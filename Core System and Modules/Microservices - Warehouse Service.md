...
- 🔴 **Reconciler cursor deadlock — batch saturation (ditemukan 2026-07-24, LIVE di prod, belum diperbaiki).** `runReconciler` hanya memanggil `writeCursor` bila ada order dengan `order_update_date` **melampaui** cursor (`if ts > newCursor` lalu `if newCursor > cursor`, `reconciler.go:133-141`). Query menarik `updated_since = cursor - 300` (overlap 5 mnt) diurutkan ASC dengan `limit=500`. Bila **≥500 order** punya `order_update_date` di dalam jendela `[cursor-300s, cursor]`, seluruh jatah batch habis oleh order yang timestamp-nya **≤ cursor** → `newCursor` tak pernah > `cursor` → cursor tak pernah ditulis → **jendela yang sama ditarik ulang tiap 60 detik selamanya**. Order di atas cursor tak pernah terjangkau. **Terjadi nyata 24 Juli 2026 09:17:25 WIB**: 514 order `TO_SHIP` mendarat dalam satu jendela 5 menit (kemungkinan batch arrange dari Desty), cursor `warehouse-reconciler` beku di `1784859445` selama ≥45 menit sementara log mengulang `TO_SHIP processed 500 orders, cursor → 1784859445` tiap menit; 82 order menumpuk di atas cursor. **Dampak nyata nihil saat itu** — 50/50 sampel order tertahan sudah masuk WMS lewat jalur webhook, dan 0 baris `upsert error` — tapi artinya **jaring pengaman reconciler sedang mati** dan baru ketahuan justru ketika dibutuhkan. Kelas bug identik dengan yang dicatat di `resi_wms_safety_net_task.go:42-47` (jalur watermark yang bisa membuat order permanen tak tertangkap), bentuknya beda: di sini penyebabnya **saturasi batch**, bukan `update_time` yang tak naik. **Pemulihan operasional**: majukan cursor `warehouse-reconciler` di `sync_cursors` melewati gerombolan (aman — `doUpsert` idempoten dan order dalam jendela sudah masuk WMS). **Perbaikan kode**: jamin progres maju — bila batch penuh dan `newCursor == cursor`, majukan paksa (mis. +1 detik) atau pakai `order_id` sebagai pemecah seri. Catatan: **open-order sweep TIDAK menutup celah ini** — sweep hanya menyegarkan order yang **sudah ada** di WMS (`open_order_sweep.go:124`), tidak bisa menarik masuk order yang belum pernah terdaftar.
