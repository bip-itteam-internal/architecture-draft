...
- **master_system_role**: _id, key, name, roles ([]string), metadata — katalog nilai role per modul, dipakai memvalidasi `system_authentication.system_roles` (key = kode MODUL seperti `it`/`hris`/`finance`, **bukan** nama departemen). ⚠️ Sejak [erp-frontend#960](https://github.com/bip-itteam-internal/erp-frontend/pull/960) (merged 2026-08-10) koleksi ini **tak punya layar tulis di web** — hanya dibaca untuk mengisi dropdown pemberian role; perubahan isinya lewat API atau langsung ke Mongo. Lihat [[CORE - RBAC dan Permission Set]]
...
- **invoice_correction_import_batches** (`KoreksiImportBatch`) 🟡 *PR bip-erp #1712, belum merge*: _id (uuid), channel, file_name, content_hash (sha256 isi file), reason, created_by, created_at, apply_mapping (bool), apply_price (bool), apply_gudang (bool, 🟡), gudang_mulai? (`YYYYMMDD`, 🟡), ringkasan (obj: total_baris, tidak_berubah, siap, perlu_persetujuan, ditolak, diabaikan, diterapkan, dilewati, gagal, faktur_tersentuh, mapping_baru, harga_baru, pindah_gudang 🟡), rows ([]KoreksiImportBaris: baris_excel ([]int), order_id, sku?, jenis, kolom, sebelum, sesudah, vonis, alasan?, invoice_id?, invoice_number?, target_invoice_id?, target_invoice_number?, target_date_wib?, master_key?, retur_ids? / retur_numbers? (🟡 GUDANG: dokumen retur yang ikut dipindah)), master? ([]PerubahanMasterImport: key, jenis MAPPING_SKU|HARGA, sku, product_name?, tiktok_item_id?, master_sku?, qty_per_unit?, harga?, effective_from?, jumlah_baris, jumlah_faktur, depends_on?, vonis?, pesan?), faktur? ([]KoreksiImportFaktur: jenis?, invoice_id, invoice_number, status RESENT|SKIPPED|FAILED, pesan?) — arsip append-only impor koreksi Rekap Lengkap (pola `fake_order_import_batches`); index `created_at:-1`. Jejak per order: `invoice_correction_logs` action `KOREKSI_IMPORT` (from_value = jenis, to_value = nama file, reason memuat batch id); 🟡 action `WAREHOUSE_OVERRIDE` (from_value = gudang sebelumnya, to_value = gudang baru).
...
- **shop_sync_states** (`ShopSyncState`) ✅ *live 2026-08-24*: _id, channel (shopee|tiktok), shop_id (string; Shopee numerik di-string-kan, TikTok = _id authorized shop), shop_name, status (ACTIVE|DISABLED), reason?, disabled_at?, updated_by? (Actor), created_at, updated_at, + Tahap 2 (🟡 PR bip-erp#1473): consecutive_auth_failures? (int — kegagalan auth beruntun dari guardian order, reset saat sukses/enable), last_auth_error?, last_auth_error_at?, last_sync_ok_at? — unique index (channel, shop_id). Toko TANPA dokumen = ACTIVE. Koleksi terpisah dari dokumen toko (nalar [[ADR - 0027 Status Sinkron Resi Terpisah dari Update_Time Marketplace]]); dipakai worker via `ListSyncableShops` (fail-open). Lihat [[ADR - 0052 Status Sinkron per Toko]]
- **shop_status_histories** (`ShopStatusHistory`) ✅ *live 2026-08-24*: _id, channel, shop_id, shop_name, event (DISABLED|ENABLED), old_status? (kosong = dokumen state pertama), new_status, reason?, actor? (Actor), created_at — append-only, tanpa TTL (volume rendah: hanya aksi manual); index (channel, shop_id, created_at desc)
...
- **ga_ruang** (`Ruang`, Booking Ruang): _id?, company_id, nama (maks 100), nama_key (huruf kecil, unik per company_id), lokasi?, kapasitas? (int), fasilitas? ([]string), jam_buka, jam_tutup ("HH:MM" dibaca WIB, menit 00/30), aktif (bool; ruang tak pernah dihapus), kunci_token?, kunci_sampai? (*time; kunci sewa persetujuan, tak dikirim ke klien), metadata. Index: (company_id, nama_key) unik. Struct `services/inventory/peminjaman_model.go`; lihat [[GA - Asset Loan & Room Booking]]
- **ga_peminjaman** (`Peminjaman`): nomor (`PJR-YYYYMMDD-nnn`, unik), company_id, jenis (`ruang`), sumber_id (hex `_id` ruang), sumber_nama (dibekukan saat diajukan), pemohon_id, pemohon_nama?, divisi?, posisi? (identitas pemohon dibekukan), no_wa? (tervalidasi wajib 8 sampai 15 angka), keperluan, keterangan?, mulai_at (time), selesai_at (time), status (`DIAJUKAN`/`DISETUJUI`/`DITOLAK`/`DIBATALKAN`), asal? ({modul, id}: modul yang melahirkan booking, kosong untuk pengajuan langsung), riwayat? ([]{aksi, oleh, nama?, posisi?, alasan?, waktu}; `oleh` = `sistem` untuk tolak otomatis dan persetujuan yang dikembalikan), metadata. Index: nomor unik; (company_id, sumber_id, status, mulai_at); (company_id, sumber_id, status, selesai_at); (pemohon_id, mulai_at desc)
- **ga_peminjaman_penyetuju** (`PenyetujuPeminjaman`): company_id (unik), penyetuju ([]{employee_id, nama?, posisi?}; wewenang hanya dari employee_id), diubah_oleh?, diubah_at? (*time). Satu daftar per perusahaan untuk semua ruang
