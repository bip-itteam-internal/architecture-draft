...
- **invoice_correction_import_batches** (`KoreksiImportBatch`) 🟡 *PR bip-erp #1712, belum merge*: _id (uuid), channel, file_name, content_hash (sha256 isi file), reason, created_by, created_at, apply_mapping (bool), apply_price (bool), ringkasan (obj: total_baris, tidak_berubah, siap, perlu_persetujuan, ditolak, diabaikan, diterapkan, dilewati, gagal, faktur_tersentuh, mapping_baru, harga_baru), rows ([]KoreksiImportBaris: baris_excel ([]int), order_id, sku?, jenis, kolom, sebelum, sesudah, vonis, alasan?, invoice_id?, invoice_number?, target_invoice_id?, target_invoice_number?, target_date_wib?, master_key?), master? ([]PerubahanMasterImport: key, jenis MAPPING_SKU|HARGA, sku, product_name?, tiktok_item_id?, master_sku?, qty_per_unit?, harga?, effective_from?, jumlah_baris, jumlah_faktur, depends_on?, vonis?, pesan?), faktur? ([]KoreksiImportFaktur: jenis?, invoice_id, invoice_number, status RESENT|SKIPPED|FAILED, pesan?) — arsip append-only impor koreksi Rekap Lengkap (pola `fake_order_import_batches`); index `created_at:-1`. Jejak per order: `invoice_correction_logs` action `KOREKSI_IMPORT` (from_value = jenis, to_value = nama file, reason memuat batch id).
