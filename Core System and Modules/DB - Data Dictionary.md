...
- **shop_sync_states** (`ShopSyncState`) 🟡 *branch `feat/oauth-shop-status-history`*: _id, channel (shopee|tiktok), shop_id (string; Shopee numerik di-string-kan, TikTok = _id authorized shop), shop_name, status (ACTIVE|DISABLED), reason?, disabled_at?, updated_by? (Actor), created_at, updated_at — unique index (channel, shop_id). Toko TANPA dokumen = ACTIVE. Koleksi terpisah dari dokumen toko (nalar [[ADR - 0027 Status Sinkron Resi Terpisah dari Update_Time Marketplace]]); dipakai worker via `ListSyncableShops` (fail-open). Lihat [[ADR - 0052 Status Sinkron per Toko]]
- **shop_status_histories** (`ShopStatusHistory`) 🟡 *idem*: _id, channel, shop_id, shop_name, event (DISABLED|ENABLED), old_status? (kosong = dokumen state pertama), new_status, reason?, actor? (Actor), created_at — append-only, tanpa TTL (volume rendah: hanya aksi manual); index (channel, shop_id, created_at desc)
