...
- **master_system_role**: _id, key, name, roles ([]string), metadata — katalog nilai role per modul, dipakai memvalidasi `system_authentication.system_roles` (key = kode MODUL seperti `it`/`hris`/`finance`, **bukan** nama departemen). ⚠️ Sejak [erp-frontend#960](https://github.com/bip-itteam-internal/erp-frontend/pull/960) (belum merge) koleksi ini **tak punya layar tulis di web** — hanya dibaca untuk mengisi dropdown pemberian role; perubahan isinya lewat API atau langsung ke Mongo. Lihat [[CORE - RBAC dan Permission Set]]
