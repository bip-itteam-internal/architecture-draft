...
- `/public/recruitment/psikotes/:token` dan `/:token/*` (GET dan POST) → recruitment-service (kandidat mengerjakan **psikotes online** lewat magic link, tanpa JWT; yang menjaganya token acak di URL, bukan sesi login). **Satu rute umum** sejak 2026-09-11 (bip-erp `feat/api-gateway-psikotes-catch-all`, belum merged), menggantikan lima rute yang dulu ditulis satu per satu: tes berpaket menambah endpoint per bagian dan subtes, dan rute per endpoint membuat tiap endpoint baru butuh rute gateway baru yang gagal senyap bila terlupa (beacon `/abandon` pernah mendarat di "Cannot POST"). Lihat [[HRIS - Psikotes Kraepelin]] dan [[HRIS - Bank Soal dan Paket Psikotes]]
	- **Penyaring** (`psikotes_publik.go`): token hanya `^[A-Za-z0-9_-]{16,128}$`; sisa path hanya segmen `^[A-Za-z0-9_.-]+$`, tanpa `.` dan `..`, tanpa `%`, `?`, `#`, maks 256 karakter. Yang tak lolos dibalas 404 yang sama dengan token tak dikenal **tanpa menyentuh service**. Ini sekaligus menutup temuan lama token yang tak di-`url.PathEscape` (`%2F` yang ter-decode jadi `/`)
	- **Gambar soal** (`.../gambar/<kunci>`): Content-Type ditentukan dari isi balasan (`http.DetectContentType`, karena `InternalRequest` tak meneruskan Content-Type service), ditambah `X-Content-Type-Options: nosniff` dan `Cache-Control: private, max-age=3600`
	- **Limiter sendiri**: rute ini dikeluarkan dari `standardLimiter` grup `/public` (lihat Lain-lain) dan dijaga **300 permintaan per menit per token** plus **600 per menit per IP**. Alasannya terukur: satu subtes CFIT bergambar penuh (belasan soal × 7 gambar) melampaui 60 per menit, dan jawaban yang ikut tertolak 429 hilang sesudah antrean klien menyerah. Dikunci test dengan kontrol (rute `/public` lain tetap 429 di permintaan ke-61); di dev 2026-09-11, 120 permintaan gambar kandidat nol 429 sementara kontrol `/public/recruitment/postings` kena 429 di 10 dari 70
- ✅ `/public/recruitment/track/:token` **dibuang dari gateway** di bip-erp #1824 (merged 2026-09-10, `a0260e4d`). Rute ini sebelumnya YATIM: masih terdaftar dan meneruskan ke `<recruitment>/public/track/<token>`, padahal handler-nya sudah dihapus dari recruitment-service (`a298ba70`, 2026-07-24), sehingga membalas **404 Fiber**. Biner API-Gateway dev sudah bersih (string `recruitment/track` = 0, diukur 2026-09-10); prod bersih sejak api-gateway prod di-deploy ulang 2026-09-11 (string yang sama = 0, diukur hari itu). Lihat [[API - Recruitment Service]] §Publik
...
**Lain-lain**
- CORS dan rate limiting di level gateway: `strictLimiter` 25 per menit (login, SSO redeem, `/debug`) dan `standardLimiter` 60 per menit (grup `/onboarding` dan `/public`, **kecuali** rute kandidat psikotes yang punya limiter sendiri sejak 2026-09-11). ⚠️ Kunci limiter bawaan adalah `c.IP()`, dan app memakai `ProxyHeader: X-Forwarded-For` (`main.go`) tanpa validasi IP, jadi nilainya diambil dari header itu apa adanya: klien bisa memilih embernya sendiri dengan mengirim header palsu. Sudah begitu sebelum psikotes; dicatat, belum diubah
