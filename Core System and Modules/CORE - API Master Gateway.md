...
- `/public/recruitment/psikotes/:token` dan `/:token/*` (GET dan POST) → recruitment-service (kandidat mengerjakan **psikotes online** lewat magic link, tanpa JWT; yang menjaganya token acak di URL, bukan sesi login). **Satu rute umum** sejak 2026-09-11 (bip-erp #1838 merged `b73862af`, belum prod), menggantikan lima rute yang dulu ditulis satu per satu: tes berpaket menambah endpoint per bagian dan subtes, dan rute per endpoint membuat tiap endpoint baru butuh rute gateway baru yang gagal senyap bila terlupa (beacon `/abandon` pernah mendarat di "Cannot POST"). Lihat [[HRIS - Psikotes Kraepelin]] dan [[HRIS - Bank Soal dan Paket Psikotes]]
