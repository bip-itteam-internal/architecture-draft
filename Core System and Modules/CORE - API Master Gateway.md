...
- [[Microservices - Form Builder Service]] — form dinamis IT/HRGA (⚠️ merged 2026-08-01, belum live di dev). ⚠️ **`FORM_BUILDER_MODULE_URL` masuk map `InternalURL` yang divalidasi**, jadi nilai kosong = gateway panic saat boot = seluruh ERP mati. Pastikan `.env` server memuat `FORM_BUILDER_SERVICE_PORT` dan gateway dideploy dengan `docker-compose.yml` yang ikut ter-merge. Rute `/internal/compliance`-nya adalah contoh penerapan [[ADR - 0031 Prefix internal Bukan Batas Keamanan]]: karena catch-all di atas membuatnya terbuka ke internet, rute itu mengunci identitas ke header dan mengabaikan query param dari request pemakai.
