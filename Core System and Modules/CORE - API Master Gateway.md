...
- `/public/recruitment/psikotes/:token` (+ `/start`, `/columns/:index`, `/finish`, `/abandon`) → recruitment-service (kandidat mengerjakan **psikotes online** lewat magic link, tanpa JWT — yang menjaganya token acak di URL, bukan sesi login). Lihat [[HRIS - Psikotes Kraepelin]]
	- ⚠️ Token **tidak** di-`url.PathEscape` saat disusun ke URL internal. Token yang benar-benar diterbitkan server aman (base64url, tanpa `/`, `?`, `#`, `%`), tapi `c.Params("token")` menerima string apa pun dari pemanggil; input ber-`%2F` bisa berubah jadi `/` di `req.URL.Path` saat diparse ulang. Terdokumentasi di komentar kodenya sebagai temuan terbuka, belum diperbaiki
