...
*Cara membuat login Web ERP mendukung redirect SSO, dan langkah menambah aplikasi konsumen baru.*

⛔ **KOREKSI 2026-09-03 — kalimat lama di sini SALAH dan sudah menyesatkan dua kali.** Dokumen ini sebelumnya menyatakan menambah konsumen baru **"tidak butuh perubahan backend"** karena `ticket`/`redeem` agnostik aplikasi. Itu benar HANYA untuk konsumen yang menukar `redeem` dari **server** (`services/vault-mcp`). Konsumen berbasis **peramban** selalu menuntut PR ke `api-gateway/main.go` **plus deploy gateway**, sebab `AllowOrigins` adalah daftar literal yang **dikompilasi ke dalam biner Go** — bukan env, bukan pola subdomain, bukan wildcard. Terbukti dua kali: Portal Karir (PR [#460](https://github.com/bip-itteam-internal/bip-erp/pull/460)) dan aplikasi Audit Internal (PR [#1690](https://github.com/bip-itteam-internal/bip-erp/pull/1690)). Kegagalannya muncul sebagai galat CORS di peramban, yang terbaca seperti masalah jaringan — bukan seperti izin yang kurang.

- ⛔ **FE outbound launch TIDAK ADA.** Baris ini sebelumnya menyebut `erp-frontend/src/features/erp/auth/hooks/use-task-manager-sso.ts` (`useTaskManagerSSO`); berkas itu **tidak pernah ada di repo** — dikonfirmasi `git grep`, nol hit, dan `git ls-files src/features/erp/auth/*` tidak memuatnya. Nama itu hanya hidup di dokumen rencana lama. Satu-satunya pemanggil `/auth/sso/ticket` di erp-frontend adalah `src/app/login/page.tsx`. Konsekuensinya: **seluruh SSO harus dimulai dari sisi aplikasi konsumen** (inbound `?redirect_url=`); tak ada peluncur dari dalam ERP.
...
1. **Sisi ERP (IdP) — allowlist redirect.** `erp-frontend/src/app/login/page.tsx` sudah mengizinkan semua subdomain `*.bharatainternasional.com` serta `localhost`/`127.0.0.1`, jadi subdomain baru tidak perlu ditambahkan ke env. ⚠️ **Dua batasnya yang tidak disebut sebelumnya**: pemeriksaannya hanya atas **hostname, bukan skema** — `http://` di subdomain itu lolos dan ERP JWT dikirim plaintext; dan `localhost` lolos **tanpa cek port**. Keduanya permukaan yang layak dipersempit sebelum konsumen produksi berikutnya ditambahkan.

3. **Env & CORS.** App Konsumen: set `GATEWAY_BASE_URL` (prod `https://api.bharatainternasional.com`) & `ERP_BASE_URL`. ⛔ **Sisi ERP BUKAN "tidak perlu setting tambahan"**: bila konsumen menukar `redeem` dari peramban, origin-nya **wajib** ditambahkan ke `AllowOrigins` di `api-gateway/main.go` lalu gateway **dibangun ulang dan di-deploy**. Sebutkan port secara harfiah dan kunci port itu di skrip `dev`/`start` aplikasinya — port yang bergeser sama dengan CORS ditolak, dan gejalanya tak menyebut port sama sekali.
...
- **Tidak semua pemegang akun adalah karyawan.** Field `account_type` di `system_authentication` membedakan `employee` (kosong = ini) dari `external` (vendor/mitra). Akun `external` punya **masa berlaku** yang ditegakkan di empat jalur penerbitan token — tiga jalur login **dan** `GET /auth/refresh`, karena refresh menerbitkan token baru tanpa kredensial sehingga menjaga login saja tak cukup. `valid_until` kosong dianggap kedaluwarsa (fail-closed). Detail: [[Microservices - Employee Service]] §Akun pihak luar.
- **Hak akun luar dipasang eksplisit per-akun, lewat dua sumbu.** Jalur hak-dari-posisi mati bagi vendor (`positionSetKeys` berhenti bila `work_data.Department` kosong, dan akun luar tak punya `work_data`), jadi satu-satunya sumber haknya adalah `permission_sets` (modul ber-RBAC) **atau** `system_roles` (modul yang menggerbang di situ, mis. warehouse) yang disetel IT supervisor. Memilih sumbu yang salah tak menghasilkan galat apa pun — vendor cuma tetap tertolak. Prosedurnya: [[RUN - Onboarding Akun Eksternal (Vendor & Mitra)]].
- **Klaim izin akun luar tak pernah kosong.** `izinAkun` selalu menyisipkan penanda tanpa-hak `account.$type.external`, sehingga setiap fallback konsumen yang berpatokan "klaim `permissions` absen" otomatis tertutup — termasuk modul yang belum ditulis. Penanda ini bukan permission katalog dan tak akan pernah lolos `HasPermission`. Konsekuensi bagi konsumen: **jangan** menyimpulkan "belum punya paket" dari klaim yang absen bila pemakainya bisa berupa akun luar.
...
- Klaim JWT **`company_id`** + header **`BIP-Company-ID`** (fallback `"BIP"`) membawa perusahaan/tenant pemakai (multi-perusahaan); diisi employee-service saat login dari `work_data.company_id`. ⚠️ **Akun pihak luar (vendor/mitra) tak punya `work_data`**, jadi nilainya diambil dari `external_account.company_id` (`companyIDAkun`); tanpa itu tiap vendor jatuh ke default `"BIP"` diam-diam. Dasar isolasi data lintas-perusahaan: [[ADR - 0029 Multi-Tenant Presensi Row-Level company_id]].
...
- [[RUN - Onboarding Akun Eksternal (Vendor & Mitra)]] — prosedur menerbitkan & memberi hak akun pihak luar
