...
**Modul `secretary` (Kesekretariatan) — peleburan `legal` + `rnd`, 2026-08-13** (branch `feat/secretary-permission-set` + `feat/secretary-permission-set-fe`, ⚠️ **belum merge**). Modul ini menggantikan `legal` sepenuhnya dan memberi `rnd` lapisan izin pertamanya. Enam izin menggerbang **25 rute** lewat `gateSecretary` di employee-service: `secretary.legal.{view,work,manage}` untuk 15 rute Legal, `secretary.rnd.{view,work,manage}` untuk 10 rute R&D. Tujuh paket bawaan (Legal Lihat/Pelaksana/Admin, R&D Lihat/Pelaksana/Admin, plus **Lihat Semua** lintas-area), reach `all`. Sakelar: `SECRETARY_PERMISSION_ENFORCEMENT` (dengan `LEGAL_PERMISSION_ENFORCEMENT` tetap dibaca sebagai alias) + `SECRETARY_TIER_FALLBACK`. Enam hal yang membedakannya, dan layak dibaca sebelum menggarap modul berikutnya:
