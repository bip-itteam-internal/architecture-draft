...
- **Katalog yang benar-benar terdaftar per 2026-08-10 ada 13**, terverifikasi dari `origin/main` (berkas `shared-library/common/catalog_*.go` + titik `common.RegisterCatalog`): `finance`, `formbuilder`, `hris`, `kaskecil`, `kpi`, `legal`, `menu`, `monitoring`, `payroll`, `procurement`, `recruitment`, `ticket`, `training`. **`ga` menyusul jadi yang ke-14** (branch `feat/ga-permission-set`, ⚠️ belum merge — lihat §Modul `ga`). Dua yang baru sejak angka "11" ditulis: **`legal`** sudah mendarat di `main` ([#1117](https://github.com/bip-itteam-internal/bip-erp/pull/1117), 2026-08-09) dan **`formbuilder`** menyusul ([#1138](https://github.com/bip-itteam-internal/bip-erp/pull/1138), 2026-08-10). Daftar ini yang mengisi `GET /api/employee/master/permission-modules`, jadi ia juga yang menentukan modul mana yang bisa dipilih HR saat menyusun paket.
