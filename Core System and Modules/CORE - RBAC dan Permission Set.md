...
- **Status**: ⚠️ Implemented (ada catatan). Lapisan **posisi-memegang-hak sudah live** (migrasi + resolver + layar Hak per Posisi & Siapa Boleh Apa), dan penegakan per-aksi jalan di **ticket**, **payroll**, **procurement**, **monitoring**, **hris**, **recruitment**, serta **training**. **finance** baru punya katalog + paket, **belum ada endpoint yang memeriksanya**. Modul **`menu`** (menu terbatas) sudah berkatalog + bergerbang tapi **belum merge dan belum berfungsi penuh** (celah akun-tanpa-paket, lihat §Belum Diimplementasikan). Modul lain masih bertumpu pada `system_roles` tier.
...
- **Katalog 9 modul lain belum ditulis** (kpi, hrdoc, wms, warehouse, integration, insentive, ga, notification, admin). `procurement` dan `monitoring` **sudah** — dikoreksi 2026-07-31 setelah ditemukan masih tercatat belum ditulis di sini padahal keduanya hidup di kode.
	...
	**`training` sudah, sejak 2026-08-09** — modul KEDUA yang dipecah keluar dari kategori sidebar `hris`, dan batasnya paling jelas dari semuanya karena ia sudah punya service sendiri (`learning`). 21 rute digerbang: 6 master (jenis pelatihan & pelatih), 7 operasional, 8 baca. Tier default memberi ketiga tier SELURUH izin — bukan kelalaian, melainkan cermin kenyataan bahwa semua rute tulis (termasuk master) digerbang `RequireHRISStaff` yang meloloskan staff; pembedaan yang berguna karena itu ada di PAKET (“Pelatihan: Pelaksana” tanpa master), bukan di tier. Sakelar: `TRAINING_PERMISSION_ENFORCEMENT` + `TRAINING_TIER_FALLBACK`.
	...
	🟡 **`kpi` SENGAJA DITUNDA, dan alasannya bukan kerumitan melainkan risiko.** Berbeda dari recruitment/training yang gerbangnya middleware seragam, kontrol akses KPI hidup **di dalam handler** lewat `common.RequireKPIDepartmentRBAC(c, department)` dan bersifat **relasional terhadap departemen yang diminta**: `hasAnyDepartmentAccess` (`roles.go:256`) meloloskan pemegang role supervisor/admin di modul MANA PUN yang memetakan ke departemen itu. Praktisnya seorang Manufacturing Supervisor tanpa role `hris` sama sekali hari ini sah membaca KPI departemennya — begitu pula SPV Quality, Procurement, dan Finance. Memberi `kpi.view` fallback tier `hris` seperti dua modul sebelumnya akan **mencabut akses seluruh supervisor non-HR tanpa satu pun galat**, persis kelas kegagalan yang diperingatkan di `catalog_hris.go`. Tak ada tier datar yang bisa menggantikannya karena aturannya bukan tingkat melainkan **cakupan** — sumbu ketiga ADR 0030 (`reach: division`) yang belum ditegakkan di mana pun selain task-management. **Prasyarat sebelum katalog `kpi` ditulis: penegakan reach divisi.** Gejala yang sama sudah tercatat di [[ADR - 0030 RBAC Tiga Sumbu dengan Hak Menempel di Posisi]] (gerbang `scope=team` berbasis keberadaan bawahan, bukan izin).
