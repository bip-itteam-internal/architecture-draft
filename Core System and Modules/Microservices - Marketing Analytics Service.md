...
- **Di PROD per 2026-09-11, sesi ditutup hanya saat SELURUH hostnya sudah absen pulang, BUKAN saat jam shift berakhir.** `selesai` diisi jam clock-out **terakhir** di antara hostnya (`waktuTutupSesi`, `live_shift_autoclose.go`) dan `ditutup_otomatis: true`. Sesi **tetap terbuka** bila ada host yang belum absen pulang, tak punya entri absensi, atau clock-out-nya lebih awal dari `mulai` (milik shift sebelumnya). Di branch ADR 0088 §1 penutupan ini ikut menulis `alasan_selesai: "clock_out"`.
- **Jam shift berakhir hanya memicu pengingat** `shift_usai`, 30 menit sesudahnya; bila jadwalnya tak diketahui, yang tersisa pengingat `jam_10`. **Ambang 12 jam tidak menutup sesi**, ia hanya menandai `perlu_koreksi` (porsi GMV host 0) setiap kali riwayat dibaca. Tutup otomatis di jam berakhir shift + 60 menit ([[ADR - 0088 Ambil Alih Sesi Live oleh Host Terjadwal dan Tutup Otomatis Akhir Shift]] §1) **sudah dikodekan di branch** bip-erp `feat/marketing-analytics-tutup-akhir-shift` dan erp-frontend `feat/live-shift-badge-alasan-tutup`, 🟡 belum merged per 2026-09-11; cara kerjanya di §Ambil alih & tutup otomatis akhir shift di bawah.
...
#### Ambil alih & tutup otomatis akhir shift (🟡 diusulkan, [[ADR - 0088 Ambil Alih Sesi Live oleh Host Terjadwal dan Tutup Otomatis Akhir Shift]])

> 🟡 **Per 2026-09-11: tutup otomatis akhir shift sudah dikodekan di branch** (bip-erp `feat/marketing-analytics-tutup-akhir-shift`, erp-frontend `feat/live-shift-badge-alasan-tutup`), belum merged dan belum di-deploy. **Ambil alih belum ada kodenya.** Begitu masing-masing naik ke prod, statusnya diganti dan rincian rutenya pindah ke [[API - Marketing Analytics Service]].
...
- **Tutup otomatis kedua** (branch; `batasTutupShift` + `keputusanTutup` di `live_shift_autoclose.go`): sesi berjalan yang melewati jam berakhir shift + 60 menit (`jedaTutupSesudahShift`) ditutup dengan `selesai` **tepat di batas itu** (bukan jam tik), `ditutup_otomatis: true`, dan `alasan_selesai: "shift_berakhir"`. Aturan clock-out tetap berlaku dan menang bila lebih awal (seri jatuh ke clock-out).
...
- 🟡 **Ambil alih & tutup otomatis akhir shift** ([[ADR - 0088 Ambil Alih Sesi Live oleh Host Terjadwal dan Tutup Otomatis Akhir Shift]]): tutup otomatis akhir shift **dikodekan di branch** (bip-erp `feat/marketing-analytics-tutup-akhir-shift`, erp-frontend `feat/live-shift-badge-alasan-tutup`, 2026-09-11), belum merged; ambil alih belum ada kodenya. Sampai tutup otomatis naik ke prod, sesi yang ditinggal host yang tak absen pulang hanya bisa dilepas supervisor IT, dan lewat 12 jam porsinya hangus sampai dikoreksi lewat tulis DB. Rincian di §Ambil alih & tutup otomatis akhir shift.
