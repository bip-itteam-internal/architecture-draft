...
Sampai penghalang di atas terjawab, **host Shopee tetap dicatat manual** lewat `/live-shifts`.
...
- **Tutup otomatis kedua** (di PROD sejak 2026-09-11 16:55 WIB; `batasTutupShift` + `keputusanTutup` di `live_shift_autoclose.go`): sesi berjalan yang melewati jam berakhir shift + 60 menit (`jedaTutupSesudahShift`) ditutup dengan `selesai` **tepat di batas itu** (bukan jam tik), `ditutup_otomatis: true`, dan `alasan_selesai: "shift_berakhir"`. Aturan clock-out tetap berlaku dan menang bila lebih awal (seri jatuh ke clock-out).
  ...
  - Riwayat web memilih label badge dari `alasan_selesai` ("shift berakhir" atau "clock-out"); sesi tanpa alasan tetap "(clock-out)".
...
- [[ADR - 0063 Siaran Serentak Dicatat sebagai Sesi Terpisah per Akun]] · [[ADR - 0088 Ambil Alih Sesi Live oleh Host Terjadwal dan Tutup Otomatis Akhir Shift]] (Sesi Live Host: siaran serentak, ambil alih, tutup otomatis akhir shift)
