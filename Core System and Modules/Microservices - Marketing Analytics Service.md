...
**45 route** di berkas produksi `origin/main` per enumerasi ulang 2026-09-11 (pendaftaran `app.Get|Post|Patch|Put|Delete` di `services/marketing-analytics/*.go` non-test; tak ada `Group`/`Mount`); audit kode 2026-08-27 menghitung 41. Branch Analisis Live menambah `GET /lives/analisis` dan `GET /live-shifts/performa`, jadi **47** bila merged. Berkas pendaftarnya: `routes.go` · `handler_mart.go` · `main.go` · `toko.go` · `divisi.go` · `price_floor_handler.go` · `ambang_handler.go` · `pagu_handler.go` · `kurva_alokasi_handler.go` · `live_shift_handler.go` · `live_shift_akun.go` · `jobs.go` · `penjadwal_status.go`. Daftar lengkap per-route: [[API - Marketing Analytics Service]], yang per audit ini **memuat keempat-puluhnya** (sebelumnya 12 route tak punya baris di mana pun). ⚠️ Per 2026-09-11 `/icc/sla-chat` dan `/kpi/sla-chat-cs` belum punya baris di sana. Seluruhnya `GET` kecuali disebut lain.
...
| `/lives` | Sesi live dari `mart_live_sessions` (**4.836 sesi** per 2026-08-22; dulu kosong). Urutan tetap `gmv` menurun lalu `session_id`; branch Analisis Live menambah `_id` sebagai penutup urutan supaya pembacaan `skip`/`limit` tak menggandakan atau melewatkan sesi ber-GMV sama. `limit` bawaan 500, maks 5.000 (`jinakkanLimit`) |
| `/lives/analisis` | 🟡 Branch, belum merged (2026-09-11). Ringkasan manajemen atas **seluruh** sesi rentang itu, lihat § Analisis Live di bawah |

### Analisis Live (`/lives/analisis`)

> 🟡 **Status per 2026-09-11**: di branch bip-erp `feat/marketing-analytics-analisis-live` (ditumpuk di atas `fix/marketing-analytics-rasio-live-berpasangan`), belum merged, belum di-deploy, dan belum pernah dijalankan lewat gateway. Konsumennya halaman Analisis Live di [[APP - Web ERP]].
...
**Pembilang dan penyebut BERPASANGAN** (branch `fix/marketing-analytics-rasio-live-berpasangan`, 2026-09-11, belum merged). Tiap rasio dijumlah hanya dari sesi yang **kedua** sisinya terukur (`bahanTraffic.tambah`): klik adalah jangkar, jadi order dan keranjang dari sesi yang kliknya tak terukur tak ikut. Sisi yang terukur sendirian memberi pembilang tanpa penyebut, dan hasilnya angka yang masuk akal tapi salah tanpa satu pun galat. Terukur prod 2026-09-11: pada Agustus 2026, 71 sesi host punya klik tanpa data keranjang (pengambilnya baru jalan pertengahan bulan), sehingga `add_to_cart_rate` terbaca 3,31% padahal sesi yang terukur 6,24%; `carevolution.hub` punya keranjang tanpa klik karena toko non-utama tak membawa blok interaksi. Muatan `/kpi/kinerja-live` membawa pasangannya (`orders_berklik`, `klik_keranjang`, `views_bertonton`) plus `ada_klik_keranjang`, padanan `ada_klik` untuk `add_to_cart_rate`. ⚠️ Field `add_to_cart` di muatan kini hanya keranjang dari sesi yang kliknya terukur; `conversion` tetap cacahan seluruh order. Jalur individu memakai pembilang yang sama. Halaman Analisis Live, `/live-shifts/performa`, kartu Konversi halaman Live, dan `performa-host.ts` di frontend memakai pasangan yang sama, supaya tak ada dua angka konversi untuk sesi yang sama.
...
- 🟡 **Analisis Live, `/live-shifts/performa`, dan rasio live berpasangan masih di branch per 2026-09-11** (bip-erp `fix/marketing-analytics-rasio-live-berpasangan` lalu `feat/marketing-analytics-analisis-live` yang ditumpuk di atasnya; erp-frontend `feat/marketing-analytics-analisis-live`). Belum merged, belum di-deploy, belum dijalankan lewat gateway. Urutan deploy di § KPI Host Live.
