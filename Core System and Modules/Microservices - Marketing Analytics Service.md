...
- **Status**: ⚠️ Implemented dengan catatan (audit kode 2026-08-26). Berjalan di production dengan channel **TikTok + Shopee**, penjadwal internal 48 jam, dan **53 baris pendaftaran rute** di berkas produksi `origin/main` (enumerasi ulang 2026-09-12 dengan metode baris, lihat § Endpoint; enumerasi sebelumnya 45 pada 2026-09-11 dan 41 pada audit 2026-08-26, tak sebanding langsung; branch Analisis Account Specialist menambah satu, belum PR). `mart_live_sessions` kini **terisi 4.836 sesi** (verifikasi produksi 2026-08-22). Catatan: `mart_buyer_cohort` masih kosong, lock job hanya in-process. (`/matrix/sku-shop` **tidak lagi** stub — terimplementasi sejak 2026-08-02, 576 baris + 631 baris test; dok ini menyebutnya stub selama tiga minggu lebih.) **Pencatatan sesi live oleh host (`/live-shifts`) sudah live di PROD dan dipakai host sejak 2026-09-09** (catatan lama "0 dokumen" dicoret 2026-09-11, lihat § Belum Diimplementasikan). **Sejak 2026-09-17 di PROD**: `live_shifts` menerima ceklis kesiapan siaran + Live Support yang bertugas, dan rute baru `GET /kpi/kesiapan-live` (di luar hitungan 53 baris di atas yang bertanggal 2026-09-12) melayani KPI posisi Live Support ([[ADR - 0101 Kesiapan Siaran Dicatat Host saat Mulai sebagai Dasar KPI Live Support]]). Sejak 2026-09-11 klien pencatatnya hanya MyBharata: halaman web Sesi Live Host dihapus ([[ADR - 0091 Pencatatan Sesi Live Host Hanya di MyBharata, Halaman Web Dihapus]]; erp-frontend [#1540](https://github.com/bip-itteam-internal/erp-frontend/pull/1540), merged 2026-09-11, di PROD sejak 2026-09-12 lewat image `frontend-hris` 07:35 WIB). 🟡 **Merged 2026-09-17, belum deploy**: register setoran tema & teaser Live Support (koleksi `live_support_karya`, enam rute `/live-support/*`, `GET /kpi/karya-live-support`) yang memanggil employee-service untuk penyetuju departemen ([[ADR - 0106 Tema dan Teaser Live Support Disetor dan Diputus Penyetuju Departemen sebagai Dasar KPI]]). ⚠️ **Merged 2026-09-18, di DEV (terverifikasi lewat gateway), belum PROD**: dua rute baca monitoring sesi live `/live-support/sesi*` untuk leader marketing dan Live Support ([[ADR - 0108 Monitoring Sesi Live di Web Hanya Baca untuk Leader dan Live Support]]). 🟡 **Branch `feat/live-shift-validasi-akun-toko` (2026-09-21, belum PR)**: penjaga pasangan `akun_live`+`shop_id` di Mulai dan ambil alih, plus field body opsional `konfirmasi_toko`; tidak menambah rute. Lihat § Penjaga pasangan akun+toko saat Mulai.
...
**53 baris pendaftaran rute** di berkas produksi `origin/main` per enumerasi ulang 2026-09-12 (baris `app.Get|Post|Patch|Put|Delete(` di `services/marketing-analytics/` non-test yang bukan komentar; tak ada `Group`/`Mount`). Metode ini menghitung baris, jadi angkanya tak sebanding langsung dengan enumerasi sebelumnya: 45 (2026-09-11) dan 41 (audit 2026-08-27). Branch `feat/marketing-analytics-analisis-penanggung-jawab` (belum PR) menambah tepat satu, `GET /penanggung-jawab/analisis`, jadi **54**; lihat § Analisis Account Specialist. Hitung ulang dengan metode yang sama bila rute lain menyusul. Berkas pendaftarnya: `routes.go` · `handler_mart.go` · `main.go` · `toko.go` · `divisi.go` · `price_floor_handler.go` · `ambang_handler.go` · `pagu_handler.go` · `kurva_alokasi_handler.go` · `live_shift_handler.go` · `live_shift_akun.go` · `jobs.go` · `penjadwal_status.go`. Daftar lengkap per-route: [[API - Marketing Analytics Service]], yang per audit ini **memuat keempat-puluhnya** (sebelumnya 12 route tak punya baris di mana pun). ⚠️ Per 2026-09-11 `/icc/sla-chat` dan `/kpi/sla-chat-cs` belum punya baris di sana. Seluruhnya `GET` kecuali disebut lain.
...
| `/lives` | Sesi live dari `mart_live_sessions` (**4.836 sesi** per 2026-08-22; dulu kosong). Urutan tetap `gmv` menurun lalu `session_id`; Analisis Live (#1841) menambah `_id` sebagai penutup urutan supaya pembacaan `skip`/`limit` tak menggandakan atau melewatkan sesi ber-GMV sama. `limit` bawaan 500, maks 5.000 (`jinakkanLimit`) |
| `/lives/analisis` | ✅ bip-erp #1841, di PROD sejak 2026-09-11. Ringkasan manajemen atas **seluruh** sesi rentang itu, lihat § Analisis Live di bawah |

### Analisis Live (`/lives/analisis`)

> ⚠️ **Status per 2026-09-11**: merged (bip-erp #1840, #1841; erp-frontend #1530) dan di PROD: employee-service 15:32, marketing-analytics 15:33, frontend 15:41 WIB. Terverifikasi: DEV lewat gateway dengan login sungguhan (200; 400 untuk `sort_by` dan rentang 121 hari; 403 non-leader; 404 rute karangan); PROD lewat probe handler dari dalam container (400/403/200/404) dan angka 30 hari yang **sama persis** dengan query Mongo independen (2.402 sesi, GMV Rp358.525.172, GMV toko Rp4.807.317.770, porsi 7,4579%); index `start_time_1` ada di Mongo PROD. **Belum terbukti**: pemanggilan lewat gateway PROD dengan akun sungguhan (PROD tak punya akun uji; dibuktikan lewat peramban). ⚠️ Deploy PROD dijalankan **agent** atas penegasan ulang user, menyimpang sadar dari skill `deploy-bip-erp` §0 (keputusan tim 2026-08-14). Konsumennya halaman Analisis Live di [[APP - Web ERP]].

Satu permintaan yang menjawab pertanyaan manajemen atas **seluruh** sesi live pada rentang itu: berapa yang dibawa live dan porsinya dari penjualan toko, siapa yang memikulnya (akun studio vs akun lain), berapa GMV per jam siaran tiap akun studio, jam tayang mana yang produktif, di mana penonton berhenti, dan kreator luar mana yang layak diajak kerja sama. Kinerja per **orang** sengaja tidak di sini melainkan di `GET /live-shifts/performa` (§ Pencatatan sesi live oleh host). Berkas: `analisis_live.go` (fungsi murni, masuk daftar `TestHalamanAgregatTidakMembangunPipelineSendiri`), `analisis_live_handler.go`, `sumber_analisis_live.go`, `baca_live_penuh.go`. Kontrak per field: [[API - Marketing Analytics Service]].

- **Membaca utuh, bukan 500 teratas.** `bacaLivePenuh` memutar pembacaan 5.000 baris (`limitReturMaks`) per putaran dengan `skip`, pola `bacaProfitPenuh`: galat mengembalikan 500, bukan hasil separuh, dan batas putaran tersentuh juga galat. Inilah alasan rute ini ada: `/lives` memotong di 500 sesi ber-GMV tertinggi (terukur prod 2026-09-11, 30 hari: 500 dari 2.502 sesi, 105 dari 393 akun). Urutan `/lives` karena itu ditutup `_id`.
- **Saringan & rentang.** Saringan channel/toko/divisi lewat `bacaFilterMart` yang sama dengan `/lives`, tetapi rentangnya diubah jadi jendela hari WIB untuk `start_time` (`jendelaHariWIB`, batas atas eksklusif). Rentang lebih dari 92 hari dan parameter milik halaman lain (`sort_by`, `sort_dir`, `granularitas`, `lingkup`, `matang`, `limit`, `bulan`) ditolak **400**. Batas 92 hari memakai fungsi yang sama dengan `/live-shifts` (`rentangMelampauiBatas`), karena halaman ini memanggil keduanya dengan rentang yang sama. Tidak digerbang peran di luar JWT gateway.
- **Akun studio (keputusan user 2026-09-11, khusus rute ini).** Akun studio = akun yang membawa blok interaksi (`views` tak nil) pada sedikitnya satu sesi di jendela WIB yang sama **tanpa saringan toko/divisi/channel** (`bacaAkunBerinteraksi`), digabung akun yang pernah dipakai host di Sesi Live Host (`live_shifts.akun_live`, seluruh waktu, `bacaAkunLiveTercatat`). Tanpa saringan dengan sengaja: proxy dari sesi yang sudah disaring membuat akun yang sama jadi studio tanpa saringan tetapi "akun lain" begitu disaring ke toko yang sesinya tak membawa interaksi, sehingga GMV studio per toko tak menjumlah ke totalnya. Kriteria interaksinya sama dengan `akunHostDariInteraksi` jalur KPI. Akun berpenonton gagal dibaca → 500 bersebab; `live_shifts` gagal dibaca → halaman tetap jalan dengan `klasifikasi_parsial: true`. ⚠️ Ini **bukan** definisi "akun host" jalur KPI departemen, yang tetap proxy interaksi saja; menyamakannya keputusan pemilik metrik (TBD). Sesi tanpa nama akun tetap GMV akun lain, tetapi bukan baris kreator.
- **GMV per jam berpasangan.** GMV per jam studio = GMV sesi yang sudah selesai ÷ jam sesi yang sama; sesi yang masih berlangsung tak punya jam, dikeluarkan dari pembilang, dan dicacah terpisah. Jam di sini jam **akun**: siaran serentak di dua akun dihitung per akun. Jam kerja orang (jam dinding) ada di `/live-shifts/performa`.
- **Peta jam tayang adalah perkiraan.** API TikTok tak memberi dimensi waktu di dalam sesi, jadi GMV sesi dibagi rata menurut lama sesi di tiap sel hari × jam WIB (preseden `jodohkanSesiDenganPorsi`), dan tiap sel membawa cacah sesinya supaya sel bersampel kecil tak terbaca sebagai temuan.
- **Corong** views → klik → keranjang → order, akun studio saja, memakai `bahanTraffic` dan fungsi rasio KPI apa adanya (rasio berpasangan, § KPI Host Live). Rasio yang penyebutnya nol dikirim `null`, bukan 0%.
- **Porsi dari GMV toko DIPASANGKAN per toko + tanggal WIB.** `porsi_dari_toko` = GMV live TikTok dari sesi yang (toko, tanggal WIB mulai)-nya punya baris `tt_shop_performance_daily` bernilai > 0, dibagi Σ `gmv_amount` seluruh baris rentang itu dengan saringan toko/divisi yang sama. Live dari toko/hari tanpa baris penjualan (pasti terjadi untuk hari berjalan yang belum tersinkron) tak ikut pembilang dan dikirim sebagai `gmv_live_tanpa_performa`. `null` beserta `sebab_porsi_kosong` saat tak dapat dihitung (saringan channel tanpa TikTok, sumber gagal dibaca, atau GMV toko nol). Aturan pemakaiannya di § Aturan Pemakaian Angka.
- **GMV live bukan laba.** Biaya iklan per sesi tak ada di API TikTok, jadi tak ada laba atau ROI yang boleh diturunkan dari muatan ini.

### Analisis Account Specialist (`/penanggung-jawab/analisis`)

> 🔜 **Status per 2026-09-12**: di branch bip-erp `feat/marketing-analytics-analisis-penanggung-jawab` (belum PR). Konsumennya halaman Analisis Account Specialist di [[APP - Web ERP]] (branch erp-frontend, belum PR). Terverifikasi: uji fungsi murni dan handler lewat Fiber, kontrol mutasi (gerbang peran, laba matang, urutan baca, tag JSON `/beranda`), dan layar headless dengan stub berbentuk respons. **Belum terbukti**: pemanggilan lewat gateway DEV/PROD dengan akun sungguhan, dan urutan baca terhadap Mongo sungguhan (dibuktikan lewat bentuk tahap `$sort` hasil rakitan, bukan eksekusi).

Satu permintaan yang membandingkan kontribusi dan efisiensi toko per **pemegang toko** (Account Specialist; kunci datanya tetap `icc`) pada satu rentang tanggal WIB, di sumbu mart laba (tanggal order, tanpa opex, tanpa angka insentif). Berkas: `analisis_penanggung_jawab.go` (fungsi murni, masuk daftar `TestHalamanAgregatTidakMembangunPipelineSendiri`) dan `analisis_penanggung_jawab_handler.go`. Kontrak per field: [[API - Marketing Analytics Service]].

- **Digerbang `common.RequireMarketingLeader`**, himpunan peran yang sama dengan `GET /live-shifts/performa` (definisinya di `shared-library/common/roles.go`). Rute baca laba lain di service ini hanya bergantung pada JWT gateway, termasuk `/beranda` yang ikut mengirim laba per orang di blok `penanggung_jawab`, jadi gerbang ini membatasi **halaman perbandingan**, bukan kerahasiaan angka per orang. Account Specialist sendiri (mis. `kyura: staff`) mendapat 403 dan memakai Performa Saya.
- **Atribusi dan vonis dipinjam, bukan ditulis ulang.** Per orang lewat `rakitPenanggungJawab` (kolom `icc` tiap baris toko), yang kini juga merakit `TokoIDs`, `Retur`, dan `LabaMatang` di loop yang sama. Status lewat `statusVonis` + `roasAgregat`, aturan yang sama dengan kartu vonis `/beranda` termasuk materialitas laba yang belum matang. `LabaMatang` = gross profit baris yang **tidak** bertanda `settlement_belum_matang`, penanda yang sama dengan `labaTakPulih` di `hitungBeranda`.
- **Ringkasan dirakit dari BARIS TOKO, bukan dari jumlah kolom per orang.** Toko yang dipegang dua orang dihitung penuh untuk masing-masing di `per_orang`; aturan pemakaiannya di § Aturan Pemakaian Angka.
- **Atribusi = pemegang yang tercatat saat sync**, tanpa riwayat penugasan (keputusan user 2026-09-12). Laba toko yang berganti pemegang tetap tercatat pada pemegang saat barisnya disinkron, dan layar menyebutnya dalam satu kalimat.
- **Dibaca penuh** lewat `bacaProfitPenuh` level shop dengan urutan **eksplisit** (`KolomGrossProfit` menurun). ⛔ `bacaFilterMart` tak mengisi `UrutKunci`/`UrutArah`, dan nilai nol Go merakit `$sort {"": 0}` yang ditolak Mongo (alasan lengkap di komentar `hitungBeranda`). Tanpa baris penyetel itu **setiap permintaan sungguhan menjadi 500**, sementara seluruh uji bersumber palsu tetap hijau karena sumber palsu tak memeriksa urutan. Ditemukan review 2026-09-12 sebelum PR, dikunci `TestAnalisisPJUrutanBacaSahUntukMongo`. Pemanggil baru `bacaProfitPenuh` wajib menyetelnya, sama dengan `/beranda`, `/kurva-alokasi`, dan `/matrix/sku-shop`.
- **Saringan.** Diterima: `dari`, `sampai`, `divisi`, `channel`, `shop_id`, dan `matang` (`jinakkanKematangan`; nilai tak dikenal 400 + `matang_valid`). Ditolak **400**: `sort_by`, `sort_dir`, `granularitas`, `lingkup`, `limit`, `bulan`, `entity_id`, `campaign_id`. Dua terakhir ditolak karena `cocokProfitMart` akan memasangnya ke `$match` dan diam-diam menyempitkan ringkasan. `dari` melewati `sampai` dan rentang lebih dari 92 hari (`rentangMelampauiBatas`, aturan yang sama dengan Analisis Live) juga 400. Rutenya ikut kontrak `routeYangWajibMenerimaDivisi`.
- **Ambang dibaca lebih dulu** (`DaftarAmbang` + `ambangBerlaku(sampai)`). Ambang atau laba gagal dibaca → 500 bersebab, bukan 200 berisi nol.
- **`/beranda` TIDAK ikut bertambah field.** `TokoIDs`, `Retur`, dan `LabaMatang` di `BarisPenanggungJawab` bertanda `json:"-"`, lalu dikirim ulang oleh `BarisAnalisisPenanggungJawab` dengan nama JSON yang sama (field luar menang atas field sematan). Keputusan user 2026-09-12 dan menyimpang sadar dari rencana yang semula menambahkannya ke `/beranda`; dikunci `TestFieldAnalisisPJTakTerkirimLewatBeranda`.
- ⚠️ **Cakupan laten: toko yang dipegang departemen saja.** Baris ber-`icc.status = ditetapkan` yang seluruh pemegangnya `asal: "tim"` (dari `department_shops`) dihitung ditetapkan, tak masuk `per_orang`, dan tak masuk `cakupan.toko_tanpa_penanggung_jawab`, sehingga kartu "toko tanpa pemegang" bisa menulis nol walau ada toko tanpa Account Specialist. Terukur PROD 2026-09-12 (level shop, 30 hari): 50 toko bertransaksi, **50 punya pemegang orang**, 0 dipegang departemen saja. Belum berdampak; ukur ulang sebelum mengandalkan kartunya.
...
| `conversion_rate` | Σ `sku_orders` ÷ Σ `product_clicks` × 100, **keduanya dari sesi yang kliknya terukur** | 70 · 2% |
| `avg_viewing_duration` | Σ(`avg_watch_sec` × `views`) ÷ Σ `views`, **penyebutnya views dari sesi yang durasi tontonnya terukur**; satuan **detik** | 20 · 60 detik |
| `add_to_cart_rate` | Σ `add_to_cart` ÷ Σ `product_clicks` × 100, **keduanya dari sesi yang klik DAN keranjangnya terukur** | 10 · 5% |
...
**Pembilang dan penyebut BERPASANGAN** (bip-erp #1840, di PROD sejak 2026-09-11: employee-service 15:32 lalu marketing-analytics 15:33 WIB, sesuai urutan di bawah). Tiap rasio dijumlah hanya dari sesi yang **kedua** sisinya terukur (`bahanTraffic.tambah`): klik adalah jangkar, jadi order dan keranjang dari sesi yang kliknya tak terukur tak ikut. Sisi yang terukur sendirian memberi pembilang tanpa penyebut, dan hasilnya angka yang masuk akal tapi salah tanpa satu pun galat. Terukur prod 2026-09-11: pada Agustus 2026, 71 sesi host punya klik tanpa data keranjang (pengambilnya baru jalan pertengahan bulan), sehingga `add_to_cart_rate` terbaca 3,31% padahal sesi yang terukur 6,24%; `carevolution.hub` punya keranjang tanpa klik karena toko non-utama tak membawa blok interaksi. Muatan `/kpi/kinerja-live` membawa pasangannya (`orders_berklik`, `klik_keranjang`, `views_bertonton`) plus `ada_klik_keranjang`, padanan `ada_klik` untuk `add_to_cart_rate`. ⚠️ Field `add_to_cart` di muatan kini hanya keranjang dari sesi yang kliknya terukur; `conversion` tetap cacahan seluruh order. Jalur individu memakai pembilang yang sama. Halaman Analisis Live, `/live-shifts/performa`, kartu Konversi halaman Live, dan `performa-host.ts` di frontend memakai pasangan yang sama, supaya tak ada dua angka konversi untuk sesi yang sama.

⚠️ **Urutan deploy: employee-service, lalu marketing-analytics, lalu frontend-hris.** Employee baru + marketing-analytics lama = perilaku lama, karena field pasangan di sisi employee bertipe pointer dan nil jatuh ke bahan lama. Marketing-analytics baru + employee lama meloloskan `add_to_cart_rate` yang penyebut keranjangnya kecil lewat penjaga 100 klik lama yang membaca seluruh klik. Frontend terakhir karena panel ICC dan halaman Analisis Live tak punya fallback ke rute lama. Skor yang sudah dibekukan tidak dihitung ulang ([[ADR - 0048 Skor KPI Otomatis Penuh Dibekukan Sistem]]); rasio berpasangan berlaku mulai periode yang masih terbuka.

`add_to_cart` **tidak ada di `shop_lives/performance`** — sumbernya endpoint terpisah `/analytics/202512/shop/{live_id}/products_performance`, satu panggilan per sesi saat sync, dan **hanya untuk sesi host** (148 panggilan/bulan, bukan ribuan). Fieldnya pointer: nil = belum pernah diambil, bukan nol orang memasukkan ke keranjang. ⚠️ **Per 2026-08-27 nol dari 2.365 sesi Agustus punya field ini** — kode pengambilnya baru merge dan sync belum berjalan sejak itu, sehingga `add_to_cart_rate` melaporkan 0% untuk keadaan yang sebenarnya belum terukur. ✅ Sesudah pemasangan (#1840) keadaan itu tak lagi terbaca 0%: klik tanpa data keranjang tak masuk penyebut, dan bila tak satu klik pun berkeranjang, `ada_klik_keranjang: false` membuat employee-service menggalatkan metriknya.
...
- **Lazada** nihil analytics sesi live sama sekali (`ReasonLazadaNoLive`, `envelope.go`) — itu yang benar-benar tidak ada.
...
Temuan intinya **sudah ada di kode sejak 2026-08-02**: `ReasonShopeeLiveUserAuth` (`envelope.go`) mencatat modul livestream Shopee ada di dok resmi, ber-`api_type: "User"`, dan panggilan `get_session_detail` dengan kredensial toko dibalas `error_param "There is no user_id in query"` — hanya saja dok ini tak pernah mengikutinya. ⚠️ Kalimat penutup konstanta itu dan [[ADR - 0086 Metrik Live Lintas Channel Digabung Satu Angka, Rincian Tetap per Channel]] (*"sampai dilakukan otorisasi akun streamer — keputusan bisnis"*) **sudah terbantah per 2026-09-10**: otorisasi toko biasa pada app Livestream menerbitkan `user_id`, dan 3 toko sudah terotorisasi (rincian di bawah). Penghalang yang tersisa bukan otorisasi, melainkan enumerasi sesi dan perpanjangan token.

Diukur ke backend dokumentasi resmi Shopee (`doc/module/?version=2`): **441 API**, di antaranya modul **`Livestream` berisi 25 endpoint** (`get_session_metric`, `get_session_detail`, `get_session_item_metric`, dst.) plus **3 endpoint livestream di BrandPortal** (`principal.get_shop_livestream_performance`, `get_session_livestream_performance`, `get_principal_livestream_performance`). ⚠️ Cache lokal `API Reference/Shopee Open API v2/Index.md` (367 endpoint dari SDK) **tidak memuat modul ini**, jadi pencarian yang berhenti di cache akan mengkonfirmasi klaim yang salah — tembak backend-nya. *(Versi pertama koreksi ini menulis 29; salah hitung, dibetulkan 2026-09-10.)*

Endpoint `/api/v2/livestream/*` menuntut ekor signature **`user_id`**, bukan `shop_id` — terverifikasi dua arah 2026-09-10 dengan app yang BENAR (partner `2038358`, kredensial toko Beautyhacks.id, kontrol positif `shop.get_shop_info` HTTP 200):

| Yang dicoba | Balasan | Artinya |
|---|---|---|
| ekor `shop_id` | `error_param` — *"There is no user_id in query."* | ditolak di pintu |
| **ekor `user_id`** | `error_data` — *"The session(session_id:N) is not belong to you"* | **otentikasi LOLOS**; ditolak di pemeriksaan kepemilikan data, karena `session_id`-nya karangan |

⛔ **Koreksi kedua (2026-09-10): `user_id` TIDAK menuntut alur otorisasi khusus.** Versi pertama koreksi ini menyimpulkan "modul menuntut otorisasi level akun" dari probe yang memakai **partner `2032314` (app ERP_SYSTEM)** — app yang salah, dan `error_param` yang sama akan muncul untuk app apa pun yang bukan app Livestream. Kenyataannya otorisasi **toko biasa** (`/api/v2/shop/auth_partner`) pada app Livestream menerbitkan **`shop_id` DAN `user_id` sekaligus**; `user_id` baru terbit saat kode ditukar, bukan di redirect. Terukur di prod: Beautyhacks Indonesia `shop 1811407105 / user 7894179133`, Beautyhacks.id `shop 898309034 / user 898362461`. Riwayat lengkapnya di [[Microservices - Integration Service]] § app keempat.

**Yang masih menghalangi sinkronisasi sesi Shopee ke `mart_live_sessions`** (TBD, belum dikerjakan):

- ⛔ **Tak ada cara mengenumerasi sesi milik sendiri, dan inilah penghalang yang tersisa.** Modul `livestream` (25 endpoint) tidak punya `get_session_list`; `create_session`/`start_session`/`end_session` untuk **menerbitkan** siaran, bukan membaca yang sudah lewat. Jadi endpoint hanya menjawab bila `session_id`-nya sudah diketahui. Yang bisa mengenumerasi hanya jalur `principal` (Brand Portal), yang menuntut `principal_id` — dan kedua kredensial prod memulangkan `principal_id_list` **kosong**.
- ⚠️ **`shopee_affiliate_content_performance` BUKAN sumber `session_id` itu**, walau ia menyimpan `content_id` ber-`channel: LiveStreaming` yang terlihat menjanjikan (judul siaran sungguhan, mis. `223885342` "monggo sekeca.aken belanja"). Diuji 2026-09-10: `get_session_metric` atas `content_id` itu dibalas *"is not belong to you"* — sesi tersebut milik **afiliator** yang mempromosikan produk toko, bukan siaran toko itu sendiri. Hipotesis yang wajar dan salah; dicatat supaya tak diulang.
- ⛔ **Semantik `orders` Shopee BERBEDA dari TikTok dan tidak boleh disamakan begitu saja.** Dok Shopee: *placed orders (**paid and unpaid**) during Livestream, **including cancelled orders***; `gmv` juga termasuk order batal. KPI Host Live memakai `sku_orders` TikTok = **dibayar saja**. Menyuapkan angka Shopee apa adanya ke metrik yang sama membuat host Shopee **sistematis terlihat lebih unggul** — tanpa galat dan tanpa test yang menangkapnya. Kelas yang sama dengan `iklan_sia_sia`. Aturan penjaganya sudah diputuskan [[ADR - 0086 Metrik Live Lintas Channel Digabung Satu Angka, Rincian Tetap per Channel]] §4: channel hanya boleh ikut digabung **setelah semantik field-nya diperiksa**, dan yang belum diperiksa **digalatkan**, bukan dilewati. Perbedaan `orders` di atas adalah temuan pemeriksaan semantik itu untuk Shopee.
- `conversion_rate` versi `principal` = orders ÷ **views**, sedangkan rumus KPI di dok ini = orders ÷ **product_clicks** (dikunci ADR 0086 §2). `livestream.get_session_metric` justru punya `co` (orders ÷ clicks) yang cocok persis.
- ✅ **Perpanjangan token yang merusak kredensial Livestream sudah diperbaiki** (terukur rusak 2026-09-10; bip-erp [#1829](https://github.com/bip-itteam-internal/bip-erp/pull/1829) diperpanjang lewat `user_id`, terverifikasi Beautyhacks.id 2026-09-11) — lihat [[Microservices - Integration Service]] § app keempat. ⚠️ Perpanjangan itu **satu arah**: token kredensial Livestream sesudah diperpanjang **tidak bisa** memanggil API toko, jadi konsumen live wajib memakai kredensial `LIVESTREAM`, sedangkan API toko tetap memakai `ERP_SYSTEM`.
- ⏸️ **Membuat sesi dari ERP (`create_session`) — diuji 2026-09-11, DITUNDA untuk pertimbangan.** Gagasannya: host menekan Mulai di MyBharata → ERP membuat sesi → `session_id` didapat tanpa input manual. Terukur di prod (Beautyhacks Indonesia, sesi uji `224243162`, `is_test=true`):
  - ✅ `upload_image` → `create_session` → `session_id` terbit; `get_session_detail` langsung memuat **`stream_url_list` berupa ARRAY 4 entri** (`push_url` RTMP + `push_key` + `domain_id` 1/22/23). ⚠️ Array, bukan objek: decode ke struct objek gagal senyap dan terbaca "alamat streaming kosong" — kekeliruan yang sempat dilaporkan.
  - ❌ Sesi itu **tidak muncul** di aplikasi Shopee maupun Shopee Seller (link `share_url` tetap membuka pratinjau di peramban). Sesi API dirancang menerima video lewat RTMP (OBS/aplikasi streaming), bukan dimulai dari kamera aplikasi Shopee. **Belum diuji:** apakah `is_test=false` mengubah visibilitasnya.
  - ⛔ **Sesi yang menggantung di status Initial memblokir host memulai live dari aplikasi** (dugaan kuat: host gagal mulai live selama sesi uji menggantung). `end_session` **menolak** sesi yang belum mulai (*"is not ongoing"*); satu-satunya cara menutupnya `start_session` (butuh `domain_id`) lalu `end_session` → status 2. Fitur apa pun yang membuat sesi wajib menjamin sesinya selalu ditutup.
  - ⚠️ `push_key` memuat rahasia stream (`speSecret`/`txSecret`, berlaku ±72 jam dari `speTime`) — jangan pernah dicetak/di-log.
  - Pilihan yang tersisa (belum diputuskan): **A.** MyBharata jadi aplikasi kamera yang mendorong RTMP sendiri + mengelola keranjang produk lewat API (besar; izin siaran baru terbukti saat video pertama dikirim); **B.** buktikan dulu `is_test=false` muncul di aplikasi Shopee, sebaiknya bersamaan dengan live sungguhan; **C.** host menempel link share live (`...share?session=<id>`) di MyBharata; **D.** Brand Portal (`principal`) yang bisa mengenumerasi sesi tanpa `session_id`. Semua jalur selain D masih perlu satu bukti: apakah `get_session_metric` bisa membaca sesi yang dibuat dari **aplikasi HP**.
...
### Setoran karya Live Support (`/live-support/*`, `/kpi/karya-live-support`)

🟡 **Merged 2026-09-17 17:52 WIB, belum deploy** (bip-erp [#1954](https://github.com/bip-itteam-internal/bip-erp/pull/1954) `feat/marketing-analytics-karya-live-support`). Register tema host live dan teaser yang disetor Live Support dan diputus penyetuju departemen penyetornya; bahan dua metrik KPI Live Support yang tak punya sumber lain. Keputusannya [[ADR - 0106 Tema dan Teaser Live Support Disetor dan Diputus Penyetuju Departemen sebagai Dasar KPI]]. Berkas: `karya_live_support.go` (aturan murni), `karya_live_support_store.go`, `karya_live_support_handler.go`, `penyetuju_departemen.go`, `kpi_karya_live_support.go`. Rute per baris: [[API - Marketing Analytics Service]] § Setoran karya Live Support.

**Gerbang dikerjakan di handler, bukan lewat peran.** Penyetor ditentukan JABATAN (`common.PosisiCocok(c, "Live Support")` atas header `BIP-Position`), peninjau ditentukan relasi penyetuju departemen. Tak satu pun terwakili `system_roles`; pemegang posisi Live Support di prod bahkan tak punya `system_roles` (terukur 2026-09-17).

| Aturan | Di mana ditegakkan |
|---|---|
| Identitas, nama, departemen, perusahaan, status, periode dicap server; body tak bisa memalsukannya | `handleSetorKarya` (test `TestSetorKaryaMencapIdentitasDariHeaderBukanBody`) |
| Periode = bulan setor pertama menurut WIB, tak ditulis ulang saat kirim ulang atau diputus | `periodeWIB`; `updateKirimUlang` tak menyentuh `periode`/`disetor_pada` |
| Toko wajib toko TikTok departemen penyetor (nama toko dari master, bukan body) | `tokoDipilih` lewat `sumberTokoMongo.TokoKarya`, pembaca `department_shops` bercache yang sama dengan penurunan channel sesi live; saringan `channel == TIKTOK` persis `tokoTikTokDepartemen` |
| Satu tautan teaser terhitung sekali per orang | indeks unik parsial `{company_id, employee_id, jenis, tautan_kunci}` `{jenis: teaser}`; tabrakan dikenali dari `keyPattern` (`duplikatTautanTeaser`) lalu **409** |
| Hanya pemilik yang memperbaiki, hanya setoran ditolak, jenis tak bisa diganti | `bolehSunting` (bukan pemilik → **404**, bukan 403) + filter `UpdateOne` `{_id, employee_id, status: ditolak}` |
| Peninjau = penyetuju departemen penyetor, bukan penyetor sendiri, hanya setoran menunggu | `bolehPutus` + filter `UpdateOne` `{_id, status: menunggu}` |
| Penyetuju tak bisa dipastikan (env kosong, transport, non-2xx, jawaban tanpa `employee_id`) | `penyetujuDepartemen` **gagal-tertutup** → **502**; kunci yang HILANG dibedakan dari string kosong lewat `json.RawMessage` |
| Field dokumen yang disimpan | **daftar-izin eksplisit** `dokumenKarya`, dikunci test refleksi `TestDokumenKaryaMemuatSeluruhField` (pelajaran `dokumenLiveShift`) |

**Antrean** (`GET /live-support/karya/antrean`) membaca setoran perusahaan pemanggil, menanyakan penyetuju **sekali per departemen**, lalu hanya meneruskan baris departemen yang penyetujunya pemanggil, atau seluruhnya bagi supervisor IT. Tiap baris membawa `boleh_putus` (penyetuju, bukan setoran sendiri, status menunggu); supervisor IT selalu `false`. Resolver galat untuk departemen mana pun → **502 seluruh antrean**, bukan antrean sebagian.
...
⚠️ **Merged 2026-09-18 01:04 WIB, di DEV, belum PROD** (bip-erp [#1966](https://github.com/bip-itteam-internal/bip-erp/pull/1966) `feat/marketing-analytics-monitoring-sesi-live`). DEV diverifikasi lewat gateway 01:15 WIB: 200 untuk kedua rute (`rows` kosong karena `live_shifts` dev nol dokumen), 400 tanpa rentang, 403 untuk akun non-pemantau, 404 untuk rute karangan, dan literal rute ada di biner container yang jalan. Layar web sesi berjalan dan riwayat per sesi untuk leader marketing dan jabatan Live Support, keputusannya [[ADR - 0108 Monitoring Sesi Live di Web Hanya Baca untuk Leader dan Live Support]]. Berkas `live_support_sesi.go`; rute per baris di [[API - Marketing Analytics Service]] § Monitoring sesi live.

**Rute baca baru, bukan memperluas `/live-shifts*`.** Gerbang `/live-shifts*` adalah middleware peran (`RequireLiveShiftUser`) yang menolak Live Support tanpa `system_roles`, dan gerbang yang sama menjaga Mulai, Jeda, Akhiri, serta Ambil alih. Store (`DaftarShift`, `ShiftBerjalanSemua`), pembaca sesi TikTok (`SesiRentang`), cache `department_shops` (`st.bacaTokoDepartemenBergalat`), `RingkasShiftBanyak`, dan `batasTutupShift` dipakai ulang apa adanya; tak ada query atau index baru.

| Aturan | Di mana ditegakkan |
|---|---|
| Identitas wajib, lalu leader (`common.IsMarketingLeader`) diperiksa LEBIH DULU dari jabatan (`common.PosisiCocok(c, posisiPenyetorKarya)`); selain keduanya **403** | `pemantauSesi` |
| Lingkup `departemen`: hanya sesi yang `shop_id`-nya termasuk toko `department_shops` departemen header (semua channel, dicocokkan persis), disaring SEBELUM diringkas | `tokoDepartemenSesi`, `lingkupPantau.memuat` |
| Lingkup `departemen` tanpa angka: `penjualan: null` dan sesi TikTok **tidak dibaca**; `department_shops` gagal dibaca → **503** (gagal-tertutup); departemen tanpa toko → 200 `toko_kosong: true` | `rakitBarisPantau`, handler |
| Lingkup `semua`: angka ikut (`gmv`, `orders`, `ada_data`, `porsi_host` tanpa `laba_kotor`); sesi TikTok gagal dibaca → **503**; nama toko gagal dibaca TIDAK menggagalkan (dicatat log, `shop_name` kosong) | handler |
| Status permintaan ambil alih diturunkan saat dibaca (`status_permintaan` dari `keadaanPermintaan`), karena `kedaluwarsa` dan `sesi_berakhir` tak pernah ditulis ke dokumen | `rakitBarisPantau` |
| `/berjalan` TIDAK menjalankan `tutupSesiYangHostnyaPulang` (beda dari `/live-shifts/berjalan`); baris membawa `batas_tutup_otomatis` dari `batasTutupShift` | `handleSesiBerjalanPantau` |

**Rentang baca sesi TikTok (leader)**: `/berjalan` dari `mulai` paling awal dikurangi 12 jam (`batasSesiMenggantung`) sampai sekarang; riwayat `dari - 12 jam` sampai `sampai (eksklusif) + 12 jam`, sama dengan riwayat lama. Keduanya dikunci test. ⚠️ Sesi yang menggantung berhari-hari memanjangkan pembacaan `/berjalan` di setiap pembaruan layar.

**Test** (`live_support_sesi_test.go`, lewat `app.Test`): gerbang per persona termasuk jabatan dan peran sah tanpa employee id, lingkup departemen (toko lain tak ikut, `"penjualan":null`, pembaca TikTok tak dipanggil), toko kosong dan galat, penjualan dan tumpang tindih leader, rentang baca, status permintaan turunan, tanpa tutup otomatis, rentang tanggal tak sah, dan rute tanpa prefiks modul. Lima penjaga terpenting dibuktikan merah lewat mutasi `go test -overlay`: pembuangan angka, penyaringan toko, penjaga identitas, rentang baca, dan status turunan.

### Pencatatan sesi live oleh host (`/live-shifts`)

> ⛔ **Klien pencatat sejak 2026-09-11 hanya MyBharata.** Halaman web Sesi Live Host (`/marketing-analytics/live-shift`) dihapus beserta menunya ([[ADR - 0091 Pencatatan Sesi Live Host Hanya di MyBharata, Halaman Web Dihapus]], erp-frontend [#1540](https://github.com/bip-itteam-internal/erp-frontend/pull/1540), merged 2026-09-11, di PROD sejak 2026-09-12 lewat image `frontend-hris` 07:35 WIB). Rute di bawah tidak berubah, dan `GET /live-shifts/performa` tetap dibaca Analisis Live dan panel ICC. Keterangan layar web di bagian-bagian berikut adalah riwayat. Layar web BACA sesi kembali ada lewat rute terpisah `/live-support/sesi*` (§ Monitoring sesi live hanya baca di atas, [[ADR - 0108 Monitoring Sesi Live di Web Hanya Baca untuk Leader dan Live Support]]), bukan lewat rute di bawah.

Enam route digerbang `common.RequireLiveShiftUser` (`live_shift_handler.go`); route ketujuh, `GET /live-shifts/performa` (bip-erp #1841, di PROD sejak 2026-09-11), digerbang `common.RequireMarketingLeader`. Fitur ini ada karena **atribusi live per orang mustahil dari API TikTok** — dibuktikan lewat panggilan nyata, bukan asumsi: `room_id` di Get Order Detail terbukti US-only (probe `code=0` untuk toko region ID mengembalikan field itu tidak ada), Product Stats API menuntut scope Creator yang tak tersedia di portal, dan `auto_combine_order_id` terkirim kosong pada seluruh 326.153 order. Jalur `affiliate_orders.content_id` = `session_id` **terbukti cocok** (407 dari 432, kontrol negatif VIDEO 0 dari 432) tetapi hanya mencakup **11,8% GMV** karena host penyumbang terbesar siaran atas nama toko, bukan sebagai afiliator.
...
| `GET /live-shifts/performa` | ✅ bip-erp #1841, di PROD sejak 2026-09-11. Kinerja per **orang** untuk seluruh tim, konsumen blok kinerja host halaman Analisis Live dan kartu Performa Tim Host Live ICC. Seluruh angka dari jalur KPI individu (`ringkasKinerjaIndividu` + `bungkusKinerjaIndividu`), tanpa rumus baru: jam = jam dinding union ([[ADR - 0063 Siaran Serentak Dicatat sebagai Sesi Terpisah per Akun]] §2), GMV & conversion = porsi orang itu, rasio berpasangan tak dibagi antar co-host. `gmv_per_jam` hanya dari shift yang tak perlu koreksi, dan jadi batas bawah selama ada shift tak terjodoh. Gerbang `RequireMarketingLeader` (staf IT termasuk, ADR 0063 §6): host biasa membaca riwayatnya sendiri, bukan kinerja rekannya. ⚠️ **Cakupannya beda dengan skor KPI**: skor membatasi shift ke toko TikTok departemen orang itu dalam periode KPI, rute ini seluruh toko pada rentang yang diminta, jadi angkanya sama persis hanya bila seluruh shift orang itu di toko departemennya dan rentangnya sama dengan periode KPI (`live_shift_performa.go`) |
...
> **Status per 2026-08-30**: yang sudah berjalan hanyalah **menampilkan** N sesi (web tayang prod, mobile selesai di branch). Kedua aturan yang menyentuh **uang** — jam dinding union dan tumpang tindih lintas akun yang sah — **belum dikerjakan sama sekali**, jadi dua konsumen frontend di bawah masih menggandakan jam sampai hari ini. **Pembaruan 2026-09-11**: jam dinding sudah dipakai jalur KPI sejak 2026-09-09, dan panel Performa Tim Host Live ICC pindah ke `GET /live-shifts/performa` (jam dinding, rumus jalur KPI individu) lewat erp-frontend #1530, di PROD sejak 2026-09-11 15:41 WIB. **Pembaruan kedua 2026-09-11** ([[ADR - 0091 Pencatatan Sesi Live Host Hanya di MyBharata, Halaman Web Dihapus]]): tampilan N sesi di web ikut hilang bersama halaman Sesi Live Host; di MyBharata tampilan itu merged ke `dev` lewat my-bharata #128 (2026-08-31).
...
#### `channel` pada `live_shifts` (2026-09-10)

`LiveShift.Channel` ada sejak [[ADR - 0086 Metrik Live Lintas Channel Digabung Satu Angka, Rincian Tetap per Channel]] §3, dan ikut ke **predikat penjodohan** `jodohkanSesiDenganPorsi`. Tanpanya satu shift menjodoh ke sesi dua platform yang kebetulan sama toko, akun, dan jendela waktunya; gagalnya senyap karena GMV, orders, klik produk, dan masuk-keranjang tiap sesi sekadar terjumlah dan angkanya tetap masuk akal. Dipasang **sebelum** sesi non-TikTok pertama masuk, karena sesudahnya ada periode yang angkanya sudah tercampur dan tak bisa dipisah lagi.

- ⛔ **DITURUNKAN dari `department_shops`, tidak diterima dari klien.** `channelPerToko` (`divisi.go`) memetakan `shop_id` → `channel`; `handleShiftMulai` memakainya lewat `sumberTokoMongo.ChannelPerToko`. `permintaanMulai` sengaja tak punya field `channel` — alasan yang sama dengan `dibuat_oleh`, `shift_id`, dan `status_jadwal` yang juga tak diterima dari body.
- ⛔ **Dua sebab gagal dibedakan: 400 vs 503.** Toko tak terpetakan → **400** dengan pesan yang menyebut **ICC Management**; `department_shops` tak terbaca → **503**. Karena itu jalur ini **tidak** memakai `bacaTokoDepartemenAman` yang sengaja menelan galat demi dropdown `/toko`; `bacaTokoDepartemenBergalat` diekstrak darinya supaya bacaannya tetap satu. Menyamakan keduanya jadi 400 menuduh master data atas gangguan infrastruktur, dan orang yang membacanya pergi membetulkan data yang sudah benar.
- **Kosong berarti `TIKTOK`** (`channelNormal`), dan normalisasinya dipakai di **kedua sisi** pembandingan. Aturan ini untuk dokumen yang ditulis sebelum field-nya ada, dan **sah hanya selama seluruh mart TikTok**. Terukur prod 2026-09-10: `mart_live_sessions` 6.363 dari 6.363 `TIKTOK`; `live_shifts` **0 dari 29** dokumen punya `channel`; `department_shops` 49 TIKTOK + 9 SHOPEE + 3 LAZADA; **21 dari 21** toko yang dipakai mart sudah terpetakan, jadi jalur 400 belum menyala untuk satu host pun.
- **SATU `sumberTokoMongo` dibagi `/toko` dan penurunan channel** (`routes.go`). Dua sumber berarti dua cache dengan TTL yang kedaluwarsa pada menit berbeda, sehingga toko bisa tampil di dropdown tetapi ditolak saat host menekan Mulai.
- ⚠️ **Index unik tetap `{akun_live}` saja, tanpa `channel`** — sengaja, lihat [[API - Marketing Analytics Service]] untuk resep migrasinya bila kelak ada handle yang dipakai serentak di dua platform.
- **Sisi frontend**: `dialog-mulai.tsx` sebelumnya hanya membaca HTTP status dan membuang `error` di body, sehingga pesan yang menyebut ICC Management tak pernah sampai ke mata host. Diperbaiki bersamaan; 400 dan 503 kini menampilkan pesan server apa adanya lewat daftar-izin status (500 tetap teks generik karena membawa `err.Error()` mentah). Dialog web itu dihapus 2026-09-11 ([[ADR - 0091 Pencatatan Sesi Live Host Hanya di MyBharata, Halaman Web Dihapus]]).
...
~~Konsumen yang MASIH menggandakan jam, dua-duanya di frontend: kartu metrik `halaman-live-shift.tsx` dan `ringkas-performa-host-live.ts` (layar tim ICC), keduanya menjumlahkan `durasi_efektif_detik` per baris tanpa dedup jendela waktu~~: **SUDAH TIDAK BERLAKU, dicoret 2026-09-11.** Panel ICC membaca `GET /live-shifts/performa` sejak erp-frontend #1530, dan `halaman-live-shift.tsx` dihapus bersama halaman web Sesi Live Host ([[ADR - 0091 Pencatatan Sesi Live Host Hanya di MyBharata, Halaman Web Dihapus]]). Di bip-erp sendiri **nol** konsumen `porsi_host`/`durasi_efektif_detik` di luar service ini.
...
#### Tutup otomatis, resolusi jadwal & pengingat (bergantung attendance)

Tiga perilaku sesi live yang **tidak** dipicu tombol host. Dua di antaranya menanyakan attendance, dan yang ketiga memakai hasil salah satunya:

| Perilaku | Pemicu | Rute attendance | Bila attendance gagal |
|---|---|---|---|
| **Resolusi jadwal**: `host[].shift_id`, `host[].status_jadwal` (`dalam_shift` / `luar_shift` / `tak_diketahui`), `jam_shift_berakhir` | `POST /live-shifts` (`resolusiJadwal`, `live_shift_jadwal.go`) | `GET /internal/jadwal-resolusi`, hari ini dan kemarin (shift lintas tengah malam) | turun ke `tak_diketahui`; sesi tetap tercatat |
| **Tutup otomatis** | tik 10 menit (`jalankanTutupOtomatis`) **dan** setiap `GET /live-shifts/berjalan` | `GET /internal/status-tap`, D-1 / D / D+1 dari hari mulai sesi | sesi tetap terbuka (arah aman) |
| **Pengingat** inbox "Sesi live belum diakhiri" | tik 10 menit (`prosesPengingat`) | tidak memanggil attendance; memakai `jam_shift_berakhir` hasil resolusi di atas | hanya pengingat jam ke-10 yang tersisa |

- **Jalur pertama, clock-out: sesi ditutup saat SELURUH hostnya sudah absen pulang.** Sampai 2026-09-11 16:55 WIB hanya jalur ini yang ada di PROD. `selesai` diisi jam clock-out **terakhir** di antara hostnya (`waktuTutupSesi`, `live_shift_autoclose.go`) dan `ditutup_otomatis: true`. Sesi **tetap terbuka** bila ada host yang belum absen pulang, tak punya entri absensi, atau clock-out-nya lebih awal dari `mulai` (milik shift sebelumnya). Sejak ADR 0088 §1 naik ke PROD (2026-09-11 16:55 WIB) penutupan ini ikut menulis `alasan_selesai: "clock_out"`.
- **Jam shift berakhir memicu pengingat** `shift_usai` 30 menit sesudahnya (teksnya menyebut jam tutup otomatis), lalu jalur kedua 60 menit sesudahnya; bila jadwalnya tak diketahui, yang tersisa pengingat `jam_10`. **Ambang 12 jam tidak menutup sesi**, ia hanya menandai `perlu_koreksi` (porsi GMV host 0) setiap kali riwayat dibaca. Tutup otomatis di jam berakhir shift + 60 menit ([[ADR - 0088 Ambil Alih Sesi Live oleh Host Terjadwal dan Tutup Otomatis Akhir Shift]] §1) **di PROD sejak 2026-09-11 16:55 WIB** (bip-erp #1847, erp-frontend #1539); cara kerjanya di §Ambil alih & tutup otomatis akhir shift di bawah.
- ⛔ **Kedua rute attendance menuntut header `BIP-Gateway-ID` DAN `?key=`.** Attendance memasang `validation.ValidateGateway` global sebelum rute internalnya, lalu `GerbangKunciAbsensi` memeriksa `ATTENDANCE_SERVICE_KEY` (rincian di [[Microservices - Attendance Service]]). Perakitnya satu tempat, `permintaanAttendance` (`attendance_permintaan.go`). `routes.InternalRequest` sengaja tak dipakai walau memasang header: ia tanpa context dan batas waktunya tetap 10 detik, sedangkan jalur lazy `/berjalan` memanggil attendance berurutan dengan batas 3 detik per panggilan. Galat transport dibersihkan dari URL (`common.GalatTransportAman`) karena URL itu memuat kunci layanan.
- ✅ **Perbaikan header ter-deploy di PROD 2026-09-11 11:30 WIB** (bip-erp [#1835](https://github.com/bip-itteam-internal/bip-erp/pull/1835), cukup `marketing-analytics-service`, tanpa env baru). Sebelumnya pemanggil hanya mengirim `?key=`, jadi setiap panggilan dibalas **401** sejak rutenya lahir (27 Agustus 2026): 51 dari 51 sesi 9 s.d. 11 Sep ber-`status_jadwal: tak_diketahui`, nol yang `ditutup_otomatis`, dan log berbunyi `status tap ... membalas 401`. ⚠️ **Keadaan per 2026-09-11 14:17 WIB, ukur ulang sebelum dipakai**: resolusi jadwal **terbukti hidup** (12 sesi sejak deploy, 14 baris host: 10 `dalam_shift`, 4 `luar_shift`, 0 `tak_diketahui`; `jam_shift_berakhir` terisi di tiap sesi yang punya host dalam shift, kosong di sesi yang seluruh host-nya luar shift). Tutup otomatis dan pengingat **belum pernah terpicu**, dan itu belum bukti gagal: sejak deploy belum satu pun host absen pulang dan belum ada sesi yang lewat jam berakhir shift + 30 menit. Bukti pertama keduanya baru bisa diukur sesudah shift A berakhir pukul 15:00.
- ⛔ **`perlu_koreksi` tidak punya jalur koreksi.** Tak ada rute maupun layar untuk mengubah `mulai`/`selesai`; satu-satunya jalan adalah tulis DB prod, dijalankan manusia. Karena `perlu_koreksi` dihitung ulang dari `selesai - mulai` setiap kali dibaca, cukup `selesai` yang dibetulkan. Beri jejak di dokumennya (mis. `koreksi_manual {oleh, pada, selesai_asli, alasan}`) supaya pembaca tahu kenapa `selesai` berbeda dari saat tombol ditekan; kode tak membacanya, dan store hanya memakai `$set` sehingga jejak itu tak terhapus. Siapa yang berwenang menyetujui koreksi yang menyentuh insentif **TBD**.

#### Ambil alih & tutup otomatis akhir shift (tutup otomatis ✅ PROD 2026-09-11, ambil alih ⚠️ PROD 2026-09-12, alur utuh belum diverifikasi; [[ADR - 0088 Ambil Alih Sesi Live oleh Host Terjadwal dan Tutup Otomatis Akhir Shift]])

> ✅ **Tutup otomatis akhir shift di PROD sejak 2026-09-11** (bip-erp #1847 sebagai `marketing-analytics-service` 16:55 WIB, erp-frontend #1539 sebagai `frontend-hris` 16:59 WIB). Gerbang biner (empat nama fungsi baru) dan bundel (label baru id/en) lolos dengan kontrol positif dan negatif; saat biner naik ada 4 sesi berjalan dan tak satu pun tertutup. ⚠️ **Per 2026-09-12 09:34 WIB, ukur ulang sebelum dipakai**: aturan ini belum pernah terpicu di PROD. Tak satu pun sesi ditutup otomatis sejak deploy, dan kelima sesi yang dimulai sesudah deploy diakhiri host sendiri paling lambat semenit sesudah jam berakhir shiftnya, jadi belum ada sesi yang menguji batas +60 menit. DEV belum diverifikasi. ⚠️ **Ambil alih di PROD sejak 2026-09-12 08:55 WIB** (bip-erp [#1855](https://github.com/bip-itteam-internal/bip-erp/pull/1855), merged 07:31 WIB): DEV terverifikasi lewat gateway 08:02 WIB, PROD lewat gerbang nama fungsi di biner dan `GET /live-shifts/ambil-alih/menunggu` 200 dari dalam container. Klien my-bharata [#143](https://github.com/bip-itteam-internal/my-bharata/pull/143) merged ke `dev` 07:34 WIB; rilis 1.17.0+161 belum diverifikasi. Alur utuh (minta, tolak, diam lalu tereksekusi) belum dijalankan di lingkungan mana pun. Kontrak rutenya di [[API - Marketing Analytics Service]] §Ambil alih sesi.

Menjawab sesi yang ditinggal terbuka oleh host yang tidak absen pulang: tutup otomatis di atas tak pernah menutupnya, dan host berikutnya tertahan 409 tanpa jalan keluar selain supervisor IT. Terjadi 10 ke 11 Sep 2026: shift pagi tertahan 07:00 sampai 09:11, sesi malam tercatat 13 jam 8 menit, dan porsinya hangus sampai dikoreksi lewat tulis DB. Diukur atas 40 pergantian host 9 s.d. 11 Sep: bila pemegang lupa dan jalannya hanya menunggu akhir shift, host berikutnya tertahan **median 239 menit**, karena serah terima umumnya terjadi di tengah shift pemegang.

- **Tutup otomatis kedua** (di PROD sejak 2026-09-11 16:55 WIB; `batasTutupShift` + `keputusanTutup` di `live_shift_autoclose.go`): sesi berjalan yang melewati jam berakhir shift + 60 menit (`jedaTutupSesudahShift`) ditutup dengan `selesai` **tepat di batas itu** (bukan jam tik), `ditutup_otomatis: true`, dan `alasan_selesai: "shift_berakhir"`. Aturan clock-out tetap berlaku dan menang bila lebih awal (seri jatuh ke clock-out).
  - ⛔ **Tidak menunggu attendance.** Loop lama melewati sesi yang tak mendapat jawaban attendance (`continue`), jadi attendance yang mati menahan SELURUH penutupan, persis selama 401 27 Agu s.d. 11 Sep 2026. Batas akhir shift kini tetap dievaluasi; dikunci `TestTutupSesiShiftBerakhirWalauAttendanceGagal`.
  - **Jam berakhirnya yang paling akhir** di antara host `dalam_shift` (`terapkanVonisJadwal`, `live_shift_jadwal.go`). PROD dan sesi yang dibuat sebelum branch ini naik menyimpan jam host pertama; nilainya tak dihitung ulang.
  - Sesi tanpa jam berakhir (seluruh host luar shift) tidak tersentuh; batas yang tidak sesudah `mulai` diabaikan. Jeda yang masih terbuka ditutup paling awal di mulainya sendiri, supaya intervalnya tak tersimpan terbalik.
  - Host diberi tahu lewat kategori `reminder`, hanya untuk sesi yang benar-benar ditutup permintaan itu; kirimannya di goroutine supaya `GET /berjalan` tak tertahan. Pengingat `shift_usai` 30 menit sebelumnya menyebut jam tutupnya, dirumuskan dari `batasTutupShift` yang sama.
  - Riwayat web memilih label badge dari `alasan_selesai` ("shift berakhir" atau "clock-out"); sesi tanpa alasan tetap "(clock-out)". Riwayat web itu dihapus 2026-09-11 bersama halamannya ([[ADR - 0091 Pencatatan Sesi Live Host Hanya di MyBharata, Halaman Web Dihapus]]); label serupa di MyBharata dikerjakan T4 ADR 0088 di my-bharata [#143](https://github.com/bip-itteam-internal/my-bharata/pull/143) (merged ke `dev` 2026-09-12, rilis belum diverifikasi, [[APP - MyBharata]] §Sesi Live Host).
  - Di sampel 9 s.d. 11 Sep aturan ini memotong **0 dari 45** sesi tertutup (molor terbesar 2,8 menit).
- **Ambil alih dengan persetujuan 30 detik** (⚠️ PROD sejak 2026-09-12, alur utuh belum diverifikasi; `live_shift_ambil_alih.go`). Keputusan perencanaan 2026-09-11 yang membentuk kodenya: permintaan disimpan **di dokumen sesi lama**, pemegang tahu lewat rute ringan yang dipoll, eksekusi karena diam dipicu pembacaan status oleh peminta, dan atasan tidak diberi tahu.
  - **Permintaan** (`POST /live-shifts/:id/ambil-alih`, muatan sama dengan Mulai): penekan wajib termasuk `host[]` sesi baru; toko diperiksa lebih dulu lewat `channelUntukToko` (satu fungsi dengan Mulai), supaya permintaan yang pasti gagal dirakit tak sempat menahan sesi pemegang; sesi harus masih berjalan dan akunnya sama dengan muatan; penekan yang sudah host sesi itu ditolak. Jadwal penekan diresolusi dengan `resolusiJadwal` yang sama dengan Mulai lewat `gerbangJadwalPeminta`: `dalam_shift` lolos, `luar_shift` 403, selainnya 503 (gagal-tertutup, pesan menyebut pengelola jadwal Host Live dan IT Supervisor). Permintaan ditulis ke `permintaan_ambil_alih` dengan filter `filterPermintaanBaru`, jadi dua peminta yang lolos pembacaan bersamaan tak sama-sama tersimpan; peminta yang sama mengulang dibalas permintaan lama, bukan batas baru.
  - **Keadaan** dihitung satu fungsi, `keadaanPermintaan`. `menunggu`, `ditolak`, dan `dijalankan` tersimpan; `sesi_berakhir` (sesi lama sudah tidak berjalan) dan `kedaluwarsa` **diturunkan saat dibaca**, tak pernah ditulis. Batasnya 30 detik menurut jam server (`batasPersetujuanAmbilAlih`); sesudahnya permintaan tetap menahan sesi selama **tenggang 20 detik** (`tenggangMenungguPeminta`), satu-satunya jendela saat pembacaan status oleh peminta boleh mengeksekusi karena pemegang diam. Lewat tenggang → `kedaluwarsa`, sesi pemegang tak disentuh.
  - **Jawaban**: Setujui dan Tolak hanya dari host sesi lama (`pemanggilAdalahHost`, **bukan** `bolehKelolaSesi`: pembuat sesi dan supervisor IT tidak ikut, karena persetujuan milik orang yang siarannya akan diputus) dan hanya sebelum batas. Eksekusi karena diam dipicu `GET /:id/ambil-alih` milik **peminta sendiri**, bukan timer server: timer tak membuktikan peminta masih menunggu, dan hilang saat service restart.
  - **Eksekusi** (`eksekusiAmbilAlih`): toko dan jadwal peminta diperiksa ulang **sebelum** menulis apa pun. Sesi lama ditutup pada detik eksekusi dengan filter `filterPermintaanMenunggu` (`selesai: null` + id permintaan + status menunggu), sehingga Akhiri, tutup otomatis, atau jawaban lain yang menang lebih dulu membuat eksekusi ini kalah, bukan menimpa. Jeda yang masih terbuka ikut ditutup dengan syarat **posisional** `jeda.N.selesai: null`, pola yang sama dengan tutup otomatis: snapshot sesi dibaca sebelum panggilan attendance (sampai sekitar 6 detik), dan tanpa syarat itu Jeda/Lanjutkan yang ditekan pemegang di celah tersebut tertimpa. Sesi peminta lalu disimpan pada instan yang sama lewat `rakitSesiBaru`, perakit yang juga dipakai Mulai. Jejak di sesi lama: `alasan_selesai: ambil_alih`, `ditutup_otomatis: false`, `diakhiri_oleh`/`diakhiri_oleh_nama`, `persetujuan_ambil_alih` (`disetujui`/`tak_dijawab`), `permintaan_ambil_alih.status: dijalankan` beserta `dijawab_*`, ditambah tulisan ketiga yang best-effort, `permintaan_ambil_alih.sesi_baru_id`.
  - ⚠️ **Dua tulisan, bukan satu transaksi.** Sesi lama ditutup lebih dulu karena index unik `{akun_live}` menolak sesi baru selama sesi lama berjalan. Bila sesi baru kalah balapan dari host lain (`errSesiBerjalanSudahAda`), peminta menerima 409 yang menyebut pemegang barunya; galat simpan lain dibalas 500 berkalimat "sesi lama sudah ditutup, tetapi sesi baru gagal disimpan: tekan Mulai lagi". Sesi lama tetap tertutup di kedua kasus.
  - **Notifikasi** kategori `reminder`, dikirim di goroutine dan best-effort: permintaan → seluruh host sesi lama; ditolak → peminta, menyebut penolaknya; dijalankan → seluruh host sesi lama, menyebut pengambil alih dan jamnya dalam WIB. **Atasan langsung tidak diberi tahu** (ditunda: resolver atasan per karyawan belum ada), jadi tak ada env baru.
  - **`GET /live-shifts/ambil-alih/menunggu`** dipoll halaman Sesi Live MyBharata tiap 3 detik, jadi sengaja **tanpa tutup otomatis dan tanpa attendance**, beda dari `/berjalan` yang memanggil status-tap per sesi.
- ⛔ **Bukan perluasan `bolehKelolaSesi`.** Jeda dan Akhiri sesi orang lain tetap hanya untuk host, pembuat, dan supervisor IT; ambil alih jalur terpisah bergerbang jadwal. Memasukkannya ke predikat itu mengubah arti `boleh_kelola` di `/berjalan`.
- ⛔ **Jangan menyerahkan akun dengan menambahkan pengganti ke `host[]`.** `host[]` berarti co-host: porsinya dibagi rata atas seluruh sesi, jadi pengganti menerima bagian dari jam sebelum ia datang. Jeda juga bukan jalan serah terima: ia tak melepas akun dan tak mengurangi porsi.
- ⚠️ **Kebenaran jadwal kini menentukan dua hal**: siapa bisa mengambil alih, dan kapan sesi tertutup. Host yang jadwalnya salah terkunci dari keduanya.

#### Memulai beberapa akun sekaligus (web 2026-08-30, dicabut 2026-09-11)

erp-frontend [#1325](https://github.com/bip-itteam-internal/erp-frontend/pull/1325). Dialog Mulai memilih **beberapa akun sekaligus lewat combobox ber-search**, termasuk akun dari **toko yang berbeda**, lalu menerbitkan N sesi dalam satu tindakan. ⛔ **Dialog web ini dihapus 2026-09-11** bersama halaman Sesi Live Host ([[ADR - 0091 Pencatatan Sesi Live Host Hanya di MyBharata, Halaman Web Dihapus]]). Per catatan 2026-08-30 MyBharata masih satu akun per dialog dengan `shop_id` diketik tangan; per 2026-09-11 `origin/dev` MyBharata sudah memanggil daftar toko dan `GET /live-shifts/akun`, sedangkan apakah beberapa akun bisa dimulai dalam satu dialog belum diperiksa.
...
~~Layar Sesi Live Host belum terdokumentasi di APP - Web ERP~~: **tak lagi relevan, dicoret 2026-09-11.** Layar webnya dihapus ([[ADR - 0091 Pencatatan Sesi Live Host Hanya di MyBharata, Halaman Web Dihapus]]); klien pencatatnya kini [[APP - MyBharata]].
...
**Terukur di PROD 2026-09-21**: dari 334 sesi, 66 tak terjodohkan, dan **29** di antaranya satu pola. Akun `carevolution.hub` dicatat di toko **Beautyhacks.co** (`7495537354419308702`) padahal akun itu berhenti bersiaran di sana sejak 2026-08-29; siarannya di **Beautyhacks.store** (`7495537364189547259`). Nol dari 29 punya shift kembar di toko yang benar, jadi itu catatan satu-satunya bagi Vani Nur Zakiyah, Kusmi Andrianti N, dan Riska Adelia Sari; Rp 912.676 GMV tak masuk ke siapa pun dan Rp 2.591.403 tercatat atas nama shift lain.

⛔ **Sebabnya bukan kelalaian host, melainkan nama toko yang tak bisa dibedakan.** Pemilih menampilkan `Beautyhacks.co` dan `Beautyhacks.store` berdampingan, dan dua shop_id lain memakai nama **identik** `Beautyhack's` (`401556928228` dan `7494710464840632749`). Menambah pelatihan tak akan menutup ini; yang menutupnya penjaga di sisi server, dan pembenahan master data namanya (belum diputuskan, **bukan** pekerjaan kode).

Tiga cabang, dan yang tengah alasan penjaga ini ada:

| Keadaan akun di `mart_live_sessions` | Hasil |
|---|---|
| Belum pernah bersiaran **di mana pun** | **LOLOS** — jalur "Akun Baru" MyBharata memang sah; menolaknya mematikan akun baru di hari pertamanya |
| Pernah bersiaran **di toko itu** | **LOLOS** |
| Dikenal, tetapi **di toko LAIN** | **400**, pesan menyebut nama toko yang benar + tanggal siaran terakhirnya |

**Gagal-terbuka, dan itu keputusan.** Mart tak terbaca atau resolver tak terpasang berarti penjaga dilewati dan permintaan tetap 201, bukan 503: gangguan Mongo yang memblokir seluruh host jauh lebih mahal daripada beberapa sesi yang perlu ditambal belakangan ([[ADR - 0101 Kesiapan Siaran Dicatat Host saat Mulai sebagai Dasar KPI Live Support]] §2).
...
Aturan **"nama toko = `$last` `entity_name` pada baris `level: shop` di `mart_profit_attribution`"** kini hidup di dua tempat: `pipelineDaftarToko` (`toko.go`, yang mengisi **pemilih**) dan `namaTokoUntukPesan` (`live_shift_akun_toko.go`, yang mengisi **pesan galat** penjaga di atas).

Dinaikkan ke dok ini karena aturannya sebelumnya hanya hidup sebagai komentar Go, dan yang merancang layar tak punya cara mengetahuinya. Bila salah satunya berubah, pesan galat akan menyebut nama yang **berbeda dari yang tertulis di pemilih**, sehingga host disuruh memilih toko yang tak ada di layarnya — persis kelas kegagalan yang penjaga itu dibuat untuk mencegah, cuma bergeser satu lapis.

Penyatuannya **tidak** dilakukan sekarang dan itu disengaja: pipeline `toko.go` menggabungkan filter tanggal dan `AdaData` dalam satu lintasan, jadi yang bisa dibagi cuma dua baris `$match`/`$group`, dan mengangkat abstraksi pada pemakai kedua mengunci bentuk yang belum tentu benar. Yang berlaku sampai ada pemakai ketiga: **siapa pun yang mengubah sumber nama toko wajib menyunting keduanya.**
...
- 🟡 **Narasi AI (irisan 2) belum dipasang**, jadi seluruh baris berstatus `menunggu`. FE wajib merender `ringkasan`, bukan penanda menunggu. Diukur 2026-09-25 ke `origin/main`: tak satu baris kode pun menulis `siap`/`gagal` maupun `Narasi`/`NarasiJejak`.
- 🟡 **Irisan 2 kini berbentuk KEPUTUSAN, bukan narasi** ([[ADR - 0127 Laporan Asisten Analisa Membawa Keputusan AI, Orang Menjalankan atau Menolak]], Diusulkan). Cara kerjanya yang sudah diputuskan: satu panggilan model per **kiriman** (bukan per analisa), dirakit **sebelum** notifikasi dikirim dengan batas waktu; model memilih dari katalog tertutup sepuluh tindakan (tabel ADR 0127 §3, termasuk dua tindakan kebersihan data `lengkapi_hpp` dan `tetapkan_penanggung_jawab`) yang **kelayakannya dihitung kode** dari ember yang sudah ada (`klasifikasiVideoBoros`, `pilihPenggerus`/`pilihPeluang`, kelompok Account Specialist, vonis `susunKeputusan`); sasaran wajib id entitas yang ada di hasil hitung dan disaring ulang seperti `RekomendasiScreening` recruitment. ⛔ **Aturan pemakaian yang wajib ikut ke validator**: `hentikan_iklan` tak pernah layak dari ember video "belum diketahui" (revenue level video hanya terisi 8,9%, nol = tak diketahui) maupun dari laba `belum_matang`; `roas` null = tak terhitung, bukan nol; retur sudah terpotong di settlement, jangan dikurangkan lagi. ⛔ Keputusan laba memakai jendela yang sudah matang, bukan jendela laporan: kiriman prod 2026-09-25 berjendela 2 hari mencetak laba -Rp212 juta dengan settlement cair 0% (ADR 0127 §3a). Gagal model → kiriman tetap jalan dengan `Ringkasan`, `narasi_status` = `gagal`. Jawaban jalankan/tolak per keputusan dicatat sebagai ukuran berhenti (ADR 0127 §9).
- ⚠️ **Klien AI belum punya env di service ini.** `AI_*` hanya dideklarasikan di blok recruitment compose; wajib ditambahkan ke blok marketing-analytics prod dan dev, dan deploy-nya `--force-recreate`.
...
| `mart_live_sessions` | `channel + session_id` | Sesi live hasil sync TikTok. **4.836 baris** per 2026-08-22. `username` di sini adalah **akun toko**, bukan orang — satu akun dipakai bergantian banyak host, dan itulah alasan `live_shifts` ada. Index baca **non-unik** `start_time` (bip-erp #1841; `start_time_1` terbukti ada di Mongo PROD 2026-09-11) untuk saringan rentang `/lives` dan `/lives/analisis`; 6.455 dokumen per 2026-09-11 (komentar `index.go`) |
| `live_shifts` | `akun_live` (unik **parsial**: hanya saat `selesai` null) | Catatan sesi live yang ditulis **host lewat tombol**, sengaja terpisah dari `mart_live_sessions` yang ditulis mesin. Index parsialnya menegakkan "satu akun hanya boleh punya satu sesi berjalan" di Mongo, bukan di kode yang bisa dilewati balapan cek-lalu-tulis. Terisi sejak awal September 2026 (51 sesi dicatat 9 s.d. 11 Sep saja, diukur 2026-09-11) |
| `live_support_karya` | `company_id + employee_id + jenis + tautan_kunci` (unik **parsial**: hanya `jenis: teaser`) | 🟡 Belum deploy (2026-09-17). Satu dokumen per tema atau teaser yang disetor Live Support, `_id` berprefiks `KL-` (tak menanam input pengguna). Index baca non-unik `{employee_id, periode}` (daftar milik + KPI) dan `{company_id, status, periode}` (antrean). Koleksi BARU, tak ada index lama yang perlu di-`dropIndex`. Lihat § Setoran karya Live Support |
...
- ⛔ **Urutan baris keluaran adalah urutan yang sudah ditetapkan sumbernya, dan rute ini TIDAK mengurutkan ulang sesudahnya.** Jalur laba mengurutkan di Go lewat `urutkanAgregat` (pemecah seri `video_id` selalu menaik) lalu memotong `limit`; jalur organik menyerahkannya ke `$sort` + `$limit` Mongo dengan pemecah seri `_id`. Yang memilih jalur adalah kunci urutnya sendiri (`sumberUtamaUrut`), jadi tiap `sort_by` yang sah selalu mendarat di koleksi yang benar-benar memiliki kolom itu: `views`/`gmv` ke deret harian, `revenue`/`ads_cost`/`gross_profit`/`hpp`/`hari_berdata` ke `mart_profit_attribution`. 🟡 **Diukur DILANGGAR di PROD 2026-09-15**: `gabungBarisPeriode` mengurutkan ulang keluarannya menurut `tiktok_item_id` di akhir, sehingga `sort_by=gross_profit&sort_dir=asc&limit=50` MEMILIH 50 video paling rugi dengan benar tetapi MENGEMBALIKANNYA urut ID (49 dari 49 pasangan menaik menurut ID, bukan menurut laba). Seleksi dan angkanya benar, yang salah cuma urutannya, jadi tak ada satu pun galat yang menyertainya; yang terlihat pengguna hanyalah blok "Perlu tindakan" yang tak pernah memberi vonis. Komentar lamanya menyatakan pengurutan tampilan dikerjakan Mongo dan masukannya map, dan keduanya sudah tak benar sejak urutan pindah ke Go. Perbaikan berikut test regresinya di bip-erp [#1907](https://github.com/bip-itteam-internal/bip-erp/pull/1907) (`555ad1c2`, merged 2026-09-16 lewat merge commit `8a1e5be0`), **belum di PROD per 2026-09-16**. Terverifikasi di DEV 2026-09-16 lewat gateway sungguhan (`/api/marketing-analytics/videos/periode`, rentang 60 hari, `sumber=gmv_max,campuran`, image container dibangun 46 detik sesudah merge): `sort_by=gross_profit&sort_dir=asc&limit=50` membalas 50 baris dengan NOL pasangan melanggar urutan (-145.008 menaik ke -34.119), dan kontrol arah sebaliknya `sort_by=revenue&sort_dir=desc` juga nol. Kontrol negatifnya ikut sehat: rute karangan 404, tanpa token 401.
- **Konsumen kedua: blok "Perlu tindakan" tab GMV Max** di [[APP - Web ERP]] (erp-frontend [#1583](https://github.com/bip-itteam-internal/erp-frontend/pull/1583), merged 2026-09-15, di PROD sejak 12:50 WIB hari itu; belum dicek dengan data sungguhan). Ia memanggil rute ini dengan `sort_by=gross_profit&sort_dir=asc&limit=50&sumber=gmv_max,campuran` untuk mengambil video paling rugi pada rentang halaman. Karena `gross_profit` kolom laba, populasinya datang dari `mart_profit_attribution` (aturan populasi di atas), dan `limit` dipotong sesudah pengurutan di Go. Blok itu bergantung pada urutan server: FE memeriksa urutannya sendiri dan tak memvonis "tak ada yang rugi" bila responsnya tak menaik. Mengubah arti urutan atau cara `limit` dipotong di rute ini mengubah isi blok itu.
...
### `gmv_live_tanpa_performa` vs `gmv_toko` di `/lives/analisis` (`analisis_live.go`)

⛔ **JANGAN menjumlahkan `gmv_live_tanpa_performa` ke `gmv_toko`, dan jangan menambahkannya ke pembilang porsi.** Ia GMV live TikTok dari toko/hari yang **tak punya** baris penjualan toko: rupiah yang justru tak punya penyebut. Menambahkannya mengembalikan porsi yang menggelembung, bahkan bisa melampaui 100%, tanpa galat. Layar menampilkannya sebagai catatan di samping porsi, bukan komponen. `gmv_toko` = Σ `gmv_amount` seluruh baris `tt_shop_performance_daily` rentang itu, termasuk toko/hari tanpa live, karena porsi menjawab "berapa bagian penjualan toko yang lahir dari live".

⛔ **`ringkasan.gmv` bukan pembilang porsi.** Ia menjumlah seluruh sesi seluruh channel pada saringan itu, sedangkan pembilang porsi hanya live TikTok yang toko + tanggalnya tercakup. Membagi `ringkasan.gmv` dengan `gmv_toko` di layar mengulang kesalahan yang dicegah di atas.

### `per_orang`, `retur`, dan laba matang di `/penanggung-jawab/analisis` (`analisis_penanggung_jawab.go`)

⛔ **JANGAN menjumlahkan kolom `per_orang` jadi total.** Toko yang dipegang lebih dari satu orang dihitung penuh untuk masing-masing, jadi Σ `per_orang.revenue` (dan kolom lain) melebihi kenyataan. Total yang benar ada di `ringkasan`, dirakit dari baris toko. Terukur PROD 2026-09-12: tak ada toko yang dipegang dua orang, jadi selisihnya belum terlihat, dan justru karena itu penjumlahan di layar akan lolos semua pemeriksaan sampai pemegang ganda pertama muncul. Tabel di layar sengaja tanpa baris total.

⛔ **`retur` adalah informasi, bukan pengurang laba.** Nilai retur sudah terpotong di settlement, jadi `gross_profit` sudah memuatnya (`retur_dobel_test.go`). Mengurangkan `retur` dari `gross_profit` atau `laba_matang` menghitung rupiah yang sama dua kali.

⛔ **`laba_matang` + `laba_belum_matang` = `gross_profit`**, per orang maupun di `ringkasan`: keduanya belahan, bukan komponen tambahan. Saat `matang=matang_saja`, baris yang belum cair tak terbaca di sumber, jadi `laba_belum_matang` bernilai 0 karena disembunyikan, bukan karena settlement-nya sudah cair.
...
- ~~Dua route ada di kode tetapi belum terdokumentasi: `/toko` dan `/profit/items`~~ — **SUDAH TIDAK BENAR, dicoret 2026-09-08.** Keduanya sudah punya baris di [[API - Marketing Analytics Service]] §Daftar acuan untuk saringan FE / §Laba sejak audit 2026-08-26 (lihat §Endpoint di atas: "Per 2026-08-26 tak ada route yang tanpa baris"). Baris ini bertahan salah lebih dari seminggu — kelas kesalahan yang sama dengan catatan `/matrix/sku-shop` di bawah.
- ⚠️ **`/toko` menggabungkan toko `department_shops` yang belum pernah punya baris mart** (fix 2026-09-08, branch `feat/marketing-analytics-toko-tanpa-data`) — ditandai `ada_data: false`, dibaca independen (bukan lewat `IndeksDivisi`) dan dicache 30 menit sendiri (kunci `"toko_departemen"`, terpisah dari cache `/divisi`). Dipicu kasus nyata: toko "GHunee" (Beauty Hacks, TikTok) sudah benar terotorisasi & dipetakan tapi tak pernah bertransaksi, sehingga hilang total dari dropdown — beda dari toko yang cuma sepi pada rentang terpilih (sudah tertangani sejak awal). Rincian: [[API - Marketing Analytics Service]] §Daftar acuan untuk saringan FE.
...
- ~~`live_shifts` masih 0 dokumen~~: **SUDAH TIDAK BENAR, dicoret 2026-09-11.** Terukur 51 sesi dicatat host 9 s.d. 11 Sep 2026 saja, di akun brand (glowbooster, carevolution, aphroditecosme), jadi fiturnya kini dipakai sungguhan dan GMV-nya sudah terjodoh ke sesi TikTok. Resolusi jadwal terbukti hidup sejak perbaikan header ter-deploy 2026-09-11 11:30 WIB; tutup otomatis dan pengingat belum terpicu per 14:17 WIB hari itu. Lihat §Tutup otomatis, resolusi jadwal & pengingat.
...
- ~~`luar_shift` belum bermakna, resolusi jadwal belum disambung~~: **SUDAH TIDAK BENAR, dicoret 2026-09-11.** Body `POST /live-shifts` tak lagi menerima `luar_shift` maupun `shift_id`; `status_jadwal` dan `shift_id` per host diisi server lewat `GET /internal/jadwal-resolusi` milik attendance, jadi urutan menang jadwalnya tetap diselesaikan attendance dan tak disalin ke service ini. Di PROD vonisnya hidup sejak perbaikan header gateway ter-deploy 2026-09-11 11:30 WIB (sebelumnya selalu `tak_diketahui`), lihat §Tutup otomatis, resolusi jadwal & pengingat.
- ⚠️ **Analisis Live, `/live-shifts/performa`, dan rasio live berpasangan sudah di PROD sejak 2026-09-11** (bip-erp #1840, #1841; erp-frontend #1530); rincian verifikasinya di § Analisis Live. Yang belum terbukti hanya pemanggilan lewat gateway PROD dengan akun sungguhan. ⚠️ Deploy frontend yang sama ikut menayangkan 9 PR frontend lain (#1526, #1527, #1528, #1529, #1531, #1532, #1533, #1535, #1537) sementara backend api-gateway, form-builder, integration, manufacture, procurement, recruitment, dan task-management belum di-build di PROD pada saat itu.
- ⚠️ **Ambil alih** ([[ADR - 0088 Ambil Alih Sesi Live oleh Host Terjadwal dan Tutup Otomatis Akhir Shift]] §2) **di PROD sejak 2026-09-12 08:55 WIB** (bip-erp [#1855](https://github.com/bip-itteam-internal/bip-erp/pull/1855); klien my-bharata [#143](https://github.com/bip-itteam-internal/my-bharata/pull/143) merged ke `dev`, rilis belum diverifikasi; alur utuh dengan dua host sungguhan belum dijalankan). Tutup otomatis akhir shift (§1) **di PROD sejak 2026-09-11 16:55 WIB** (bip-erp #1847, erp-frontend #1539), jadi sesi yang ditinggal host yang tak absen pulang kini tertutup sendiri satu jam sesudah shift berakhir. Sampai host memakai MyBharata 1.17.0+161, akun yang ditinggal sebelum batas itu tetap tertahan (409); sesi tanpa jam shift (host luar shift) tetap hanya tertutup lewat clock-out atau supervisor IT, dan host luar shift tidak bisa mengambil alih. Rincian di §Ambil alih & tutup otomatis akhir shift.
- ⛔ **Halaman web Sesi Live Host dihapus** (2026-09-11, [[ADR - 0091 Pencatatan Sesi Live Host Hanya di MyBharata, Halaman Web Dihapus]]; erp-frontend [#1540](https://github.com/bip-itteam-internal/erp-frontend/pull/1540), merged 2026-09-11, di PROD sejak 2026-09-12 lewat image `frontend-hris` 07:35 WIB): pencatatan hanya dari MyBharata, leader tak lagi punya riwayat per sesi seluruh tim di layar mana pun, dan tombol Ambil alih versi web (ADR 0088 T3) batal. `frontend-hris` sudah di PROD sejak 2026-09-12; tidak tercatat apakah prasyaratnya (host aktif sudah memakai build MyBharata yang memuat Sesi Live) dicek lebih dulu.
...
- **Sumber data (baca-saja)**: `integration_db` milik [[Microservices - Integration Service]] — `transaction_orders`, `product_sku_mappings`, `product_costs`, `tt_business_*`, `tt_shop_video_performances`, `tt_shop_video_performance_daily` (deret harian video, sumber kolom organik `/videos/periode`), `shopee_gms_*`, `affiliate_orders`, `icc_account_mappings`, `icc_affiliate_accounts`, `department_shops` (sejak fase Contract 2026-08-29, [[ADR - 0045 Identitas Tim Tunggal dan Peta Kepemilikan Marketing]] — dipakai TIGA konsumen: saringan `/divisi`, kolom `icc`/"Konteks per departemen" di bawah, DAN `/toko` (toko terdaftar tanpa data mart, fix 2026-09-08, cache terpisah dari `/divisi`)); plus `insentive_db` milik [[Microservices - Insentive Service]] (`employee_performance_mappings`, env `INSENTIVE_MONGO_URI`). Lihat juga [[Sales - Marketplace Integration]]. ✅ **`team_shops`/`marketing_teams` DICABUT TOTAL 2026-09-15** (Fase Contract) — pembaca dead code-nya (`bacaTokoTim`/`bacaNamaTim`) sudah dihapus fisik, bukan lagi sekadar tak terpanggil.
...
- **Panggilan HTTP ke service lain (Sesi Live Host)**: [[Microservices - Attendance Service]] `GET /internal/jadwal-resolusi` + `GET /internal/status-tap` (header `BIP-Gateway-ID` **dan** `?key=`; env `ATTENDANCE_MODULE_URL`, `ATTENDANCE_SERVICE_KEY`, `INTERNAL_GATEWAY_KEY`), serta notification-service `/inbox/send` untuk pengingat (env `NOTIFICATION_MODULE_URL`, `NOTIFICATION_SERVICE_KEY`). Env dibaca langsung, bukan lewat `InternalURL`, sehingga env kosong hanya mematikan fiturnya, bukan service-nya. Lihat §Tutup otomatis, resolusi jadwal & pengingat. ⚠️ Ambil alih (bip-erp #1855, PROD sejak 2026-09-12) memakai pengirim dan env yang sama dengan kategori `reminder` untuk tiga notifikasinya (permintaan, ditolak, dijalankan), jadi tidak menambah env maupun kategori inbox.
- **Panggilan HTTP ke service lain (setoran karya Live Support, 🟡 belum deploy)**: [[Microservices - Employee Service]] `GET /internal/department-approver?department=&company_id=` dengan header `BIP-Gateway-ID` dan `BIP-Employee-ID` **peninjau** (rute menolak 403 tanpa identitas), batas waktu 5 detik, env **baru** `EMPLOYEE_MODULE_URL` (compose prod dan dev; container wajib `--force-recreate`). Env kosong tidak mematikan service, tetapi antrean dan keputusan dibalas 502. Ini penyimpangan sadar dari [[ADR - 0101 Kesiapan Siaran Dicatat Host saat Mulai sebagai Dasar KPI Live Support]] §3, dicatat di [[ADR - 0106 Tema dan Teaser Live Support Disetor dan Diputus Penyetuju Departemen sebagai Dasar KPI]]. Notifikasi keputusannya lewat `/inbox/send` yang sama dengan kategori **baru** `live-support-karya-decided`, jadi marketing-analytics dan notification-service wajib naik bersama.
- **Gerbang**: [[CORE - API Master Gateway]] · **Konsumen**: dashboard marketing di [[APP - Web ERP]]; pencatatan Sesi Live Host (dan ambil alih sejak 1.17.0+161, my-bharata #143) di [[APP - MyBharata]]
...
- [[ADR - 0106 Tema dan Teaser Live Support Disetor dan Diputus Penyetuju Departemen sebagai Dasar KPI]] (setoran karya Live Support, penyetuju departemen via employee-service)
- [[ADR - 0063 Siaran Serentak Dicatat sebagai Sesi Terpisah per Akun]] · [[ADR - 0088 Ambil Alih Sesi Live oleh Host Terjadwal dan Tutup Otomatis Akhir Shift]] · [[ADR - 0091 Pencatatan Sesi Live Host Hanya di MyBharata, Halaman Web Dihapus]] (Sesi Live Host: siaran serentak, ambil alih, tutup otomatis akhir shift, klien hanya MyBharata)
