...
- **Tutup otomatis kedua** (di PROD sejak 2026-09-11 16:55 WIB; `batasTutupShift` + `keputusanTutup` di `live_shift_autoclose.go`): sesi berjalan yang melewati jam berakhir shift + 60 menit (`jedaTutupSesudahShift`) ditutup dengan `selesai` **tepat di batas itu** (bukan jam tik), `ditutup_otomatis: true`, dan `alasan_selesai: "shift_berakhir"`. Aturan clock-out tetap berlaku dan menang bila lebih awal (seri jatuh ke clock-out).
  ...
  - Riwayat web memilih label badge dari `alasan_selesai` ("shift berakhir" atau "clock-out"); sesi tanpa alasan tetap "(clock-out)". Riwayat web itu dihapus 2026-09-11 bersama halamannya ([[ADR - 0091 Pencatatan Sesi Live Host Hanya di MyBharata, Halaman Web Dihapus]]); label serupa di MyBharata termasuk T4 ADR 0088 dan belum dikerjakan.
  - Di sampel 9 s.d. 11 Sep aturan ini memotong **0 dari 45** sesi tertutup (molor terbesar 2,8 menit).
- **Ambil alih dengan persetujuan 30 detik**: dari penolakan Mulai, host yang jadwalnya sedang berjalan dan termasuk host sesi baru mengajukan permintaan (membawa id sesi yang ditampilkan, jadi pemegang yang sudah berganti dijawab 409 baru; resolusi jadwal gagal berarti **ditolak**). Pemegang menerima notifikasi dan, bila halaman Sesi Live MyBharata terbuka, jendela bawah otomatis berisi siapa yang meminta dengan tombol Setujui/Tolak dan hitungan mundur. **Setujui, atau tak dijawab 30 detik menurut jam server, berarti ambil alih berjalan** (yang terakhir hanya bila peminta masih menunggu); Tolak menggugurkan permintaan. Saat eksekusi gerbang diperiksa ulang, sesi lama ditutup **pada detik eksekusi** (tak bisa dimundurkan), dan sesi peminta dimulai pada instan yang sama; bersentuhan tepat tidak dihitung tumpang tindih. Jejaknya siapa yang mengakhiri, alasan `ambil_alih`, dan cara persetujuannya; host lama dan atasan langsungnya diberi tahu lewat kategori `reminder` yang sudah ada.
...
- 🟡 **Ambil alih** ([[ADR - 0088 Ambil Alih Sesi Live oleh Host Terjadwal dan Tutup Otomatis Akhir Shift]] §2) belum ada kodenya. Tutup otomatis akhir shift (§1) **di PROD sejak 2026-09-11 16:55 WIB** (bip-erp #1847, erp-frontend #1539), jadi sesi yang ditinggal host yang tak absen pulang kini tertutup sendiri satu jam sesudah shift berakhir. Yang tersisa: sebelum batas itu akunnya tetap tertahan (409) sampai ambil alih ada, dan sesi tanpa jam shift (host luar shift) tetap hanya tertutup lewat clock-out atau supervisor IT. Rincian di §Ambil alih & tutup otomatis akhir shift.
...
- **Panggilan HTTP ke service lain (Sesi Live Host)**: [[Microservices - Attendance Service]] `GET /internal/jadwal-resolusi` + `GET /internal/status-tap` (header `BIP-Gateway-ID` **dan** `?key=`; env `ATTENDANCE_MODULE_URL`, `ATTENDANCE_SERVICE_KEY`, `INTERNAL_GATEWAY_KEY`), serta notification-service `/inbox/send` untuk pengingat (env `NOTIFICATION_MODULE_URL`, `NOTIFICATION_SERVICE_KEY`). Env dibaca langsung, bukan lewat `InternalURL`, sehingga env kosong hanya mematikan fiturnya, bukan service-nya. Lihat §Tutup otomatis, resolusi jadwal & pengingat.
