...
- **Tutup otomatis kedua** (di PROD sejak 2026-09-11 16:55 WIB; `batasTutupShift` + `keputusanTutup` di `live_shift_autoclose.go`): sesi berjalan yang melewati jam berakhir shift + 60 menit (`jedaTutupSesudahShift`) ditutup dengan `selesai` **tepat di batas itu** (bukan jam tik), `ditutup_otomatis: true`, dan `alasan_selesai: "shift_berakhir"`. Aturan clock-out tetap berlaku dan menang bila lebih awal (seri jatuh ke clock-out).
  ...
  - Riwayat web memilih label badge dari `alasan_selesai` ("shift berakhir" atau "clock-out"); sesi tanpa alasan tetap "(clock-out)". Riwayat web itu dihapus 2026-09-11 bersama halamannya ([[ADR - 0091 Pencatatan Sesi Live Host Hanya di MyBharata, Halaman Web Dihapus]]); label serupa di MyBharata dikerjakan T4 ADR 0088 di branch my-bharata `feat/live-shift-ambil-alih` (belum merge, [[APP - MyBharata]] §Sesi Live Host).
  - Di sampel 9 s.d. 11 Sep aturan ini memotong **0 dari 45** sesi tertutup (molor terbesar 2,8 menit).
- **Ambil alih dengan persetujuan 30 detik** (⚠️ branch, belum merge; `live_shift_ambil_alih.go`). Keputusan perencanaan 2026-09-11 yang membentuk kodenya: permintaan disimpan **di dokumen sesi lama**, pemegang tahu lewat rute ringan yang dipoll, eksekusi karena diam dipicu pembacaan status oleh peminta, dan atasan tidak diberi tahu.
...
- ⚠️ **Ambil alih** ([[ADR - 0088 Ambil Alih Sesi Live oleh Host Terjadwal dan Tutup Otomatis Akhir Shift]] §2) **kodenya di branch, belum merge** (bip-erp `feat/marketing-analytics-ambil-alih`, klien my-bharata `feat/live-shift-ambil-alih`; per 2026-09-12 sudah direview, belum di-push, belum dipanggil lewat gateway DEV). Tutup otomatis akhir shift (§1) **di PROD sejak 2026-09-11 16:55 WIB** (bip-erp #1847, erp-frontend #1539), jadi sesi yang ditinggal host yang tak absen pulang kini tertutup sendiri satu jam sesudah shift berakhir. Yang tersisa sampai ambil alih naik: sebelum batas itu akunnya tetap tertahan (409), dan sesi tanpa jam shift (host luar shift) tetap hanya tertutup lewat clock-out atau supervisor IT. Rincian di §Ambil alih & tutup otomatis akhir shift.
...
- **Panggilan HTTP ke service lain (Sesi Live Host)**: [[Microservices - Attendance Service]] `GET /internal/jadwal-resolusi` + `GET /internal/status-tap` (header `BIP-Gateway-ID` **dan** `?key=`; env `ATTENDANCE_MODULE_URL`, `ATTENDANCE_SERVICE_KEY`, `INTERNAL_GATEWAY_KEY`), serta notification-service `/inbox/send` untuk pengingat (env `NOTIFICATION_MODULE_URL`, `NOTIFICATION_SERVICE_KEY`). Env dibaca langsung, bukan lewat `InternalURL`, sehingga env kosong hanya mematikan fiturnya, bukan service-nya. Lihat §Tutup otomatis, resolusi jadwal & pengingat. ⚠️ Branch ambil alih (belum merge) memakai pengirim dan env yang sama dengan kategori `reminder` untuk tiga notifikasinya (permintaan, ditolak, dijalankan), jadi tidak menambah env maupun kategori inbox.
- **Gerbang**: [[CORE - API Master Gateway]] · **Konsumen**: dashboard marketing di [[APP - Web ERP]]; pencatatan Sesi Live Host (dan ambil alih, di branch) di [[APP - MyBharata]]
