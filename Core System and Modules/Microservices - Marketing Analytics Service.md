...
- 🟡 **Irisan 2 kini berbentuk KEPUTUSAN, bukan narasi** ([[ADR - 0127 Laporan Asisten Analisa Membawa Keputusan AI, Orang Menjalankan atau Menolak]], Diusulkan). Cara kerjanya yang sudah diputuskan: satu panggilan model per **kiriman** (bukan per analisa), dirakit **sebelum** notifikasi dikirim dengan batas waktu; model memilih dari katalog enam tindakan (`hentikan_iklan`, `kurangi_belanja`, `naikkan_belanja`, `periksa`, `tunda_penilaian`, `pertahankan`) yang **kelayakannya dihitung kode** dari ember yang sudah ada (`klasifikasiVideoBoros`, `pilihPenggerus`/`pilihPeluang`, kelompok Account Specialist, vonis `susunKeputusan`); sasaran wajib id entitas yang ada di hasil hitung dan disaring ulang seperti `RekomendasiScreening` recruitment. ⛔ **Aturan pemakaian yang wajib ikut ke validator**: `hentikan_iklan` tak pernah layak dari ember video "belum diketahui" (revenue level video hanya terisi 8,9%, nol = tak diketahui) maupun dari laba `belum_matang`; `roas` null = tak terhitung, bukan nol; retur sudah terpotong di settlement, jangan dikurangkan lagi. Gagal model → kiriman tetap jalan dengan `Ringkasan`, `narasi_status` = `gagal`. Jawaban jalankan/tolak per keputusan dicatat sebagai ukuran berhenti (ADR 0127 §9).
