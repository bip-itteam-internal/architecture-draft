...
- **Jam shift berakhir hanya memicu pengingat** `shift_usai`, 30 menit sesudahnya; bila jadwalnya tak diketahui, yang tersisa pengingat `jam_10`. **Ambang 12 jam tidak menutup sesi**, ia hanya menandai `perlu_koreksi` (porsi GMV host 0) setiap kali riwayat dibaca.
...
- ⚠️ **Status 2026-09-11: di PROD ketiga perilaku ini belum pernah bekerja.** Pemanggil lama hanya mengirim `?key=`, jadi setiap panggilan dibalas **401** sejak rutenya lahir (27 Agustus 2026). Terukur: 51 dari 51 sesi 9 s.d. 11 Sep ber-`status_jadwal: tak_diketahui`, nol yang `ditutup_otomatis`, dan log berbunyi `status tap ... membalas 401`. Diperbaiki di branch `fix/marketing-analytics-header-gateway-attendance` (🟡 belum merged per 2026-09-11; deploy cukup `marketing-analytics-service`, tanpa env baru). Sesudah deploy, ketiganya menyala **untuk pertama kalinya** di prod, jadi ukur ulang sebelum mengandalkannya.
...
- ~~`live_shifts` masih 0 dokumen~~: **SUDAH TIDAK BENAR, dicoret 2026-09-11.** Terukur 51 sesi dicatat host 9 s.d. 11 Sep 2026 saja, di akun brand (glowbooster, carevolution, aphroditecosme), jadi fiturnya kini dipakai sungguhan dan GMV-nya sudah terjodoh ke sesi TikTok. Yang **belum** terbukti adalah tiga perilaku yang bergantung attendance, lihat §Tutup otomatis, resolusi jadwal & pengingat.
...
- ~~`luar_shift` belum bermakna, resolusi jadwal belum disambung~~: **SUDAH TIDAK BENAR, dicoret 2026-09-11.** Body `POST /live-shifts` tak lagi menerima `luar_shift` maupun `shift_id`; `status_jadwal` dan `shift_id` per host diisi server lewat `GET /internal/jadwal-resolusi` milik attendance, jadi urutan menang jadwalnya tetap diselesaikan attendance dan tak disalin ke service ini. Di PROD vonisnya masih selalu `tak_diketahui` sampai perbaikan header gateway ter-deploy, lihat §Tutup otomatis, resolusi jadwal & pengingat.
