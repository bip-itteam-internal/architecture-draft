...
- **Status**: ⚠️ Implemented dengan catatan (audit kode 2026-08-26). Berjalan di production dengan channel **TikTok + Shopee**, penjadwal internal 48 jam, dan **45 route** di berkas produksi `origin/main` (enumerasi ulang 2026-09-11, termasuk `/health`; audit 2026-08-26 menghitung 41; Analisis Live menambah dua lewat bip-erp #1841, merged dan di PROD 2026-09-11, jadi 47 saat merge; hitung ulang bila rute lain menyusul). `mart_live_sessions` kini **terisi 4.836 sesi** (verifikasi produksi 2026-08-22). Catatan: `mart_buyer_cohort` masih kosong, lock job hanya in-process. (`/matrix/sku-shop` **tidak lagi** stub — terimplementasi sejak 2026-08-02, 576 baris + 631 baris test; dok ini menyebutnya stub selama tiga minggu lebih.) **Pencatatan sesi live oleh host (`/live-shifts`) sudah live di PROD dan dipakai host sejak 2026-09-09** (catatan lama "0 dokumen" dicoret 2026-09-11, lihat § Belum Diimplementasikan). Sejak 2026-09-11 klien pencatatnya hanya MyBharata: halaman web Sesi Live Host dihapus ([[ADR - 0091 Pencatatan Sesi Live Host Hanya di MyBharata, Halaman Web Dihapus]]; erp-frontend [#1540](https://github.com/bip-itteam-internal/erp-frontend/pull/1540), merged 2026-09-11, belum di PROD).
...
> ⛔ **Klien pencatat sejak 2026-09-11 hanya MyBharata.** Halaman web Sesi Live Host (`/marketing-analytics/live-shift`) dihapus beserta menunya ([[ADR - 0091 Pencatatan Sesi Live Host Hanya di MyBharata, Halaman Web Dihapus]], erp-frontend [#1540](https://github.com/bip-itteam-internal/erp-frontend/pull/1540), merged 2026-09-11, belum di PROD). Rute di bawah tidak berubah, dan `GET /live-shifts/performa` tetap dibaca Analisis Live dan panel ICC. Keterangan layar web di bagian-bagian berikut adalah riwayat.
...
#### Ambil alih & tutup otomatis akhir shift (tutup otomatis ✅ PROD 2026-09-11, ambil alih ⚠️ PR bip-erp #1855, belum merge; [[ADR - 0088 Ambil Alih Sesi Live oleh Host Terjadwal dan Tutup Otomatis Akhir Shift]])

> ✅ **Tutup otomatis akhir shift di PROD sejak 2026-09-11** (bip-erp #1847 sebagai `marketing-analytics-service` 16:55 WIB, erp-frontend #1539 sebagai `frontend-hris` 16:59 WIB). Gerbang biner (empat nama fungsi baru) dan bundel (label baru id/en) lolos dengan kontrol positif dan negatif; saat biner naik ada 4 sesi berjalan dan tak satu pun tertutup. ⚠️ **Per 2026-09-11 17:25 WIB, ukur ulang sebelum dipakai**: bukti perilaku pertama baru bisa diukur 2026-09-12 00:30 WIB (pengingat) dan 01:00 WIB (tutup), dari dua sesi dalam shift yang berjalan saat deploy bila tidak diakhiri; DEV belum diverifikasi. ⚠️ **Ambil alih: kode selesai dan sudah direview di branch, belum merge** (2026-09-12): bip-erp PR [#1855](https://github.com/bip-itteam-internal/bip-erp/pull/1855) (7 commit, belum merge; merge dengan `origin/main` yang sudah maju 36 commit terbukti tanpa bentrok lewat `git merge-tree`), klien my-bharata PR [#143](https://github.com/bip-itteam-internal/my-bharata/pull/143) ke `dev`. Belum di DEV maupun PROD. Kontrak rutenya di [[API - Marketing Analytics Service]] §Ambil alih sesi.
...
- **Tutup otomatis kedua** (di PROD sejak 2026-09-11 16:55 WIB; `batasTutupShift` + `keputusanTutup` di `live_shift_autoclose.go`): sesi berjalan yang melewati jam berakhir shift + 60 menit (`jedaTutupSesudahShift`) ditutup dengan `selesai` **tepat di batas itu** (bukan jam tik), `ditutup_otomatis: true`, dan `alasan_selesai: "shift_berakhir"`. Aturan clock-out tetap berlaku dan menang bila lebih awal (seri jatuh ke clock-out).
  ...
  - Riwayat web memilih label badge dari `alasan_selesai` ("shift berakhir" atau "clock-out"); sesi tanpa alasan tetap "(clock-out)". Riwayat web itu dihapus 2026-09-11 bersama halamannya ([[ADR - 0091 Pencatatan Sesi Live Host Hanya di MyBharata, Halaman Web Dihapus]]); label serupa di MyBharata dikerjakan T4 ADR 0088 di my-bharata PR [#143](https://github.com/bip-itteam-internal/my-bharata/pull/143) (belum merge, [[APP - MyBharata]] §Sesi Live Host).
  - Di sampel 9 s.d. 11 Sep aturan ini memotong **0 dari 45** sesi tertutup (molor terbesar 2,8 menit).
- **Ambil alih dengan persetujuan 30 detik** (⚠️ PR bip-erp #1855, belum merge; `live_shift_ambil_alih.go`). Keputusan perencanaan 2026-09-11 yang membentuk kodenya: permintaan disimpan **di dokumen sesi lama**, pemegang tahu lewat rute ringan yang dipoll, eksekusi karena diam dipicu pembacaan status oleh peminta, dan atasan tidak diberi tahu.
...
- ⚠️ **Ambil alih** ([[ADR - 0088 Ambil Alih Sesi Live oleh Host Terjadwal dan Tutup Otomatis Akhir Shift]] §2) **kodenya di PR, belum merge** (bip-erp [#1855](https://github.com/bip-itteam-internal/bip-erp/pull/1855), klien my-bharata [#143](https://github.com/bip-itteam-internal/my-bharata/pull/143); per 2026-09-12 sudah direview, belum dipanggil lewat gateway DEV). Tutup otomatis akhir shift (§1) **di PROD sejak 2026-09-11 16:55 WIB** (bip-erp #1847, erp-frontend #1539), jadi sesi yang ditinggal host yang tak absen pulang kini tertutup sendiri satu jam sesudah shift berakhir. Yang tersisa sampai ambil alih naik: sebelum batas itu akunnya tetap tertahan (409), dan sesi tanpa jam shift (host luar shift) tetap hanya tertutup lewat clock-out atau supervisor IT. Rincian di §Ambil alih & tutup otomatis akhir shift.
- ⛔ **Halaman web Sesi Live Host dihapus** (2026-09-11, [[ADR - 0091 Pencatatan Sesi Live Host Hanya di MyBharata, Halaman Web Dihapus]]; erp-frontend [#1540](https://github.com/bip-itteam-internal/erp-frontend/pull/1540), merged 2026-09-11, belum di PROD): pencatatan hanya dari MyBharata, leader tak lagi punya riwayat per sesi seluruh tim di layar mana pun, dan tombol Ambil alih versi web (ADR 0088 T3) batal. Prasyarat deploy `frontend-hris`: host aktif sudah memakai build MyBharata yang memuat Sesi Live.
...
- **Panggilan HTTP ke service lain (Sesi Live Host)**: [[Microservices - Attendance Service]] `GET /internal/jadwal-resolusi` + `GET /internal/status-tap` (header `BIP-Gateway-ID` **dan** `?key=`; env `ATTENDANCE_MODULE_URL`, `ATTENDANCE_SERVICE_KEY`, `INTERNAL_GATEWAY_KEY`), serta notification-service `/inbox/send` untuk pengingat (env `NOTIFICATION_MODULE_URL`, `NOTIFICATION_SERVICE_KEY`). Env dibaca langsung, bukan lewat `InternalURL`, sehingga env kosong hanya mematikan fiturnya, bukan service-nya. Lihat §Tutup otomatis, resolusi jadwal & pengingat. ⚠️ PR ambil alih bip-erp #1855 (belum merge) memakai pengirim dan env yang sama dengan kategori `reminder` untuk tiga notifikasinya (permintaan, ditolak, dijalankan), jadi tidak menambah env maupun kategori inbox.
- **Gerbang**: [[CORE - API Master Gateway]] · **Konsumen**: dashboard marketing di [[APP - Web ERP]]; pencatatan Sesi Live Host (dan ambil alih, my-bharata PR #143) di [[APP - MyBharata]]
