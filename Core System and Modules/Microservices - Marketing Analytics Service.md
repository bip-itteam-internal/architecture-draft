...
- **Sesi ditutup saat SELURUH hostnya sudah absen pulang, BUKAN saat jam shift berakhir.** `selesai` diisi jam clock-out **terakhir** di antara hostnya (`waktuTutupSesi`, `live_shift_autoclose.go`) dan `ditutup_otomatis: true`. Sesi **tetap terbuka** bila ada host yang belum absen pulang, tak punya entri absensi, atau clock-out-nya lebih awal dari `mulai` (milik shift sebelumnya).
- **Jam shift berakhir hanya memicu pengingat** `shift_usai`, 30 menit sesudahnya; bila jadwalnya tak diketahui, yang tersisa pengingat `jam_10`. **Ambang 12 jam tidak menutup sesi**, ia hanya menandai `perlu_koreksi` (porsi GMV host 0) setiap kali riwayat dibaca. Tutup otomatis di jam berakhir shift + 60 menit **diusulkan** di [[ADR - 0088 Ambil Alih Sesi Live oleh Host Terjadwal dan Tutup Otomatis Akhir Shift]] (kode belum ada), lihat §Ambil alih & tutup otomatis akhir shift di bawah.
...
> 🟡 **Kode belum ada** per 2026-09-11. Bagian ini cara kerja yang sudah diputuskan; begitu naik ke prod, statusnya diganti dan rincian rutenya pindah ke [[API - Marketing Analytics Service]].
...
- **Tutup otomatis kedua**: sesi berjalan yang melewati jam berakhir shift + 60 menit ditutup dengan `selesai` **tepat di batas itu** (bukan jam tik) dan alasan `shift_berakhir`. Aturan clock-out tetap dan menang bila lebih awal. Jam berakhirnya yang **paling akhir** di antara host `dalam_shift`; sekarang `jam_shift_berakhir` diambil dari host pertama yang jadwalnya ketemu. Sesi tanpa jam berakhir (seluruh host luar shift) tidak tersentuh. Di sampel 9 s.d. 11 Sep aturan ini memotong **0 dari 45** sesi tertutup (molor terbesar 2,8 menit).
- **Ambil alih**: dari penolakan Mulai, host yang jadwalnya sedang berjalan dan termasuk host sesi baru menutup sesi lama **pada detik itu** (tak bisa dimundurkan), lalu sesinya dimulai pada instan yang sama; bersentuhan tepat tidak dihitung tumpang tindih. Permintaan membawa id sesi yang ditampilkan, jadi pemegang yang sudah berganti dijawab 409 baru. Resolusi jadwal gagal berarti **ditolak**. Jejaknya siapa yang mengakhiri dan alasan `ambil_alih`; host lama dan atasan langsungnya diberi tahu lewat kategori `reminder` yang sudah ada.
...
- 🟡 **Ambil alih & tutup otomatis akhir shift** ([[ADR - 0088 Ambil Alih Sesi Live oleh Host Terjadwal dan Tutup Otomatis Akhir Shift]]) diusulkan 2026-09-11, kode belum ada. Sampai naik, sesi yang ditinggal host yang tak absen pulang hanya bisa dilepas supervisor IT, dan lewat 12 jam porsinya hangus sampai dikoreksi lewat tulis DB. Rincian di §Ambil alih & tutup otomatis akhir shift.
