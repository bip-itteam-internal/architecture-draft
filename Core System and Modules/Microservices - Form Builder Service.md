...
>
> **Di prod yang terbukti baru sampai boot**, dan batas itu ditulis jujur di sini alih-alih diklaim setara dev: koleksi `form_type_rules` beserta index unik `(company_id, department)` terbentuk saat boot (hanya kode baru yang membuatnya), hitungan handler `61` sama persis dengan dev, log bersih, dan backfill senyap — benar, karena prod 0 dokumen `request`. **Alur berautentikasi belum dijalankan di prod** karena belum ada akun prod ber-`it:supervisor`/`it:admin` yang diketahui, dan pembuktiannya menuntut menulis satu baris aturan ke basis data produksi.
>
> Dua percobaan membuktikan rute lewat probe tanpa token **gagal memberi informasi**, dan keduanya ketahuan justru karena disertai kontrol negatif: lewat gateway, rute karangan pun dibalas `401`; langsung ke service, sama saja, karena penjaga kunci-gateway dipasang **global** sebelum routing. Kalau kontrol negatifnya dilewatkan, `401` pada rute baru akan terbaca sebagai "rute terdaftar" padahal tak membuktikan apa-apa.
