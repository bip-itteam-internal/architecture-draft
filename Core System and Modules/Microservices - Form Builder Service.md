...
**Analisa mengirim `file_fields`, terpisah dari `fields`** (⚠️ bip-erp [#1852](https://github.com/bip-itteam-internal/bip-erp/pull/1852) merged 2026-09-11, deploy belum diukur, belum diuji lewat gateway). `aggregateResponses` mengeluarkan berkas dari `fields` karena kartu ringkasan atas deretan id unggahan tak berguna, tetapi tanpa daftar kedua klien tak tahu pertanyaan mana yang berisi lampiran, dan tab Individu serta Pertanyaan yang mengulang `fields` tak pernah menampilkannya. `file_fields` `[{key,label}]` diambil dari susunan pertanyaan periode yang sama dengan `fields`, urut sesuai form, dan **absen** pada form tanpa berkas. Nilainya sendiri tetap dibaca dari `/forms/:id/responses` dan dibuka lewat `GET /forms/:id/uploads/:uploadId/preview`.
...
> **Status**: ⚠️ **T1+T2 merged 2026-09-11** (bip-erp PR [#1849](https://github.com/bip-itteam-internal/bip-erp/pull/1849)), **deploy belum diukur, belum diuji lewat gateway**. Editor web (sakelar penanda, field foto, lampiran di analitik) (T4: bip-erp #1852 merged, erp-frontend #1542 **belum merge**). Skor dan KPI dari cek ulang (T3), rekap web (T5), dan menu MyBharata (T6) belum dikerjakan. Keputusan dan alasannya: [[ADR - 0090 Inspeksi Satgas 5R dan K3 di Form Builder dengan Nilai dari Cek Ulang Terakhir]]. Daftar task: `Workspace/ANALISA - Inspeksi Satgas 5R dan K3.md`.
