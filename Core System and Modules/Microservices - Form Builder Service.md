...
>
> ⚠️ **Klien baru sebagian (diperiksa ke kode 2026-09-12).** Editor web kini bisa membuat field `file`, dan pengelola membuka lampirannya dari tab Individu dan Pertanyaan analitik (T4 ADR 0090: erp-frontend [#1542](https://github.com/bip-itteam-internal/erp-frontend/pull/1542) + bip-erp [#1852](https://github.com/bip-itteam-internal/bip-erp/pull/1852) merged dan **live dev** 2026-09-11, prod belum). MyBharata `origin/dev` masih tak bisa mengisinya (`survey_field.dart`, jatuh ke `unknown`); T6 my-bharata (PR draft [#144](https://github.com/bip-itteam-internal/my-bharata/pull/144), 2026-09-12) menambah tipe `file` dan `boolean`: foto kamera atau galeri dikompres di bawah 3,8 MB lalu diunggah lebih dulu, hanya lewat field `file`. Editor tetap **menolak pertanyaan berkas yang wajib** sampai PR draft erp-frontend [#1550](https://github.com/bip-itteam-internal/erp-frontend/pull/1550) di-merge sesudah adopsi versi T6 terukur, dan uji end-to-end di bawah tetap dijalankan langsung ke API. Tipe `report` yang mewajibkan field berkas masih tak bisa dibuat dari editor. Batas yang perlu diketahui perancang: satu field berisi satu `upload_id` (`validate.go`, cabang `FieldFile` di `validateAnswerValue`), jadi banyak berkas berarti beberapa field; jenis berkas tidak diperiksa server, dan objeknya tersimpan `application/octet-stream` sehingga PDF mungkin terunduh alih-alih tampil (belum diuji). Rencana klien: [[ADR - 0090 Inspeksi Satgas 5R dan K3 di Form Builder dengan Nilai dari Cek Ulang Terakhir]].
...
> **Status**: ⚠️ **T1+T2 merged 2026-09-11** (bip-erp PR [#1849](https://github.com/bip-itteam-internal/bip-erp/pull/1849)); biner form-builder dev memuatnya (diukur 2026-09-11), **employee-service dev belum diukur dan gerbang Satgas belum diuji lewat gateway**. Editor web (sakelar penanda, field foto, lampiran di analitik) merged dan **live dev** (T4, #1852 + #1542, 2026-09-11), prod belum. **T3** (nilai per orang, `GET /internal/satgas/metrics` + sumber KPI `nilai_inspeksi_satgas`, cek ulang lintas bulan) ⚠️ **selesai di branch 2026-09-12, belum PR, belum merge, belum deploy**: bip-erp `feat/satgas-nilai-kpi`, lalu `feat/satgas-cek-ulang` yang bertumpu padanya (§Nilai dan cek ulang lintas bulan di bawah). Rekap web (T5) belum dikerjakan; menu MyBharata (T6) ada di PR draft my-bharata [#144](https://github.com/bip-itteam-internal/my-bharata/pull/144). Keputusan dan alasannya: [[ADR - 0090 Inspeksi Satgas 5R dan K3 di Form Builder dengan Nilai dari Cek Ulang Terakhir]]. Daftar task: `Workspace/ANALISA - Inspeksi Satgas 5R dan K3.md`.
...
⚠️ Di branch T3 respons `/me/satgas` juga membawa kartu cek ulang bulan lalu, dan tiap kartu membawa `fields` serta `owner_department` (§Nilai dan cek ulang lintas bulan).

⚠️ **Di `main`, tab analitik "Yang Dinilai" masih merata-ratakan** (`overallOf`) seluruh jawaban form Satgas, jadi sampai T3 ter-merge dan ter-deploy analitik bukan tempat membaca nilai Satgas. Di branch T3 aturan itu sudah diganti (§Nilai dan cek ulang lintas bulan).
...
### Nilai dan cek ulang lintas bulan (T3, branch)

> **Status**: ⚠️ **Selesai di branch 2026-09-12, belum PR, belum merge, belum deploy, belum diuji lewat gateway.** bip-erp `feat/satgas-nilai-kpi` memuat aturan nilai, tab analitik, `GET /internal/satgas/metrics`, sumber KPI `nilai_inspeksi_satgas` di employee-service, dan env compose. `feat/satgas-cek-ulang` bertumpu padanya dan menambah cek ulang lintas bulan beserta kartunya di `/me/satgas`. Nomor PR belum ada. Seluruh isi bagian ini belum berlaku di `main`.
...
**Env `FORM_BUILDER_SERVICE_KEY`** (`common.Env.FormBuilderServiceKey`) ditulis di blok `form-builder-service` **dan** `employee-service` compose dengan nilai yang sama, terpisah dari `INTERNAL_GATEWAY_KEY`. Kosong di form-builder = rute menolak semua pemanggil; kosong di employee-service = metrik gagal hitung dengan pesan yang menyebut env itu; nilai tak sama = `401` yang pesannya menyebut env di kedua service. Env dibaca saat container dibuat, jadi menambahkannya menuntut `docker compose up -d --force-recreate form-builder-service employee-service`, bukan `restart`. Isi `.env` dev dan prod belum diukur.
