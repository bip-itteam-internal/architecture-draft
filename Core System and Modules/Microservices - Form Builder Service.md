...
### 🟡 Rencana: kehadiran via scan, rating dipisah, master program
Diusulkan 2026-09-09 (belum ada di kode) — [[ADR - 0084 Kehadiran Program Culture via Scan Menggantikan Feedback, plus Master Program]]. Perubahan inti: "hadir" bergeser dari **jumlah pengisi `culture_feedback`** menjadi **jumlah scan kehadiran fisik** (koleksi baru `culture_attendance`, index unik `(program_id, employee_id)`), sehingga `partisipasi` di `hitungSkorProgram` dihitung dari scan — `clamp100` & bobot 30/30/40 tetap. QR kehadiran = **token per-program yang berputar** (meniru alur Guestbook [[Microservices - Attendance Service]]) ditampilkan officer di [[APP - MyBharata]], peserta men-scan; walk-in di luar `target` tetap dihitung (clamp menahan dampak). Rating **dipisah** dari kehadiran dan didorong lewat `GET /culture/feedback/pending` (program yang di-hadiri tapi belum dinilai) + pop-up persisten di MyBharata. **Master program** baru (`master_culture_program`: `nama` + `pilar` + `pelaksanaan` harian/mingguan/bulanan/tahunan) mengisi form pembuatan program otomatis. Bentuk `GET /internal/culture/metrics` **tetap** ([[ADR - 0032 Kepemilikan kpi_score dan Batas Pengumpul Metrik]]). Daftar task: `Workspace/ANALISA - Program Culture Scan Kehadiran & Master Program.md`.

### ⚠️ Tipe program non-event + jadwal di master (T1 merged, T2–T5 branch)
[[ADR - 0093 Tipe Program Culture Non-Event Dinilai Terlaksana dengan Approval SPV HR, plus Jadwal di Master]] (2026-09-12). **Status kode**: T1 (field `tipe`) **merged ke `main`** (bip-erp PR #1861); **T2–T5** di branch `feat/culture-tipe`, belum merge/prod — jadwal master (`jadwal_hari`/`jadwal_tanggal`), koleksi baru **`culture_terlaksana`** + rute (`POST/GET /culture/programs/:id/terlaksana`, `GET /culture/terlaksana/pending`, `PUT /culture/terlaksana/:id`), dan cabang skor non-event. Kontrak rinci: [[API - Form Builder Service]]. ⚠️ **Notifikasi loop approval belum ada** (follow-up terpisah). Modul saat ini **event-centric**: tiap program di-resolve target-nya lalu dinilai dari **scan kehadiran + rating**, sehingga program budaya yang **bukan acara** (mis. **Kamis Batik**, tanpa daftar hadir) tak punya tempat — dipaksa dibuat, komposit-nya **0** dan menyeret turun rata-rata KPI officer (tak dilewati). Perubahan inti: field **`tipe` (event | non_event)** pada `CultureProgram` & `MasterCultureProgram` (default `event`, kompatibel mundur), sumbu **berbeda** dari `jenis` (resolve target) dan `pelaksanaan` (frekuensi). Non-event **melewati** resolusi target, scan/QR, dan rating; `tanggal` jadi opsional. Skor di `hitungSkorProgram` bercabang: non-event = **terlaksana-yang-disetujui SPV HR → 100, else 0**, masuk **array `komposit[]` yang sama** sehingga sumber KPI `program_culture` + reduksi `rata_rata` + target **tak diubah** (struktur KPI diatur SK, bukan ADR). Officer menandai terlaksana bebas kapan saja; **approval SPV HR** (gerbang `requireCultureManager`) menahan self-report — dua lapis bersama program-wajib-dari-master. `MasterCultureProgram` menyimpan **jadwal hari/tanggal** (arti bergantung `pelaksanaan`: mingguan→hari, bulanan→tanggal) yang **mem-pra-isi** `tanggal` saat program dibuat, tetap disesuaikan (acuan, **bukan** generator cron). ⚠️ Modul culture sudah **live di web ERP & MyBharata** (fitur mobile ter-commit), jadi build mobile perlu **guard** agar non-event tak muncul bisa-di-scan. Daftar task: `Workspace/ANALISA - Tipe Non-Event dan Jadwal Master Program Culture.md`.
...
>
> ⚠️ **Klien baru sebagian (diperiksa ke kode 2026-09-12; status PR diperbarui 2026-09-14).** Editor web kini bisa membuat field `file`, dan pengelola membuka lampirannya dari tab Individu dan Pertanyaan analitik (T4 ADR 0090: erp-frontend [#1542](https://github.com/bip-itteam-internal/erp-frontend/pull/1542) + bip-erp [#1852](https://github.com/bip-itteam-internal/bip-erp/pull/1852) merged dan **live dev** 2026-09-11, prod belum). Sebelum T6, MyBharata tak bisa mengisinya (`survey_field.dart`, jatuh ke `unknown`); T6 my-bharata (PR [#144](https://github.com/bip-itteam-internal/my-bharata/pull/144), merged ke `dev` 2026-09-14, uji perangkat belum) menambah tipe `file` dan `boolean`: foto kamera atau galeri dikompres di bawah 3,8 MB lalu diunggah lebih dulu, hanya lewat field `file`. Larangan editor atas **pertanyaan berkas yang wajib** dicabut lewat erp-frontend PR [#1550](https://github.com/bip-itteam-internal/erp-frontend/pull/1550), merged 2026-09-14 **sebelum** adopsi MyBharata 1.18.0 terukur (frontend dev dibangun manual hari itu karena pipeline tak me-rebuild-nya); uji end-to-end di bawah tetap dijalankan langsung ke API. Tipe `report` yang mewajibkan field berkas masih tak bisa dibuat dari editor. Batas yang perlu diketahui perancang: satu field berisi satu `upload_id` (`validate.go`, cabang `FieldFile` di `validateAnswerValue`), jadi banyak berkas berarti beberapa field; jenis berkas tidak diperiksa server, dan objeknya tersimpan `application/octet-stream` sehingga PDF mungkin terunduh alih-alih tampil (belum diuji). Rencana klien: [[ADR - 0090 Inspeksi Satgas 5R dan K3 di Form Builder dengan Nilai dari Cek Ulang Terakhir]].
...
**Analisa mengirim `file_fields`, terpisah dari `fields`** (bip-erp [#1852](https://github.com/bip-itteam-internal/bip-erp/pull/1852) merged; ✅ **terverifikasi lewat gateway dev** 2026-09-11: form uji berberkas membalas `file_fields [{foto_uji}]` dengan `fields` tanpa berkas, form tanpa berkas tak mengirim kuncinya; ⚠️ prod belum). `aggregateResponses` mengeluarkan berkas dari `fields` karena kartu ringkasan atas deretan id unggahan tak berguna, tetapi tanpa daftar kedua klien tak tahu pertanyaan mana yang berisi lampiran, dan tab Individu serta Pertanyaan yang mengulang `fields` tak pernah menampilkannya. `file_fields` `[{key,label}]` diambil dari susunan pertanyaan periode yang sama dengan `fields`, urut sesuai form, dan **absen** pada form tanpa berkas. Nilainya sendiri tetap dibaca dari `/forms/:id/responses` dan dibuka lewat `GET /forms/:id/uploads/:uploadId/preview`.
...
> **Status**: ⚠️ **T1+T2 merged 2026-09-11** (bip-erp PR [#1849](https://github.com/bip-itteam-internal/bip-erp/pull/1849)); biner form-builder dev memuatnya (diukur 2026-09-11), employee-service dev memuat kode T3 (diukur 2026-09-14), dan **gerbang Satgas diuji lewat gateway dev 2026-09-14** (tanpa paket kiriman `403` berpesan paket, berpaket `201`). Editor web (sakelar penanda, field foto, lampiran di analitik) merged dan **live dev** (T4, #1852 + #1542, 2026-09-11), prod belum. **T3** (nilai per orang, `GET /internal/satgas/metrics` + sumber KPI `nilai_inspeksi_satgas`, cek ulang lintas bulan) ⚠️ **merged 2026-09-14** (bip-erp PR [#1866](https://github.com/bip-itteam-internal/bip-erp/pull/1866) + [#1867](https://github.com/bip-itteam-internal/bip-erp/pull/1867)), dev memuatnya, **prod belum deploy** (§Nilai dan cek ulang lintas bulan di bawah). Rekap web (T5) belum dikerjakan; menu MyBharata (T6) merged ke `dev` lewat my-bharata PR [#144](https://github.com/bip-itteam-internal/my-bharata/pull/144) 2026-09-14, uji perangkat belum. Keputusan dan alasannya: [[ADR - 0090 Inspeksi Satgas 5R dan K3 di Form Builder dengan Nilai dari Cek Ulang Terakhir]]. Daftar task: `Workspace/ANALISA - Inspeksi Satgas 5R dan K3.md`.
...
⚠️ Sejak T3 respons `/me/satgas` juga membawa kartu cek ulang bulan lalu, dan tiap kartu membawa `fields` serta `owner_department` (§Nilai dan cek ulang lintas bulan).

⚠️ **Sebelum T3, tab analitik "Yang Dinilai" merata-ratakan** (`overallOf`) seluruh jawaban form Satgas, jadi sampai T3 ter-deploy analitik prod bukan tempat membaca nilai Satgas. T3 mengganti aturan itu (§Nilai dan cek ulang lintas bulan).
...
Rumus konversi skala 1-5 ke 0-100 **diputuskan 2026-09-12**: `(v-1)/4`, rumus sistem yang sudah ada (`normalisasiSkala`, `skor_gabungan.go:61`, membuat 3 = 50), bukan `v/5` praktik lembar HRD yang membuat 3 = 60 (ADR 0090 §8). T3 memakainya lewat `overallOf`, tanpa rumus kedua.

### Nilai dan cek ulang lintas bulan (T3)

> **Status**: ⚠️ **Merged 2026-09-14, dev memuatnya, prod belum deploy.** bip-erp PR [#1866](https://github.com/bip-itteam-internal/bip-erp/pull/1866) (`feat/satgas-nilai-kpi`) memuat aturan nilai, tab analitik, `GET /internal/satgas/metrics`, sumber KPI `nilai_inspeksi_satgas` di employee-service, dan env compose. PR [#1867](https://github.com/bip-itteam-internal/bip-erp/pull/1867) (`feat/satgas-cek-ulang`) bertumpu padanya dan menambah cek ulang lintas bulan beserta kartunya di `/me/satgas`. Keduanya merged 00:31 UTC; biner form-builder dan employee-service dev memuat kode T3 (diukur 2026-09-14).
>
> **Uji end-to-end dev 2026-09-14** (form Satgas uji, akun petugas uji berpaket sementara; data uji sudah dibersihkan): JWT karyawan biasa tanpa `key` ke `/internal/satgas/metrics` dibalas `401` `Unauthorized gateway`, dari dalam employee-service dengan kunci `200` berkontrak sempit; form terbit tanpa snapshot dan tanpa kiriman `has_form:false`; `/me/satgas` berpaket `allowed:true` dengan `fields`, `owner_department`, `period_key`; kiriman ber-`period_key` 2026-08 `409` "sudah ditutup 5 September 2026", 2026-13 `400`; unggahan tanpa berkas 2026-08 `409`, 2026-13 `400`, periode berjalan `400` berkas wajib; temuan skor 2 menghasilkan `nilai` 25, `menunggu_cek_ulang`, batas `2026-10-05T23:59:59+07:00`; cek ulang skor 5 menjadikannya `resolved`, analitik `overall` 100, `responses` 2, `subject_rule: "last_per_period"`, `nilai` 100; kiriman ulang identik dalam 2 menit `duplicate:true` dan tak tersimpan.
>
> **Belum teruji**: jendela 1-5 Oktober 2026 (kartu cek ulang lintas bulan, kiriman tercatat ke bulan temuan, `409` bagi orang yang sudah selesai atau tanpa temuan, dedup cek ulang lintas bulan).

**Aturan nilai tinggal di satu berkas** (`satgas_nilai.go`), dibaca status menu (`ringkasSatgas`), tab analitik (`rekapSasaranSatgas`), dan endpoint KPI. Pembaca yang menghitung sendiri akan menyimpang diam-diam begitu salah satu aturannya berubah, dan angkanya tetap tampak masuk akal.

- **Kiriman yang dihitung**: yang **terakhir** per orang per periode (`terakhirPerSasaran`, seri waktu dipecah ObjectID), bukan rata-rata, karena cek ulang menggantikan temuan. Status menu memakai pemilih yang sama, jadi menu dan KPI tak mungkin memilih kiriman berbeda.
- **Nilai satu kiriman**: `overallOf` atas kiriman itu saja (`nilaiKiriman`), jadi skala 1..5 menjadi (v-1)/4. Absen, bukan 0, bila kiriman terakhir tak menjawab pertanyaan skala yang ikut skor.
- **Gabungan antar form**: rata-rata nilai akhir tiap form yang sudah bernilai, satu form satu suara. Form yang belum menilai orang itu tak ikut dirata-rata, tapi tetap terhitung di `forms_total` supaya cakupannya terbaca jujur.
- **Kapan final**: kiriman terakhir berstatus `open_finding` belum final sampai `batasCekUlang`, yaitu tanggal **5 bulan berikutnya pukul 23:59:59** di zona `sekarang()` (compose memasang `TZ: Asia/Jakarta` untuk form-builder). Batasnya dihitung dari awal bulan periode, bukan `AddDate` dari tanggal lain, karena `AddDate(0, 1, 0)` pada 31 Januari menghasilkan 3 Maret. **Kecuali form sudah `closed`**: form yang ditutup tak menerima kiriman, jadi temuannya mustahil dicek ulang dan nilainya langsung final.
- **Form tanpa pertanyaan skala**: `validateSatgas` tak menuntut skala, jadi form semacam itu bisa terbit. Ia dilaporkan sebagai salah konfigurasi (`form_tanpa_skala`), bukan belum dinilai.

**Tab analitik "Yang Dinilai"**: `rekapSasaran` (`analytics.go`) memilih aturan dari penanda form, tak diwariskan diam-diam. Form Satgas memilih kiriman terakhir per orang per periode lebih dulu, lalu memakai agregasi yang sama dengan form lain: dengan `?period=` hasilnya nilai kiriman terakhir bulan itu, tanpa `?period=` rata-rata nilai akhir tiap bulan. Jumlah kiriman per orang tetap dari seluruh kiriman, karena temuan dan cek ulang sama-sama terjadi. Responsnya membawa `subject_rule: "last_per_period"` (absen pada form lain) supaya layar menyebut aturan itu alih-alih membiarkan angkanya terbaca sebagai rata-rata.

**`GET /internal/satgas/metrics?period=YYYY-MM`** (`satgas_metrics.go`) melaporkan nilai itu ke sumber KPI `nilai_inspeksi_satgas` di [[Microservices - Employee Service]] (`kpi_sumber_inspeksi_satgas.go`), yang dipasang HR di metrik Office Boy dan Security. Yang menulis `kpi_score` tetap employee-service ([[ADR - 0032 Kepemilikan kpi_score dan Batas Pengumpul Metrik]]).

- **Gerbang `gerbangKunciLayananKPI`**: `?key=` wajib sama dengan `FORM_BUILDER_SERVICE_KEY` (perbandingan constant-time). Header gateway bukan bukti, karena gateway memasang `BIP-Gateway-ID` pada setiap permintaan yang lolos JWT ([[ADR - 0031 Prefix internal Bukan Batas Keamanan]]) sementara muatannya nilai per orang. Header gateway saja, kunci salah, dan kunci gateway yang dipakai sebagai `key` sama-sama `401`. Env kosong **menutup** rute: env yang lupa dipasang saat deploy tak boleh berubah jadi pintu terbuka. `ValidateGateway` tetap berlaku seperti rute lain, jadi pemanggil mengirim keduanya.
- Sesudah gerbang: `period` kosong, mingguan, atau cacat dibalas `400` (tanpa nilai bawaan); database belum terhubung `500`, bukan panik yang terbaca 502. `company_id` dari header, bila kosong dari query.
- **Form yang dibaca** (`filterFormSatgasUntukNilai`): Satgas `published` **dan** `closed`, tak terhapus. Berbeda dari `/me/satgas`, form yang ditutup tetap dibaca supaya nilai bulan saat ia masih berjalan tak hilang. Form tanpa snapshot periode dan tanpa kiriman pada periode itu dilewati, sebab menghitungnya menurunkan cakupan orang yang dinilai lewat form penggantinya. Tak tersisa satu pun = `has_form:false`.
- Susunan pertanyaan dari **snapshot periode yang diminta**; gagal membacanya `500`, bukan jatuh ke susunan terbaru, karena nilai KPI dari pertanyaan yang salah tetap tampak wajar.
- **Batas baca 20.000** (`maxExportRows`). Pembacaan urut naik menurut `submitted_at`, jadi yang terpotong justru kiriman terbaru, tepat kiriman yang menentukan nilai. Kiriman satu form yang mencapai batas itu dibalas `500` dengan pesan menyebut judul form, dan employee-service meneruskan badan galat itu ke pesan metriknya.
- **Muatan sempit**: `{has_form, period_key, orang[{employee_id, nilai, forms_dinilai, forms_total, ada_kiriman, menunggu_cek_ulang, batas_cek_ulang, form_tanpa_skala}]}`, tanpa nama, departemen, jabatan, daftar form, atau rincian per form, supaya kunci yang bocor hanya memaparkan angka per `employee_id`. Rincian kaya tetap ada di `nilaiSatgasPeriode` untuk pembaca di dalam proses, dengan struct yang sengaja tanpa tag JSON. Penyusun muatannya fungsi murni `susunMuatanNilaiSatgas`; kontraknya dikunci literal `contohMuatanNilaiSatgas` di test kedua service, dan test employee-service memeriksa kedua salinan tetap sama.
- Di employee-service, `has_form:false`, belum ada kiriman, kiriman terakhir tanpa skor, dan `menunggu_cek_ulang` jatuh ke "belum dapat dihitung" (metrik itu saja kembali ke penilaian manual). Orang yang tak ada di muatan dan `form_tanpa_skala` dilaporkan sebagai galat konfigurasi.

**Env `FORM_BUILDER_SERVICE_KEY`** (`common.Env.FormBuilderServiceKey`) ditulis di blok `form-builder-service` **dan** `employee-service` compose dengan nilai yang sama, terpisah dari `INTERNAL_GATEWAY_KEY`. Kosong di form-builder = rute menolak semua pemanggil; kosong di employee-service = metrik gagal hitung dengan pesan yang menyebut env itu; nilai tak sama = `401` yang pesannya menyebut env di kedua service. Env dibaca saat container dibuat, jadi menambahkannya menuntut `docker compose up -d --force-recreate form-builder-service employee-service`, bukan `restart`. `.env` dev belum memuatnya saat merge; diisi 2026-09-14 lalu kedua container dibuat ulang `--force-recreate`. Isi `.env` prod belum diukur.

**Cek ulang lintas bulan** (`satgas_cek_ulang.go`). Kiriman cek ulang ditulis ke `period_key` **bulan temuan**, bukan periode yang sedang buka. Atribusinya disimpan di data, bukan dihitung ulang tiap pembaca, sehingga KPI, analitik, dan rekap cukup membaca `period_key` apa adanya. Satu keputusan, `periodeKiriman` (fungsi murni), dipakai `POST /me/forms/:id/responses` **dan** `POST /me/forms/:id/uploads` (field multipart `period_key`), supaya lampiran dan jawabannya tak mungkin jatuh ke periode berbeda (`cocokkanUnggahan` memakai `periodeWajibSama`):

- `period_key` kosong: periode berjalan, persis perilaku lama, jadi klien lama tak perlu berubah;
- periode berjalan: diterima bila putarannya buka;
- bulan lalu pada form Satgas bulanan sampai batas cek ulang (`periodeCekUlang`): diterima sebagai cek ulang, **termasuk di rentang mati awal bulan** saat putaran berjalan belum buka. Bulan lalu dihitung dari tanggal 1 bulan berjalan, karena `AddDate(0, -1, 0)` pada 31 Oktober menghasilkan 1 Oktober;
- bulan lampau pada form Satgas yang batasnya sudah lewat: `409` `Cek ulang temuan <bulan> sudah ditutup <tanggal>. Nilai bulan itu memakai kiriman terakhirnya.`;
- selain itu: `400` `period_key "<nilai>" tidak sah untuk form ini`.

Pada kiriman cek ulang, `submitResponse`:

- membaca snapshot pertanyaan bulan temuan lewat `findPeriod` **tanpa membuatnya**, karena snapshot itu lahir saat putarannya buka. Tak ada: validasi jatuh ke `fields` form. **Gagal dibaca: `500`**, berbeda dari `ensurePeriod` periode berjalan yang tak pernah menolak kiriman, karena kunci "Ada temuan?" yang memutuskan boleh tidaknya bulan itu ditulis dibaca dari susunannya, dan menebaknya dari susunan terbaru bisa membuka bulan lalu bagi orang yang temuannya sudah selesai. Kartu `/me/satgas` tetap jatuh ke susunan terbaru pada keadaan itu, karena ia hanya menawarkan;
- menjalankan **`cekUlangBerlaku`**, predikat KETAT atas seluruh kiriman atas orang itu di bulan temuan dari petugas siapa pun: hanya jawaban "Ada temuan?" `true` eksplisit pada kiriman terakhir yang membuka bulan itu untuk ditulis. Selain itu `409` berpesan: temuan sudah dinyatakan selesai (dengan arahan memuat ulang menu Satgas), orang itu tak punya temuan terbuka pada bulan itu, jawaban terakhirnya tak terbaca, atau susunan pertanyaan bulan itu tak punya tepat satu pertanyaan "Ada temuan?". Tanpa penjaga ini, kiriman yang tiba sesudah petugas lain menyelesaikan temuannya menimpa status "selesai" bulan itu, dan orang yang tak pernah bertemuan bisa dinilai ulang ke bulan yang sudah lewat;
- menjalankan penjaga itu **sesudah** penjaga idempotensi, supaya kiriman ulang identik dalam jendela 2 menit (balasan pertamanya hilang di jaringan) dibalas `duplicate:true`, bukan `409` "sudah dinyatakan selesai" yang dipicu kiriman petugas itu sendiri;
- membalas `201` dengan `period_key` tempat kiriman dicatat.

Unggahan tak membawa orang yang dinilai, jadi syarat "masih bertemuan" baru diperiksa saat jawaban dikirim.

**Dua sikap, satu pembaca jawaban.** Tampilan menu tetap konservatif (`statusDariKiriman`: jawaban tak terbaca tampil `open_finding`, supaya tak ada temuan yang tersembunyi). Jalur tulis memakai predikat ketat, sebab sikap konservatif di sana membuka pintu menulis ulang orang yang sudah selesai ke bulan yang sudah lewat hanya karena kunci pertanyaannya berganti atau jawabannya tak terbaca. Keduanya membaca jawaban lewat satu fungsi, `jawabanTemuan`.

**Kartu cek ulang di `GET /me/satgas`.** Sampai batas cek ulang, tiap form Satgas bulanan yang terbit bisa membawa entri untuk **bulan lalu**, diletakkan sebelum kartu putaran berjalan form yang sama (temuan yang batasnya dekat lebih mendesak) dan tetap tampil di rentang mati. Isinya `cek_ulang: true`, `period_key` bulan temuan, `batas_cek_ulang`, `fields` snapshot bulan temuan (jatuh ke susunan terbaru bila tak ada atau tak terbaca), `owner_department`, serta hanya orang yang cek ulangnya sah menurut `cekUlangBerlaku` yang sama dengan gerbang kiriman, supaya kartu tak pernah menawarkan orang yang lalu ditolak. Tanpa `opens_at`/`closes_at`. Tanpa orang yang sah, kartunya tak dikirim. Kartu putaran berjalan kini juga membawa `fields` dan `owner_department`, sehingga halaman isi di aplikasi tak bergantung pada `/me/forms`, yang menyembunyikan form di rentang mati.
