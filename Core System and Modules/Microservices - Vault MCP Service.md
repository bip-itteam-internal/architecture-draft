...
- **Status**: ⚠️ **Irisan 1 (baca) dan irisan 2 (tulis) sama-sama HIDUP di prod; irisan 3 belum ada kode.** Irisan 2 merged 2026-08-30 (PR [#1543](https://github.com/bip-itteam-internal/bip-erp/pull/1543), 155 test) dan di-deploy 2026-09-01. Diukur, bukan diingat: image `bip-erp-vault-mcp` dibangun 18:03, container dibuat ulang 18:50 sesudah `VAULT_MCP_AUTHOR_EMAIL` dan kredensial push dipasang di `.env` prod; log boot berbunyi `tool tulis aktif, author commit memakai bharataitteam@gmail.com`, dan `git push --dry-run` dari dalam container membalas `Everything up-to-date`. ⛔ **Aktif bukan berarti lunas.** Diukur ulang 2026-09-06, lima hari sesudah tulis diaktifkan: masih **nol commit** hasil tulisan lewat jalur ini, dan **nol baris log permintaan** dalam 16 jam sejak container terakhir dibuat. Sudah ada satu sesi ber-hak tulis sejak 2026-09-05, jadi yang menghalangi bukan lagi hak akses melainkan pemakaian. Angka adopsi sengaja hanya disimpan di satu tempat, [[RUN - Menyambungkan Claude ke Vault MCP]] § Keadaan adopsi, beserta perintah untuk mengukurnya ulang; menyalinnya ke sini akan melahirkan dua angka yang menyimpang. PR [#1488](https://github.com/bip-itteam-internal/bip-erp/pull/1488) dan [#1489](https://github.com/bip-itteam-internal/bip-erp/pull/1489) merged 2026-08-27. Diukur di prod 2026-08-30: container `Vault-MCP` sehat, `POST /mcp` membalas 401 ber-`WWW-Authenticate` yang benar (`scope="vault:read"`), kedua `.well-known` 200, `/oauth/authorize` menolak client tak dikenal dengan 400, sembilan `employee_id` terdaftar di `VAULT_MCP_ALLOWED_EMPLOYEES`, dan **tiga sesi sudah terbentuk di `mcp_sessions`** sehingga alur OAuth penuh terbukti pernah dilalui orang sungguhan.
...
| `write_note` | `vault:write` | ✅ aktif di prod 2026-09-01 | Buat dokumen baru atau timpa penuh |
| `patch_note` | `vault:write` | ✅ aktif di prod 2026-09-01 | Sunting satu bagian saja (tambah/ganti di bawah heading tertentu) |
...
Status per 2026-09-01, diukur bukan diingat:
...
- ⛔ **Deploy key TIDAK BISA DIPAKAI: fiturnya dimatikan di level organisasi `bip-itteam-internal`.** Halaman `Settings → Deploy keys` repo berbunyi *"Deploy keys are disabled for this repository"* dan *"Disabled by bip-itteam-internal"*, dengan saran GitHub memakai GitHub Apps. Berlaku untuk **seluruh** repo org, jadi jangan merancang service lain di sini dengan asumsi deploy key tersedia. ⚠️ `POST repos/.../keys` membalas **404**, dan 404 itu tak terbedakan dari "akun bukan admin repo"; yang menentukan hanya layar Settings. `user.name`/`user.email` committer sudah disiapkan di Dockerfile sejak irisan 1
- ✅ **`VAULT_MCP_REPO_URL` terisi https ber-kredensial**: fine-grained PAT milik akun `BIP-ITTeam`, hanya repo ini, `Contents: Read and write`, kedaluwarsa 2027-09-02. Bentuk SSH yang semula direncanakan gugur bersama deploy key. ⚠️ **Kedaluwarsanya mati SENYAP**: tulisan tetap jadi commit lokal dan tool melaporkannya apa adanya, tapi tak seorang pun melihat dokumennya berubah di GitHub. ⚠️ Kredensial yang **salah** pun tak bergejala di jalur baca, karena repo `architecture-draft` publik sehingga `pull` anonim tetap berhasil: placeholder `<username>:<PAT>` sempat hidup di prod tanpa satu pun tanda sampai `git push --dry-run` dicoba. Mengubah URL CUKUP lewat env; service menyelaraskan URL origin clone yang sudah ada saat boot, terbukti di log `URL origin berbeda dari VAULT_REPO_URL, diselaraskan`. ⛔ Konsekuensi urutan: **pasang kredensialnya dulu, baru ubah env.** Env yang menunjuk kredensial belum sah mematikan `pull` juga, dan jalur baca yang sedang hidup ikut menolak menjawab begitu lewat `VAULT_MCP_STALE_AFTER`
- ✅ **`VAULT_MCP_AUTHOR_EMAIL`** terisi `bharataitteam@gmail.com` ([[ADR - 0064 Author Commit Tunggal untuk Vault MCP]]). Kosong = tool tulis tidak didaftarkan sama sekali dan service tetap naik melayani jalur baca. Itulah yang terjadi 2026-08-30 sampai 2026-09-01, dan seluruh gejalanya cuma satu baris `WARN` dari `docker compose` plus satu baris log boot
- ⚠️ **Sesi lama harus menyambung ulang, dan ini BELUM dilakukan.** Scope berubah, dan `BolehTulis` fail-closed untuk sesi terbitan irisan 1, jadi tool tulis tidak akan muncul bagi mereka sampai connector-nya disambungkan ulang. Beri tahu pemakainya, jangan biarkan mereka menemukannya sebagai kegagalan
