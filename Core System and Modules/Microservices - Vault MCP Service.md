...
- **Status**: ⚠️ **Irisan 1 (baca saja) HIDUP di prod. Irisan 2 (tulis) MERGED ke `main` 2026-08-30 (PR [#1543](https://github.com/bip-itteam-internal/bip-erp/pull/1543), 155 test) tetapi BELUM di-deploy, dan deploy key berhak tulis belum ada. Irisan 3 belum ada kode.** ⛔ **Merged bukan berarti jalan**: `docker build` belum pernah dicoba, sembilan verifikasi hidup semuanya menunggu deploy key, dan sembilan pemakai di daftar-izin belum diberi tahu bahwa connector mereka harus disambung ulang. PR [#1488](https://github.com/bip-itteam-internal/bip-erp/pull/1488) dan [#1489](https://github.com/bip-itteam-internal/bip-erp/pull/1489) merged 2026-08-27. Diukur di prod 2026-08-30: container `Vault-MCP` sehat, `POST /mcp` membalas 401 ber-`WWW-Authenticate` yang benar (`scope="vault:read"`), kedua `.well-known` 200, `/oauth/authorize` menolak client tak dikenal dengan 400, sembilan `employee_id` terdaftar di `VAULT_MCP_ALLOWED_EMPLOYEES`, dan **tiga sesi sudah terbentuk di `mcp_sessions`** sehingga alur OAuth penuh terbukti pernah dilalui orang sungguhan.
