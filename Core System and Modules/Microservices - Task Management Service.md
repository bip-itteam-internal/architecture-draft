...
> Status: **kode di `main`** (BE PR [#1504](https://github.com/bip-itteam-internal/bip-erp/pull/1504) `feat/engagement-assign`, FE PR [#1287](https://github.com/bip-itteam-internal/erp-frontend/pull/1287)); **belum diverifikasi lewat gateway** dev maupun prod. ⛔ **Alurnya belum bisa dipakai utuh** — audit 2026-08-29 menemukan tiga cacat yang masing-masing sendirian cukup menghentikan alurnya, dirinci di [[Sales - Engagement Team (Modul)]] § Cacat yang Diketahui. Konsep bisnisnya di dokumen yang sama; kontrak endpoint di [[API - Task Management Service]].
...
- **State machine sendiri** (`engagement_state.go`), lima status tetap di kode — bukan stage space yang dinamis:
...
| `IN_PROGRESS` → `DONE_BY_TEAM` | pengerja | wajib ada lampiran bukti |
...
- ⛔ **Tak ada penyaringan departemen/space di jalur pemakai.** `antrianEngagement` dan `dashboardEngagement` menghitung seluruh koleksi; `detailTiket`/`riwayatTiket` tak memeriksa keterkaitan pemanggil dengan tiket. Gerbangnya cuma `requireRoles("staff","supervisor","admin")` = setiap pemakai ERP. Standarnya sudah ada di service ini (`division_scope.go`) dan tidak dipakai modul ini sama sekali. 🟡 **Aturan penggantinya sudah diputuskan** di [[ADR - 0060 Cakupan Keterlihatan Tiket Engagement]] — field `requester_department` + penyaringan berbasis keterkaitan + migrasi tiket lama; **belum ada di kode**.
...
- **Modul Engagement Tim: tiga cacat yang menghentikan alurnya** (audit 2026-08-29) — (a) tak ada rute unggah bukti sementara `done` mewajibkan lampiran, sehingga `DONE_BY_TEAM`/`CLOSED` tak pernah tercapai dan KPI-nya selalu nol; (b) `pekerjaanSaya` menyaring `claimed_by`, field model lama yang tak pernah ditulis, jadi tab "Pekerjaan Saya" selalu kosong dengan `200`; (c) `assigned_name` tak pernah diisi sehingga kolom Pengerja menampilkan `employee_id` mentah. Rincian + usulan perbaikan: [[Sales - Engagement Team (Modul)]].
- **Modul Engagement Tim: keterlihatan tiket belum dibatasi sama sekali** — `EngagementTicket` tak menyimpan `department`, dan antrian/dashboard/detail tak menyaring apa pun. Menutupnya menuntut field baru + migrasi, dan cakupannya keputusan SPV (TBD).
