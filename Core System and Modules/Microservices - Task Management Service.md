...
### Modul Engagement Tim ⚠️

> Status: **kode di `main`** (BE PR [#1504](https://github.com/bip-itteam-internal/bip-erp/pull/1504) `feat/engagement-assign`, FE PR [#1287](https://github.com/bip-itteam-internal/erp-frontend/pull/1287)); **belum diverifikasi lewat gateway** dev maupun prod. Audit 2026-08-29 menemukan tiga cacat yang masing-masing sendirian cukup menghentikan alurnya; ketiganya **sudah diperbaiki** (dirinci di [[Sales - Engagement Team (Modul)]] § Cacat yang Diketahui). Satu cacat lain ditemukan 2026-09-21: `volume_realisasi` tak punya penulis sehingga metrik Quantity selalu 0, diperbaiki di branch `feat/engagement-realisasi-per-jenis` (**belum merge, belum deploy**, [[ADR - 0117 Realisasi Engagement Dilaporkan Pengerja per URL per Jenis Pekerjaan sampai Akhir Bulan KPI]]). Konsep bisnisnya di dokumen yang sama; kontrak endpoint di [[API - Task Management Service]].

Modul kedua di service ini: tiket **boosting media sosial** dari Account Specialist ke tim Engagement (dulu bernama *Buzzer*). Berbagi proses, notifikasi, scheduler, dan file-service dengan tiket IT, tetapi **datanya terpisah total**.

- **Koleksi sendiri**: `engagement_tickets` · `engagement_ticket_items` (baris target URL, terpisah karena `volume_realisasi` diperbarui per baris) · `engagement_logs` (audit trail, sekaligus riwayat penugasan). Alasan pemisahan: [[ADR - 0058 Tiket Engagement Memakai Koleksi dan State Machine Sendiri]].
- **State machine sendiri** (`engagement_state.go`), enam status tetap di kode (`ANTRE` di depan `OPEN` untuk request yang melewati plafon 5 per hari; ia belum punya pengerja dan hanya keluar lewat promosi otomatis terjadwal, bukan lewat tabel transisi) — bukan stage space yang dinamis:

| Dari → Ke | Peran yang boleh | Catatan |
|---|---|---|
| `OPEN` → `IN_PROGRESS` | pengerja | menandai mulai; **opsional** dalam alur |
| `OPEN` → `DONE_BY_TEAM` | pengerja | boleh langsung, tanpa menekan "mulai" |
| `IN_PROGRESS` → `DONE_BY_TEAM` | pengerja | bukti pengerjaan **opsional** (keputusan SPV) |
| `DONE_BY_TEAM` → `CLOSED` | requester | verifikasi diterima |
| `DONE_BY_TEAM` → `IN_PROGRESS` | requester | revisi; `assigned_to` **tetap** orang yang sama |
| `IN_PROGRESS` → `OPEN` | requester, admin | penugasan ulang |
| `OPEN`/`IN_PROGRESS` → `CANCELLED` | requester, admin | `DONE_BY_TEAM` **tak bisa** dibatalkan |
| `CLOSED` → `IN_PROGRESS` | admin (admin/supervisor/leader) | buka kembali; satu-satunya jalan keluar dari `CLOSED`, `CANCELLED` tetap terminal |

- **Peran = relasi terhadap tiket, bukan jabatan.** `pengerja` = `assigned_to`, `requester` = `requester_id`, `admin` = tier admin/supervisor. Orang yang sama bisa jadi requester di satu tiket dan pengerja di tiket lain.
- `bolehTransisi` adalah **satu-satunya** gerbang perpindahan status, dipanggil handler **sebelum** menulis. Daftar peran per transisi adalah setengah dari gerbangnya: tanpa itu pengerja bisa menutup tiketnya sendiri — menilai hasil pekerjaan sendiri. Status terminal (`CLOSED`/`CANCELLED`) read-only. Status asing ditolak `400`, tidak dinormalisasi diam-diam.
- **Penugasan langsung, bukan antrian bersama** ([[ADR - 0059 Penugasan Langsung Menggantikan Antrian Bersama]]): `assigned_to` **wajib** saat membuat tiket, tunggal (bukan array seperti `Task.AssignTo`). ⚠️ Nilai `OPEN` **maknanya berubah** jadi "sudah ditugaskan, belum disentuh" **tanpa berubah nama** — nilai itu sudah tersimpan di dokumen produksi. Query papan kerja wajib memasukkan `OPEN`.
- **Kandidat penugasan disaring `department`, bukan `position`** (`engagement_assign.go`). Menyaring `position == "Engagement Team"` akan terikat pada nama jabatan yang baru saja di-rename dari *Buzzer*, dan rename berikutnya mengosongkan daftarnya **tanpa satu pun galat**. Departemennya dari header identitas, bukan query klien; akun non-aktif dibuang; daftar kosong dijawab `200` + `catatan`, bukan galat. 🟡 **Akan berubah**: [[ADR - 0060 Cakupan Keterlihatan Tiket Engagement]] §4 mengganti sumbunya jadi `position_key` lintas departemen — departemen **pemanggil** keliru karena requester (Account Specialist Kyura/Beauty Hacks) tak selalu sedepartemen dengan pengerjanya.
- **Alias pencarian `buzzer`** (`engagement_alias.go`): pencarian dengan nama lama tetap mengenali modul ini, `Contains`, satu arah, tak mengubah data. Awalan/kemiripan (`buzz`, `engage`) sengaja ditolak — alias longgar membuat pencarian apa pun mengembalikan seluruh tiket.
- **Nomor tiket `ENG/YYYYMM/NNNN`** (`engagement_nomor.go`), bulan menurut **WIB** (memakai ulang `feedWIB` modul kalender, bukan memuat zona kedua). Urutan dari `CountDocuments` bulan berjalan; penjaga nomor ganda adalah **index unique `no_tiket`**, bukan pengecekan aplikasi. ⚠️ Komentarnya menjanjikan pemanggil me-retry saat tabrakan, **tetapi pemanggilnya tidak** — permintaan bersamaan berujung `500`.
- **Notifikasi** memakai ulang `notifyMany()` tiket IT (Mongo + WS + inbox + FCM); yang berbeda hanya penerimanya. Delapan tipe `engagement_*` dipetakan satu per satu di `inboxCategoryFor` (`fcm.go`) dan **tidak menumpang** kategori tiket IT. Identitas tiket dititipkan lewat `meta` (`modul`/`engagement_ticket_id`/`no_tiket`), sebab `Notification.TaskID` bertipe ObjectID tugas dan mengisinya membuat klien membuka detail tugas yang tak ada. ⚠️ Dua tipe (`engagement_ticket_open`, `engagement_released`) terdaftar lengkap **tapi tak pernah dikirim**, dan daftar penjaga di test memuat satu tipe dua kali sekaligus melewatkan satu tipe lain — tiga test yang mengiterasinya tidak menguji apa pun untuk tipe itu, sementara semuanya hijau.
- **Eskalasi tiket menganggur** (`engagement_eskalasi.go`), menumpang scheduler per jam yang sama dengan SLA tiket IT — dua penjadwal di satu service berarti dua tempat yang harus diperiksa saat sesuatu tak jalan. Ambangnya **2 jam ATAU setengah sisa deadline, mana duluan**: batas tetap saja membuat tiket berdeadline 1 jam telat sebelum ambang menyala, setengah-deadline saja membuat tiket berdeadline seminggu menganggur 3,5 hari. Deadline kosong jatuh ke batas tetap; deadline yang sudah lewat matang seketika. Hanya status `OPEN`. Penerimanya **pengerja + requester**, bukan seluruh tim (tim flat tanpa lead; cuma dua orang itu yang bisa menindak). Penanda anti-spam `escalated_at` **ditulis lebih dulu** dan hanya bila belum bertanda, sehingga dua pemindaian bertumpuk tak bisa sama-sama menang — tanpa itu satu tiket mengirim 24 notifikasi sehari.
- **KPI** `GET /kpi/engagement` (`kpi_engagement.go`) — agregat satu orang satu periode, digerbang **kunci layanan** (bukan `staffOrSup`), dikonsumsi sumber `kinerja_engagement` di [[Microservices - Employee Service]]. Aturan lengkapnya di [[API - Task Management Service]]. Fungsi peringkasnya **murni** supaya aturan seperti "periode dari `closed_at`" bisa diuji tanpa Mongo.
- **Rincian jenis per URL dan realisasi** (`engagement_rincian.go` murni, `engagement_realisasi_handlers.go` glue; branch `feat/engagement-realisasi-per-jenis`, belum merge, [[ADR - 0117 Realisasi Engagement Dilaporkan Pengerja per URL per Jenis Pekerjaan sampai Akhir Bulan KPI]]): `EngagementItem.Rincian` memuat `{jenis, volume_target, volume_realisasi}`, dan total baris (`volume_target`/`volume_realisasi`) tetap disimpan lewat **satu** penulis, `terapkanRincian`. Pengerja melapor lewat `PUT .../items/:itemId/realisasi`; putusan boleh-tidaknya (`keputusanIsiRealisasi`: peran, lalu status, lalu jendela) juga dipakai untuk `boleh_isi_realisasi` di detail. **Bulan jendela dan bulan KPI berasal dari satu fungsi**, `periodeKPIEngagement` (UTC), yang juga menentukan isi tab Pekerjaan Saya. Definisi "baris belum dilaporkan" satu, `barisBelumDiisi`, dipakai penanda daftar dan kelengkapan KPI. `RealisasiDiisiAt` yang membedakan belum dilaporkan dari nol.
- ⚠️ **Urutan pendaftaran rute mengikat**: rute literal (`/tickets/queue`·`mine`·`requested`, `/kandidat`, `/dashboard`) **wajib sebelum** `/tickets/:id`. Rute yang tertelan membalas `200` berisi bentuk yang masuk akal, bukan `404`. Dikunci `TestRuteLiteralTakTertelanParam`.
- ⚠️ **Gateway membuang prefix `/api/task-management`**, jadi rute didaftarkan sebagai `/engagement/...`, bukan `/task-management/engagement/...`. Salah menaruhnya membuat seluruh permintaan lewat jalur normal membalas `404` sementara unit test tetap hijau (test memanggil path lokal langsung ke Fiber).
- ⚠️ **`pastikanIndexEngagement` tidak mematikan service saat gagal** — index yang belum jadi membuat query lambat, sementara service yang menolak start membuat **modul tiket IT ikut padam karena tetangganya**. Kecuali index unique `no_tiket`, yang kegagalannya dicatat mencolok.
- ⛔ **Catatan lama "tak ada penyaringan departemen/space di jalur pemakai" tidak berlaku lagi.** `antrianEngagement`, `detailTiket`, dan `PUT .../realisasi` memakai `keterlihatanFilter`/`cocokKeterlihatan` (`engagement_visibility.go`): field `requester_department` distempel server saat tiket dibuat, migrasi tiket lama di `engagement_migrasi.go`, aturan di [[ADR - 0060 Cakupan Keterlihatan Tiket Engagement]]. Gerbang rute tetap `requireRoles("staff","supervisor","admin")`; yang membatasi adalah handler.
...
- **Modul Engagement Tim: tiga cacat yang menghentikan alurnya SUDAH DIPERBAIKI** (audit 2026-08-29): bukti pengerjaan dilonggarkan jadi opsional (konsekuensinya metrik KPI `reporting` terstruktur nol karena tetap tak ada rute pengisi `attachments`), `pekerjaanSaya` menyaring `assigned_to`, dan `assigned_name` diisi. Rincian: [[Sales - Engagement Team (Modul)]]. Yang masih terbuka di modul ini: rute lampiran bukti, dan tiket lama yang belum punya rincian per jenis.
- **Modul Engagement Tim: keterlihatan tiket SUDAH dibatasi** cakupan departemen (`engagement_visibility.go`, [[ADR - 0060 Cakupan Keterlihatan Tiket Engagement]]); catatan lama "belum dibatasi sama sekali" tidak berlaku.
- **Modul Engagement Tim: sisa model "antrian bersama" masih berserak** — index `ix_pemegang` dan `hitungWIP` menunjuk `claimed_by`, dua tipe notifikasi tak pernah dikirim, `anggotaSpaceEngagement` dan `namaOrangAtau` dead code, dan beberapa komentar kepala berkas masih menjanjikan siaran ke seluruh space. Lihat [[ADR - 0059 Penugasan Langsung Menggantikan Antrian Bersama]] § Consequences.
...
- **MongoDB** sendiri (`task_management_db`) untuk koleksi `tasks`, `space`, `notifications`, `audits`, ditambah **tiga koleksi modul Engagement** (`engagement_tickets`, `engagement_ticket_items`, `engagement_logs`). Lihat [[DB - Overview and Notes]].
...
- [[Sales - Engagement Team (Modul)]] — konsep bisnis modul Engagement Tim yang ditampung service ini
