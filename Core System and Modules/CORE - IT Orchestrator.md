...
- **431 karena `ReadBufferSize` 4 KB — diperbaiki, live di DEV, PROD masih tertinggal.** Sama dengan [[CORE - HRIS Orchestrator]]: `orchestrator/it/main.go` dulu tidak menyetelnya, jadi SELURUH `/api/it/*` membalas 431 untuk akun berizin banyak. Terukur di dev 2026-09-04: `/api/it/v2/multi` **431 → 200** dengan header probe 6 KB yang sama, sementara rute service ber-32 KB tetap 200 sebagai kontrol. Kini `konfigFiber()` menyetel 32 KB dan dikunci uji perilaku di `konfig_fiber_test.go`. Aturan dan sebabnya di [[CORE - API Master Gateway]]; PR [bip-erp#1708](https://github.com/bip-itteam-internal/bip-erp/pull/1708) (merged `df03a674`). ⛔ **PROD masih biner lama** (image `2026-09-03T13:22`, diukur 2026-09-04) — status bergerak, ukur ulang. Yang membuat ini lebih menggigit di sini daripada kelihatannya: `/roles/*`, `/network*`, dan `/v2/multi` belum punya pengganti di service mana pun (lihat peringatan di atas), jadi tak ada jalur cadangan saat rutenya mati.
