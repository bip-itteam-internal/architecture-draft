...
- **`POST /employee-salary/bulk-bpjs-base`** — isi massal DUA dasar upah BPJS dari berkas Excel HR (bip-erp [#1482](https://github.com/bip-itteam-internal/bip-erp/pull/1482) + erp-frontend [#1269](https://github.com/bip-itteam-internal/erp-frontend/pull/1269), **merged 2026-08-27**). Ada karena pemisahan dasar upah mengosongkan seluruh 120 record, dan mengisinya lewat modal berarti membuka 120 formulir. Gerbangnya `PermPayrollSalaryWrite`, **sama persis** dengan menulis satu record: mengisi 120 orang sekaligus tak boleh lebih longgar daripada mengisi satu.
	- ⛔ **TANPA `upsert`, dan itu keputusan yang menentukan.** Karyawan yang belum punya penetapan gaji **dilaporkan gagal**, bukan dibuatkan record. Record yang lahir hanya berisi dasar upah bergaji pokok nol, dan `recalcRun` mengambil SELURUH koleksi (`Find(ctx, bson.M{})`) sehingga record setengah jadi itu ikut terhitung sebagai karyawan bergaji nol di run berikutnya. Sekelas dengan peringatan di §Master Badan Usaha soal record kosong sebagai wadah `company_id`.
	- ⛔ **`$set` HANYA memuat dua dasar upah** plus `updated_by`/`updated_at`. `PUT /employee-salary/:employeeId` adalah upsert PENUH, jadi memakainya untuk mengisi dasar upah menuntut pemanggil menyalin ulang record lama dengan sempurna; satu field terlewat menjadikannya nol, dan pada impor massal kekeliruan itu berlipat 120 kali. Isi `$set` dikunci test berikut kontrol negatif atas `basic_salary` dan `component_values`.
	- ⚠️ **TANPA validasi bentuk `employee_id`.** Diukur ke prod 2026-08-27: ada **dua** pola, `BIP-9999-99-99` (201 orang) dan **`BIP-MG-9999-99-99` (8 orang)**. Validator regex akan menolak delapan karyawan yang sah dengan galat yang tak menyebut sebab sebenarnya. Yang menentukan sah adalah **ada atau tidaknya di koleksi**, dan itu kebal terhadap pola baru.
	- **Dua perjalanan ke Mongo, bukan satu per baris**: satu `Find` ber-`$in` untuk tahu siapa yang punya penetapan gaji, lalu satu `BulkWrite` (`SetOrdered(false)`). Pelaporan per baris tak berkurang karena yang belum punya record sudah teridentifikasi di langkah pertama, dan penjaga anti-`upsert` jadi berlapis.
	- **Kegagalan dilaporkan PER BARIS** (`{diperbarui, tanpa_ubah, gagal:[{employee_id, alasan}]}`). Batch 120 baris yang batal total gara-gara satu orang memaksa HR mengulang semuanya. `tanpa_ubah` dibedakan dari `diperbarui` supaya angka yang dilaporkan bisa dipercaya.
	- **Rute didaftarkan SEBELUM saudara ber-`:employeeId`.** Metodenya sudah berbeda (POST vs GET/PUT) sehingga tak mungkin tertelan, tapi urutannya dijaga sebagai sabuk pengaman kedua dan **dikunci test**. Rute literal yang kalah urutan membalas 200 berisi bentuk yang masuk akal, bukan 404 — lihat kelas yang sama di §Belum Diimplementasikan.
	- **Excel diurai di FRONTEND**, bukan di sini: tak ada `excelize`, tak ada multipart, tak ada dependensi Go baru. Layarnya di [[APP - Web ERP]]. Konsekuensinya server tak pernah menebak orang dari nama — yang tiba selalu `employee_id` yang pasti.
- Field: `basic_salary`, **`upah_bpjs_kesehatan`** + **`upah_bpjs_ketenagakerjaan`** (DUA dasar BPJS, terpisah dari gaji pokok), `ptkp_status` (TK/0…K/3), `component_values[]`, `bpjs_enrollment`, `effective_date`
- ⚠️ **Dasar upah BPJS ada DUA sejak [#1478](https://github.com/bip-itteam-internal/bip-erp/pull/1478), merged 2026-08-27.** HR mendaftarkan upah yang berbeda ke kedua lembaga: Kesehatan umumnya gaji pokok + tunjangan tetap, Ketenagakerjaan sering sebatas UMK. Satu field `upah_bpjs` untuk kelima program karenanya membuat salah satunya pasti salah. Pemetaan program → dasar upah hidup di **satu** fungsi, `programBpjs` (`payroll_calc.go`); sisi karyawan, sisi perusahaan (`bpjsCompanyTotal`), dan perakitan baris slip semuanya lewat sana supaya keduanya mustahil menyimpang.
	- **Sisi perusahaan ikut dipecah, dan itu bukan kerapian.** `JKK` & `JKM` ber-`employee_rate` **0**, jadi keduanya tak pernah muncul di slip dan `/employer-cost` adalah **satu-satunya** jalur tempat kedua program itu berpengaruh. Bila sisi perusahaan memakai dasar upah sendiri, kedua centang itu jadi dekorasi tanpa satu pun test menangkapnya.
	- **Nilai `upah_bpjs` lama TIDAK disalin ke keduanya.** Backfill boot `ensureUpahBpjsTerpisah()` mem-`$unset` field lama (idempoten), dan `setEmployeeSalary` ikut mem-`$unset`-nya tiap record disentuh. Alasannya di §Kondisi Pemakaian di Produksi: nol dari 120 record punya angka yang masuk akal sebagai dasar upah, jadi menyalinnya hanya mengawetkan kekeliruan yang sama di dua field. Record dibiarkan **kosong** supaya HR mengisi ulang.
	- **Kosong tidak senyap.** `lembagaBpjsTanpaDasar` menandai lembaga yang programnya diikuti tapi dasarnya belum diisi; baris payroll run ditandai (`warn_codes: bpjs_base_missing`) dan `/employer-cost` memulangkan `catatan`. Tanpa itu potongannya nol dan slipnya terbit dengan angka yang wajar tapi terlalu kecil.
	- ⚠️ **Branch lokal `feat/bpjs-management` (erp-frontend, 3 commit, terakhir 2026-08-06, TIDAK ada di remote) akan konflik**: ia membangun layar Kelola BPJS di atas `upah_bpjs` tunggal dan ikut menyunting `bpjs-estimate.ts`. Lihat [[APP - Web ERP]].
- Referensi `employee_id` ke [[Microservices - Employee Service]] — NPWP/no.BPJS/rekening **di-join di FE**, tidak disalin.
- ⚠️ **Keduanya DASAR upah, bukan nominal potongan.** Engine memakainya sebagai pengali (`computeBpjsEmployee`), jadi mengisinya dengan nominal iuran membuat potongan mengecil sebesar rate itu sendiri (isi 4% dari dasar, potongan jadi 4% dari 4%, alias 25 kali lebih kecil). **Tidak ada validasi yang menahannya**: schema FE hanya `min(0)` dan `validateEmployeeSalary` hanya menolak negatif, jadi angka yang keliru lolos diam-diam sampai slip terbit. Yang menahannya cuma peringatan di layar, karena ada kasus sah dasar upah di bawah gaji pokok (paruh waktu, atau dasar di-cap ke UMK).
	...
	- ⛔ **Seluruh pengukuran di atas menyangkut field `upah_bpjs` TUNGGAL yang sudah dipensiunkan** ([#1478](https://github.com/bip-itteam-internal/bip-erp/pull/1478), merged 2026-08-27 dan **sudah live di prod**: field lamanya terbukti hilang, 90 dari 120 dasar upah baru sudah terisi HR per 2026-08-27). Angkanya dipertahankan di sini karena ia yang menjelaskan **kenapa** nilainya tidak disalin ke dua field baru: menyalin 120 record yang nol-nol-nya benar hanya akan mengawetkan kekeliruan di dua tempat, bukan satu. Sesudah deploy, seluruh 120 record berdasar upah **kosong** dan menunggu HR mengisi ulang; itu keadaan yang diinginkan, bukan regresi. **Ukur ulang dengan nama field yang baru**, jangan memakai angka di atas sebagai kondisi terkini.

## Endpoint / Fitur (Sudah Diimplementasikan — Fase 2: Payroll Run)

- **Kalkulasi** (`buildPayslip`): Gaji Pokok = `basic_salary` (bukan komponen → hindari double-count; komponen manual bernama "Gaji Pokok" di-skip sbg guard) + komponen manual + Tunjangan Kehadiran penuh + lembur − BPJS (dari **dua** dasar upah + config, lewat `programBpjs`) − potongan kehadiran − **PPh21 (TER)**. Hanya komponen `manual` diambil dari `component_values`; yang `computed` dihitung engine.
...
- **Batch run**: `POST /payroll-runs` (metadata `title`, `pay_period_start/end`, `pay_date`, `notes`, **`scope`** — **penggajian BULANAN**, tak ada mingguan; `period` label diturunkan dari `pay_period_start` bila kosong; hitung karyawan **dalam lingkup**, simpan snapshot per orang; supplement gagal per-orang ditandai, tak gagalkan run) · `GET /payroll-runs` · `GET /payroll-runs/:id` (+ lines) · `POST /:id/recalculate` (draft) · `POST /:id/approve` (approver) · `POST /:id/publish` (approver; approved → published) · `GET /:id/lines/:employeeId`. Status **draft → approved → published**.
	- ⚠️ **`period` yang tersimpan BUKAN periode kehadiran run itu** — lihat §Periode Kehadiran. Ia label yang diturunkan dari `pay_period_start` dan di prod meleset sebulan; yang menentukan kehadiran adalah bulan `pay_period_end`.
	- **`GET /payroll-runs` membawa ringkasan terhitung** ([#1485](https://github.com/bip-itteam-internal/bip-erp/pull/1485), merged 2026-08-27): `jumlah_karyawan`, `total_gross`, `total_net` dari satu agregasi `$in`+`$group` atas `payroll_run_line` (`run_ringkasan.go`). ⛔ Ketiganya ber-`bson:"-"` — **DIHITUNG saat dibaca, tak disimpan**. Kalau disimpan, `recalculate` wajib ikut memperbaruinya, dan begitu itu terlupa daftar memamerkan total lama sementara slipnya sudah berubah. Tanpa tag itu pun `createPayrollRun` akan menuliskannya ke Mongo sebagai nol dan melahirkan salinan kedua yang terlihat sah tapi selamanya nol.
...
- **Service-to-service**: panggil [[Microservices - Attendance Service]] `GET /payroll-supplement?employee_id=…&period=YYYY-MM` (`payout_pct` **persentase 0–100** → prorata Tunjangan Kehadiran + lembur) via `InternalRequest`. Periodenya wajib dan cara menurunkannya tidak sepele — lihat §Periode Kehadiran.

### Periode Kehadiran (✅ [#1500](https://github.com/bip-itteam-internal/bip-erp/pull/1500), merged 2026-08-28)

⛔ **Sampai 2026-08-28 payroll TIDAK PERNAH mengirim `period`.** `attendance-service` sudah menerimanya sejak lama dan hanya jatuh ke bulan berjalan bila tak diberi, jadi **setiap** run — periode apa pun, dihitung kapan pun — memakai kehadiran **bulan berjalan**. Potongan kehadiran adalah ~90% dari seluruh potongan yang ada di prod, jadi ini menyentuh hampir semua angka yang bukan BPJS.

Tiga konfirmasi yang saling bebas: kedua run prod berperiode berbeda punya `attendance_basis` **identik byte-per-byte**; payroll memang tak pernah menyusun param `period`; attendance memang default `time.Now()`.

⛔ **Periodenya diturunkan dari `pay_period_end`, BUKAN `run.Period`.** Ini yang paling mudah salah, karena `run.Period` justru namanya yang paling terdengar benar. `Period` diturunkan `derivePeriodLabel` dari `pay_period_start`, dan di prod itu **meleset satu bulan**:

| Judul run | `pay_period_start` | `pay_period_end` | `period` tersimpan | periode kehadiran yang BENAR |
|---|---|---|---|---|
| GAJI BULAN AGUSTUS | 2026-07-26 | 2026-08-25 | **2026-07** | **2026-08** |
| Gaji Bulan Juli | 2026-06-25 | 2026-07-26 | **2026-06** | **2026-07** |

Jendela attendance untuk periode P adalah 26 bulan-sebelum-P s.d. 25 bulan-P, jadi 26 Jul–25 Agu **adalah** periode 2026-08. Memakai `run.Period` hanya menukar satu bug (selalu bulan berjalan) dengan bug lain yang lebih sulit dilihat (selalu mundur sebulan); keduanya membalas 200 berisi angka yang masuk akal. Bulan dari `pay_period_end` juga tak menuntut payroll menyalin angka **26** milik attendance ke sini sebagai fakta kedua.

- **Periode yang tak terparse TIDAK dikirim.** Attendance membalas **400 `period must be YYYY-MM`**, yang akan menggagalkan SELURUH baris run — jelas lebih buruk daripada cacat yang diperbaiki. Barisnya ditandai `warn_codes: run_period_unknown` supaya kejatuhan ke bulan berjalan tidak senyap.
- **`/employer-cost` ikut**: ia sudah membaca `?period=` sejak awal tapi **hanya menyalinnya ke respons**, tak pernah memakainya mengambil kehadiran. Modul insentif memanggilnya per periode, jadi beban bulan lalu pun dihitung dari kehadiran bulan berjalan.
- **Run THR tak tersentuh** — `computeThrRunLines` memang tak memanggil attendance sama sekali.
- Aturannya hidup di satu berkas, `periode_kehadiran.go`, dan `computeRunLines` kini menerima `PayrollRun` utuh (bukan `runID` saja) karena periodenya milik run.

### Lingkup Run — semua / karyawan / magang (✅ [#1502](https://github.com/bip-itteam-internal/bip-erp/pull/1502), merged 2026-08-28)

Sebelumnya tiap run selalu mencakup **seluruh** karyawan yang punya `employee_salary`. `PayrollRun.scope` kini `semua` (default) | `karyawan` | `magang`.

**Disimpan di run, bukan diputuskan saat menghitung** — kalau tidak, `recalculate` menghasilkan kumpulan orang menurut layar saat itu, bukan kumpulan yang sama dengan saat run dibuat.

Tipe kepegawaian diambil dari [[Microservices - Employee Service]] `/internal/export/all` yang **memang sudah** mengirim `employment_type`; payroll saja yang belum membacanya. **Nol perubahan di employee-service.**

Terukur di prod 2026-08-27, `work_data.employment_type`: PKWT **195** · PKWT (Evaluasi) **5** · PKWTT **1** · **Magang 8** — dan kedelapan magang ikut terhitung di kedua run yang ada.

- ⚠️ **Kegagalan mengambil identitas tak lagi selalu non-fatal.** Untuk run berlingkup ia **menggagalkan run**: penyaringnya ikut di data identitas yang sama, jadi memperlakukannya seperti sebelumnya membuat run `magang` diam-diam membayar 209 orang. Nama yang hilang cuma kosmetik; lingkup yang hilang bukan. Ini menyimpang dari aturan di §Fase 5 (`employee_name`/`position` gagal ambil tidak menggagalkan run), dan penyimpangannya disengaja.
- **Scope tak dikenal ditolak 400**, bukan dijatuhkan ke `semua`: salah ketik yang *melebarkan* lingkup berarti orang yang tak dimaksud ikut terbayar. Kosong tetap sah dan berarti semua (FE lama tak mengirim field ini).
- **`employment_type` kosong dihitung karyawan**, bukan dilewati. Orang yang datanya belum lengkap tetap terbayar; run yang kurang satu orang jauh lebih sulit disadari daripada satu orang di batch yang salah.
- ⛔ **Magang TIDAK disimpulkan dari prefiks `employee_id`.** Kedelapan magang prod kebetulan ber-ID `BIP-MG-` dan itu menggoda karena 100% cocok hari ini, tapi prefiks itu konvensi penamaan yang tak dijaga apa pun. Aturannya `employment_type`, dan ia hidup di satu tempat (`lingkup_run.go`).
...
- ⚠️ **`period` baru benar-benar dipakai sejak [#1500](https://github.com/bip-itteam-internal/bip-erp/pull/1500)** (2026-08-28). Sebelumnya ia hanya **disalin ke respons** dan tak pernah diteruskan ke attendance, jadi beban bulan lalu pun dihitung dari kehadiran bulan berjalan tanpa satu pun tanda. Yang tak terparse tetap tak dikirim (attendance membalas 400) tapi dicatat di `catatan`.
...
- **`computeBpjsCompany`** (kembaran `computeBpjsEmployee`) — memakai `CompanyRate` + batas upah yang sama, dan **dasar upah per lembaga yang sama** lewat `programBpjs`. Rate-nya **sudah ada di config sejak awal tapi tak pernah dihitung**, karena iuran pemberi kerja bukan potongan karyawan sehingga tak muncul di slip. Slip nyata: bruto 4.328.500 + iuran 329.728 = **4.658.228** (vs gaji bersih 4.094.423 — selisih 13,8%).
- ⚠️ **`catatan` kini terisi bila dasar upah BPJS belum diisi**, dan [[Microservices - Insentive Service]] **meneruskannya** ke `peringatan[]` baris dashboard sehingga `LayakBayar` gugur. Sebelumnya `ambilBiayaKaryawan` hanya memungut `total` dan **membuang** `catatan`, jadi iuran perusahaan yang terhitung nol (~7% biaya per orang) mengecilkan biaya operasional, membuat profit tampak lebih besar, dan insentif dibayar terlalu besar **tanpa satu pun tanda di layar**. Ini kelas yang berbeda dari "belum ada penetapan gaji": di sana angkanya tak ada dan layar jelas kosong, di sini angkanya terisi dan terlihat wajar. **Bentuk respons tidak berubah**, jadi bukan perubahan kontrak — tapi insentive-service tetap harus naik bersama payroll-service karena `func.go`-nya ikut berubah.
...
- **Insentif** = komponen manual di slip. Integrasi dengan [[Finance - Incentive]] sejauh ini **satu arah**: modul insentif **menarik** beban karyawan dari sini (`/employer-cost`) untuk dipakai sebagai biaya operasional. Arah sebaliknya — nominal insentif masuk ke slip — **diputuskan lewat slip** (🟡 [[ADR - 0125 Insentif Profit Dibayar lewat Slip Gaji dari Snapshot yang Disetujui Finance]], 2026-09-25, belum di kode): komponen baru `Insentif` terpisah dari `Bonus`, nilainya hanya dari snapshot insentif yang disetujui Finance, diterima impor hanya bila cocok dengan snapshot, dan snapshot ditandai terbayar saat run di-publish. ⛔ `Insentif` **ditolak** di `component_values` master, karena nilai master ikut ke `/employer-cost` dan insentif akan menjadi beban gaji yang mengurangi profit insentif (melingkar). ⚠️ Flag `Taxable` & `BpjsBase` **tidak dibaca hitungan** (hanya ditulis CRUD `component_handlers.go:92-93`): PPh 21 selalu TER atas seluruh pendapatan dan dasar BPJS adalah field master `upah_bpjs_*`, jadi mengubah flag tidak mengubah pajak apa pun. Perlakuan pajak/BPJS insentif TBD konfirmasi Finance dan HR. Komponen `Insentif` sendiri ✅ merged PR #2073 (T3). ⚠️ **T4 (branch `feat/insentif-payroll-snapshot`, belum merged per 2026-09-25)**: impor mencocokkan baris `Insentif` dengan snapshot yang disetujui lalu **mengklaimnya** di insentive, hapus draft melepas klaim, publish menandai terbayar (tanda `insentif_tanda_tertunda` + rute ulang). Rincian di [[API - Payroll Service]]. ⛔ `INSENTIVE_MODULE_URL` sengaja **di luar** map `InternalURL` yang divalidasi `ValidateInternalURL` (map itu memanik bila ada URL kosong); URL kosong = impor ber-Insentif ditolak berpesan, bukan service mati. Env baru di blok payroll-service compose: naik dengan `--force-recreate`.
...
- **Slip self-service ada di DUA klien**, bukan lagi web saja. [[APP - Web ERP]] dan **[[APP - MyBharata]]** (Flutter) sama-sama memanggil `GET /payroll-runs/my` dan `…/pdf`; sisi mobile merged ke `dev` pada commit `3fbf44d4`. Catatan lama "integrasi MyBharata menyusul" **sudah tidak berlaku** (diverifikasi ke kode 2026-09-01). ⚠️ MyBharata **belum dirilis ke store**, jadi merged bukan berarti sudah ada di HP orang.
	- Jenis run **tak menghalangi** sisi mobile: `isThr` di sana perbandingan positif dan `MyPayslip` tak punya `payout_pct`, jadi run `import` mengalir tanpa perubahan apa pun di aplikasi.
