...
- ⚠️ **Dasar upah BPJS ada DUA sejak [#1478](https://github.com/bip-itteam-internal/bip-erp/pull/1478), **belum merge**.** HR mendaftarkan upah yang berbeda ke kedua lembaga: Kesehatan umumnya gaji pokok + tunjangan tetap, Ketenagakerjaan sering sebatas UMK. Satu field `upah_bpjs` untuk kelima program karenanya membuat salah satunya pasti salah. Pemetaan program → dasar upah hidup di **satu** fungsi, `programBpjs` (`payroll_calc.go`); sisi karyawan, sisi perusahaan (`bpjsCompanyTotal`), dan perakitan baris slip semuanya lewat sana supaya keduanya mustahil menyimpang.
...
- ⚠️ **`warn_codes[]` = SEBAB baris ditandai, sebagai kode stabil dan bukan kalimat**, supaya layar bisa menerjemahkannya ([[ADR - 0010 Internasionalisasi (i18n) Dua Bahasa]]) — sealasan dengan `attendance_basis` yang disimpan sebagai angka. Nilai: `attendance_unavailable`, `attendance_days_missing`, `thr_join_date_missing`, `bpjs_base_missing`. `error` tetap memuat detail teknisnya.
...
BPJS** (seluruh 120 record kosong sesudah pemisahan, menunggu HR mengisi; lihat §Gaji per
Karyawan) → pengali mangkir yang menyimpang dari Pasal 20 → sign-off tabel TER → cakupan gaji
yang belum separuh. Tiga yang pertama membuat angka di slip **salah**, bukan sekadar kosong,
dan slip yang sudah `published` men-snapshot kesalahannya.
