...
- ⚠️ **Dasar upah BPJS ada DUA sejak branch `feat/payroll-upah-bpjs-terpisah` (belum merge).** HR mendaftarkan upah yang berbeda ke kedua lembaga: Kesehatan umumnya gaji pokok + tunjangan tetap, Ketenagakerjaan sering sebatas UMK. Satu field `upah_bpjs` untuk kelima program karenanya membuat salah satunya pasti salah. Pemetaan program → dasar upah hidup di **satu** fungsi, `programBpjs` (`payroll_calc.go`); sisi karyawan, sisi perusahaan (`bpjsCompanyTotal`), dan perakitan baris slip semuanya lewat sana supaya keduanya mustahil menyimpang.
