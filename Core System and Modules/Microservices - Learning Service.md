...
- **Status**: ⚠️ **Implemented (ada catatan)** — service **live di dev + produksi 2026-08-06** (bip-erp PR [#1020](https://github.com/bip-itteam-internal/bip-erp/pull/1020), frontend [erp-frontend#814](https://github.com/bip-itteam-internal/erp-frontend/pull/814)), data 4 koleksi dipindah di kedua lingkungan dengan jumlah terverifikasi cocok, dan pengajuan + evaluasi + `/me` **terverifikasi lewat gateway hidup 2026-08-19**. **Catatannya bukan di service melainkan di alur pemakainya**: karyawan biasa tak punya jalan bermenu sejak menu web dicabut 2026-08-13 sementara my-bharata [#115](https://github.com/bip-itteam-internal/my-bharata/pull/115) belum rilis, sehingga `training_request` di produksi masih **0**. Seluruh fitur LMS (course, materi, kuis, kurikulum, Talent Pool) **belum ada**. Konsep & rencana lanjutan: [[HRIS - Training Program]]
...
- ⚠️ **Terpasang penuh tapi BELUM DIPAKAI sama sekali.** Hitungan `learning_db` produksi 2026-08-19: `training_request` **0**, `trainer_evaluation` **0**, `training` 1, `training_participant` 1, `trainer` 2, `training_type` 1. Pengajuan dan evaluasi live sejak 2026-08-11, jadi nol di kedua koleksi itu **pertanyaan, bukan kabar baik** — sebabnya alur karyawan yang terputus di bawah, bukan servicenya. Angka nol yang mencurigakan diperlakukan sebagai pertanyaan; lihat pola yang sama di [[Microservices - Form Builder Service]].
- ⛔ **Alur karyawan biasa TERPUTUS sejak 2026-08-13, dan masih terputus per 2026-08-19.** Menu self-service di web sudah dicabut, tapi penggantinya di aplikasi belum sampai ke orang: my-bharata [#115](https://github.com/bip-itteam-internal/my-bharata/pull/115) (Pengajuan Pelatihan) **masih OPEN**, belum masuk `dev` maupun `main`, dan [#112](https://github.com/bip-itteam-internal/my-bharata/pull/112) (Pelatihan Saya) baru di `dev` sementara rilis yang terpasang di HP pemakai masih **1.14.5+135**. Akibatnya karyawan tanpa bawahan dan tanpa izin `training.work` **tak punya satu pun jalan bermenu** untuk mengajukan pelatihan atau melihat pelatihannya sendiri; `/hris/training/requests` masih menyimpan tab `self` berikut formulirnya, tapi menunya digerbang penyetuju. Konsekuensi bagi service ini: `POST /training/requests` sudah live berminggu-minggu **tanpa satu pun pemanggil**. Lihat [[APP - Web ERP]] dan [[APP - MyBharata]].
