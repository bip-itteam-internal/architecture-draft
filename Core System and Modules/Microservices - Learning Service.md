...
- **MongoDB `learning_db`** — koleksi `training_type`, `trainer`, `training`, `training_participant`, `training_request`, `trainer_evaluation`, `course`, `quiz`, `quiz_attempt`, dan (di branch, **belum merged**) `training_plan_item`, `training_certificate`, `training_certificate_counter`, `training_certificate_setting` (`models_training.go:24-27,38-41`). Lihat [[DB - Overview and Notes]].
