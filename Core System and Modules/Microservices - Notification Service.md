...
- **Push web (browser) — fan-out dari `/inbox/send`, bukan dari tiap pengirim** (2026-08-20):
	...
	- **`PushWebCategories`** (`shared-library`) = daftar-izin TERPISAH dan lebih pendek dari `InboxCategories`. "Layak masuk inbox" dan "layak menyela pekerjaan orang" bukan pertanyaan yang sama: kalau tiap komentar tugas memunculkan notifikasi desktop, orang yang terganggu memblokir izin **permanen** — browser tak menyediakan cara memintanya kembali dari dalam aplikasi. Isi awal: `request-*` (kecuali `ignored`), `warning-*`, `task-assigned`, `task-approval`, `form-published`, `report-pending`.
	...
	- Pencarian token **sengaja tanpa `company_id`**. `/inbox/send` dipanggil pakai service key tanpa identitas karyawan, jadi `common.CompanyID(c)` jatuh ke BIP dan karyawan perusahaan lain tak akan pernah ditemukan tokennya — push hilang tanpa satu pun galat. `employee_id` sudah menunjuk satu orang.
