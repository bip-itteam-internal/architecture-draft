...
- **Alur approval/freeze skema profit** — 🟡 **di kode, belum merged** (branch `feat/insentif-snapshot`, 2026-09-25; [[ADR - 0125 Insentif Profit Dibayar lewat Slip Gaji dari Snapshot yang Disetujui Finance]] T1). Koleksi `incentive_snapshots` (indeks unik parsial `{level, entity_id, periode}` atas `aktif:true`) dan `incentive_snapshot_jobs` (satu job `berjalan` per periode). Mesin status `dibekukan → disetujui → terbayar`, `dibekukan|disetujui → dibatalkan`; `terbayar` diisi payroll di T4. Pembekuan asinkron di goroutine ber-`recover` (service ini tak memasang recover middleware, jadi panik di goroutine sendiri akan mematikan proses). Status final periode **dibaca** dari integration (`final`/`cutoff_at` pada `/profit/incentive/summary`), tidak disalin. Kontrak rute di [[API - Insentive Service]]. Sampai merged + deploy, keadaan prod tetap: tidak ada approval, `disetujui` target tak pernah `true`.
