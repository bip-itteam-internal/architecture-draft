...
- **Input HPP**: upload xlsx format finance (sheet per brand-group, kolom `Produk` + `HPP/Pcs`; parser tolak format angka ambigu). Authz upload/mapping: header `system_roles` role `admin|finance` modul integration; lainnya view-only.
