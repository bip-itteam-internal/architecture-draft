...
- **Klasifikasi Komisi**: `fee_commission` = platform + referral + **dynamic commission + flash-sale + voucher-xtra** (dynamic terverifikasi prod > platform commission, Jun 2026: 310jt vs 225jt — sebelumnya nyangkut di residual Proses); `fee_affiliate` = organik + partner + **affiliate ADS commission**; `fee_process` = residual, isi terukur (Jun 2026): **cashback program ~56% + biaya infrastruktur tetap (`vn_fix_infrastructure_fee`) ~26% + mall service fee ~4% + pajak/lainnya** — pecahan informasi dibawa breakdown (`fee_proc_cashback/infra/mall`, ikut Add/Scale/estimasi) dan ditampilkan sub-baris waterfall panel Rincian Biaya FE.
