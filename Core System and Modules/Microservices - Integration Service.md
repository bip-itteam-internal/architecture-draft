...
- **Klasifikasi Komisi**: `fee_commission` = platform + referral + **dynamic commission + flash-sale + voucher-xtra** (dynamic terverifikasi prod > platform commission, Jun 2026: 310jt vs 225jt — sebelumnya nyangkut di residual Proses); `fee_process` = residual murni biaya transaksi/pembayaran/pajak.
- **Arus dana** (`GET /profit/cash-flow`): kartu **Sudah Cair** (Σ `income.total_settlement_amount` order `income_status=fetched` — actual) vs **Uang Gantung** (COMPLETED belum settle — estimasi `GMV × rate_toko`, shrinkage `(net+global×K)/(gmv+K)` K=100jt, window belajar order settled 21/45 hari terdekat; backtest Apr+Mei total ±3,0%, per-toko 4–6%) + rincian toko×bulan + `accuracy_30d` (track-record rumus dihitung on-the-fly). Granularitas repo per (toko, hari WIB) — `AggregateCashFlowDaily`.
