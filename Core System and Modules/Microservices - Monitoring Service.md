...
- ~~**Belum ada konsumen yang benar-benar memakainya.**~~ **Tidak berlaku lagi, kedua bagiannya.** Catatan lama menyebut belum ada `kpi_template` ber-`auto` bersumber `uptime_sistem` DAN dashboard IT belum memanggil `/uptime`; keduanya sudah terlampaui:
  - **Sisi KPI**: metrik `Network` pada template `IT Support` ber-`auto` sejak 2026-08-28 dengan sumber `uptime_sistem` (lihat [[HRIS - Matriks KPI per Departemen]] bab Tech Development). ⚠️ Bentuknya masih perlu ditinjau: uptime terhadap `target_per_karyawan` 90 sementara realisasinya di kisaran 99,9% memberi nilai penuh setiap bulan, sehingga metrik berbobot 0,4 itu tidak membedakan bulan baik dari bulan buruk.
  - **Sisi dashboard**: `GET /uptime?periode=` kini dipanggil blok **"Uptime Bulan Penilaian"** di Ringkasan Divisi IT (`erp-frontend/src/features/it/ringkasan/components/kartu-uptime-periode.tsx`, erp-frontend [#1452](https://github.com/bip-itteam-internal/erp-frontend/pull/1452)). Ia sengaja berdampingan dengan blok Kesehatan Infrastruktur yang memakai jendela berjalan 24 jam, supaya bedanya terbaca: yang satu "apakah ada yang mati sekarang", yang satu lagi "apakah bulan itu memenuhi target".

  ⚠️ **Frontend memanggil `/uptime`, BUKAN `/kpi/uptime`**, dan bedanya menentukan: yang kedua digerbang `MONITORING_SERVICE_KEY` yang tidak dipegang browser, jadi memanggilnya dari layar akan selalu 401 dengan gejala yang terbaca sebagai "izin saya kurang". Pilihan rute itu dikunci test (`use-uptime-periode-query.test.tsx`), sebab tak ada tipe maupun kompilasi yang menahannya.
