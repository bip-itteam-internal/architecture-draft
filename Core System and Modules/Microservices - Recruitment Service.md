...
- **Hasil tes** babak ber-`form_type: "test"` (baku: **Psikotest** & **Technical Test**) direkam per kandidat: `POST /candidates/:id/test-result`, **upsert per (`candidate_id`, `round_id`)**, isi `score` (opsional) + `result` (**Pass/Fail/Pending**) + `notes`. `round_id` yang bukan babak tes ditolak `400`. **Tanpa lampiran** — tak ada tempat menyimpan report PDF
...
- **Psikotes tetap dikerjakan di luar aplikasi**; yang masuk sistem hanya hasilnya. Psikotes online (test-engine + bank soal) = fase lanjut, belum ada kode
...
- **Backbone internal:** `job_requisition` · `job_posting` · `candidate` (+`progress`/`status`) · `interview` · `interview_round` (katalog babak **global**) · `interview_feedback` · `candidate_test_result` · `background_check` · `offer`
...
- **Report PDF psikotes belum ada tempatnya.** `candidate_test_result` tak punya field lampiran dan `background_check` juga tidak, jadi desain lama "skor per-aspek + interpretasi + report PDF di MinIO" ([[HRIS - Recruitment]]) belum terealisasi sama sekali. Yang tersimpan cuma satu angka `score` + `notes` bebas. ⚠️ Kunci i18n untuk layarnya (`stageReportTitle`/`stageReportUpload` "Report Psikotes", `stagePsychotest`, `stageTestType`, `stageMode`, `stageScores`, `stageInterpretation`) **masih ada di `id.ts`/`en.ts` erp-frontend tapi tak dipakai satu berkas pun** — membaca locale saja bisa membuat orang menyimpulkan fiturnya sudah jalan.
