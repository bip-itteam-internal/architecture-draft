...
- **Alur target berjenjang** (Finance → SPV → Direktur, lewat `/kpi/target-marketing`) sengaja digambar terpisah dengan garis putus-putus: rutenya ada di biner produksi dan halaman `/finance/target-marketing` sudah ter-build, tetapi koleksi `kpi_target_marketing` masih kosong dan antrean persetujuan Direktur belum ter-deploy. Ia belum jadi bagian alur yang berjalan. 🟡 **Diputuskan dipensiunkan 2026-09-07** ([[ADR - 0079 Target Profit Satu Pintu di Insentif, KPI Membacanya]], belum dicabut dari kode): target profit ditulis satu kali di insentif dan dibaca KPI; alur ini tak pernah terisi, mengecualikan Advertiser, dan bertentangan dengan keputusan SPV menulis langsung.
