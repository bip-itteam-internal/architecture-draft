...
- **Tampilan gabungan lintas departemen** (`GET /kpi?merge=<daftar departemen>`): departemen yang disebut ditampilkan sebagai **satu kelompok berlabel gabungan**, departemen lain tetap terpisah. Dipakai bila beberapa departemen **berbagi satu supervisor** — saat ini **Human Resource + General Affair** (lihat [[HRIS - Organization Structure]]). **Data karyawan tidak diubah**: `work_data.department` tiap orang tetap di departemen masing-masing, yang disatukan hanya tampilannya, sehingga pemisahan kembali cukup ubah konfigurasi FE tanpa migrasi data.
	...
	- `merge` hanya **mengelompokkan ulang** baris yang sudah lolos filter `department` + RBAC, tidak pernah menarik baris baru. Bila filter menyisakan kurang dari dua departemen, penggabungan **batal** dan label kembali ke nama departemen aslinya.
	- Pasangan departemennya **hardcode di FE** (`erp-frontend/src/features/hris/kpi/constants.ts`), belum jadi master data. Memindahkannya ke `master_department` butuh field relasi baru = **TBD** (lihat [[HRIS - Organization Structure]] §posisi rangkap/matriks).
