...
- **Template per posisi**: `GET/POST/DELETE /kpi/templates` (filter `?department=&position=`). Tiap template = `{name, department, position, metrics[]}`; metrik `{label, weight, description}` (nilai 0–100, bobot total tiap template).
	- **`employee_count`** (✅ live 2026-08-18, bip-erp commit `74da98d0` + FE `3d807952`): `GET /kpi/templates` kini menyertakan jumlah karyawan AKTIF pada (department, position) tiap template. Semantik & penyaring keaktifan **sama** dengan `karyawan_aktif` di Otomasi KPI (`akunAktif`) supaya angkanya tak menyimpang antar-layar; satu posisi dengan beberapa template → semuanya menerima jumlah pemegang posisi yang sama (kelayakan posisi, bukan "sudah dinilai template ini"). WorkData dibaca proyeksi sempit (hindari decode utuh yang jatuh pada dokumen tanggal-string). FE menampilkannya sebagai kolom **"Karyawan"** di tabel KPI Templates; fallback "–" bila field belum ada (BE deploy dulu), 0 tetap 0. Terverifikasi dev lewat gateway (Fullstack Developer = 3, cocok query manual).
