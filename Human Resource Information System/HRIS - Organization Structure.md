...
- **Departemen ter-scope per perusahaan** — `master_department.company_id` ([[ADR - 0029 Multi-Tenant Presensi Row-Level company_id]]). **`key` unik PER perusahaan, bukan global**; kosong pada data lama berarti BIP. Perusahaan baru mulai dengan daftar departemen kosong. Endpoint `/data-type/department`, `/data-type/position`, dan `/master/departments` menyaring memakai `EffectiveCompanyID`. ⚠️ Supervisi/RBAC (`allMasterDepartments`, `system-list`) sengaja **tetap global**, jadi jangan disamakan dengan penyaringan tampilan.
	...
	- **`printing` ("Printing") ditambahkan ke BIP 2026-08-19** sebagai tujuan perpindahan karyawan CV Elit ([[ADR - 0044 Mutasi Antar-Tenant Mempertahankan employee_id]] aturan #7: departemen & jabatan tujuan wajib ada lebih dulu di perusahaan TUJUAN). 13 jabatan disalin dari `pct` milik ELT. ✅ **Live di DEV dan PRODUKSI, terverifikasi lewat gateway.** Perpindahan 14 karyawannya **sudah dijalankan** 2026-08-19 11:40 WIB; rincian dan urutannya di [[ADR - 0044 Mutasi Antar-Tenant Mempertahankan employee_id]].
		- ⚠️ **Sensus produksi 2026-09-14: nama departemen ini kini `Percetakan`, bukan `Printing`**, baik di `master_department` (key tetap `printing`, kini 19 jabatan) maupun di `work_data` (14 karyawan). Migrasi 2026-08-19 menulis `Printing`; kapan dan lewat jalur apa namanya berganti belum diketahui. Yang tidak ikut berganti: `employee.DeptPrinting` dan `deptKeyToNames` di kode, serta 4 `kpi_template` berdepartemen `Printing`. Akibatnya picker jadwal attendance menolak departemen ini (400) dan template KPI-nya tak cocok di pratinjau KPI Saya. Rincian dan buktinya di [[REF - Peta Departemen]].
...
- **Org chart** — ✅ dibangun 2026-08-01, **merged** (FE PR [#673](https://github.com/bip-itteam-internal/erp-frontend/pull/673); BE `services/employee/org_chart.go`). `GET /org-chart` mengirim seluruh karyawan **aktif** satu perusahaan beserta daftar departemennya dalam satu panggilan; halaman `/hris/org-chart` (menu **HRIS → Personalia → Bagan Organisasi**) menggambarnya.
	...
	- **DUA MODE sejak 2026-08-28** (merged ke `main` erp-frontend `5ae2f31c`, diverifikasi 2026-09-14; deploy produksi belum diukur): toggle **Bagan / Daftar** di kepala halaman. Keduanya membaca pohon yang sama dari `bangunPohon`; yang berbeda cuma cara menggambarnya.
...
- **Relasi supervisi berjenjang** — saat ini sengaja dibatasi SATU tingkat. Organisasi bertingkat (divisi → departemen → sub-departemen) perlu keputusan tersendiri
	- ⚠️ **Divisi lahir sebagai master data tersendiri** (`master_divisi`, [[ADR - 0113 Actual vs Planning MPP Dihitung Sistem per Bulan, Berpijak pada Jejak Keluar Bertanggal]], disetujui 2026-09-21). 🔜 **Kodenya sudah ada, belum merge & belum deploy** (bip-erp `feat/employee-master-divisi`, erp-frontend `feat/hris-master-divisi`): koleksi ter-scope per perusahaan, CRUD di Pengaturan → Organisasi, departemen menunjuknya lewat `divisi_key`, dan modul lain membacanya lewat `GET /data-type/divisi`. Empat divisi di-seed **tanpa pemetaan** — memetakan departemen ke divisi pekerjaan HRD. Pemicunya lembar HRD "Actual vs Planning MPP" yang bertingkat Divisi > Departemen > Posisi, dan HRD meminta divisinya bisa dipakai modul mana pun, bukan daftar tetap di satu layar. Empat divisi awal: Commercial, Operational, Supporting, Management, dipetakan ke 12 departemen BIP yang ada. **Ini tidak membatalkan batasan satu tingkat di atas**: yang diputuskan barulah pengelompokan untuk pelaporan, bukan rantai supervisi berjenjang. Pecahan tugasnya di [[ANALISA - Actual vs Planning MPP]] T2.
	- ⚠️ Jangan bingungkan dengan **`supervision_label`** yang sudah ada (mis. `HRGA` atas Human Resource dan General Affair): itu label supervisi antar-departemen, bukan divisi, dan lembar HRD memakai keduanya sekaligus.
...
- [[REF - Peta Departemen]]: tiap departemen di master data dipetakan ke kunci modul dan dok induknya di vault
