...
- **Status**: ⚠️ Sebagian diimplementasikan — **catatan terminasi sudah live di produksi** (koleksi `employee_resign`, sejak 2026-08-05), dan **kartu turnover bulan berjalan** sudah ada di kode (⚠️ belum merge & belum deploy). Dashboard penuh (ikhtisar, detail per departemen, demografi) masih 🟡 konsep; contohnya masih spreadsheet.
...
### Kartu turnover bulan berjalan — ⚠️ ada di kode, belum merge & belum deploy

Cicilan pertama Dashboard: empat kartu di halaman Resign ([[APP - Web ERP]]) yang disuplai `GET /resign/summary`. Rumusnya **rata-rata headcount**, `keluar / ((awal + akhir) / 2) × 100`, dengan target **5% bulanan** yang dikirim server (bukan disalin frontend, supaya warna kartu tak pernah berbeda pendapat dengan servernya).
