...
- **Status**: ⚠️ **Implemented (ada catatan)**: T1 sampai T6 dikodekan 2026-09-11 di empat branch (bip-erp `feat/recruitment-psikotes-multi-jenis` dan `feat/api-gateway-psikotes-catch-all`, erp-frontend `feat/psikotes-multi-jenis`, career-bharata `feat/psikotes-multi-jenis`) dan **belum merged**. Terverifikasi di DEV hari itu lewat gateway dan sebagai HR serta kandidat di Chrome (recruitment-service dan api-gateway dinaikkan dari branch; career portal dijalankan lokal terhadap gateway dev). Prod belum. Status ini bergerak, ukur ulang sebelum mengandalkannya. Papan kerja: `Workspace/ANALISA - Psikotes Multi-Jenis.md`
