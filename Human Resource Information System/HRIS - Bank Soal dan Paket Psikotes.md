...
- **DISC tanpa kunci skor** (keputusan user 2026-09-15: kunci skor DISC tidak dimuat ke sistem, HR menilai DISC sendiri dari layar): dikodekan di branch bip-erp `feat/recruitment-disc-lembar-jawaban` dan erp-frontend `feat/psikotes-disc-lembar-jawaban`, **belum merged**. Teks bertanda *(tanpa kunci)* di bawah menggambarkan kode di branch itu; sampai merged, `main` masih mewajibkan huruf dimensi DISC dan laporan DISC hanya membawa hitungan. Keputusan dan penyimpangannya: [[ADR - 0087 Katalog Tipe Psikotes Jadi Master Data, Tiga Bentuk Jawaban Tetap Kode]] butir (9).
