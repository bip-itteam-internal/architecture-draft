...
- *(persen)* ⚠️ **PR terbuka, belum merged** (erp-frontend [#1618](https://github.com/bip-itteam-internal/erp-frontend/pull/1618), 2026-09-17). Laporan bagian pilihan ganda menambahkan **persen** di baris total yang sudah ada: `38 benar dari 50 soal · 76%` (key `totalBenarDariPersen`). Aturannya:
  - **Hanya bagian tuntas** (`bagianTuntas`, satu aturan yang juga dipakai label status bagian). Bagian terputus dan yang masih dikerjakan tetap menampilkan hitungan mentah **tanpa** persen: soal yang tak pernah dicapai terhitung salah, jadi persennya terbaca seperti nilai sungguhan (aturan T0).
  - **Nol soal = tanpa persen**; "0 dari 0 · 0%" terbaca seperti gagal total.
  - **Per bagian, bukan per paket.** Di dalam blok bagiannya sendiri persen tak mewakili paket campuran, jadi butir di atas tak dilanggar.
  - Diformat di render menurut bahasa aktif (`59,3` di id, `59.3` di en). Angka faktual, tanpa kategori atau norma ([[ADR - 0087 Katalog Tipe Psikotes Jadi Master Data, Tiga Bentuk Jawaban Tetap Kode]] Decision 7). Tak ada field backend baru: persen diturunkan dari `total_benar` dan `total_soal` yang sudah ada di respons laporan.
- ⛔ **Jangan membangun di atas `skor_otomatis` pada `/candidates/:id/test-results`.** Field objek itu (`{benar, total, persen, sumber, selesai_pada}`) beserta kotak "Skor sistem" di kartu Hasil Tes **sudah merged** 2026-09-17 (bip-erp [#1931](https://github.com/bip-itteam-internal/bip-erp/pull/1931), erp-frontend [#1616](https://github.com/bip-itteam-internal/erp-frontend/pull/1616)) dan **sedang dicabut** (bip-erp [#1933](https://github.com/bip-itteam-internal/bip-erp/pull/1933) revert, #1618 di atas). Alasannya dari `/review` sesudah merge, dan ketiganya berlaku untuk percobaan serupa berikutnya:
  1. **Duplikat.** Total benar sudah tercetak di laporan pada kartu yang sama; satu-satunya fakta baru adalah persen.
  2. **Nama bertabrakan.** `skor_otomatis` sudah dipakai `disc.skor_otomatis` (butir tanpa kunci di atas) sebagai **bool** dengan arti lain, di berkas Go yang sama (`psikotes_paket_skor.go`). **Cari nama field lebih dulu sebelum menamai field baru.**
  3. **Pemasangan lewat `round_id` saja tak aman.** `round_id` babak Psikotest sama untuk semua kandidat; kebenarannya bergantung penuh pada filter `candidate_id`, dan menghapus filter itu tetap membuat seluruh test hijau.
...
| Bukti kunci | `test_buat_cfit.py` memakai pemeriksa yang ditulis TERPISAH dari pembangkitnya: seri dan matriks dihitung ulang dari aturannya, klasifikasi menurunkan pasangan jawabannya dari sifat yang dimaksud soal, kondisi diuji geometri (ada wilayah yang muat titik). ⛔ Versi pertamanya **tautologis** untuk Klasifikasi (katalog sifat di test sama dengan milik generator, jadi testnya mustahil merah) dan baru ketahuan saat review; kontrol negatif yang hanya memutasi KUNCI tak bisa mengeksposnya, yang memutasi OPSI bisa |
...
- ⚠️ **Paket bawaan Staff belum siap sampai bank soalnya diisi.** Di prod ini keadaan hari pertama, bukan kasus pinggir; dialog Kirim Tes karena itu menautkan alasan "belum siap" ke Bank Soal. *(internal)* Di DEV sudah siap sejak 2026-09-16 (CFIT diisi soal internal, DISC diisi 24 grup tanpa kunci); di prod masih berlaku apa adanya.
