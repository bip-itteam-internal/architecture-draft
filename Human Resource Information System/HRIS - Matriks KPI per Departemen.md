...
| Bobot | Label | Target / keterangan | Sumber di sistem erp | Rekomendasi |
|---:|---|---|---|---|
| 0.25 | `Monitoring AR & Collection ... < 5% dari total AR.` | Mengurangi piutang aging > 60 hari sampai < 5% dari total AR. | **Koreksi 2026-08-26**: sumber `kinerja_ar` metrik `piutang_lewat_60_persen`, sudah terdaftar dan **sudah ter-deploy di prod** (`kpi_sumber_ar.go`). Konfigurasi identik sudah hidup di template `AR STAFF PIUTANG`. | ✅ **Bisa dinyalakan sekarang, tanpa deploy** — cukup konfigurasi `{sumber: kinerja_ar, metrik: piutang_lewat_60_persen, formula: rata_rata, target: 5, arah: turun, scope: department}`. ⚠️ Angkanya akan **sama persis dengan AR Staff**: `kinerja_ar` menarik piutang se-perusahaan tanpa parameter departemen. ⚠️ **`scope` di sini KOSMETIK** — lihat catatan cakupan di bawah tabel. |
| 0.25 | `Rasio EBITDA 45%` | Kontrol OPEX dengan Budget Compliance 95% (Varians antara budget vs realisasi OPEX ≤ ±5%) | ⚠️ **Ralat 2026-08-31.** Klaim lama *"Budget TIDAK tersimpan di ERP mana pun"* sudah usang: master anggaran OPEX **ada** (`anggaran_opex` di integration-service) dan sumber KPI `varians_anggaran` (`kpi_sumber_varians_anggaran.go`) sudah di `main` sejak 13 Agustus 2026 dengan metrik `varians_absolut_persen` yang memang dibentuk untuk target dua-arah "≤ ±5%". Yang **belum masuk `main`** adalah penyempitan per-departemen: PR [bip-erp#1546](https://github.com/bip-itteam-internal/bip-erp/pull/1546) (branch `feat/kpi-varians-departemen`, commit `e5cc5470`, sudah di-review dan disetujui) mengirim `&departemen=<departemen karyawan yang dinilai>` ke `GET /accounting/anggaran/varians`. | 🟡 **Menunggu merge PR#1546, lalu bisa dinyalakan tanpa pekerjaan tambahan** — `{sumber: varians_anggaran, metrik: varians_absolut_persen, formula: rata_rata, target: 5, arah: turun}`. ⚠️ **Prasyarat DATA yang bisa membatalkannya**: `saringVarians` mencocokkan `anggaran_opex.departemen` dengan `work_data.department` sebagai string PERSIS. Bila kedua daftar tak beririsan, metriknya gagal hitung untuk setiap karyawan — itu pekerjaan data, bukan kode. **TBD:** irisan kedua daftar belum diperiksa ke basis data produksi; cara verifikasi: `scripts/kpi-spv-finance/f0-prasyarat.js` §D di bip-erp branch `bip-erp/t_957e449a-…`. |
| 0.2 | `Net income 20%` | Kontrol Beban Non-Operasional ≤ 2% | ⚠️ **Ralat 2026-08-31.** Klaim lama *"bisa otomatis sekarang"* menyesatkan: proxy Accurate memang ada, tapi **tak ada satu pun metrik KPI yang menghitung rasio itu** sampai 31 Agustus 2026. Pembilangnya sudah lama ada (3 akun `DaftarAkunAdminNonOps`, tersalin harian); **penyebutnya tidak pernah ditentukan** — "≤ 2% dari APA" adalah pertanyaan yang baru dijawab sekarang. Metrik `admin_non_ops/penurunan_yoy_persen` yang sudah ada **bukan** jawabannya (ia mengukur penurunan terhadap baseline tahun lalu, bukan rasio terhadap pendapatan). | 🟡 **Menunggu merge PR [bip-erp#1548](https://github.com/bip-itteam-internal/bip-erp/pull/1548) + [erp-frontend#1332](https://github.com/bip-itteam-internal/erp-frontend/pull/1332)** (branch `feat/kpi-rasio-non-ops`). Keputusan SPV: **penyebut = pendapatan periode itu sendiri**, diambil dari baris top-total yang menyebut "pendapatan" pada `GET /accounting/profit-loss`. Sesudah merge: `{sumber: admin_non_ops, metrik: rasio_beban_non_ops_persen, formula: rata_rata, target: 2, arah: turun}`. ⚠️ `rasio_terdefinisi: false` (labarugi belum tersalin / tak ada baris total) menjadi **galat**, bukan 0 — pada arah turun, 0 adalah nilai SEMPURNA. ⚠️ `scope` di sini juga **KOSMETIK**. |
| 0.1 | `Return on Operation = 2,75` | Cashflow Forecasting mingguan dengan akurasi ≥ 95%. | ⚠️ **Ralat 2026-08-31 — sama dengan Cost Control #4.** Deskripsinya adalah *cashflow* forecasting, bukan demand planning, dan sumber `forecast_kas` metrik `akurasi_forecast_kas` sudah ada di `main` sejak 14 Agustus 2026 (`kpi_sumber_forecast_kas.go:29`). ⚠️ **Namun label dan deskripsinya berbeda hal**: label mengukur `Return on Operation = 2,75`, deskripsinya mengukur akurasi forecast. Satu baris, dua penilaian. | ✅ **Bagian akurasi forecast bisa dinyalakan sekarang, tanpa deploy** — `{sumber: forecast_kas, metrik: akurasi_forecast_kas, formula: rata_rata, target: 95, arah: naik}`. ⚠️ Angkanya akan **identik dengan Cost Control #4**: sumber yang sama, tanpa dimensi karyawan maupun departemen. ⚠️ `scope` **KOSMETIK**. ⛔ Bagian `Return on Operation = 2,75` **tetap perlu dipecah dengan pemilik metrik** — belum jelas angka mana di sistem yang dimaksud (TBD; jawabannya hanya bisa datang dari pemilik metrik, bukan dari kode). |
...
#### ⚠️ `scope` pada F1, F3, dan F4 tidak mengubah angka apa pun
...
⚠️ F2 (`varians_anggaran`) **dulu juga buta-departemen** — itulah yang diperbaiki PR#1546. Sesudah PR itu merge, F2 menjadi satu-satunya dari lima yang benar-benar per-departemen.
...
|---|---|---:|---:|
| Hari-1, tanpa merge PR apa pun | F1 + F4 (F5 menyala tapi belum berangka) | 0,25 + 0,10 = **0,35** | **35** |
| + F5 berangka (staf Finance berotomasi penuh) | F1 + F4 + F5 | 0,55 | 55 |
| + PR#1546 & #1548 merge dan dinyalakan | F1 + F2 + F3 + F4 + F5 | 1,00 | 100 |
...
| 0.2 | `Culture 2` (metrik `culture` di template aktif) | Persentase terlaksananya program culture yang sesuai jadwal | 🟡 **TERPETAKAN — sumber `program_culture`** (branch `feature/workspace-position`, belum merge/prod). Menarik `GET /internal/culture/metrics` dari form-builder: form realisasi program culture (di-seed dari template generik) dibaca per periode; % program yang jawaban `terlaksana_sesuai_jadwal`-nya "Ya". Identifikasi form lewat `template_id`, bukan `metric_key`. Detail: [[Microservices - Form Builder Service]], [[ADR - 0065 Template Form Generik untuk Realisasi Program (Culture)]]. | Tinggal diisi HR di "Atur Target": sumber `program_culture` (tanpa sub-metrik, tanpa scope), formula `rata_rata`, target 100, arah naik. Data kosong digalatkan (jatuh manual), bukan 0. |
