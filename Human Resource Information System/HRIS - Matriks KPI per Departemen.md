...
- **Data `learning_db`**: `training` 2 (keduanya `Ongoing`, satu bertaut materi), `training_participant`
  3 (2 hadir), `course` 1, `quiz` 1 (**hanya jenis `post`**), `quiz_attempt` 0, `trainer_evaluation` 0,
  `training_plan_item` 0, `training_certificate` 0, `training_request` 0.
- ⛔ **Materi satu-satunya belum punya bank soal PRE.** Dengan aturan Tahap 3b, pre-test tak
  ditawarkan, dan begitu kelas bertautnya ditandai `Completed`, post-test terkunci permanen untuk
  SELURUH pesertanya. Bank soal pre wajib disusun HR selagi kelas masih berjalan.

Ringkasnya: **4 dari 6 metrik (bobot 0,75) sudah punya sumber otomatis yang jalan di prod**, 0 yang
dikonfigurasi, dan 2 metrik (bobot 0,25) belum punya sumber sama sekali.
...
dipakai.** Per 2026-09-17 `learning_db` prod berisi `training` 2 (keduanya `Ongoing`),
`training_participant` 3, `trainer` 2, `training_type` 4, `course` 1, `quiz` 1, sedangkan
