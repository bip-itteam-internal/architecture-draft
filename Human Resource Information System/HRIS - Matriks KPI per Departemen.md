...
| Bobot | Label | Target / keterangan | Sumber di sistem erp | Rekomendasi |
|---:|---|---|---|---|
| 0.25 | `Monitoring AR & Collection ... < 5% dari total AR.` | Mengurangi piutang aging > 60 hari sampai < 5% dari total AR. | **Koreksi 2026-08-26**: sumber `kinerja_ar` metrik `piutang_lewat_60_persen`, sudah terdaftar dan **sudah ter-deploy di prod** (`kpi_sumber_ar.go`). Konfigurasi identik sudah hidup di template `AR STAFF PIUTANG`. | ✅ **Sudah dinyalakan dan berangka** (2026-08-31): `{sumber: kinerja_ar, metrik: piutang_lewat_60_persen, formula: rata_rata, target: 5, arah: turun, scope: individu}`, realisasi 0,76 → **100**. ⛔ **Arah `turun` di sini BENAR dan tidak boleh diseragamkan dengan F4**: bunyi KPI-nya "< 5%", jadi makin kecil memang makin baik dan 0,76 < 5 layak bernilai penuh — beda hal dengan F4 yang metriknya mengembalikan akurasi. ⚠️ Angkanya akan **sama persis dengan AR Staff**: `kinerja_ar` menarik piutang se-perusahaan tanpa parameter departemen. ⚠️ **`scope` di sini KOSMETIK** — lihat catatan cakupan di bawah tabel. |
| 0.25 | `Rasio EBITDA 45%` | Kontrol OPEX dengan Budget Compliance 95% (Varians antara budget vs realisasi OPEX ≤ ±5%) | ⚠️ **Ralat 2026-08-31.** Klaim lama *"Budget TIDAK tersimpan di ERP mana pun"* sudah usang: master anggaran OPEX **ada** (`anggaran_opex` di integration-service) dan sumber KPI `varians_anggaran` (`kpi_sumber_varians_anggaran.go`) sudah di `main` sejak 13 Agustus 2026 dengan metrik `varians_absolut_persen` yang memang dibentuk untuk target dua-arah "≤ ±5%". Penyempitan per-departemen (PR [bip-erp#1546](https://github.com/bip-itteam-internal/bip-erp/pull/1546), branch `feat/kpi-varians-departemen`) **SUDAH merge** ke `origin/main` 31 Agustus 2026, merge commit `d5d50ff3` (diverifikasi `git merge-base --is-ancestor`): `GET /accounting/anggaran/varians` kini menerima `&departemen=<departemen karyawan yang dinilai>`. | ⚠️ **Sudah dinyalakan, tapi bergalat**: *"total anggaran periode ini nol"*. Konfigurasi terpasang `{sumber: varians_anggaran, metrik: varians_absolut_persen, formula: rata_rata, target: 5, arah: turun}` — **arah turun BENAR** di sini ("≤ ±5%", dan metriknya `math.Abs(TotalVarians)`, `kpi_sumber_varians_anggaran.go:81-83`). Yang kurang **data**: belum ada baris anggaran OPEX untuk departemen **Finance**. Unggah lewat menu **"Anggaran OPEX"** (`/finance/anggaran`, izin `finance.accounting.view`) — pesan galat backend menyebut "menu Master Anggaran", nama yang **tidak ada** di sidebar. ⚠️ Prasyarat: `saringVarians` mencocokkan `anggaran_opex.departemen` dengan `work_data.department` sebagai string PERSIS; anggaran departemen lain **tidak menolong**. ⛔ Galatnya jangan diganti 0 — pada arah turun 0 adalah nilai SEMPURNA. |
| 0.2 | `Net income 20%` | Kontrol Beban Non-Operasional ≤ 2% | ⚠️ **Ralat 2026-08-31.** Klaim lama *"bisa otomatis sekarang"* menyesatkan: proxy Accurate memang ada, tapi **tak ada satu pun metrik KPI yang menghitung rasio itu** sampai 31 Agustus 2026. Pembilangnya sudah lama ada (3 akun `DaftarAkunAdminNonOps`, tersalin harian); **penyebutnya tidak pernah ditentukan** — "≤ 2% dari APA" adalah pertanyaan yang baru dijawab sekarang. Metrik `admin_non_ops/penurunan_yoy_persen` yang sudah ada **bukan** jawabannya (ia mengukur penurunan terhadap baseline tahun lalu, bukan rasio terhadap pendapatan). PR [bip-erp#1548](https://github.com/bip-itteam-internal/bip-erp/pull/1548) (`fea804a7`) + [erp-frontend#1332](https://github.com/bip-itteam-internal/erp-frontend/pull/1332) (`de1f24ea`) **SUDAH merge** ke `origin/main` masing-masing repo, 31 Agustus 2026. | ⚠️ **Sudah dinyalakan, tapi bergalat**: *"INTEGRATION_SERVICE_KEY belum diatur"*. Konfigurasi terpasang `{sumber: admin_non_ops, metrik: rasio_beban_non_ops_persen, formula: rata_rata, target: 2, arah: turun}` — **arah turun BENAR** ("≤ 2%"). Keputusan SPV: **penyebut = pendapatan periode itu sendiri**, dari baris top-total yang menyebut "pendapatan" pada `GET /accounting/profit-loss`. Yang kurang **env di server**, bukan kode: `docker-compose.yml` `origin/main` sudah memuat kedua blok (employee-service & integration-service) merujuk satu variabel `${INTEGRATION_SERVICE_KEY}` yang sama, jadi mustahil terisi beda antar-blok **asal** compose-nya versi `bed49ad9` atau lebih baru. Langkah lengkapnya di [[HRIS - Otomasi Skor KPI]]. ⛔ Kunci kosong **menutup** rutenya (401) — disengaja. `rasio_terdefinisi: false` jadi **galat**, bukan 0. ⚠️ `scope` di sini **KOSMETIK**. |
| 0.1 | `Return on Operation = 2,75` | Cashflow Forecasting mingguan dengan akurasi ≥ 95%. | ⚠️ **Ralat 2026-08-31 — sama dengan Cost Control #4.** Deskripsinya adalah *cashflow* forecasting, bukan demand planning, dan sumber `forecast_kas` metrik `akurasi_forecast_kas` sudah ada di `main` sejak 14 Agustus 2026 (`kpi_sumber_forecast_kas.go:29`). ⚠️ **Namun label dan deskripsinya berbeda hal**: label mengukur `Return on Operation = 2,75`, deskripsinya mengukur akurasi forecast. Satu baris, dua penilaian. | ⛔ **SUDAH dinyalakan di produksi, TAPI ARAHNYA TERBALIK** (terukur 2026-08-31): terpasang `arah: turun`, seharusnya **`naik`** seperti yang tertulis di kolom ini sejak awal — `{sumber: forecast_kas, metrik: akurasi_forecast_kas, formula: rata_rata, target: 95, arah: naik}`. Akibatnya akurasi **38,03%** dinilai **100/100**; arah yang benar memberi 40. Kejadian, rantai buktinya, dan tindakan perbaikannya di [[HRIS - Otomasi Skor KPI]] §"Kesalahan arah F4"; pelajaran umumnya di [[RUN - Menambah Metrik KPI Otomatis]] §"Arah mengikuti BUNYI KPI-nya". ⚠️ Angkanya akan **identik dengan Cost Control #4**: sumber yang sama, tanpa dimensi karyawan maupun departemen. ⚠️ `scope` **KOSMETIK**. ⛔ Bagian `Return on Operation = 2,75` **tetap perlu dipecah dengan pemilik metrik** — belum jelas angka mana di sistem yang dimaksud (TBD; jawabannya hanya bisa datang dari pemilik metrik, bukan dari kode). |
| 0.2 | `Performance Monitoring Team` | KPI Tim minimal skor 70 | ✅ **Sudah menyala di produksi** (terverifikasi dari layar Atur Target periode berjalan 2026-08-31: penanda cakupan **"Se-departemen"** hanya muncul bila `Auto.Scope` terisi). Konfigurasi: `skor_tim` + `rata_rata` + `scope: department`. ⚠️ **Tapi belum menghasilkan angka**, layar berbunyi *"belum dapat dihitung: belum ada satu pun pengukuran pada periode ini"* (`kpi_team.go:106`, dipicu `len(c.Nilai)==0`). | ⛔ **Bukan cacat, dan bukan pekerjaan di template SPV ini.** F5 hidup dari BAWAH — lihat catatan cakupan & urutan penilaian di bawah tabel. Menyentuh konfigurasi F5 tidak akan mengubah apa pun. |
...
#### ⚠️ `scope` pada F1–F4 tidak mengubah angka apa pun (F2 dengan alasan berbeda)

Ketiga sumber di balik baris-baris itu — `kinerja_ar`, `admin_non_ops`, `forecast_kas` — **tidak memanggil `DaftarkanScopeSumber` dan tidak menyentuh `k.Karyawan` sama sekali** (dicacah 2026-08-31 ke `main`: nol kemunculan di ketiganya; bandingkan `kpi_sumber_cost_control.go` yang punya satu). `kpi_sumber.go:176` menyatakan sumber yang tak mendaftarkan cakupan **dianggap mengabaikan cakupan**, dengan baku jatuh ke `individu`.

Artinya `scope: department` yang tertulis di konfigurasi **tersimpan sah tetapi tidak menyaring apa pun**: angkanya se-perusahaan, sama untuk siapa pun yang dinilai. Buta-departemen ini **DISENGAJA** — komentar kepala tiap berkas menyebut alasannya (AR: seluruh toko dikelola bersama tim AR Sales dan endpoint sumbernya tak punya dimensi karyawan; `forecast_kas` & `admin_non_ops`: keduanya lahir untuk memasok metrik KPI **Cost Control**). Jadi ini **bukan cacat yang perlu diperbaiki**. Yang dicatat di sini agar orang berikutnya tidak mengira datanya sudah disaring per departemen padahal tidak.

⚠️ **Konsekuensi yang perlu disadari**: karena F3 dan F4 memakai sumber yang komentar kepalanya menyatakan diri memasok metrik **Cost Control** (#2 bobot 10% dan #4 bobot 15%), SPV Finance dinilai atas **angka yang identik dengan Cost Control** untuk dua metrik. Hal yang sama berlaku antara F1 dan AR Staff. Sah secara mesin; keputusan apakah itu yang dikehendaki ada di pemilik metrik (**TBD**).

⚠️ F2 (`varians_anggaran`) **dulu juga buta-departemen** — itulah yang diperbaiki PR#1546, yang **sudah merge** (`d5d50ff3`, 31 Agustus 2026). Sesudahnya F2 menjadi satu-satunya dari lima yang benar-benar per-departemen.

⛔ **Tetapi penanda "Individu" pada F2 juga TIDAK berpengaruh — dengan alasan yang berbeda, dan itu penting supaya tak salah diperbaiki.** F2 menyempitkan ke departemen karyawan **tanpa syarat**, tak membaca `cfg.Scope` sama sekali; komentarnya eksplisit di kode (`kpi_sumber_varians_anggaran.go:186-192`): *"Cakupan TIDAK didaftarkan lewat DaftarkanScopeSumber, sengaja — sumber ini SELALU menyempitkan ke departemen karyawan yang dinilai, tak bergantung pada nilai Scope template."* Jadi nilai scope apa pun menghasilkan permintaan yang sama. Bedanya dengan F1/F3/F4: di sana scope tak berpengaruh karena datanya memang se-perusahaan; di F2 scope tak berpengaruh karena penyempitannya sudah dipaksakan di kode.

⚠️ **Konsekuensi yang berlaku untuk keempatnya**: layar Atur Target menawarkan pilihan scope yang **tidak mengubah apa pun** pada F1–F4. Itu jebakan tampilan, bukan cacat perhitungan. **Jangan mengubah nilai scope untuk "memperbaikinya"** — mengubah nilai yang tak berpengaruh menambah risiko tulisan tanpa manfaat, dan `individu` adalah nilai yang sah menurut `ValidateKPIAutoConfig` (`kpi_source.go:183-188`).
...
| 0.15 | `Kebersihan 3` | Pelaksanaan 5R di Area Pantry dan Area Tanggung Jawab Kebersihan | 🟡 **Direncanakan 2026-09-11, kode belum ada**: inspeksi Satgas 5R & K3 oleh petugas OD & IR lewat Form Builder, nilai per orang = cek ulang terakhir dalam periode. Hari ini diisi manual dari lembar HRD dengan skala campur (1-5 dan 0-100). Keputusan: [[ADR - 0090 Inspeksi Satgas 5R dan K3 di Form Builder dengan Nilai dari Cek Ulang Terakhir]]. | Tunggu ADR 0090. **Jangan** memakai `nilai_layanan_pribadi`: sumber itu melebur seluruh form penilaian bertanda di departemen, jadi skor 5R akan tercampur rating pelayanan. |
