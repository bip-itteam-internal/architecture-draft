...
> | `pelatihan` (2026-09-15) | `kehadiran_peserta_persen` · `skor_post_test_persen` · `peningkatan_post_test_persen` · `kepuasan_trainer_skala10` · `kesesuaian_materi_skala10` | **Diukur prod 2026-09-24**, yang benar-benar terpasang hanya dua: People and Development `Training Attendance Rate` (`kehadiran_peserta_persen`, 0,1) dan `Skor Penilaian Training All Karyawan > 70 ` (**`peningkatan_post_test_persen`**, 0,35 — bukan `skor_post_test_persen` seperti tercatat sebelumnya). ⚠️ `Training Satisfaction Score ( 1 - 10 )` masih **manual**, dan Organizational Development **seluruhnya manual** (0,00 otomatis) |
> | `rencana_pelatihan` (2026-09-16) | — | People and Development `Terlaksananya kegiatan training & performance officer sesuai dengan rencana` (0,2). **Terpasang**, terukur prod 2026-09-24 |
>
> ⚠️ **Tidak ada sumber `stok`.** Metrik yang kolom sumbernya menyebut koleksi
> `manufacture_*` tetap **belum bisa dinyalakan** betapapun lengkap datanya.
>
> ~~**Ralat 2026-09-18 untuk paruh rekrutmen:** sumber `rekrutmen` belum ter-deploy dan belum
> dipasang ke template mana pun.~~
>
> ✅ **Ralat 2026-09-24 — paruh rekrutmen SUDAH BERGESER, ukur ke `employee_db` prod.** Sumber
> **`rekrutmen`** kini ter-deploy **dan terpasang**, dan template `Recruitment` sudah menilai
> memakainya di periode **2026-08**. Empat dari enam metriknya terpakai:
> `pemenuhan_tepat_waktu` (0,25) · `review_evaluasi_tepat_waktu` (0,2) ·
> `buffer_mpp_kritikal_persen` (0,2) · `retensi_masa_probation` (0,1). Totalnya **0,75 bobot
> otomatis** di posisi Recruitment & Onboarding, naik dari nol. Sisa manualnya satu baris,
> `Ketersediaan Dokumen Jobdesk diseluruh posisi` (0,25), dan itu terhalang ketiadaan
> penyimpanan jobdesk, bukan ketiadaan sumber. Baris-baris rekrutmen di dokumen ini
> **klasifikasinya bergeser**; yang belum ditulis ulang per bab ditandai di tempatnya.
>
> ⚠️ Yang tidak berubah: `pemenuhan_tepat_waktu_kritikal` dan `buffer_mpp_persen` ada di sumber
> tetapi **tidak dipakai** template mana pun. Paruh `stok` juga tidak berubah.
>
...
> **Segar 2026-09-17** (baca `employee_db` + `task_management_db` prod). Template aktif peran ini **`Building and Maintenance Staff`** (dibuat 2026-08-22), **3 metrik**, dipakai menilai satu staf pemegang peran ini (`GA Staff`) sejak skor 2026-08. Template 4-metrik `Building & Maintenance` yang tercatat di salinan 2026-08-01 dan versi `BUILDING & MAINTENANCE` berstatus **arsip**; metrik `Daily Report` tidak ada lagi. Posisi `GA Staff` juga punya template aktif kedua, `General Asset Staff HRGA`. Template yang benar-benar dipakai ditentukan skor terakhir orangnya, karena `kpi_template_assignment` kedua staf GA masih menunjuk template arsip dan penetapan ke template arsip diabaikan (`pilihTemplateEfektif`, `services/employee/kpi_auto_scores.go`). Hitungan bab General Affair di atas (5 template, 24 metrik) **tidak** diukur ulang.

| Bobot | Label | Target / keterangan | Sumber di sistem erp | Rekomendasi |
|---:|---|---|---|---|
| 0.3 | `Realisasi Preventif Maintenance Building & Fasilitas` | Target 100% | TIDAK ADA modul checklist berjadwal (patroli, 5R, GMP, preventive maintenance). Konsepnya di GA - Checklist Management. Space task-management `Building Maintenance` tidak punya tipe tiket, jadi pekerjaan preventif tak terpisah dari perbaikan (2026-09-17). | Belum bisa otomatis. Perlu daftar periksa berjadwal, atau tipe tiket "Preventif" di space bila GA memutuskan mencatatnya sebagai tiket. |
| 0.35 | `Menyelesaikan kerusakan secara cepat dan tepat ( jumlah Perbaikan Berhasil : Total Perbaikan )` | Target 100% | 🟡 **Sumber `kinerja_tiket` metrik `selesai_dinilai`** (bip-erp PR [#1962](https://github.com/bip-itteam-internal/bip-erp/pull/1962) + erp-frontend PR [#1638](https://github.com/bip-itteam-internal/erp-frontend/pull/1638), merged 2026-09-17, live di DEV dan terverifikasi lewat gateway 2026-09-17, belum deploy PROD). Definisi keputusan user: perbaikan berhasil = tiket **selesai DAN sudah dinilai pengaju** (nilai dari penangan tiket sendiri tak dihitung), dibagi tiket ditugaskan. Data: space task-management `Building Maintenance` (divisi General Affair, anggota dua `GA Staff`), **9 tiket** Juni-September 2026, 6 masih Todo termasuk tiket uji penugasan. Pemegang peran: Juni-Juli **nol tiket**, Agustus 1 dari 3, September 2 dari 2; ketiga nilai dari pengaju non-penangan. Skor manual metrik ini pada bulan yang sama jauh di atas angka dari tiket, karena tiket yang tak ditutup ikut menjadi penyebut. Rincian sumbernya di [[Microservices - Employee Service]]. | Sesudah deploy (task-management dulu): HR pasang di Atur Target, sumber Kinerja tiket / `selesai_dinilai`, formula `rasio_ambang`, ambang = bintang minimal 1..5 (backend menolak di luar itu), target 100, arah naik. **Sebelum dinyalakan GA merapikan space** (tiket basi dan tiket uji), kalau tidak angka Agustus terbaca 33% karena tiket yang tak dirawat. KPI bulan lalu sebaiknya dinilai sesudah pengaju sempat menilai; metrik berstatus `semi` selama ada tiket selesai yang menunggu nilai. Bulan tanpa tiket menjatuhkan metrik ke isian manual. |
| 0.35 | `Mengontrol biaya tanpa menurunkan kualitas (Realisasi/Budget) x 100% All Project Perbaikan` | Target 100% | Budget TIDAK tersimpan di ERP mana pun. Realisasi ada di Accurate; perlu master anggaran lebih dulu. | Belum bisa otomatis. Pengeluarannya sudah tercatat, tapi anggarannya belum pernah dimasukkan ke sistem, jadi tidak ada yang bisa dibandingkan. |
...
> |---|---|---|
> | `Time to Fulfilment Rate ( < 30 Hari ) All Vacant` | `pemenuhan_tepat_waktu` | tiap requisition Approved/Posted = `max(1, jumlah_dibutuhkan)` slot; slot dinilai di periode tempat tenggat **30 hari** sejak persetujuan jatuh, 100 bila hire-nya kurang dari 30 hari sesudah disetujui |
> | (versi posisi kritikal, lihat [[#HRD Supervisor]]) | `pemenuhan_tepat_waktu_kritikal` | sama, disaring posisi yang `is_kritikal` di MPP tahun persetujuannya |
> | `Presentase Data Base Buffer Kebutuan MPP` | `buffer_mpp_persen` · `buffer_mpp_kritikal_persen` | `hitungCoverage` milik recruitment-service (rumus yang sama dengan layar MPP, tidak disalin ke employee); hanya bulan berjalan atau bulan lalu |
> | `Membuat rencana jadwal dan pelaksanaan onboarding karyawan Masa Percobaan.` | `review_evaluasi_tepat_waktu` | kontrak `PKWT (Evaluasi)` yang `end_date`-nya jatuh di periode; 100 bila keputusan review onboarding-nya jatuh **di dalam** masa kontrak itu dan tidak melewati akhir kontrak |
> | `Tingkat turnover Masa Probation 0%` | `retensi_masa_probation` | karyawan dalam **3 bulan** pertama masa kerja (`work_data.join_date`); 0 bila resign "Mengundurkan Diri" efektif di periode dan masih di dalam 3 bulan itu |
>
> **Target tetap diisi tangan di template** (`Target 100%`): `kpi_template` tidak punya field
> target yang dibaca mesin, jadi sumber ini hanya memasok **realisasi**; ambang yang hidup di
> kode cuma tenggat **30 hari** (`tenggatPemenuhanHari`) dan masa probation **3 bulan**
> (`masaProbationBulan`). Metrik `Ketersediaan Dokumen Jobdesk ...` sengaja **tidak** dibuatkan
> sumber: tak ada tempat menyimpan jobdesk per posisi (baris di bawah tetap berlaku).
...
> **Ralat 2026-09-18 atas paragraf di atas** (benar saat ditulis 2026-09-02, kini tidak lagi):
> sumber `rekrutmen` sudah ditulis dan merged, jadi grup `sdm` bertambah satu nama dan
...
>
> 🟡 **Diperbarui lagi 2026-09-17: tema dan teaser (bobot 0,70) kini punya sumber yang sudah merged 2026-09-17, belum deploy.** Live Support menyetor tiap tema dan teaser di web ERP, penyetuju departemennya memutus, dan sumber `karya_live_support` menghitung yang disetujui ([[ADR - 0106 Tema dan Teaser Live Support Disetor dan Diputus Penyetuju Departemen sebagai Dasar KPI]]). Sesudah merged, deploy, dan ada setoran nyata, keempat metrik bisa dipasangi blok `auto`, dan skor posisi ini baru bisa dibekukan otomatis.
...
| 0.35 | `Membuat tema hostlive perbulan` (`kualitas-tema`) | Minimal 10 tema | 🟡 Sumber `karya_live_support` → **`tema_disetujui`** (merged 2026-09-17, belum deploy): jumlah tema yang disetor Live Support di web ERP dan **disetujui** penyetuju departemennya, berperiode bulan setor pertama. Scope `individu`, reduksi `jumlah_nilai`. `live_shifts` dan `mart_live_sessions` bukan kandidat karena mencatat sesi yang berjalan, bukan tema yang disiapkan. | Pasang blok `auto` (target 10) sesudah deploy dan setoran nyata pertama diputus. ⚠️ Pasangkan ke metrik ini lewat LABEL: key-nya `kualitas-tema`, walau namanya terdengar seperti mutu. |
| 0.35 | `Membuat cuplikan konten (Teaser)` (`kuantitas-tema`) | Minimal 10 cuplikan konten | 🟡 Sumber `karya_live_support` → **`teaser_disetujui`** (merged 2026-09-17, belum deploy): teaser yang disetor dengan tautan video dan disetujui; satu tautan terhitung sekali per orang. **`tt_shop_video_performances` terbukti bukan kandidat** (diukur 2026-09-17): 4 dari 123.144 judul memuat teaser/cuplikan, tak satu pun teaser live, dan videonya tertaut ke akun kreator, bukan karyawan. | Pasang blok `auto` (target 10) sesudah deploy dan setoran nyata pertama diputus. Setoran yang masih menunggu tinjauan saat skor dibekukan tak terhitung, dan tersebut di `Catatan`. |
