...
Template `KPI Cost Control`, **3 metrik** — direvisi perusahaan pada sheet KPI September 2026, menggantikan susunan tujuh metrik yang tercatat di sini sebelumnya. Bobot barunya **0,40 / 0,25 / 0,35** dan berjumlah tepat 1,0. Tiga metrik lama — `Pengelolaan Kas Iklan`, `Minimal 5 ide inovasi baru dari tim`, dan `Pertemuan 1-on-1` — tidak lagi ada di sheet, sehingga vonis "belum bisa otomatis" atasnya tak lagi menghalangi apa pun; dua di antaranya bergantung pada modul Kaizen dan log 1-on-1 yang memang tidak pernah dibangun.

| Bobot | Label | Target / keterangan | Sumber di sistem erp | Rekomendasi |
|---:|---|---|---|---|
| 0.40 | `Review Cash Outflow Mingguan` | Efisiensikan Realisasi OPEX −5% dari budget. Kolom `SISTEM ERP`: *"realisasi opex tanpa iklan dengan input budget"* | Sumber KPI `varians_anggaran` (`kpi_sumber_varians_anggaran.go`, di `main` sejak 13 Agustus 2026) lewat `GET /accounting/anggaran/varians`; populasinya **57 komponen OPEX**, bukan 57 dikurangi iklan. | ⚠️ **Golongan B.** Angkanya hidup di produksi, tetapi populasinya berbeda dari yang diminta kolom `SISTEM ERP`: `6107 Beban Iklan` Rp 3.579.122.639 adalah ≈70% dari anggaran Juli Rp 5.118.934.685, sehingga membuangnya mengganti metriknya, bukan menyaringnya. Tidak ada satu pun endpoint yang menerima parameter pengecualian akun — `/anggaran/varians` hanya punya `akun_no` yang **memilih satu** akun. Butuh keputusan Finance lebih dulu. |
| 0.25 | `Analisis biaya berulang dan rekomendasi perbaikan` | Penurunan biaya non-operasional ≥ 2% YoY. Kolom `SISTEM ERP`: *"Komparasi biaya bulan ini tahun ini vs bulan lalu tahun lalu"* | Dua sumber terpisah untuk dua paruh kalimatnya: `kinerja_cost_control` · `rekomendasi_efisiensi` (finance-service, mencacah rekomendasi) dan `admin_non_ops` · `penurunan_yoy_persen` (integration-service, 3 akun admin & non-ops). Keduanya jalan sejak 14 Agustus 2026. | ⚠️ **Golongan B.** Baseline sistem adalah **bulan yang sama tahun sebelumnya × 1,25** (`usecase.PeriodeBaseline` dan `PengaliBaselineYoY`), sedangkan sheet menulis *"bulan lalu tahun lalu"* dan tak menyebut pengali sama sekali. Sampai keduanya disamakan, angka Finance dan angka ERP menjawab pertanyaan yang berbeda. Pemecahan bobot 0,25 menjadi dua metrik adalah keputusan HR. |
| 0.35 | `Analisis Deviasi Forecast vs Aktual` | Realisasi anggaran maksimal 95% dari RAPB. Kolom `SISTEM ERP`: *"PENAMBAHAN FITUR LAPORAN ANGGARAN VS REALISASI"* | Sumber KPI `forecast_kas` · `akurasi_forecast_kas` lewat `GET /accounting/anggaran/mingguan/kpi`; populasinya **6 akun kas-keluar**, rumus simetris terhadap 100%. | ✅ **Golongan A.** Rantainya utuh dari ujung ke ujung dan angkanya terverifikasi ke rupiah. Cacat "nol di awal bulan" sudah ditambal (`ee6bdf38`, 14 Agustus 2026): tak satu pun sel terukur kini menghasilkan `akurasi_terdefinisi: false`, bukan 0%. Yang tersisa hanya memastikan konfigurasi templatenya benar. |

> ⚠️ **Bobot baru tidak boleh dipasang pada daftar metrik lama.** Sheet menyebut tiga metrik; template produksi masih memuat tujuh. Memasang 40/25/35 di atas tujuh baris menghasilkan total yang bukan 100, dan mesin KPI **tidak** menormalisasi ulang bobot metrik yang gagal dihitung — lihat [[HRIS - Otomasi Skor KPI]]. Penggantian daftar metriknya adalah pekerjaan HR, bukan konsekuensi otomatis dari perubahan bobot.

> ⚠️ **Baris 1 dan baris 3 menuliskan target yang sama tetapi berdiri di atas hitungan yang terpisah.** *"Realisasi OPEX −5% dari budget"* dan *"realisasi anggaran maksimal 95% dari RAPB"* identik secara aritmatika, namun populasinya (57 komponen OPEX vs 6 akun kas-keluar), versi anggarannya (RAPB Rev 2 vs RAPB asli), dan rumusnya (rasio terpakai vs simetris terhadap 100%) berbeda — sehingga terpakai 62,7% dan akurasi 47,81% untuk Juli 2026 **tidak dapat diperbandingkan**. Arah kebaikannya pun berlawanan: berhemat menaikkan baris 1 dan menurunkan baris 3. Dengan bobot baru, kedua baris ini bersama menguasai **75%** penilaian posisi. Rinciannya di [[Finance - Rancangan Finance Service]], bab "Sheet KPI revisi September 2026".
