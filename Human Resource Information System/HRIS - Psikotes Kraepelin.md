...
- ⚠️ **DUA implementasi mesin tes kandidat hidup berdampingan**, dan ini perlu diputuskan:
	...
	**Di prod yang dibuka kandidat adalah versi career portal.** Sampai deploy prod 2026-09-14 caranya `ERP_FRONTEND_URL` di container `Recruitment-Service` prod diisi `https://career.bharatainternasional.com` (dibaca 2026-09-10 dan 2026-09-12), dan akibatnya tautan karyawan yang membaca env yang sama (penilaian onboarding, feedback interview) ikut ke portal karir lalu 404. ✅ **Sejak bip-erp #1870 (merged, live dev dan prod 2026-09-14)** `psikotesURL` membaca `CAREER_PORTAL_URL` tanpa jatuh ke `ERP_FRONTEND_URL` ([[Microservices - Recruitment Service]] increment **Tautan Email per Penerima**). **Di dev**: sebelum perubahan itu tautan dev berawalan `https://erp-dev.bharatainternasional.com/psikotes/` (terukur 2026-09-10), jadi kandidat uji membuka versi erp-frontend; sesudahnya tautan dev mengikuti `CAREER_PORTAL_URL` dev yang berisi portal karir **prod** (career portal tidak ada di VM dev), sehingga tautan dari dev terbaca tidak berlaku, dan layar career portal hanya bisa diuji di prod atau lewat portal karir lokal. Keputusan resmi mana yang dipertahankan belum ada ([[ADR - 0087 Katalog Tipe Psikotes Jadi Master Data, Tiga Bentuk Jawaban Tetap Kode]] §Belum Diputuskan).
