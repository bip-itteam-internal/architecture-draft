...
- **Jadwal berulang menuju backend**: jadwal klub yang di sini masih konstanta FE direncanakan pindah ke **master program culture** (field jadwal hari/tanggal yang mem-pra-isi form), diusulkan [[ADR - 0093 Tipe Program Culture Non-Event Dinilai Terlaksana dengan Approval SPV HR, plus Jadwal di Master]]. ADR yang sama menambah **tipe program non-event** (mis. Kamis Batik) yang tak butuh absensi.
  ⚠️ **Sebagian sudah jalan, tapi BUKAN di layar ini** (2026-09-22, bip-erp [#1996](https://github.com/bip-itteam-internal/bip-erp/pull/1996) belum merged): **MyBharata** sudah membaca jadwal klub dari master lewat `GET /culture/clubs/mobile` ([[API - Form Builder Service]] § Culture untuk karyawan). Layar **Jadwal Club di web ERP ini masih konstanta FE** `data/clubs.ts`, jadi sejak saat itu ada **dua sumber jadwal klub yang bisa menyimpang**: master program (dibaca aplikasi) dan konstanta FE (ditampilkan web). Menyatukannya task tersendiri, dan sampai itu terjadi yang benar adalah master.
