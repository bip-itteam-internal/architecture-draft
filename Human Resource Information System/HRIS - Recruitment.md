...
- **Status**: ⚠️ **Sebagian besar sudah diimplementasi; yang menahan bukan fitur, melainkan alur yang belum pernah ditutup sampai `Hired`** (diukur PROD 2026-09-21, lihat §Pemakaian Nyata di Produksi). Fase 1-3 + adopsi struktur ERPGo (Fase A–F) live di [[Microservices - Recruitment Service]]; **portal karir publik sudah ada** ([[APP - Portal Karir Bharata]]); **psikotes online sudah dibangun dan kini multi-jenis** ([[HRIS - Psikotes Kraepelin]], [[HRIS - Bank Soal dan Paket Psikotes]]); ✅ **AI CV screening SUDAH ADA di kode** (`services/recruitment/cv_screening.go` + `ai_prompt_cv.go`, hasil disimpan di `candidate.cv_screening`; terpakai 1 kandidat di prod per 2026-09-21) — catatan lama "menyusul" sudah tidak berlaku. Yang **benar-benar belum**: WhatsApp kandidat (nol berkas di `services/recruitment`) dan sinkronisasi job board (Glints hanya label sumber di `seed_lookups.go`, bukan integrasi). ⚠️ **Rekrutmen lintas perusahaan: merged 2026-09-12, backend, Web ERP, dan portal karir live di prod, paket belum dipasang** (diukur 2026-09-12, lihat bagian "Rekrutmen Lintas Perusahaan" di bawah dan [[ADR - 0092 Rekrutmen Lintas Perusahaan lewat Paket Izin]]) · ✅ **Alur kerja HR (titik putus dibuka, kotak Langkah berikutnya, tombol Setujui offer dari `can_approve`): SUDAH di `origin/main`** (diperiksa 2026-09-21: `can_approve` di `services/recruitment/offer_setujui.go`; `dari_kandidat`, `Kirim Psikotes`, `mintaRevisi` ada di erp-frontend) — catatan "belum merge" 2026-09-12 sudah usang · 🔜 **Form Penilaian Masa Onboarding (20 butir + keputusan status kepegawaian HRD): kode ada di branch LOKAL `feat/recruitment-form-penilaian-onboarding-v2` yang belum pernah di-push**, jadi belum ada di remote mana pun (diperiksa 2026-09-21; lihat §Masa Evaluasi & Performance Review Onboarding) · ⚠️ **MPP: Posisi Kosong per resign + hak tulis MPP hanya SPV HRD dan staf rekrutmen: merged 2026-09-17 (bip-erp #1961, erp-frontend #1637), deploy dan paket posisi belum diverifikasi** (lihat §Rencana Tenaga Kerja (MPP)) · 🔜 **Lembar Actual vs Planning MPP belum bisa dihasilkan sistem**: [[ADR - 0113 Actual vs Planning MPP Dihitung Sistem per Bulan, Berpijak pada Jejak Keluar Bertanggal]] T1 merged 2026-09-21, T2–T5 belum ada kodenya
...
5. **Interview** — penjadwalan, **multi-tahap**: HR Interview → User Interview → **Technical Test (tes skill — terpisah dari psikotes)** → Final Interview (jumlah tahap tergantung posisi, mis. SPV) — HR + hiring manager/SPV dept; scoring + catatan per tahap. Lolos → **Background Check**. **Orkestrasi (✅ #498/#356):** sesi bisa ditautkan ke **babak** (`round_id`). **Feedback via link email (✅ #536/#381, 2026-07-18):** untuk stage **User & Final**, tiap pewawancara di panel (snapshot nama+email dikirim FE saat menjadwalkan, terpisah dari `Lokasi`) menerima **email undangan branded** (jadwal, lokasi/link meeting, tombol **"Buka Form Feedback"**) menuju halaman **login-gated tanpa sidebar** (`/interview-feedback/<id>`) tempat ia mengisi feedback sendiri (bentuk form mengikuti `form_type` babak: aspek bernilai + catatan bertipe + keputusan 3-arah untuk HR/User Interview, 3 rating + rekomendasi untuk babak generic; editable/upsert) — **menggantikan** menu **"Interview Saya"** (dihapus dari navigasi Portal Saya; komponen `MyInterviewsPage` dormant, sedangkan endpoint `/interviews/assigned` tetap dipakai halaman tautan). **(bip-erp [#1886](https://github.com/bip-itteam-internal/bip-erp/pull/1886) + erp-frontend [#1577](https://github.com/bip-itteam-internal/erp-frontend/pull/1577), merged dan live prod 2026-09-15; dev belum)** nilai dipilih lewat **tombol angka 1-5 dengan arti ujung "Sangat Kurang" sampai "Sangat Baik"** (sebelumnya bintang tanpa angka maupun arti); bentuk form diambil dari **sesi**, sehingga atasan non-HR kini mendapat form User Interview, bukan form generic; galat memuat sesi punya pesan + Coba lagi; rekap HR di detail sesi menampilkan catatan bertipe. Detail: [[Microservices - Recruitment Service]] increment Form Feedback Interview. Stage **HR**: tanpa email — HR isi feedback dari menu **Interviews** (dialog Lihat). **Kandidat** juga menerima email undangan jadwal (**semua stage**, tanpa link form). **Pengelolaan interview terpusat di menu Interviews (✅ FE):** jadwalkan (pilih kandidat aktif) + edit + hapus + salin link + lihat/isi feedback (terisi/total) + **tandai hasil sesi** (Selesai/Batal/Tidak hadir, 2026-09-16), semua dari sini; **detail kandidat menampilkan interview read-only** (jenis tahap lain — screening/tes/background check — tetap dicatat dari detail kandidat). Halaman tautan pewawancara `/interview-feedback/<id>` ikut menampilkan status sesi: untuk sesi batal, form penilaian dan tombol Gabung Meeting **ditarik** — tautan emailnya tetap hidup sesudah HR membatalkan dan tak ada notifikasi apa pun, jadi halaman itu satu-satunya tempat pewawancara bisa tahu. Belum: kalender/reminder ICS, auto-advance tahap dari hasil, **notifikasi ke panel saat sesi dibatalkan**.
6. **Psikotes / Tes keahlian** — ✅ **terimplementasi sebagai babak bertipe tes**, bukan modul tersendiri. Psikotest dan Technical Test adalah dua babak ber-`form_type: "test"` di katalog babak; keduanya **tidak dijadwalkan** lewat Proses Seleksi, melainkan direkam sebagai **form hasil per kandidat** di tab "Hasil Tes" pada detail kandidat: `result` (Pass/Fail/Pending) + `score` opsional + catatan. Menyimpan hasil lewat form itu **memindahkan tahap kandidat ke babak itu** dan menderivasi status (Pass → `Pending`, Fail/Pending → `Hold`). ✅ **(sudah di `origin/main`, diperiksa 2026-09-21 lewat `Kirim Psikotes` di erp-frontend; catatan lama "belum merge" usang)** menerbitkan psikotes (terbit maupun terbit ulang) juga memindah progress ke babak Psikotest tanpa menyentuh status, dan tab Hasil Tes kini menawarkan **Kirim Psikotes** untuk kandidat yang belum punya sesi (dulu tombol pertamanya hanya ada di sel Progress, dan hanya saat progress sudah Psikotest)
	...
	- ⚠️ **Sisa gap vs desain di dokumen ini:** **report PDF psikotes** masih belum ada tempatnya (laporan hanya tampil di layar HR, tak ada lampiran tersimpan). ✅ Koreksi 2026-09-15: **bank soal CFIT dan DISC sudah ada** sejak 2026-09-11 ([[HRIS - Bank Soal dan Paket Psikotes]]); isi soalnya tanggung jawab HRD, dan DISC dinilai HR dari lembar jawaban tanpa kunci skor (keputusan user 2026-09-15; merged 2026-09-15 bip-erp #1900 dan erp-frontend #1589). Ambang lulus/tidak tetap keputusan HR, tak dikodekan — sistem sengaja berhenti di `Pending`. *(internal)* Koreksi 2026-09-16: **bank CFIT tidak jadi diisi HRD** — HRD tak punya bahannya, jadi isinya 50 soal penalaran gambar **orisinal buatan internal** berformat CFIT dan tipenya dinamai "Penalaran Gambar (internal)" (skor jumlah benar mentah, tanpa norma dan tanpa IQ). Dimuat di DEV 2026-09-16, prod belum
...
## Pemakaian Nyata di Produksi — diukur 2026-09-21

⚠️ **Status yang bergerak. Ukur ulang sebelum mengandalkannya**; angka di bawah potret satu hari, bukan keadaan permanen.

Bagian ini ada karena pertanyaan "fitur rekrutmen sudah lengkap belum" berulang kali dijawab dari daftar fitur, padahal yang menahan justru **alur yang tak pernah ditutup**. Isi `recruitment_db` di produksi:

| Tahap | Jumlah |
|---|---|
| Job requisition | 10 (5 `Approved`) |
| Lowongan | 5 (4 `Open`) |
| Kandidat | 15 — progress: Onboarding 6 · Psikotest 5 · HR Interview 3 · CV Screening 1 |
| Status kandidat | Scheduled 6 · Pending 5 · Rejected 3 · Withdrawn 1 |
| Sesi interview | 8 (babak: 7) |
| Hasil tes · sesi psikotes · bank soal | 5 · 5 · 82 butir (3 tipe, 1 paket) |
| Background check | 1 |
| Offer | 5, **seluruhnya masih `Draft`** |
| Onboarding instance · review · jawaban review | 4 · 2 · 1 |
| Rencana MPP · keputusan posisi kosong | 6 · 6 |
| AI CV screening terpakai | 1 kandidat |

⛔ **Yang paling menentukan: NOL kandidat berstatus `Hired`, dan NOL kandidat tertaut `employee_id`.**

Konsekuensinya langsung ke penilaian orang: metrik KPI sumber `rekrutmen` yang sudah live (`pemenuhan_tepat_waktu`, `pemenuhan_tepat_waktu_kritikal`, `buffer_mpp_persen`, `buffer_mpp_kritikal_persen`, `review_evaluasi_tepat_waktu`, `retensi_masa_probation` — `services/employee/kpi_sumber_rekrutmen.go`) menghitung dari requisition **disetujui sampai kandidat `Hired`**. Tanpa satu pun `Hired`, "time to fulfilment" tak punya titik akhir, jadi angkanya nol **bukan karena sumbernya belum dibangun**. Menambah fitur tidak akan menggerakkan angka itu; yang menggerakkannya adalah alur dipakai sampai selesai di sistem.

Terkait: [[HRIS - Matriks KPI per Departemen]] (template `Recruitment`) · `Workspace/ANALISA - Actual vs Planning MPP.md` mencatat hal yang sama sebagai pekerjaan terpisah dari lembar MPP.

## Screening (AI sudah ada, keputusan tetap di HR)

*Penyaringan awal pelamar: HR menilai kecocokan CV/data pelamar terhadap **kriteria posisi** (dari kualifikasi yang difinalkan di requisition). Keputusan lanjut/tidak **tetap di HR**, dengan atau tanpa bantuan AI. Untuk pelamar via **Glints**, **Pertanyaan Skrining** Glints menjadi filter awal sebelum HR review.*

**Alur manual (tetap sah, dan masih jalur utama):**
...
**AI CV Screening — ✅ SUDAH ADA di kode** (diperiksa ke `origin/main` 2026-09-21, bukan lagi rencana):
- `services/recruitment/cv_screening.go` menarik teks CV lalu memanggil LLM lewat klien tipis shared-library ([[ADR - 0082 Integrasi AI lewat Klien Tipis di Shared-Library]]); prompt dan `json_schema` strict-nya di `ai_prompt_cv.go`. Hasilnya disimpan **menempel di dokumen kandidat** (`candidate.cv_screening`), bukan koleksi tersendiri — jadi jangan mencari koleksi `cv_screening` untuk mengukur pemakaiannya.
- Tetap **asisten, tanpa auto-reject**: keputusan lanjut/tidak tetap di HR, sesuai pengaman di bawah.
- ⚠️ Terpakai **1 kandidat** di produksi per 2026-09-21 (lihat §Pemakaian Nyata di Produksi), jadi jalurnya hidup tetapi praktis belum jadi kebiasaan kerja.
...
- `interview` — sesi interview (kandidat, babak `round_id`, panel pewawancara, jadwal, lokasi/meeting link); penilaian per pewawancara di `interview_feedback`
  - **`status` siklus sesi** (2026-09-16): `Scheduled · Completed · Cancelled · NoShow`, ditandai HR dari menu Interviews. **Batal & Tidak hadir wajib beralasan** (`status_reason`), dan koreksi salah tandai dilakukan dengan **membatalkan penandaan** (kembali ke Terjadwal), bukan loncat antar-hasil. Sesi lama tanpa field ini dibaca **Terjadwal**. Sesi batal/tidak hadir **tak menerima penilaian** — penilaian menggerakkan status kandidat, jadi sesi yang tak pernah terjadi tak boleh ikut memutuskan. **NoShow sendiri TIDAK menggerakkan status kandidat**: yang tak hadir belum tentu gugur, dan itu keputusan HR, bukan sistem
...
- `onboarding_review` + `onboarding_review_response` (✅ #493, merged) — sesi **Performance Review Onboarding** (masa evaluasi): peserta (**karyawan masa evaluasi**, `employment_type` "PKWT (Evaluasi)"), jadwal, penilai, 7 rating + 3 uraian per penilai, keputusan status. 🔜 Di branch (belum merged, 2026-09-17): jawaban ber-`form_versi` (20 butir + komentar + rekomendasi status kepegawaian), keputusan berkode status kepegawaian, dan index unik jawaban per penilai; lihat §Masa Evaluasi
...
> **Isi form disamakan dengan Google Form lama** (erp-frontend [#1565](https://github.com/bip-itteam-internal/erp-frontend/pull/1565), **merged 2026-09-14 11:46 WIB**, deploy belum diukur). ⚠️ Cabang galat memuat halaman penilai (`src/app/onboarding-review/[employeeId]/page.tsx:52`) memakai `isError` tanpa syarat data, sehingga refetch di latar yang gagal ikut menutup form yang sedang diisi; cacat yang sama ditemukan dan diperbaiki di form feedback interview 2026-09-14, perbaikannya di sini dijadwalkan sebagai PR terpisah (🔜 akhirnya ikut ditutup di branch form Penilaian Masa Onboarding 2026-09-17: layar galat hanya bila data belum pernah termuat; belum merged). Pembandingnya struktur Google Form "Form Review Performance Masa Evaluasi" yang diurai langsung dari halamannya:
>
...
> 🔜 **Form diganti: "Penilaian Masa Onboarding" (Google Form HRD, 2026-09-17).** Kode lengkap di bip-erp `feat/recruitment-form-penilaian-onboarding-v2` dan erp-frontend `feat/rekrutmen-form-penilaian-onboarding-v2`, **BELUM merged maupun deploy** (2026-09-17). Sampai merge, `main` masih berperilaku seperti uraian di atas.
>
> ⚠️ **Diperiksa ulang 2026-09-21: branch itu belum pernah di-push.** Ia tidak muncul di daftar branch remote mana pun, jadi kodenya hanya ada di worktree lokal satu mesin — bukan sekadar "belum merge". Siapa pun yang melanjutkan ini perlu mengambilnya dari mesin itu lebih dulu, dan risiko hilangnya nyata.
>
> | Bagian | Form lama (masih di `main`) | Form baru (branch) |
> |---|---|---|
> | Judul & pembuka | "Form Review Performance Masa Evaluasi", 2 paragraf soal presentasi | "Penilaian Masa Onboarding", 1 paragraf (objektif, adil, berdasarkan observasi kinerja) + panduan skala 1 = Sangat Kurang sampai 5 = Sangat Baik |
> | Penilaian | 7 aspek presentasi, skala 1–5 | **20 butir kinerja** (judul + deskripsi), skala 1–5, semua wajib: Daya Tangkap Instruksi sampai Antusiasme Belajar |
> | Uraian | 3 uraian (kelebihan, perlu ditingkatkan, saran) | 1 **Komentar** wajib |
> | Rekomendasi penilai | tidak ada | **Rekomendasi Status Kepegawaian** wajib: perpanjangan kontrak 3/6/12 bulan, Pemutusan Hubungan Kerja, Yang lain (+ penjelasan) |
> | Keputusan akhir HR | Lulus / Diperpanjang / Tidak Lulus | **Pilihan yang sama dengan rekomendasi penilai** ("tombol keputusan akhirnya sesuaikan dengan yg dari hrd"), lewat dialog konfirmasi yang menyebut peserta, pilihan, dan jumlah penilai yang belum mengisi |
>
> Keputusan user 2026-09-17 yang mengikat:
> - **Tanpa migrasi data.** Jawaban lama tetap dirender dengan 7 aspek + 3 uraian aslinya. Keputusan lama (PROD saat diukur 2026-09-17: 2 sesi, `Lulus` 1 dan `Diperpanjang` 1) tetap tampil dengan labelnya, tapi tak bisa dipilih untuk keputusan baru.
> - **Jawaban form lama terkunci.** Penilai yang sudah mengisi form lama melihat jawabannya hanya-baca, tanpa tombol kirim, dan tak bisa menambah rekomendasi (saat diukur: DEV 1 jawaban lama di sesi terbuka, PROD 0 sesi terbuka).
> - **Rekomendasi bahan pertimbangan, bukan keputusan.** Rekap HR menampilkan rangkuman rekomendasi penilai di dekat pilihan keputusan; pilihan HR tak diisi otomatis, dan keputusan tetap **tidak** menggerakkan kontrak di employee-service.
>
> Ikut berubah: rekap HR memisah rata-rata per versi form (butir dua form berbeda, jadi tak dirata-rata bersama); donut Onboarding Progress berkategori pilihan HRD + keputusan lama; daftar sesi dan ekspornya menulis label keputusan, bukan kode; butir yang belum diisi ditandai di tempatnya saat kirim ditolak; layar galat halaman penilai hanya menyala bila data belum pernah termuat. Kontrak, index baru, dan urutan deploy: [[API - Recruitment Service]] dan [[Microservices - Recruitment Service]].
>
> ⚠️ Teks 20 butir, komentar, dan pilihan status dikunci sama persis dengan Google Form HRD baru oleh `i18n-onboarding-review.test.ts`; teks form lama tetap dikunci karena jawabannya masih dirender. Peraturan Perusahaan (`mybharata-app/docs/development/BUSINESS_LOGIC_IMPLEMENTATION.md`) tidak mengatur durasi perpanjangan maupun masa evaluasi, jadi pilihan 3/6/12 bulan adalah kebijakan HRD, bukan pasal.
...
> ✅ **Sudah di `origin/main`** (diperiksa 2026-09-21 ke bip-erp `3b32b03f` dan erp-frontend
> `4ba0b2c72`): `can_approve` ada di `services/recruitment/offer_setujui.go`, dan
> `dari_kandidat`, `Kirim Psikotes`, serta `mintaRevisi` ada di erp-frontend. Catatan lama
> "kode lengkap di branch, belum merge" (2026-09-12, branch bip-erp
> `feat/recruitment-alur-kerja-hr` + erp-frontend `feat/rekrutmen-buka-alur`) **sudah usang**.
> ⚠️ Status deploy TIDAK ikut diperiksa di sini — ukur ulang sebelum mengandalkannya.
> Rincian layar: [[APP - Web ERP]] (modul Recruitment). Sisi service:
...
⚠️ **Data modul ini kini juga jadi bahan KPI otomatis** (bip-erp [#1967](https://github.com/bip-itteam-internal/bip-erp/pull/1967), erp-frontend [#1644](https://github.com/bip-itteam-internal/erp-frontend/pull/1644), merged 2026-09-18, ✅ **live prod 2026-09-21**, tetapi **belum dipasang ke template mana pun**). Sumber `rekrutmen` di [[Microservices - Employee Service]] membaca requisition beserta jejak persetujuannya, kandidat berstatus Buffer, cakupan MPP, dan keputusan Performance Review Onboarding lewat dua rute berkunci layanan di [[Microservices - Recruitment Service]]. Konsekuensinya untuk pemakai menu ini: **jejak yang tidak diisi menurunkan skor orang lain**, dan itu bukan hipotesis — diukur PROD 2026-09-21, 11 requisisi Agustus dan September tak punya SATU pun kandidat Hired yang tertaut lewat lowongannya, dan nol kandidat berstatus Buffer, sehingga metrik pemenuhan dan buffer akan berbunyi 0% begitu dipasang. Requisition tanpa audit persetujuan, kandidat Hired yang tak melamar lewat lowongan requisition-nya, dan posisi Buffer yang ejaannya tak berpadanan dengan MPP semuanya dikecualikan dari hitungan, dan jumlahnya ditampilkan sebagai baris pengecualian di rincian metrik. Metrik dan definisinya: [[HRIS - Matriks KPI per Departemen]].
...
>   non-BIP nol. Portal karir live prod 08:45 WIB (`company_name` di bundel 3 berkas, sebelumnya 0). Paket
>   `recruitment_lintas_perusahaan` **belum** dipasang (0 posisi, 0 akun, diukur 09:44 WIB), tetapi kedua
>   pemegang posisi target (HRD Supervisor, Recruitment & Onboarding) sudah admin pusat
>   (`system_roles.group = admin`), jadi lintas perusahaan sudah aktif bagi mereka.
...
> - ✅ **Halaman Permintaan Rekrutmen sempat melempar galat** saat disaring ke perusahaan tanpa departemen:
>   `GET /api/employee/data-type/department?company=` membalas `{"data":null}`, dan `useDataTypes`
>   meneruskannya ke `departments.filter(...)`. Diperbaiki erp-frontend
>   [#1548](https://github.com/bip-itteam-internal/erp-frontend/pull/1548) (hook menormalkan `null` jadi `[]`),
>   live prod 2026-09-12 09:43 WIB; perilakunya di layar (saring ke FLS) belum diuji.
>
...
- [x] **Interview: skala angka 1-5, bentuk form dari sesi, rekap catatan bertipe**: BE [#1886](https://github.com/bip-itteam-internal/bip-erp/pull/1886) / FE [#1577](https://github.com/bip-itteam-internal/erp-frontend/pull/1577) (merged dan live prod 2026-09-15, terverifikasi; dev belum), lihat langkah 5 di atas dan [[Microservices - Recruitment Service]]
- [x] ~~**Onboarding checklist per-kandidat**~~ — **DIHAPUS 2026-07-18** (dead code, komponen FE tak pernah dirender) — bekas BE #492 / FE #346
- [x] **Performance Review Onboarding (masa evaluasi)** — sesi review multi-penilai (7 rating + 3 uraian) → keputusan status — ✅ BE #493 / FE #349 (merged; halaman penilai membalas 200 di prod, terukur 2026-09-14 pada increment Tautan Email di [[Microservices - Recruitment Service]]). Isi form disamakan dengan Google Form lama: erp-frontend [#1565](https://github.com/bip-itteam-internal/erp-frontend/pull/1565) (merged 2026-09-14 11:46 WIB; deploy belum diukur), lihat §Masa Evaluasi. 🔜 Form diganti "Penilaian Masa Onboarding" (20 butir + komentar + rekomendasi; keputusan HR berpilihan HRD) di branch, belum merged (2026-09-17)
