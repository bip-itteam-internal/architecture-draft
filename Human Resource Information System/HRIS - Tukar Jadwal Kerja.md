...
- **Status**: ⚠️ **Implemented**, di `main` dan **terverifikasi jalan di DEV lewat gateway 2026-09-23** (catatan branch `feat/recruitment-service` "belum rilis" sudah usang). **Tukar Shift** = swap antar-rekan 3-langkah (consent → atasan → HRD); **Tukar Hari** = model single-person (geser hari). Keadaan PROD **belum diukur** untuk perubahan 2026-09-23; jangan menyimpulkannya dari sini. Lihat "Desain & Implementasi" + **Known limitation** di bawah.
...
- **Tenggat BEDA per tipe** (sejak bip-erp [#2033](https://github.com/bip-itteam-internal/bip-erp/pull/2033), 2026-09-23): **Tukar Shift `H+1`**, **Tukar Hari `H+3`**. Sebelumnya H+3 untuk keduanya, dan itu membuat kasus yang jadi alasan Tukar Shift ada tak pernah sekali pun muat: rekan yang izin mendadak baru diketahui sehari sebelumnya. Konstantanya `scheduleExchangeMinLeadDaysShift`/`Day` di `services/attendance/func.go`; tipe diresolusi **sebelum** tanggal divalidasi, dan tipe kosong dari klien lama jatuh ke tenggat yang lebih **ketat**.
- ⛔ **MENYETUJUI pengajuan yang tanggalnya sudah lewat DITOLAK** (400), dan **menolak tetap boleh**. Konsekuensi langsung dari H+1: persetujuan yang terlambat akan menulis ulang jam kerja hari yang sudah berlalu sehingga hitungan telat ikut berubah tanpa satu pun galat. Membetulkan masa lalu punya jalurnya sendiri lengkap dengan jejak audit ([[HRIS - Attendance Correction]], [[ADR - 0036 Roster Harian Menimpa Jadwal Dasar]] §8). Menolak sengaja dibiarkan terbuka supaya antrean basi tetap bisa dibereskan reviewer. Batasnya **awal hari**, bukan jam shift, sehingga menyetujui pagi hari untuk shift malam di hari yang sama masih sah; untuk Tukar Hari, tanggal yang paling awal di antara `work_date` dan `exchange_date` yang menentukan.
...
- **Auto-ignore & reminder** (konsisten leave/koreksi): pengajuan basi **>24 jam** → `Diabaikan` (termasuk tahap **consent rekan** yang tak direspons); **reminder** aktor saat ini (rekan/atasan/HRD) di **T+18 jam**. Cron `services/attendance/cron.go` (`cronAutoIgnoreStaleRequest` + `cronRemindStalePendingScheduleExchanges`).
	- **Pengingat KEDUA di T+2 jam untuk pengajuan mendesak** — yang `work_date`-nya tinggal kurang dari **48 jam** lagi (`urgentScheduleExchangeReminderFilter`, sejak #2033). Tanpa ini, pengajuan H+1 praktis tak pernah sempat diingatkan sebelum shiftnya berjalan padahal rantainya tiga orang. Memakai **jendela 1 jam** yang sama dengan pengingat biasa, jadi tiap pengajuan tetap diingatkan tepat sekali tanpa perlu flag "sudah diingatkan"; kedua jendela usianya berbeda (T+2 vs T+18) sehingga satu pengajuan tak mungkin terjaring dua-duanya pada tick yang sama.
...
- `exchange_date` minimal **H+1 untuk Tukar Shift**, **H+3 untuk Tukar Hari** (lihat "Aturan yang sudah diputuskan"). Pesan galatnya menyebut angka harinya sesuai tipe, jadi `at least 1 days` vs `at least 3 days` bisa dipakai membedakan tipe mana yang sedang divalidasi.
...
| PATCH  | `/schedule-exchange/review`     | Atasan/HRD setuju/tolak (body: `{ id, status, notes? }`). **Diblokir** sampai consent rekan disetujui. **`Disetujui` ditolak 400 bila tanggalnya sudah lewat**; `Ditolak` tetap boleh |
...
- **Pemohon** — pilih `work_date` → `GET /partners` → **pilih JAM tujuan** → alasan → submit (`POST /create`, payload RFC3339). Riwayat (`view`) + detail + **batal** (`cancel`).
	- ⚠️ **Sejak my-bharata [#167](https://github.com/bip-itteam-internal/my-bharata/pull/167) (2026-09-23) yang dipilih JAM, bukan ORANG.** Kandidat dari `/partners` dikelompokkan per `work_time` (`kelompokkanSlot`), tiap baris menyebut pemegangnya, dan rekannya diturunkan dari slot yang dipilih. Alasannya host menyebut kebutuhannya sebagai jam ("maju ke shift 2"); formulir lama meminta memilih orang dengan jamnya sebagai keterangan kecil, sehingga dari luar terbaca seolah opsi tukar jam tidak ada — itu bunyi keluhan yang melahirkan perubahan ini. **Payload ke BE tidak berubah** (`type:"shift"` + `partner_employee_id`, tanpa `exchange_work_time`; slot tetap diresolusi BE dari jadwal rekan).
	- Slot yang dipegang **lebih dari satu** orang menanyakan siapa lewat sheet kedua; memilihkan yang pertama diam-diam akan mengirim pengajuan ke orang yang tidak dimaksud pemohon, tanpa galat apa pun.
	- **Keadaan kosong dan keadaan gagal dibedakan tegas.** Kosong menyebut aturannya (satu departemen, terjadwal kerja pada tanggal itu, jam berbeda); gagal menampilkan pesan dari BE beserta aksi coba lagi. Menyamakan keduanya memberi tahu orang bahwa aturan rekan yang menghalanginya padahal permintaannya tak sampai.
