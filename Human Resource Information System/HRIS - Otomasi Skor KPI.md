...
⚠️ **Transisi sampai T5.** Template masih boleh menyimpan target untuk metrik profit dan mesin **mengabaikannya**; angka yang diketik HR di Atur Target tidak berpengaruh, tetapi basis dan kesimpulan kartu menyebut angka insentif sehingga tidak senyap. Penolakan validasi yang disebut ADR 0079 §7 sengaja digeser ke T5, karena frontend hari ini memblokir simpan bila `auto.target <= 0` (`auto-block-rules.ts`), sehingga membersihkannya di backend lebih dulu akan menjebak HR di template Kyura yang menyimpan `target_per_karyawan` untuk 11 orang. Tanpa migrasi data: 8 template prod yang masih menyimpan target dibersihkan bersama T5. Verifikasi lewat gateway (dev) belum dijalankan; daftarnya di artefak rencana.
