...
- **Status**: ⚠️ **24 metrik ber-`auto` di produksi per 2026-08-20**, tersebar di 11 template. Angka "tiga metrik Tech Development" di bawah adalah keadaan **6 Agustus 2026** dan sudah lama terlampaui; bab **Metrik otomatis yang sudah menyala** belum disegarkan mengikutinya. Sensus 2026-08-20 langsung ke `employee_db` prod: 24 dari 362 metrik di 83 template, menyentuh **38 dari 183 karyawan aktif**, dan **30 di antaranya berada di posisi yang otomasinya PENUH** (bobot 1,00) yaitu ICC Beauty Hacks 19 orang dan ICC Kyura 11 orang. Ketiga puluh orang itu kini dibekukan sistem, lihat [[ADR - 0048 Skor KPI Otomatis Penuh Dibekukan Sistem]]. Catatan lama "belum ada satu pun metrik yang terisi otomatis" **sudah tidak berlaku**. Mesinnya sendiri jalan sejak PR #843 (kontrak sumber nilai), #857 (mesin reduksi + arah target + registry), dan #866 (sumber `uptime_sistem`), deploy 1 Agustus 2026. **Inventaris sumber datanya ✅ grounded** ke kode `origin/main` bip-erp commit `23c6bdc8` dan sensus dokumen 15 database production per **2026-07-31**.
	...
	- ✅ **Sumber tingkat TIM naik ke produksi 2026-08-28, dan hari itu juga dipakai.** `kinerja_tiket_divisi` (enam metrik, dua kelompok space × tiga ukuran) ada di biner `Employee-Service` prod bersama `GET /kpi/space-group` di `Task-Management-Service` dan isian "Kelompok KPI" di bundel frontend. Pagi harinya rutenya masih `409` karena seluruh `space` produksi ber-`kpi_group` kosong; sore harinya kesembilan space Tech Development diisi (2 support, 7 development) dan **tujuh metrik dinyalakan sekaligus**, sehingga metrik ber-`auto` departemen itu naik 3 → 10. Rincian per metrik, target yang dipakai, dan angka pertamanya di [[HRIS - Matriks KPI per Departemen]] bab Tech Development.
	- ⚠️ **Kaizen: penghalangnya berpindah, bukan hilang.** `FORM_BUILDER_MODULE_URL` yang kosong di employee-service prod (catatan 25 Agustus) **sudah terisi** per 2026-08-28 dan endpointnya terjangkau, tetapi `GET /internal/kaizen/metrics` membalas `has_program:false` karena kedua form ber-`form_type: kaizen` di produksi sudah dihapus. Selama itu, metrik Kaizen di departemen mana pun **gagal hitung dengan sebab yang terbaca**, bukan bernilai nol — lihat [[HRIS - Kaizen (Ide Perbaikan)]].
...
| IT Support | `Network ` | 0,4 | `uptime_sistem` **tanpa metrik** (= `uptime`) | `rata_rata` | `individu` | 0,5, ditimpa `target_per_karyawan` **90** untuk `BIP-0226-01-26`, **arah naik** |
| IT Support | `Problem Solving` | 0,3 | `kinerja_tiket` metrik `ontime` | `rasio_ambang` ambang 0 | `individu` | 80, bertahap **60** untuk 2026-08 dan 2026-09 |
| IT Support | `Customer Satisfaction` | 0,15 | `kinerja_tiket` metrik `csat` | `rata_rata` | `individu` | 5 (skala 1..5) |
| Fullstack Developer | `System Development` | 0,5 | `kinerja_tiket` metrik `selesai_persen` | `rata_rata` | `individu` | 85 |
| Fullstack Developer | `Customer Satifaction` | 0,2 | `kinerja_tiket` metrik `csat` | `rata_rata` | `individu` | 5 (skala 1..5) |
| Tech Development Leader | `Revenue 240M` | 0,2 | `kinerja_tiket_divisi` metrik `support_selesai_persen` | `rata_rata` | — | 95 |
| Tech Development Leader | `Integration System Development di Q4` | 0,2 | `kinerja_tiket_divisi` metrik `development_ontime` | `rasio_ambang` ambang 0 | — | 60, bertahap **30** untuk 2026-08 dan 2026-09 |
| Tech Development Leader | `Customer Satifaction` | 0,1 | `kinerja_tiket_divisi` metrik `development_csat` | `rata_rata` | — | 5 (skala 1..5) |

> **Tujuh baris terakhir dinyalakan 2026-08-28**, sesudah `kinerja_tiket_divisi` naik ke prod dan kesembilan space Tech Development diberi `kpi_group`. Metrik ber-`auto` seluruh perusahaan naik **58 → 65**, `kpi_score` ber-`auto_value` tidak bergerak (tetap 2). Scope pada tiga metrik Leader ditulis `individu` di database tetapi **tidak berarti apa-apa**: `kinerja_tiket_divisi` mengabaikan `KPIAutoConfig.Scope`, sebab ruang lingkupnya ditentukan kelompok space di task-management.
>
> Angka pertama periode 2026-08, lewat `GET /kpi/auto-values` untuk orang sungguhan: Problem Solving 8 dari 10 tepat waktu → 100 (`otomatis`), System Development ketuntasan 80% → 94 (`otomatis`), Revenue 240M 90,9% → 96 (`otomatis`), Integration Q4 1 dari 7 tepat waktu → 48 (`otomatis`), dan **keempat metrik CSAT berstatus `semi`** dengan cakupan 14% sampai 60% karena yang menilai baru satu sampai enam pemohon. Rincian per metrik di [[HRIS - Matriks KPI per Departemen]].
...
>
> ⚠️ **Perbaikan itu dibatalkan lagi 26 Agustus 2026, dan tabel di atas sudah memuat bentuk yang berlaku sekarang.** Jejaknya ada di koleksi `kpi_template_audits` (dua baris, `BIP-0205-08-25`, 08:14 dan 08:15 WIB): langkah pertama membuang `metrik: downtime` dan mengubah scope `department` → `individu`, langkah kedua membalik arah `turun` → `naik` sambil menambah `target_per_karyawan` 90. Membuang sub-metriknya **tidak menggalatkan apa pun**: `kpi_sumber_uptime.go` sengaja memperlakukan metrik kosong sebagai `uptime`, supaya konfigurasi lama tidak mati diam-diam. Akibatnya metrik berbobot 0,4 itu kembali ke bentuk yang selalu bernilai 100 — uptime Agustus terukur **99,96%** terhadap target 90. **Sebelum menyalakan metrik Tech Development yang lain, putuskan dulu apakah bentuk ini yang dikehendaki**, sebab menambah metrik otomatis di sekitarnya membuat skor akhir makin bergantung pada satu angka yang tak bergerak.
...
`GET /kpi/auto-scores` menghitung **satu skor per karyawan** untuk kolom Otomasi di daftar Scoring dan badge skor di dropdown `/portal/kpi`; kartu skor total per-orang (tab Ringkasan → Per Karyawan) menghitung ulang dari `GET /kpi/auto-values` di frontend. Keduanya WAJIB memakai rumus yang sama: **jumlah berbobot** `Σ(bobot × usulan)` atas metrik yang berhasil dihitung, di mana **metrik yang belum terhitung dianggap 0** (BUKAN dibagi Σbobot). Karena Σbobot template selalu 1,0 (dijaga `ValidateKPIMetrics`), jumlah ini otomatis pada skala 0–100 dan **identik dengan skor final `ApplyKPIValues`** begitu seluruh metrik terisi — jadi pratinjau berjalan bertemu skor beku tanpa lompatan.

**Kenapa 0, bukan dinormalisasi (dua percobaan, keputusan akhir 2026-08-26):**

1. Semula `hitungSkorOtomatis` (`services/employee/kpi_auto_scores.go`) menjumlah tanpa dibagi (0-fill), sedangkan kartu skor total FE **menormalisasi** (`Σ(bobot×nilai) / Σbobot terisi`). Keduanya menyimpang untuk orang yang sama — SPV Kyura (cakupan 3/4) tampil 12 (jumlah) di badge vs 15 (rata-rata) di kartu.
2. Sempat "diperbaiki" dengan menyamakan keduanya ke **rata-rata** (normalisasi di backend). Itu keliru: normalisasi membagi ke metrik yang ADA, sehingga KPI yang baru satu metriknya menyala **melompat ke skor penuh**. Aan Budiyanto (Account Specialist) hanya metrik Retur (bobot 0,2) yang berdata di 100, dua lainnya belum — `(0,2×100)/0,2 = 100`, seolah KPI-nya sudah tercapai penuh padahal baru 20% bobot dikerjakan.
3. Keputusan akhir: **metrik belum terhitung = 0**, jumlah berbobot di KEDUA permukaan. Aan → `0,2×100 = 20`; SPV Kyura → 12 di badge maupun kartu. Konsisten antar-layar (motivasi awal) **dan** jujur soal cakupan.
...
> | `kinerja_ar` · metrik `*_persen` (4 terpasang, `piutang_lewat_90_persen` menyusul) | `rata_rata` | department, individu |
...
1. ✅ ~~**`shopee_shop_id` KOSONG di seluruh 12 baris `icc_account_mappings`**~~ — **SUDAH TIDAK BENAR, dikoreksi 2026-09-02.** Klaim itu benar saat diukur 2026-08-06 dan konsekuensinya nyata: `tokoMilikKaryawan` mengambil `tiktok_shop_id` dan `shopee_shop_id` lalu melewati yang kosong, sehingga profit dan revenue KPI hanya terhitung dari toko TikTok. Revenue Juli 2026 per channel: TIKTOK **Rp7.741.562.100** (20 toko) · SHOPEE **Rp1.951.681.920** (8 toko) · LAZADA Rp7.193.788 (2 toko), jadi kira-kira 20% omzet luput tanpa satu pun pesan.

	**Keadaan terukur 2026-09-02** (produksi, `integration_db`): 59 baris `icc_account_mappings`, **53 aktif**; `shopee_shop_id` terisi pada **8** baris aktif dan `tiktok_shop_id` pada **42**. Kedelapan toko Shopee yang punya baris belanja iklan **seluruhnya terpetakan** (8 dari 8), dan `mart_profit_attribution` level `shop` channel SHOPEE Agustus 2026 mengembalikan `icc.status: ditetapkan` untuk semuanya. `department_shops` sejalan: 47 TIKTOK · 8 SHOPEE · 3 LAZADA. Pemegangnya dua orang, keduanya berjabatan **`Marketplace Advertiser`**: BIP-0045-02-24 (Beauty Hacks, 4 toko) dan BIP-0086-09-24 (Kyura, 4 toko).

	⚠️ **Yang menggantikannya sebagai gap**: orang **ketiga** berjabatan `Marketplace Advertiser`, **BIP-0235-03-26** (Beauty Hacks), tidak punya **satu pun** baris `icc_account_mappings` — bukan baris berisi kosong, melainkan tak ada barisnya sama sekali. Seluruh metrik `insentif_profit` dan `kinerja_toko` miliknya karena itu tak punya toko untuk dijumlahkan, dan template Beauty Hacks hanya memberi `target_per_karyawan` kepada BIP-0045 sehingga ia jatuh ke target global Rp 2 miliar. Sama seperti pendahulunya, **ini pekerjaan data di ICC Management, bukan pekerjaan kode.**

	⚠️ **Cakupan pemetaan TikTok tidak diukur ulang** pada 2026-09-02. Angka lama "21 toko terotorisasi, 11 dipetakan" (2026-08-06) dibiarkan apa adanya dan **jangan dipakai sebagai fakta hari ini** tanpa dicek ke produksi.

	⚠️ Komentar `services/marketing-analytics/integration_db.go:249` masih berbunyi "per 2026-08-06 masih kosong". Komentar kode **belum ikut dikoreksi** (butuh PR tersendiri); yang berlaku adalah pengukuran di atas.
...
**Sumber baru `insentif_profit`** (`services/employee/kpi_sumber_insentif_profit.go`) membaca `GET /profit-dashboard` di [[Microservices - Insentive Service]]. **Empat** metrik: `profit` (realisasi apa adanya), `retur_persen` (sejak 2026-08-27), serta `roas` dan `roas_bersih` (sejak 2026-08-28). Scope `individu`→`icc`, `team`/`tim_icc`→`leader`, `department`→`supervisor`. Delapan template marketing memakainya per 2026-08-26 (2 SPV, 2 Leader, 2 Account Specialist, 2 Marketplace Advertiser).

**`roas` & `roas_bersih` DITAMBAHKAN di `insentif_profit`** (2026-08-28, commit `71aa32a1`; penyebutnya `omzet`, pembaginya `iklan_tiktok + iklan_shopee`). Alasannya sumbu periode: ROAS satu-satunya metrik marketing yang masih memakai `order_date` tanpa cutoff, sementara profit dan retur sudah pindah ke hari-kirim + cutoff tanggal 25 — satu kartu KPI karena itu menghitung "bulan Agustus" dengan dua definisi sekaligus. Terukur prod 2026-08-26 atas 15 Account Specialist: median ROAS **4,06 → 3,17**, yang lolos target 4,5 berkurang dari 5 orang jadi 4. Penurunannya tak merata (12–59%) dan itu memang yang hendak diukur: iklan sudah dibelanjakan, uangnya belum cair pada cutoff.

⚠️ **Versi `kinerja_toko` TIDAK dibuang** — beda dari `retur_persen` yang dimigrasi, di sini keduanya hidup berdampingan supaya template yang belum dipindah tetap terhitung. Konsekuensinya pemilih Sumber data menampilkan **empat** entri ROAS, dan sampai 2026-08-31 dua pasang di antaranya berlabel **sama persis** karena entri `METRIK_PER_SUMBER` di frontend tak ikut ditambahkan (erp-frontend `fix/kpi-label-roas-per-sumber`). **Pemindahan template ke `insentif_profit` adalah keputusan HR, bukan migrasi otomatis** — selama belum dipindah, tak ada angka yang bergeser. Lihat [[REF - Penamaan Metrik & Sumber KPI]].

**`retur_persen` PINDAH dari `kinerja_toko` ke `insentif_profit`** (2026-08-27, PR [#1462](https://github.com/bip-itteam-internal/bip-erp/pull/1462); migrasi boot `migrateKPIReturKeInsentif` mengganti `auto.sumber` di 8 template — **hanya sumber**, formula/target/arah/scope tak disentuh). Versi `kinerja_toko` membaca `|income.total_refund|` dari mart, dan di TikTok field itu **hanya terisi pada order CANCELLED** — yang justru dibuang saat mart dibangun. Akibatnya retur toko TikTok **selalu nol**: terukur prod 2026-08-26, tim Ade Rp35,4 juta / 370 order retur di insentif vs **Rp0** di KPI — skor retur 100/100 untuk tim ber-rasio 6%. Versi insentif membaca retur yang benar-benar dibukukan, per order, definisi yang sama dengan gerbang 7%. ⚠️ Nama metriknya **sengaja sama** di dua sumber; label pemilihnya dibedakan lewat `METRIK_PER_SUMBER` — lihat [[REF - Penamaan Metrik & Sumber KPI]].

⚠️ **Insiden 2026-08-27: seluruh metrik otomatis marketing mati.** Gerbang peringatan `insentif_profit` mencocokkan peringatan lewat **potongan kalimat** dengan penjaga fail-closed. Ketika insentive menambah kalimat baru "beban gaji tidak lengkap — dasar upah BPJS belum diisi…" (commit `2057201f`), kalimat itu muncul di SETIAP baris marketing dan mematikan **kedua** metrik untuk 8 template sekaligus — gejalanya `missing value for metric` bertubi-tubi di log, dan `ApplyKPIValues` menolak menghitung skor bila SATU metrik tak bernilai. Hotfix PR [#1480](https://github.com/bip-itteam-internal/bip-erp/pull/1480) mengenali kalimat itu sebagai boleh-lewat (kelas "gaji ADA tapi kurang ±Rp330rb iuran BPJS" ≠ "gaji tak ada sama sekali", yang tetap menahan profit). Penjaga fail-closed-nya BENAR dan dipertahankan; **akarnya belum ditutup** — kontraknya masih kalimat bebas, jadi kalimat baru berikutnya akan mengulanginya. Rencana: insentive mengirim `peringatan_rinci: [{kode, teks, dampak}]`.

⛔ **Metrik `pencapaian_target` sempat ada lalu dibuang.** Ia mengambil TARGET dari insentif, padahal KPI sudah punya targetnya sendiri di template (240 jt SPV, 50 jt AS, 3,4 M Leader). Dua target untuk satu penilaian, diketik dua orang di dua layar, tanpa apa pun yang menyamakannya. Pembagian yang benar: **realisasi dari insentif, target dari template KPI**. Dikunci test karena godaan menambahkannya kembali besar — ia *tampak* berguna, dan salahnya baru terlihat saat kedua angka menyimpang. 🟡 **DIBALIK 2026-09-07 oleh [[ADR - 0079 Target Profit Satu Pintu di Insentif, KPI Membacanya]]** (T2 dikodekan di bip-erp branch `feat/employee-kpi-target-insentif`, belum merge; lihat §Target profit dibaca dari insentif di bawah): manajemen memutuskan target insentif dan target KPI adalah SATU angka, pemiliknya insentif, dan KPI membacanya dari baris `/profit-dashboard`. Kekhawatiran di atas tetap benar; yang berubah adalah cara menutupnya, yaitu menghapus tempat kedua, bukan mempertahankan dua target. Test pengunci sudah ditulis ulang dengan arah sebaliknya (`TestSumberProfit_TargetDariInsentif`); `pencapaian_target` tetap tak ada karena pencapaian dihitung mesin, bukan diimpor.
...
- ✅ ~~Gerbang peringatan pukul-rata.~~ **DIPERBAIKI 2026-08-26.** Dulu sumber `insentif_profit` membuang baris begitu ada peringatan apa pun, sehingga Maftuhissaiin tak dinilai berbulan-bulan padahal realisasinya Rp486 juta sudah benar. Yang menahannya cuma dua hal yang tak menyentuh angka: "HPP mencakup 99,8%" (meleset 0,2%) dan "95 retur belum terbukukan" (retur memang tidak dikurangkan di rumus SK).
  ...
  Sekarang hanya peringatan yang **benar-benar mengubah angka** yang menahan penilaian: gaji belum ada, opex belum ada (dan, sampai T2 ADR 0079 di bawah, target belum diisi). Penjaganya sengaja **dibalik** — yang boleh lewat yang didaftar, dan peringatan baru yang belum dipertimbangkan **tetap menahan** sampai ada yang memutuskan sifatnya. Pencocokannya lewat potongan kalimat, yang memang rapuh; dengan arah ini rapuhnya jatuh ke sisi aman.

### Target profit dibaca dari insentif (T2 ADR 0079, 2026-09-07)

> **Status**: ✅ **live di prod 2026-09-07 22:16 WIB** (bip-erp PR [#1775](https://github.com/bip-itteam-internal/bip-erp/pull/1775), merged 21:42 WIB; dev lewat pipeline 21:44 WIB). Terverifikasi di prod dengan data sungguhan: `auto_target` Aan Budiyanto Agustus 44 jt dari insentif (template menyimpan 88 jt), Silvia Agistina 165 jt (template 116,89 jt), Aan September "belum dapat dihitung: target profit belum diisi di Master Target insentif", skor beku Annisa Agustus tak berubah. Yang masih menahan angka di prod bukan target lagi, melainkan peringatan lama seperti "proyek Accurate tidak ditemukan" pada baris Supervisor. Rencana: `.task-plans/2026-09-07-employee-kpi-target-insentif.md`.

Mesin skor mendapat satu konsep baru, **target dari sumber**: sumber yang memang memiliki target sebuah metrik mendaftar lewat `DaftarkanTargetSumber(sumber, tempat, metrik...)` (`services/employee/kpi_target_sumber.go`) dan meneruskan angkanya lewat `Cuplikan.TargetSumber`. Untuk metrik terdaftar, `hitungDariCuplikan` memakai angka itu dan **menolak jatuh ke template**: target yang belum diisi menghasilkan "belum dapat dihitung: target profit belum diisi di Master Target insentif untuk periode X", kelas yang sama dengan data yang belum ada pada sumber lain, bukan "gagal mengambil data". Pendaftar pertama dan satu-satunya: `insentif_profit` untuk metrik `profit`, dari field `target` baris `/profit-dashboard`. Level mengikuti cakupan seperti realisasinya, jadi target cakupan `team` adalah target baris Leader itu sendiri di insentif, bukan jumlah anggota. `retur_persen`, `roas`, dan `roas_bersih` tetap bertarget template.

Tiga hal yang ikut berubah:

- **`KPIMetric.AutoTarget`** menyimpan target yang **benar-benar dipakai** dan ikut ke snapshot beku, sehingga `GET /kpi/score` tak lagi menghitung ulang dari konfigurasi. Snapshot yang dibekukan sebelum field ini ada (Agustus 2026 ke belakang) tetap dibaca lewat `TargetBerlaku`, persis seperti saat dinilai. Satu fungsi `targetMetrik` menjawab "target mana yang tampil" untuk `/kpi/auto-values`, `/kpi/score`, dan `/me/kpi-score`; sebelumnya tiga pembaca itu memanggil `TargetBerlaku` sendiri-sendiri. Untuk metrik bersumber-target yang gagal dihitung, `auto_target` dikirim **null**, bukan target template yang tak pernah dipakai.
- **`GET /kpi/sumber-katalog`** mengumumkan `target_dari_sumber[]` per sumber supaya layar Atur Target (T5) membaca dari sini, bukan menyalin petanya ke frontend.
- Peringatan "target belum diisi" dari insentif kini **lewat** di gerbang peringatan sumber, sebab ia bukan komponen biaya yang hilang; mesin yang menanganinya. Akibat yang terlihat: orang tanpa target insentif tampil "belum dapat dihitung" (monitoring menghitung bobot metriknya 0, cron finalisasi melewati orangnya), bukan lagi tersembunyi sebagai "gagal sumber".

⚠️ **Transisi T5, tiga PR berurutan (2026-09-08).** Sampai PR A naik, template masih boleh menyimpan target untuk metrik profit dan mesin **mengabaikannya**; angka yang diketik HR di Atur Target tidak berpengaruh, tetapi basis dan kesimpulan kartu menyebut angka insentif sehingga tidak senyap. Penolakan validasi ADR 0079 §7 sengaja tidak dipasang sekaligus, karena frontend hari ini memblokir simpan bila `auto.target <= 0` (`auto-block-rules.ts`) dan menolaknya di backend lebih dulu akan menjebak HR di template Kyura yang menyimpan `target_per_karyawan` untuk 11 orang.

- **PR A, backend (bip-erp [#1783](https://github.com/bip-itteam-internal/bip-erp/pull/1783), live prod 2026-09-08 11:29 WIB)**: `ValidateKPIAutoConfig` dipecah jadi aturan umum + aturan target; `ValidateKPIAutoConfigTargetDariSumber` menerapkan aturan umum yang sama tetapi **menolak** target yang terisi di lapis mana pun; `validasiAutoKonfig`/`validasiAutoTemplate` (employee-service, `kpi_target_sumber.go`) memilih validator per registry `TargetDariSumber`, dan pada masa transisi **membuang** target yang masih dikirim layar lama (`BersihkanTargetOtomatis`) alih-alih menolaknya. Dipanggil `POST /kpi/templates` **sesudah** `GabungKonfigurasiOtomatis`, jadi target lama di dokumen ikut tersapu tiap kali template profit disimpan ulang; salinan dibuat sebelum dibersihkan supaya jejak `kpi_template_audits` tetap mencatat "target: X → kosong". Dua jalur pratinjau (`POST /kpi/auto-values/pratinjau`) memakai pemilihan yang sama.
- **PR B, frontend (erp-frontend [#1484](https://github.com/bip-itteam-internal/erp-frontend/pull/1484), live prod 2026-09-08 11:27 WIB)**: untuk metrik ber-`target_dari_sumber` di katalog, kolom Target di Atur Target jadi baca-saja berisi `auto_target` dari `/kpi/auto-values`, keterangan "dari Master Target insentif", dan tautan ke Master Target ("belum diisi" dan "tak terbaca" dibedakan dari nol); `bangunBlokAuto` membuang keempat lapis target dan `alasanBlokTakSah` tak menuntutnya, jadi payload simpan tak lagi membawa target profit. Bendera diturunkan lewat kunci string yang stabil dari katalog, bukan objeknya. T6 ikut di PR yang sama: halaman, menu, panel Direktur, dan rantai yatim Isi massal + hook pratinjau dihapus. Karena A dan C dimerge bersamaan sebelum deploy, backend dan frontend-hris naik berdampingan (jendela menit: simpan template profit dibalas 400 sampai keduanya naik).
- **PR C, backend (bip-erp [#1788](https://github.com/bip-itteam-internal/bip-erp/pull/1788), live prod 2026-09-08 11:29 WIB; biner terverifikasi tanpa `kpi_target_marketing.go` dan `auto-values/pratinjau`, rute membalas 404)**: pembersihan dihentikan (`bersihkanTargetTransisi = false`, dikunci `TestTransisiTargetProfitSelesai`), target profit yang masih dikirim ditolak **400** "target metrik ini ditulis di Master Target insentif, bukan di template KPI"; sekaligus T3: rute `/kpi/target-marketing*`, model, koleksi, dan `POST /kpi/auto-values/pratinjau` (kehilangan pemanggil) dicabut.
- 8 template prod yang masih menyimpan target: tersapu saat disimpan ulang, atau lewat skrip `.task-plans/2026-09-08-bersihkan-target-profit-template.ps1` (dry-run, cadangan EJSON, `$unset` empat lapis pada metrik `insentif_profit|profit` saja; dijalankan manusia, opsional).
...
Contoh Kyura Agustus 2026: 28 anggota, baru 12 yang punya skor, rata-ratanya 52,70 terhadap target 70 → skor 0. Enam belas sisanya memegang posisi yang metriknya **manual seluruhnya** (Host Live, Bootcamp Content Creator, Buzzer, Customer Support, Live Support, Meta Advertiser) — mereka menunggu penilaian atasan, bukan menunggu data. ⚠️ **Koreksi 2026-09-15 untuk Host Live**: posisi itu tidak lagi manual. Template `Host Live Kyura` seluruhnya ber-`auto` (sumber `kinerja_live`, terakhir diubah 27 Agustus 2026), dan ketujuh host Kyura sudah dibekukan sistem untuk Agustus dengan skor 94,18. Rinciannya di [[HRIS - Matriks KPI per Departemen]] bab Kyura › Host Live. Angka 28 anggota dan 12 berskor di atas tetap catatan saat itu dan tidak diukur ulang.
...
|---|---|---|---|
| Konversi iklan TikTok | ADV Marketplace, Leader, Affiliate (BHS + Kyura) | `integration_db.tt_business_gmv_max_performance_reports`, disegarkan cron harian 01:00 + intraday 4x sehari ([[IT - Background Jobs & Schedulers]]) | 712.855 |
| ROI/ROAS dan CPA | Leader, ADV Marketplace | `marketing_analytics_db.mart_profit_attribution` (level `ad`/`campaign`/`video`/`shop`/`product`) | 405.543 |
| Kinerja siaran live (conversion rate, add to cart, durasi tonton) | Host Live (BHS + Kyura). **Koreksi 2026-09-15**: dua baris di atas sempat mencantumkan Host Live; template yang berlaku per tanggal itu membaca siaran live, bukan laporan iklan | `marketing_analytics_db.mart_live_sessions` dijodohkan dengan `live_shifts`, sumber `kinerja_live` ([[Microservices - Marketing Analytics Service]] § KPI Host Live) | 6.363 sesi (diukur 2026-09-10) |
...
| Tidak ada modul maintenance gedung, 5R, patroli security, kebersihan. `guestbook_entries` adalah buku tamu, **bukan** log patroli ([[GA - Building Maintenance]], [[GA - Checklist Management]] masih konsep). ⚠️ Sejak sensus ini, perbaikan gedung dicatat sebagai tiket di space task-management `Building Maintenance`, dan metrik kerusakan Building & Maintenance punya sumber 🟡 `kinerja_tiket` / `selesai_dinilai` (bip-erp PR [#1962](https://github.com/bip-itteam-internal/bip-erp/pull/1962), merged 2026-09-17, terverifikasi DEV, belum deploy PROD); Office Boy dan Security `Kerapihan` punya `nilai_inspeksi_satgas`. Angka 12 adalah sensus lama dan tidak diukur ulang | Building & Maintenance, Office Boy Team, Security Team seluruhnya | 12 |
...
| Merujuk skor pemegangnya sendiri | 3 | Host Live BHS, Affiliate Kyura, Leader Kyura ("Skor final KPI tercapai sesuai target"). Per 2026-09-15 baris Host Live BHS tinggal riwayat: template `HOST LIVE` diarsipkan dan penggantinya, `Host Live Beautyhacks`, tak memuat metrik ini | ❌ sirkular, tetap manual |
...
| Host Live | 0,7 · 0,1 · 0,2 | `Conversion Rate` · `Add to Chart` · `Average view duration` | ✅ otomatis penuh per 2026-09-15 (template `Host Live Kyura`, sumber `kinerja_live`, scope `individu`, target 7 · 7 · 60), menggantikan `Conversion` · `ROI` yang semula tercatat di baris ini. Keadaan skor dan temuannya di [[HRIS - Matriks KPI per Departemen]] bab Kyura › Host Live |
...
12. Log 1-on-1 (9 metrik), sejalan dengan konsep [[HRIS - Work Review]]. **Rumahnya sudah ditentukan**: mesin kewajiban berulang di **irisan 3** [[Microservices - Calendar Service]], yang menyalin pola `FormPeriod` [[Microservices - Form Builder Service]]. Metrik Fullstack "Monitoring Kegiatan Sinkronisasi/Review dengan Requester" (bobot 0,2) menunggu irisan yang sama.

	⚠️ **Koreksi 2026-08-31: catatan lama "(belum ada kode)" di baris ini SALAH sejak 7 Agustus 2026.** Mesinnya sudah merged ke `main` lewat PR [#1074](https://github.com/bip-itteam-internal/bip-erp/pull/1074) — template kewajiban, penanggalan periode, cron per jam, potret peserta, dan penerbitan tagihan per orang semuanya berjalan. Yang **belum** ada adalah seluruh jalur yang menghubungkannya dengan orang: tak ada rute "kewajiban saya", tak ada cara menautkan agenda ke tagihan, tak ada cara menandai sesi selesai, tak ada papan kepatuhan, dan tak ada frontend. Konsekuensinya bukan sekadar "belum siap": **template yang dibuat hari ini akan menerbitkan tagihan yang mustahil dipenuhi, lalu cron menandai semua orang `missed`.**

	Karena itu jarak metrik ini ke otomatis bukan "bangun modulnya dari nol", melainkan tiga langkah: rute pemakai di calendar-service, rute internal ber-kunci-layanan sendiri di sana (ADR 0031 — `INTERNAL_GATEWAY_KEY` bukan gerbang), lalu `kpi_sumber_*.go` baru di [[Microservices - Employee Service]]. Rinciannya beserta dua keputusan produk yang masih menggantung ada di § Mesin kewajiban pada [[Microservices - Calendar Service]].
...
- ✅ ~~**`shopee_shop_id` kosong di seluruh 12 baris `icc_account_mappings`**~~ — **selesai, diukur ulang 2026-09-02**: 8 baris aktif terisi dan 8 dari 8 toko Shopee beriklan sudah punya penanggung jawab. 🔴 **Yang menggantikannya**: `BIP-0235-03-26` (`Marketplace Advertiser`, Beauty Hacks) tak punya satu pun baris mapping, jadi metrik otomatisnya kosong. Rinciannya di bab **Yang harus diketahui sebelum angkanya dipercaya**.
...
- 🔴 **Finalisasi otomatis tidak memeriksa tanggal masuk karyawan** (ditemukan 2026-09-15). Tak satu pun berkas jalur skor KPI membaca `work_data.join_date`: `git grep join_date` atas `services/employee/kpi*.go` dan `shared-library/models/employee/kpi*.go` di bip-erp `origin/main` `a155dcb8` hanya menemukan sumber `turnover_karyawan`. Akibatnya karyawan aktif yang templatenya otomatis penuh ikut dibekukan untuk periode yang sedang difinalisasi, walau periode itu sebelum ia bekerja. Terbukti di prod: satu host Beauty Hacks ber-`join_date` 26 Agustus 2026 menerima skor **Juli** 90 (dibekukan `OTOMASI` 28 Agustus 02:00 WIB) dan skor Agustus sebulan penuh 90,29, keduanya angka tim departemen karena sumber `kinerja_live` saat itu belum menghormati scope `individu`. Untuk Host Live kasus ini kini tertutup dengan sendirinya (tanpa shift tercatat, `penjagaIndividu` menggalatkan metriknya sehingga tak ada yang dibekukan), tetapi sumber yang menghitung dari data tim atau departemen, bukan data milik orang itu (mis. `skor_tim` scope `department`), **belum diperiksa**. **TBD**: lewati periode sebelum bulan `join_date` saat mengumpulkan kandidat finalisasi. Rincian kasusnya: [[HRIS - Matriks KPI per Departemen]] bab Kyura › Host Live.
...
- Sumber ROI/ROAS otomatis berasal dari [[Microservices - Marketing Analytics Service]] (didokumentasikan 2026-07-31, sebelumnya service ini berjalan di production tanpa dok). Perlu diperhatikan (**disegarkan 2026-08-26**): `/cohort` masih membalas kosong karena `mart_buyer_cohort` belum terisi. Dua klaim lama di baris ini sudah **tidak benar** — `/lives` terisi **4.836 sesi** sejak verifikasi produksi 2026-08-22, dan `/matrix/sku-shop` **bukan stub** melainkan terimplementasi penuh sejak 2026-08-02. Klaim `/price-floor` kosong **tidak diverifikasi** pada penyegaran ini; jangan dipakai sebagai fakta sampai dicek ke produksi. Konteks bisnisnya di [[Sales - Marketing Dashboard (Master Roadmap)]].
- 🟡 **Sumber baru `program_culture`** (branch `feature/workspace-position`, `kpi_sumber_culture.go`, **belum merge/prod**). **Apa yang diukur**: **skor komposit program culture** (blueprint 30/30/40: Partisipasi + Antusiasme + Implementasi, implementasi = part×ant÷100, otomatis) — metrik `culture` pada template aktif `HR Organizational Development` (posisi Culture & Industrial, dept Human Resource; template lama `Organizational Development` diarsipkan). **Dari mana**: `GET /internal/culture/metrics` di [[Microservices - Form Builder Service]] lewat `FORM_BUILDER_MODULE_URL` (pola identik `kaizen`: memo 30s, gerbang kunci layanan). Grup **SDM**, formula baku `rata_rata` **tanpa sub-metrik, tanpa scope**. Cuplikan: satu **skor komposit** per program (dari `culture_programs` + `culture_feedback` peserta), Populasi = jumlah program, `PopulasiNyata` true; reduksi `rata_rata` → skor KPI officer. **Data kosong** (belum ada program) → **GALAT**, metrik jatuh ke `manual`, bukan 0 (kosong bukan nol). Wiring sumber + target oleh HR lewat "Atur Target" ([[ADR - 0053 Struktur dan Target KPI Disatukan di Satu Halaman]]): formula `rata_rata`, target 100, arah naik. Keputusan mekanismenya: [[ADR - 0066 Modul Kelola Program Culture]].
...
- **Sumber otomatis yang sudah terpasang**: [[Microservices - Monitoring Service]] (`uptime_sistem`) · [[Microservices - Employee Service]] sendiri (`skor_tim`) · [[Microservices - Form Builder Service]] (`kaizen`; 🟡 **`program_culture`** — branch `feature/workspace-position`, belum merge/prod, lihat butir di bawah) · [[Microservices - Marketing Analytics Service]] (`kinerja_toko`, PR #1042) · [[Microservices - Integration Service]] (`kinerja_ar` — piutang rumpun AR Finance di `main` sejak 11 Agustus 2026; metrik retur `retur_lewat_14_persen` (12 Agustus) dan `retur_tuntas_cutoff_persen` (14 Agustus) keduanya ✅ live, realtime bulan-berjalan + popup Rincian sejak PR [#1252](https://github.com/bip-itteam-internal/bip-erp/pull/1252) 17 Agustus 2026; 🟡 **`piutang_lewat_90_persen`** — ambang ketiga untuk metrik AR Staff berbobot 0,50, branch `feat/kpi-piutang-lewat-90`, belum merge/prod, lihat butir di bawah).
  - [[Microservices - Task Management Service]] (`kinerja_tiket`, PR #1055 merged & di-deploy 6 Agustus 2026 — bukan lagi "sedang dikerjakan"). Tiga metrik dari satu agregat: `ontime` (ketepatan waktu penyelesaian, dipasangkan `rasio_ambang` ambang 0), `csat` (kepuasan pemohon skala 1..5, dinyatakan sebagai target 5 sehingga **tidak perlu konversi skala** walau KPI menulis "rating 10 dari 1-10"), dan `selesai_persen`. Rincian keputusan penyebutnya di [[API - Task Management Service]]. 🟡 Metrik keempat `selesai_dinilai` (tiket selesai yang dinilai pengaju non-penangan ÷ tiket ditugaskan, `rasio_ambang` berambang bintang 1..5) untuk Building & Maintenance ada di bip-erp PR [#1962](https://github.com/bip-itteam-internal/bip-erp/pull/1962) + erp-frontend PR [#1638](https://github.com/bip-itteam-internal/erp-frontend/pull/1638) (merged 2026-09-17, terverifikasi DEV, belum deploy PROD); rinciannya di [[Microservices - Employee Service]]
...
- ✅ [[Microservices - Integration Service]] (`kinerja_ar` · **`piutang_lewat_90_persen`**, bip-erp PR [#1579](https://github.com/bip-itteam-internal/bip-erp/pull/1579), **live di dev & prod 31 Agustus 2026**). Ambang KETIGA rumpun AR, untuk metrik `Pengawasan 100% AR aging ≤ 90 hari.` di template **`AR Staff 2026`** yang berbobot **0,50**. Konfigurasinya di template **belum dinyalakan HR**, jadi metriknya masih diisi tangan sampai itu dilakukan. Tiga hal yang membedakannya dari dua ambang sebelumnya:
	- **Menuntut perubahan di DUA service, dengan urutan yang mengikat**: `PiutangPosisiRow` di integration harus membawa `Lebih90` lebih dulu, baru employee dapat membacanya. Terbalik = field-nya absen, ter-decode jadi nol, dan nol pada metrik berarah `turun` adalah nilai **SEMPURNA**. Penjaganya pointer, bukan urutan deploy semata; rinciannya di [[Finance - Rancangan Finance Service]].
	- **Bentuk metriknya sengaja mengukur porsi yang MELEWATI ambang**, bukan komplemennya, dan pratinjau prod membenarkannya: realisasi Juli 2026 **11,76%** memberi nilai **8,5** pada target 1. Bentuk komplemennya ("porsi yang masih di bawah 90 hari", arah naik, target 100) akan memberi 88 untuk bulan yang sama, jauh lebih tumpul untuk metrik berbobot 0,50. Penerapan aturan "kalau realisasi wajar selalu melampaui target, metriknya salah bentuk" di [[RUN - Menambah Metrik KPI Otomatis]].
	- **Sekalian membetulkan penyebut yang menghitung sebagian rupiah dua kali.** `barisDariAgingSummary` menjumlahkan SELURUH bucket `/piutang/summary` jadi total, termasuk `>90` yang himpunan bagian `>60`, sehingga kedua metrik yang sudah live memakai pembagi yang menggelembung. Terukur di prod 2026-08-31: penyebut benar 1.564.893.659 vs 1.572.035.487 yang menggelembung, menggeser `piutang_lewat_60_persen` dari 0,5915% ke 0,5942% (keduanya membulat 0,59, jadi dampaknya memang masih kecil) dan membesar seiring saldo macet naik. Lihat [[API - Integration Service]].
	- ⚠️ **Angka bulan BERJALAN dan bulan TERTUTUP tidak sebanding, dan selisihnya besar.** Prod 2026-08-31: posisi live menunjukkan **0,46%** (nilai 100) sementara Juli yang sudah tertutup **11,76%** (nilai 8,5). Sebabnya populasi kedua pipeline memang berbeda dan itu disengaja — historis memakai `{SHIPPED, COMPLETED}` karena status adalah nilai KINI, sedangkan live hanya `SHIPPED`. Konsekuensinya bagi pemakai: **skor pratinjau sepanjang bulan berjalan bisa terlihat jauh lebih baik daripada skor final bulan itu**. Berlaku juga untuk ambang 14 dan 60, tetapi bobot 0,50 membuatnya paling terasa di sini. Sampaikan ke pemilik metrik sebelum target ditetapkan.
