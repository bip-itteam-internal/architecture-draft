...
- 🟡 [[Microservices - Integration Service]] (`kinerja_ar` · **`piutang_lewat_90_persen`**, branch `feat/kpi-piutang-lewat-90`, belum merge). Ambang KETIGA rumpun AR, untuk metrik `Pengawasan 100% AR aging ≤ 90 hari.` di template **`AR Staff 2026`** yang berbobot **0,50** dan sampai kini masih diisi tangan. Tiga hal yang membedakannya dari dua ambang sebelumnya:
	...
	- **Bentuk metriknya sengaja mengukur porsi yang MELEWATI ambang**, bukan komplemennya. "Porsi yang masih di bawah 90 hari" berarah naik terdengar lebih cocok dengan bunyi KPI-nya, tetapi realisasi wajarnya menempel di ujung skala (≈99,4%) sehingga metrik berbobot 0,50 itu akan bernilai penuh tiap bulan dan tak membedakan bulan baik dari bulan buruk. Ini penerapan langsung aturan "kalau realisasi wajar selalu melampaui target, metriknya salah bentuk" di [[RUN - Menambah Metrik KPI Otomatis]].
	- **Sekalian membetulkan penyebut yang menghitung sebagian rupiah dua kali.** `barisDariAgingSummary` menjumlahkan SELURUH bucket `/piutang/summary` jadi total, termasuk `>90` yang himpunan bagian `>60`, sehingga kedua metrik yang sudah live memakai pembagi yang menggelembung. Dampaknya kecil per 2026-08-31 (≈0,59%) dan membesar seiring saldo macet naik. Lihat [[API - Integration Service]].
