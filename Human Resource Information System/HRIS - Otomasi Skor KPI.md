...
**Sumber baru `insentif_profit`** (`services/employee/kpi_sumber_insentif_profit.go`) membaca `GET /profit-dashboard` di [[Microservices - Insentive Service]]. **Dua** metrik: `profit` (realisasi apa adanya) dan `retur_persen` (sejak 2026-08-27). Scope `individu`→`icc`, `team`/`tim_icc`→`leader`, `department`→`supervisor`. Delapan template marketing memakainya per 2026-08-26 (2 SPV, 2 Leader, 2 Account Specialist, 2 Marketplace Advertiser).
