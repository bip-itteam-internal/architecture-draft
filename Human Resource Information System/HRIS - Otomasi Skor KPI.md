...
- 🟡 **Sumber baru `program_culture`** (branch `feature/workspace-position`, `kpi_sumber_culture.go`, **belum merge/prod**). **Apa yang diukur**: "Persentase terlaksananya program culture yang sesuai jadwal" — metrik `culture` pada template aktif `HR Organizational Development` (posisi Culture & Industrial, dept Human Resource; template lama `Organizational Development` diarsipkan). **Dari mana**: `GET /internal/culture/metrics` di [[Microservices - Form Builder Service]] lewat `FORM_BUILDER_MODULE_URL` (pola identik `kaizen`: memo 30s, gerbang kunci layanan). Grup **SDM**, formula baku `rata_rata` **tanpa sub-metrik, tanpa scope**. Cuplikan: satu nilai per program (100 bila sesuai jadwal, 0 bila tidak), Populasi = jumlah program dilaporkan, `PopulasiNyata` true; reduksi `rata_rata` → % terlaksana sesuai jadwal. **Data kosong** (belum ada program / orang tak melapor apa pun) → **GALAT**, metrik jatuh ke `manual`, bukan 0 (kosong bukan nol). Wiring sumber + target oleh HR lewat "Atur Target" ([[ADR - 0053 Struktur dan Target KPI Disatukan di Satu Halaman]]): formula `rata_rata`, target 100, arah naik. Keputusan mekanismenya: [[ADR - 0065 Template Form Generik untuk Realisasi Program (Culture)]].
