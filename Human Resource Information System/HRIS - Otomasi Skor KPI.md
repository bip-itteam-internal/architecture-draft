...
>
> **Penghalang ketiga, dan ini soal bentuk laporannya bukan soal datanya**: rute `/report/*` di [[Microservices - Task Management Service]] hanya menerima `start_date` dan `end_date`, **tanpa penyaring per space**. Padahal KPI Leader IT memisahkan "E-Ticket Infra & IT Support" dari "E-Ticket Software Dev" sebagai dua metrik berbeda berbobot 20 masing-masing. Angka SLA dan CSAT yang keluar selalu gabungan seluruh space dalam cakupan pemanggil, dan rincian per space yang tersedia (`summary-by-department`) cuma total/open/done tanpa SLA maupun CSAT. Dua metrik itu tak bisa diambil dari layar apa adanya, berapa pun rapinya data nanti. Ditambah **skala CSAT 1–5 sedangkan ketiga KPI menargetkan skor 1–10**, sehingga konversinya perlu disepakati sebelum metrik apa pun disambungkan.
