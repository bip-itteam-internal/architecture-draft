...
> ⛔ **Model skor `turun` berubah 2026-08-27 — paragraf di atas berlaku pada model LAMA (plafon/anggaran) dan angkanya tak lagi berlaku.** Arah `turun` kini dinilai sebagai **ramp `[0, target]`**: target = **titik gagal** (skor 0), `(target − realisasi)/target × 100`. Dengan konfigurasi yang sama (target 0,5%), downtime Agustus 0,07% **tidak lagi 100 melainkan `(0,5−0,07)/0,5 = 86`**, dan skor menyentuh 0 tepat di 0,5%. Konsekuensinya konfigurasi ini — bersama 43 metrik turun lain yang dulu disetel sebagai anggaran — **perlu di-re-kalibrasi**: target 0,5% kini berarti "0,5% downtime = gagal total", bukan "0,5% masih penuh". Rincian model + rumusnya: [[RUN - Menambah Metrik KPI Otomatis]] (§ "Persentase yang menempel di ujung skala"). Alasan penggantian: metrik plafon datar 100 selama di bawah target sehingga tak membedakan bulan.
