...
**Sumber `kinerja_affiliate` — KPI Conversion PER-INDIVIDU staf Affiliate Acquisition** (🟡 branch `feature/workspace-position`, **belum merge/prod**, terverifikasi DEV 2026-08-22). Pola sama `kinerja_toko`: endpoint `GET /kpi/kinerja-affiliate` di [[Microservices - Marketing Analytics Service]] (`kpi_affiliate.go`) memaparkan angka mentah `conversion` (distinct `order_id`), `gmv`, `komisi`; sumber `kpi_sumber_affiliate.go` di employee-service memilih metrik & merakit target. **KPI = penilaian individu**: tiap staf dinilai dari akun affiliate MILIKNYA (atribusi `icc_affiliate_accounts` username→`employee_id`), scope bawaan `individu`; nol order = nilai sah (bukan galat). Dikonfig di template `AFFILIATE ACQ` (Beauty Hacks) & `AFFILIATE ACQUSITION` (Kyura), metrik `Conversion` (scope `individu`), **target PER-KARYAWAN** dari sheet KPI tiap orang (mis. Nur Istiqomah `BIP-0194-08-25` → 11.000, Maulana Pratama `BIP-0106-10-24` → 31.000 — angka berbeda per orang = bukti metrik ini per-individu, bukan departemen), fallback umum 150. ⚠️ **Kejanggalan skala**: target per-orang 11.000–31.000 ≈ TOTAL conversion seluruh departemen sebulan (~11.015), jauh di atas satu akun terbesar (~2.105/bln) — mustahil dicapai sampai tiap staf dipetakan ke PORTOFOLIO affiliator-nya (7 staf posisi Affiliate kini belum dipetakan sama sekali). Perlu klarifikasi pemilik metrik apakah target itu skala-portofolio atau angka departemen tersalin ke sheet individu.
- ⛔ **Gap yang menentukan**: 7 staf posisi "Affiliate" (yang dinilai) **belum dipetakan** ke akun di `icc_affiliate_accounts` — akun yang terpetakan justru milik 17 staf **ICC** (pemegang akun). Jadi Conversion mereka **kosong** sampai staf Affiliate dipetakan lewat **ICC Management**. Ini kelas gap yang sama dengan `shopee_shop_id` kosong: fitur & mesin siap, angkanya menunggu pekerjaan data.
- **"Jumlah Affiliate Aktif"** kini juga otomatis, metrik `affiliate_aktif` = **jumlah affiliator AKTIF** (akun affiliate ber-order distinct pada periode, = `akun_berdata` endpoint), scope individu, `target_per_karyawan` {Nur 4.500, Maulana 7.500} + fallback 4.500. ⚠️ **Bukan "baru bergabung"**: tanggal-join tak ada di data (TikTok Seller Affiliate Orders API hanya bawa waktu order) dan tak ada penanda perekrut — jadi yang terukur keaktifannya (sesuai judul metrik "Aktif"), bukan "baru bergabung" (deskripsi sheet). Skala target sama-sama menunggu pemetaan portofolio affiliator tiap staf.
