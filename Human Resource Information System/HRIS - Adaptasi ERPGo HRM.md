...
- **Komponen**: `salary-components` CRUD (earning/deduction, taxable, bpjs_base, **manual vs computed**) — di-seed 15. **Gaji per karyawan**: `employee-salary` GET/PUT (basic_salary, **upah_bpjs_kesehatan + upah_bpjs_ketenagakerjaan** — dua dasar terpisah, ptkp_status, component_values, bpjs_enrollment).
...
- **Engine nyata** (grounded ke fungsi): **PPh21 TER** (`terRate×gross`, tabel penuh PMK 168/2023) ✓ · **BPJS** (slip memuat **sisi karyawan saja**, tapi `company_rate` **kini dipakai** lewat `/employer-cost` untuk modul insentif — dikoreksi 2026-08-05. Pembulatan **beda antar sisi**: karyawan dibulatkan ke bawah ke kelipatan 100, perusahaan ke sen agar cocok Formulir 2a; selisihnya ditanggung perusahaan. Detail: [[Microservices - Payroll Service]]) ⚠️ · **prorata Tunjangan Kehadiran** (`base×payout` dari supplement) ✓ · **THR** (`basic×proporsi masa kerja`, Permenaker) ✓ · **Lembur** simplistik (`basic/173`, **tanpa multiplier 1,5×/2×** — TBD) ⚠️.
...
| Resignations · Terminations | ✅ | **Pencatatan + penonaktifan akun live di produksi 2026-08-05** (koleksi `employee_resign`, halaman `/hris/resign`), lihat [[ADR - 0035 HR Menonaktifkan Akun lewat Catatan Resign]]. ⚠️ Koleksinya masih 0 dokumen — live tapi belum dipakai. Sisa exit clearance masih konsep di [[HRIS - Personalia]]; konsumennya [[HRIS - Attrition]]. |
...
- **Tier 3 — Butuh keputusan / eksekusi konsep:**
  ...
  - **Lifecycle** — **Resignations/Terminations ✅ live di produksi**; Promotions/Transfers masih dok konsep. 🟡
