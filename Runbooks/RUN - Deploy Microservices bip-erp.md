...
> ⚠️ **`FORM_BUILDER_SERVICE_KEY`** ([[ADR - 0090 Inspeksi Satgas 5R dan K3 di Form Builder dengan Nilai dari Cek Ulang Terakhir]] T3) merged 2026-09-14 lewat bip-erp PR [#1866](https://github.com/bip-itteam-internal/bip-erp/pull/1866) (`feat/satgas-nilai-kpi`) bersama [#1867](https://github.com/bip-itteam-internal/bip-erp/pull/1867). **DEV terverifikasi 2026-09-14**: pipeline dev menaikkan form-builder-service dan employee-service **tanpa kunci** karena env ini belum ada di `.env` dev saat merge (keduanya gagal tertutup). Kunci lalu diisi (dengan backup `.env`), kedua container dibuat ulang `docker compose up -d --force-recreate --no-deps <svc>` tanpa build (image hasil pipeline sudah memuat kode baru), form-builder lebih dulu; hash kunci di kedua container cocok dengan `.env`, dan tiga pemeriksaan di bawah lolos (401 `Unauthorized gateway`; 200 `{"has_form":false,"period_key":"2026-09"}` karena dev belum punya form Satgas; `auto_value` 100 dengan `auto_gagal_sumber: false` lewat template KPI uji bersumber `nilai_inspeksi_satgas`, sesudah uji E2E dengan form Satgas uji yang lalu dihapus). Pelajarannya: merge ke `main` tidak mengisi `.env` dev, jadi kunci layanan baru diisi di `.env` dev **SEBELUM** merge; kalau tidak, container naik dengan kunci kosong dan harus dibuat ulang. **PROD, diukur 2026-09-15**: kode kedua sisi sudah ada di biner produksi (`Form-Builder-Service`: `/satgas/metrics`, `satgasMetricsHandler`, dan `gerbangKunciLayananKPI` masing-masing 1; `Employee-Service`: `nilai_inspeksi_satgas` 1; kontrol string karangan 0), tetapi **`FORM_BUILDER_SERVICE_KEY` belum ada di `.env` produksi** (nol baris, panjang 0 di kedua container). Rutenya karena itu menolak semua pemanggil dan sumbernya gagal hitung di produksi sampai kuncinya diisi lewat langkah di bawah, dijalankan manusia.

> ✅ **`LEARNING_SERVICE_KEY`** (sumber KPI `pelatihan`, bip-erp PR [#1895](https://github.com/bip-itteam-internal/bip-erp/pull/1895), merged 2026-09-15 12:59:59 WIB) mengikuti pola yang sama: **penerima** `learning-service` (`GET /kpi/pelatihan` memeriksa `key`), **pengirim** `employee-service`. PR yang sama menambah env **`LEARNING_MODULE_URL` di blok `calendar-service`** untuk feed kalender pelatihan, jadi calendar-service ikut dibuat ulang walau tak menyangkut kunci. **PROD 2026-09-15, dijalankan manusia**, berurutan: `git pull --ff-only origin main` → gerbang `git merge-base --is-ancestor 347dcc6f HEAD` → `.env` dicadangkan lalu ditambah `LEARNING_SERVICE_KEY=$(openssl rand -hex 16)` (panjang 32 dicek tanpa menampilkan isinya) → `docker compose up -d --build --force-recreate <svc> --no-deps` untuk learning, employee, lalu calendar, masing-masing dengan log ke berkas dan `grep -cE 'did not complete successfully|failed to solve'` = 0 → frontend. Gerbangnya lolos: string khas di biner 0 → 1 untuk ketiga service (kontrol `gofiber` ratusan, string karangan 0), hash sha256 kunci sama di kedua container, `GET /kpi/pelatihan` dari dalam `Employee-Service` 200 berbentuk kontrak, tanpa `key` 401, dan path karangan berprefiks sama 404. **DEV belum**: sshd VM dev tak mengirim banner sepanjang siang itu, jadi `.env` dev belum diisi dan container dev belum dibuat ulang.
>
...
> 🟡 **Tahap 2 — Training & Performance Officer** (bip-erp PR [#1903](https://github.com/bip-itteam-internal/bip-erp/pull/1903) rencana pelatihan, [#1904](https://github.com/bip-itteam-internal/bip-erp/pull/1904) sertifikat — **TERBUKA, belum merge, belum deploy dev maupun prod**, diperiksa 2026-09-16). Dua PR, dua konsekuensi deploy berbeda — jangan naikkan keduanya sebagai satu paket:
>
> - **#1903** menambah rute `GET /kpi/rencana-pelatihan` di learning-service, digerbang `GerbangKunciLearning` — **kunci yang SAMA** dengan `/kpi/pelatihan` di atas, bukan env baru (`services/learning/kpi_pelatihan.go:31,51`; `kpi_rencana_pelatihan.go:126`). Employee-service dapat sumber KPI baru, `rencana_pelatihan`, yang memanggil rute itu lewat `LEARNING_MODULE_URL` + `LEARNING_SERVICE_KEY` yang sudah ada (`services/employee/kpi_sumber_rencana_pelatihan.go:28,55-66`). Karena env-nya tak berubah, deploy-nya **cukup rebuild biasa** (`--build --no-deps`, learning-service dulu lalu employee-service, §3) — **dengan syarat** kunci itu memang sudah terisi sama di kedua blok `.env` lingkungan yang dituju. Jangan percaya begitu saja catatan PROD 2026-09-15 di atas untuk itu — env adalah keadaan yang bergerak, verifikasi ulang saat deploy; kalau ternyata kosong atau beda di salah satu blok, isi dulu lalu `--force-recreate` keduanya seperti pengisian pertama.
> - **#1904** (sertifikat) **hanya menyentuh learning-service**: tak ada pembaca baru di `services/employee` maupun kategori inbox baru di `notification` (diverifikasi Grep atas `services/employee` dan `request_notify.go` pada branch-nya — notifikasi pengajuan pelatihan tetap memakai kategori `request-*` yang sudah ada, tak menyangkut sertifikat). Deploy-nya rebuild `learning-service` saja. Dua index unik baru pada `training_certificate` dan satu pada `training_certificate_setting` dibuat otomatis saat rute registrasi (`ensureSertifikatIndexes`, `services/learning/sertifikat.go:51,458`); karena koleksinya baru (belum ada dokumen), tak ada risiko dokumen lama menabrak uniqueness saat index dibuat.
>
> Kedua PR menambah koleksi baru di `learning-mongo-db` (`training_plan_item`, `training_certificate`, `training_certificate_counter`, `training_certificate_setting`) — rincian field dan index di [[DB - Overview and Notes]]. **Frontend** (erp-frontend) yang mengonsumsi layar Rencana Pelatihan dan Sertifikat di-deploy **sesudah** kedua backend ini (backend dulu, FE belakangan untuk perubahan kontrak).

> ⚠️ **`FINANCE_SERVICE_KEY`** (jalur pembayaran per CV, [[ADR - 0096 Buku Besar 40 CV Dibangun di ERP dengan FINCON sebagai Spesifikasi]] §6 Jalur A / T2; **MERGE 2026-09-16 dan 17**, bip-erp #1926 dan #1929, di prod sejak 2026-09-17) mengikuti pola yang sama sekali lagi: **penerima** `finance-service` (`GET /internal/cv/penugasan` memeriksa `key`; env kosong **503** yang menyebut env-nya, kunci salah **401**), **pengirim** `procurement-service`, yang juga mendapat env baru **`FINANCE_MODULE_URL`**. Keduanya di `docker-compose.yml`, nilainya WAJIB sama. Bila kosong atau berbeda, yang mati hanya transfer kas CV — dengan pesan yang menyebut env-nya, dan antreannya membawa penanda `cakupan_cv_gagal` supaya layar tidak menampilkan daftar kosong yang terbaca "tak ada tugas". Transfer rekening PT tetap berjalan, sebab finance tidak dipanggil untuk orang tanpa `budget.cv.transfer`. Urutan: **finance-service lebih dulu, lalu procurement-service, keduanya `--force-recreate`**, baru frontend. Rilis ini juga menyentuh `shared-library` (izin `budget.cv.transfer`), jadi employee-service ikut naik supaya paket "Budget: Transfer Kas CV" ter-seed. ⛔ Sesudah deploy, IT memasang **DUA paket** ke Junior Accountant pemegang CV: "Budget: Transfer Kas CV" dan "Buku Besar CV: Pemegang"; salah satu saja membuat orangnya bisa menindak tanpa punya layar, atau sebaliknya. ⛔ **Yang terjadi saat rilis ini (2026-09-16 dan 17)**, tiga kegagalan senyap yang semuanya lolos test: (1) klien procurement tak mengirim `BIP-Gateway-ID`, jadi finance menolak 401 di setiap panggilan meski kuncinya benar (diperbaiki #1929; uji lewat gateway publik tak membedakan apa pun karena gateway menolak semua permintaan tanpa JWT, ujinya wajib di jaringan docker yang sama dengan pemanggil); (2) `FINANCE_SERVICE_KEY` terlewat di `.env` PROD, compose hanya memperingatkan "variable is not set. Defaulting to a blank string", dan hash kunci di kedua container SAMA karena sama-sama hash string kosong, jadi gerbangnya wajib "sama DAN bukan kosong"; (3) pipeline DEV melewatkan procurement-service dua merge berturut-turut (#1926 dan #1929), sehingga merge terbaca terpasang padahal biner lama yang berjalan; ukur umur image procurement sesudah merge dan bangun manual bila basi.

> ⚠️ **`PAYROLL_SERVICE_KEY`** (master entitas CV, [[ADR - 0096 Buku Besar 40 CV Dibangun di ERP dengan FINCON sebagai Spesifikasi]] T1; **MERGE 2026-09-16**, di prod sejak 2026-09-17) mengikuti pola yang sama dengan pasangan berbeda: **penerima** `payroll-service` (`GET /internal/badan-usaha` memeriksa `key`; env kosong membalas **503**, kunci salah **401**), **pengirim** `finance-service`, yang juga mendapat env baru **`PAYROLL_MODULE_URL`**. Keduanya ada di `docker-compose.yml`; `docker-compose.dev.yml` hanya memasang kuncinya di payroll-service karena finance-service tidak didefinisikan di compose dev. Bila kunci di finance-service kosong atau berbeda, laporan kecocokan menandai sumber payroll **galat** berikut sebabnya dan PATCH rujukan badan usaha dibalas 503; tidak ada temuan palsu. Isi `.env` dev **sebelum** merge (pelajaran `FORM_BUILDER_SERVICE_KEY` di atas), naikkan payroll-service lebih dulu lalu finance-service, keduanya `--force-recreate`. Rilis yang sama menyentuh `shared-library` (katalog `akuntansicv`), jadi employee-service ikut naik supaya paket barunya ter-seed.

> ✅ **`RECRUITMENT_SERVICE_KEY`** (sumber KPI `rekrutmen` untuk template `Recruitment` dan `KPI Supervisor HRGA`, bip-erp PR [#1967](https://github.com/bip-itteam-internal/bip-erp/pull/1967) + erp-frontend PR [#1644](https://github.com/bip-itteam-internal/erp-frontend/pull/1644), merged 2026-09-18, **live DEV 2026-09-18 dan PROD 2026-09-21**) mengulang pola yang sama: **penerima** `recruitment-service` (`GET /kpi/rekrutmen` dan `GET /kpi/review-evaluasi`, keduanya digerbang `GerbangKunciRekrutmen` **sesudah** `ValidateGateway`, jadi header `BIP-Gateway-ID` tetap wajib di samping `key`; kunci kosong **menutup** kedua rute dengan 401), **pengirim** `employee-service`, yang juga mendapat env baru **`RECRUITMENT_MODULE_URL`** (`http://recruitment-service:${RECRUITMENT_SERVICE_PORT}`, 6979 di `.env.example`). Keduanya ada di `docker-compose.yml` **dan** `docker-compose.dev.yml`; nilai kuncinya WAJIB sama di blok recruitment-service dan blok employee-service. Kosong atau berbeda → sumbernya berbunyi `RECRUITMENT_SERVICE_KEY belum diatur` atau `recruitment membalas 401` (URL kosong: `RECRUITMENT_MODULE_URL belum diatur`), semuanya berkelas `gagal mengambil data`. ⚠️ **Keadaan per 2026-09-18: BELUM di-deploy** ke DEV maupun PROD, kunci belum diisi di `.env` lingkungan mana pun, dan metriknya **belum dipasang ke template KPI mana pun** (memasangnya tindakan data HR sesudah HRD melihat pratinjau, [[HRIS - Matriks KPI per Departemen]] §Recruitment & Onboarding). Selama belum terpasang di template, kegagalan kunci tak terlihat di layar siapa pun; begitu terpasang, metrik berkelas `gagal mengambil data` tanpa nilai manual membuat **baris orang itu dilewati** (`adaSumberGagalTanpaNilai`, sama seperti pasangan lain di bagian ini), bukan cuma satu metrik yang kosong. Isi kunci di `.env` DEV **sebelum** pipeline menaikkan kedua service (pelajaran `FORM_BUILDER_SERVICE_KEY` di atas); kalau terlewat, containernya naik dengan kunci kosong dan harus dibuat ulang. PROD dijalankan **manusia**.
>
> **Urutannya: `recruitment-service` lebih dulu, lalu `employee-service`, keduanya `--force-recreate` (env dibaca saat container DIBUAT, `restart` tidak cukup), baru `erp-frontend`.** Bila employee-service naik lebih dulu, metrik rekrutmen melapor `gagal mengambil data` sampai recruitment menyusul; metrik dari sumber lain tak terpengaruh.
>
> ```bash
> docker compose up -d --build --force-recreate recruitment-service --no-deps
> docker compose up -d --build --force-recreate employee-service --no-deps
> ```
>
> Gerbangnya, dan **`docker ps` maupun `/health` tidak diterima sebagai bukti** (keduanya hijau untuk biner lama):
>
> 1. **Env-nya benar-benar ada di kedua container.** `docker inspect` `Recruitment-Service` dan `Employee-Service`: hash `RECRUITMENT_SERVICE_KEY` **sama DAN bukan hash string kosong** (`e3b0c44298fc…`) — compose hanya memperingatkan "variable is not set. Defaulting to a blank string" bila `.env` terlewat, dan dua container sama-sama kosong menghasilkan hash yang cocok (pelajaran `FINANCE_SERVICE_KEY` di atas). `Employee-Service` juga wajib memuat `RECRUITMENT_MODULE_URL`.
> 2. **Rutenya membaca, dan tertutup tanpa kunci.** Dari **dalam jaringan docker** (bukan lewat gateway publik, yang menolak semua permintaan tanpa JWT): `GET http://recruitment-service:<port>/kpi/rekrutmen?periode=<YYYY-MM>&company_id=BIP&key=<kunci>` dengan header `BIP-Gateway-ID` harus **200 berbentuk kontrak** — `data` memuat `requisisi[]` (tiap butir `id`, `posisi`, `jumlah_dibutuhkan`, `disetujui_pada`, `kritikal`, `diterima_pada[]`), `requisisi_tanpa_jejak_persetujuan`, `diterima_tanpa_posting`, dan `coverage`; **tanpa nama orang**. Permintaan yang sama **tanpa `key=`** harus **401**.
>
...
> 🔜 **`EMPLOYEE_SERVICE_KEY`** (kolom Aktual lembar MPP, T3 [[ADR - 0113 Actual vs Planning MPP Dihitung Sistem per Bulan, Berpijak pada Jejak Keluar Bertanggal]]; PR bip-erp [#1993](https://github.com/bip-itteam-internal/bip-erp/pull/1993) **merged 2026-09-22 WIB** `a6e26941`, **belum deploy** dan ketujuh gerbang verifikasinya belum satu pun dijalankan) membalik arah pasangan `RECRUITMENT_SERVICE_KEY` di atas: kali ini **penerima**-nya `employee-service` (`GET /kpi/headcount-periode`, digerbang `GerbangKunciHeadcount` **sesudah** `ValidateGateway`, jadi header `BIP-Gateway-ID` tetap wajib di samping `key`; kunci kosong **menutup** rute dengan 401 dan service menuliskannya ke log saat boot). **Pengirimnya recruitment-service menyusul di T5**, jadi hari ini env-nya dipasang di **SATU blok saja** — `employee-service` di `docker-compose.yml` dan `docker-compose.dev.yml`. Blok `recruitment-service` sudah punya `EMPLOYEE_MODULE_URL`, jadi nanti benar-benar cuma menambah satu baris.
>
> ```bash
> docker compose up -d --build --force-recreate employee-service --no-deps
> ```
...
| Metrik KPI bersumber `pelatihan` (atau `rencana_pelatihan` sejak Tahap 2, PR #1903 — kunci sama, prefiks galat sama) berbasis `gagal mengambil data: LEARNING_SERVICE_KEY belum diatur` atau `... learning membalas 401`, dan skor orangnya tak tampil | `LEARNING_SERVICE_KEY` kosong di employee-service, kosong atau berbeda nilai di learning-service, atau `.env` sudah diisi tapi container belum dibuat ulang | isi nilai yang SAMA di kedua blok, `--force-recreate` keduanya (learning lebih dulu), lalu bandingkan hash kunci di kedua container dan panggil `GET /kpi/pelatihan` (atau `GET /kpi/rencana-pelatihan`) dari dalam `Employee-Service` (catatan `LEARNING_SERVICE_KEY` di §3d) |
| Metrik KPI bersumber `rekrutmen` berbasis `gagal mengambil data: RECRUITMENT_SERVICE_KEY belum diatur`, `... RECRUITMENT_MODULE_URL belum diatur`, atau `... recruitment membalas 401`, dan skor orangnya tak tampil | kunci kosong di salah satu container, berbeda nilai antara blok recruitment-service dan employee-service, atau `.env` sudah diisi tapi container belum dibuat ulang | isi nilai yang SAMA di kedua blok, `--force-recreate` keduanya (recruitment lebih dulu), lalu dua gerbang §3d: hash kunci sama **dan** bukan hash kosong, serta `GET /kpi/rekrutmen` dari dalam jaringan docker 200 berbentuk kontrak / tanpa `key` 401 |
| Pelatihan tak muncul di kalender peserta, tanpa `learning` di `degraded` | `LEARNING_MODULE_URL` kosong di calendar-service sehingga provider dilewati diam-diam, atau env sudah ada di compose tapi container belum dibuat ulang | `--force-recreate calendar-service`, lalu pastikan `printenv LEARNING_MODULE_URL` terisi; bila kelasnya tetap tak muncul, periksa pemanggil memang terdaftar sebagai peserta, sebab feed hanya memuat kelas milik peserta |
