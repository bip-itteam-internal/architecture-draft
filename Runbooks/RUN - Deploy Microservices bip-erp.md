...
| Metrik KPI bersumber `pelatihan` berbasis `gagal mengambil data: LEARNING_SERVICE_KEY belum diatur` atau `... learning membalas 401`, dan skor orangnya tak tampil | `LEARNING_SERVICE_KEY` kosong di employee-service, kosong atau berbeda nilai di learning-service, atau `.env` sudah diisi tapi container belum dibuat ulang | isi nilai yang SAMA di kedua blok, `--force-recreate` keduanya (learning lebih dulu), lalu bandingkan hash kunci di kedua container dan panggil `GET /kpi/pelatihan` dari dalam `Employee-Service` (catatan `LEARNING_SERVICE_KEY` di §3d) |
