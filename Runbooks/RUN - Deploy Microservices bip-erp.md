...
> ⚠️ **`FINANCE_SERVICE_KEY`** (jalur pembayaran per CV, [[ADR - 0096 Buku Besar 40 CV Dibangun di ERP dengan FINCON sebagai Spesifikasi]] §6 Jalur A / T2; branch `feat/procurement-pembayaran-cv`, **belum merge per 2026-09-16**) mengikuti pola yang sama sekali lagi: **penerima** `finance-service` (`GET /internal/cv/penugasan` memeriksa `key`; env kosong **503** yang menyebut env-nya, kunci salah **401**), **pengirim** `procurement-service`, yang juga mendapat env baru **`FINANCE_MODULE_URL`**. Keduanya di `docker-compose.yml`, nilainya WAJIB sama. Bila kosong atau berbeda, yang mati hanya transfer kas CV — dengan pesan yang menyebut env-nya, dan antreannya membawa penanda `cakupan_cv_gagal` supaya layar tidak menampilkan daftar kosong yang terbaca "tak ada tugas". Transfer rekening PT tetap berjalan, sebab finance tidak dipanggil untuk orang tanpa `budget.cv.transfer`. Urutan: **finance-service lebih dulu, lalu procurement-service, keduanya `--force-recreate`**, baru frontend. Rilis ini juga menyentuh `shared-library` (izin `budget.cv.transfer`), jadi employee-service ikut naik supaya paket "Budget: Transfer Kas CV" ter-seed. ⛔ Sesudah deploy, IT memasang **DUA paket** ke Junior Accountant pemegang CV: "Budget: Transfer Kas CV" dan "Buku Besar CV: Pemegang"; salah satu saja membuat orangnya bisa menindak tanpa punya layar, atau sebaliknya.

> ⚠️ **`PAYROLL_SERVICE_KEY`** (master entitas CV, [[ADR - 0096 Buku Besar 40 CV Dibangun di ERP dengan FINCON sebagai Spesifikasi]] T1; **MERGE 2026-09-16**, belum di prod) mengikuti pola yang sama dengan pasangan berbeda: **penerima** `payroll-service` (`GET /internal/badan-usaha` memeriksa `key`; env kosong membalas **503**, kunci salah **401**), **pengirim** `finance-service`, yang juga mendapat env baru **`PAYROLL_MODULE_URL`**. Keduanya ada di `docker-compose.yml`; `docker-compose.dev.yml` hanya memasang kuncinya di payroll-service karena finance-service tidak didefinisikan di compose dev. Bila kunci di finance-service kosong atau berbeda, laporan kecocokan menandai sumber payroll **galat** berikut sebabnya dan PATCH rujukan badan usaha dibalas 503; tidak ada temuan palsu. Isi `.env` dev **sebelum** merge (pelajaran `FORM_BUILDER_SERVICE_KEY` di atas), naikkan payroll-service lebih dulu lalu finance-service, keduanya `--force-recreate`. Rilis yang sama menyentuh `shared-library` (katalog `akuntansicv`), jadi employee-service ikut naik supaya paket barunya ter-seed.
