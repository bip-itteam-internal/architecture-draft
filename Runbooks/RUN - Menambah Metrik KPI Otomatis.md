...
- **Status**: ✅ Mesinnya **deploy ke produksi 1 Agustus 2026** (PR #843 kontrak sumber, #857 mesin reduksi dan registry, #866 sumber `uptime_sistem`), dan **metrik otomatis pertama menyala 6 Agustus 2026**: tiga metrik Tech Development kini benar-benar menghasilkan angka di produksi. Prosedur di bawah final terhadap kode itu dan sudah pernah dijalankan sampai tuntas. Rinciannya di [[HRIS - Otomasi Skor KPI]].
	...
	- ⚠️ **`FORM_BUILDER_SERVICE_KEY` terpasang di DEV sejak 2026-09-14, PROD belum.** Env BARU untuk sumber `nilai_inspeksi_satgas` ([[ADR - 0090 Inspeksi Satgas 5R dan K3 di Form Builder dengan Nilai dari Cek Ulang Terakhir]] T3). Pemetaannya di blok `employee-service` dan `form-builder-service` pada `docker-compose.yml` maupun `docker-compose.dev.yml` dibawa bip-erp PR [#1866](https://github.com/bip-itteam-internal/bip-erp/pull/1866) (`feat/satgas-nilai-kpi`), merged 2026-09-14 bersama [#1867](https://github.com/bip-itteam-internal/bip-erp/pull/1867). Pipeline dev sempat menaikkan kedua container dengan kunci KOSONG karena `.env` dev belum memuatnya saat merge; kunci lalu diisi, keduanya dibuat ulang, dan tiga pemeriksaan kunci layanan di runbook deploy lolos, jadi sumber ini bisa diuji di dev. Pelajarannya: merge ke `main` tidak mengisi `.env` dev, jadi kunci layanan baru diisi di `.env` dev SEBELUM merge. PROD belum deploy dan isi `.env` PROD **belum diukur**. Gerbangnya fail-closed di dua sisi: kosong di form-builder menutup rutenya (401), kosong di employee-service membuat sumbernya gagal hitung dengan galat yang menyebut nama env-nya. Nilainya harus SAMA di kedua blok; urutan deploy dan pemeriksaannya di [[RUN - Deploy Microservices bip-erp]] §3d.
...
| `insentif_profit` | insentive lewat HTTP | **Empat** metrik: `profit`, `retur_persen`, `roas`, `roas_bersih`. ⛔ **Ketiga yang belakangan bernama SAMA dengan milik `kinerja_toko`** tetapi isinya berbeda (retur terbukukan per order vs `income.total_refund` dari mart; ROAS bersumbu hari-kirim + cutoff tgl 25 vs tanggal order tanpa cutoff) — saat menambah metrik yang namanya sudah dipakai sumber lain, entri `METRIK_PER_SUMBER` di FE **wajib ikut**, kalau tidak keduanya tampil berlabel identik. Aturan ini sudah tertulis di sini sejak `retur_persen`, dan tetap terlewat saat `roas`/`roas_bersih` masuk 2026-08-28: pemilih menampilkan empat entri, dua pasang berlabel sama persis selama tiga hari. Sejak 2026-08-31 ada jaring runtime yang membubuhkan nama sumber pada label kembar (`bedakanLabelKembar`), tetapi ia hanya menyingkap bahwa dua pilihan berbeda — yang menjelaskan BEDANYA tetap entri kamus. ⚠️ Adaptor ini meminta `mode=bergeser`: jendela periodenya SENGAJA berbeda dari insentif (lihat [[Microservices - Insentive Service]]) |
...
| `kinerja_ar` 🟡 | integration lewat HTTP | Satu-satunya contoh sumber yang **menuntut perubahan di service SEBERANG lebih dulu**, dengan urutan deploy yang mengikat. Saat `piutang_lewat_90_persen` ditambahkan, field `lebih90` harus ada di `/piutang/tren` sebelum employee membacanya; terbalik = field absen, ter-decode **nol**, dan nol pada metrik berarah `turun` adalah nilai **SEMPURNA**. Polanya: bawa field semacam itu sebagai **pointer**, jangan nilai biasa, supaya "belum dikirim" terbedakan dari "nol" dan jatuh ke galat bersebab. Juga contoh sumber yang metriknya **bersarang** (`lebih14` ⊇ `lebih60` ⊇ `lebih90`) sehingga penyebutnya gampang kelebihan hitung |
...
- **Sempitkan muatannya.** `KPIUptime` sengaja tanpa daftar monitor: nama monitor memaparkan topologi infrastruktur, sementara penilaian hanya butuh angka. Kalau kunci layanan bocor, yang terpapar sebatas itu. Terapkan hal sama pada rutemu: kirim angka dan cacahan, jangan judul tiket, nama toko, atau nama orang. Contoh yang muatannya nilai PER ORANG: `/internal/satgas/metrics` (form-builder, sumber `nilai_inspeksi_satgas`) hanya membawa `employee_id` beserta nilai, cacahan form, dan penanda cek ulang, tanpa nama, departemen, jabatan, atau rincian per form. Satu-satunya teks di dalamnya judul form yang salah konfigurasi (`form_tanpa_skala`), dan pesan galat sumbernya menyebut "karyawan ini", bukan nama.
...
> ⚠️ [[ADR - 0079 Target Profit Satu Pintu di Insentif, KPI Membacanya]] T2, bip-erp PR [#1775](https://github.com/bip-itteam-internal/bip-erp/pull/1775) **merged dan live di prod 2026-09-07**.
...
5. **Validasi template mengikuti registry** (T5 PR A bip-erp [#1783](https://github.com/bip-itteam-internal/bip-erp/pull/1783), merged 2026-09-08): `POST /kpi/templates` dan pratinjau memanggil `validasiAutoKonfig` (employee-service), yang memilih `ValidateKPIAutoConfigTargetDariSumber` untuk metrik terdaftar — aturan umum sama, tetapi target yang terisi di lapis mana pun **ditolak**, bukan diwajibkan — dan `ValidateKPIAutoConfig` lama untuk sisanya. Selama masa transisi (`bersihkanTargetTransisi = true`, PR A) target yang masih dikirim layar lama **dibuang** (`BersihkanTargetOtomatis`) supaya template tetap bisa disimpan; setelah layar Atur Target berhenti mengirimnya (erp-frontend #1484), konstanta itu `false` (PR C, dikunci `TestTransisiTargetProfitSelesai`) dan target itu dibalas **400**. Jangan memanggil `ValidateKPIAutoConfig` langsung dari handler simpan baru: yang menentukan validator mana yang berlaku adalah registry, bukan pemanggil.
...
- [ ] ⛔ **Bila metrikmu menuntut FIELD BARU dari service seberang: field itu bertipe pointer di sisi pembaca, dan urutan deploy ditulis di PR.** Field yang belum dikirim ter-decode jadi **nol** tanpa galat, dan pada metrik berarah `turun` nol adalah nilai SEMPURNA — orangnya dapat skor penuh justru karena datanya belum ada. Ingat juga cache respons di service sumber (mis. `/piutang/tren` ber-TTL 10 menit): bentuk lama masih dapat tersaji sesaat sesudah deploy yang benar, jadi pesan galatnya jangan cuma menyalahkan urutan deploy
...
- [ ] ⚠️ **Bila sumbermu bermetrik, TIAP METRIKNYA butuh pendaftaran sendiri di beberapa tempat — dan penjaganya TIDAK menangkap yang terlewat.** Sumber berlabel lengkap tidak menolong metriknya: `kinerja_ar` berlabel sepanjang waktu sementara `piutang_lewat_60_persen` miliknya tampil tanpa keterangan dan bersatuan salah, padahal sudah dipakai template produksi. Daftar tempatnya ada di [[REF - Penamaan Metrik & Sumber KPI]]; jangan menebaknya dari ingatan
