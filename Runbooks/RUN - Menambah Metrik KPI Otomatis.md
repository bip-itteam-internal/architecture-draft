...
| `insentif_profit` | insentive lewat HTTP | Dua metrik: `profit` dan `retur_persen`. ⚠️ `retur_persen` **bernama sama** dengan milik `kinerja_toko` tetapi isinya berbeda (retur terbukukan per order vs `income.total_refund` dari mart) — saat menambah metrik yang namanya sudah dipakai sumber lain, entri `METRIK_PER_SUMBER` di FE wajib ikut, kalau tidak keduanya tampil berlabel identik. ⚠️ Adaptor ini meminta `mode=bergeser`: jendela periodenya SENGAJA berbeda dari insentif (lihat [[Microservices - Insentive Service]]) |
