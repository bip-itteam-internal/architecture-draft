...
5. **Validasi template mengikuti registry** (T5 PR A bip-erp [#1783](https://github.com/bip-itteam-internal/bip-erp/pull/1783), belum merge per 2026-09-08): `POST /kpi/templates` dan pratinjau memanggil `validasiAutoKonfig` (employee-service), yang memilih `ValidateKPIAutoConfigTargetDariSumber` untuk metrik terdaftar — aturan umum sama, tetapi target yang terisi di lapis mana pun **ditolak**, bukan diwajibkan — dan `ValidateKPIAutoConfig` lama untuk sisanya. Selama masa transisi (`bersihkan=true`) target yang masih dikirim layar lama **dibuang** (`BersihkanTargetOtomatis`) supaya template tetap bisa disimpan; setelah layar Atur Target berhenti mengirimnya, pemanggil beralih ke `false` dan target itu dibalas **400**. Jangan memanggil `ValidateKPIAutoConfig` langsung dari handler simpan baru: yang menentukan validator mana yang berlaku adalah registry, bukan pemanggil.
