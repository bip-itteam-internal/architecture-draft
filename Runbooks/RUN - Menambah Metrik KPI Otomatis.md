...
- **Status**: ✅ Mesinnya **deploy ke produksi 1 Agustus 2026** (PR #843 kontrak sumber, #857 mesin reduksi dan registry, #866 sumber `uptime_sistem`), dan **metrik otomatis pertama menyala 6 Agustus 2026**: tiga metrik Tech Development kini benar-benar menghasilkan angka di produksi. Prosedur di bawah final terhadap kode itu dan sudah pernah dijalankan sampai tuntas. Rinciannya di [[HRIS - Otomasi Skor KPI]].
	...
	- ⚠️ **`FORM_BUILDER_SERVICE_KEY` belum terpasang di DEV maupun PROD.** Env BARU untuk sumber `nilai_inspeksi_satgas` ([[ADR - 0090 Inspeksi Satgas 5R dan K3 di Form Builder dengan Nilai dari Cek Ulang Terakhir]] T3). Blok `employee-service` dan `form-builder-service` di `docker-compose.yml` maupun `docker-compose.dev.yml` baru memetakannya di bip-erp branch `feat/satgas-nilai-kpi` (2026-09-12, belum PR, belum merge, belum deploy), jadi sampai T3 di-deploy tak ada biner di server mana pun yang membacanya. Isi `.env` kedua server untuk env ini **belum diukur**. Gerbangnya fail-closed di dua sisi: kosong di form-builder menutup rutenya (401), kosong di employee-service membuat sumbernya gagal hitung dengan galat yang menyebut nama env-nya. Nilainya harus SAMA di kedua blok; urutan deploy dan pemeriksaannya di [[RUN - Deploy Microservices bip-erp]] §3d.
