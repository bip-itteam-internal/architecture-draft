...
- **Opname & akurasi Perlengkapan via rekonsiliasi Accurate (🟡 Direncanakan)**: rekonsiliasi GA↔Accurate DIPERLUAS ke barang **kategori Perlengkapan**. Beda dari aset tetap: dicocokkan per-**KUANTITAS** — staff GA meng-input **hitung fisik** saat opname, dibandingkan ke qty Accurate (Accurate = kebenaran; `ga_stok` cuma pembanding), skornya → **KPI** (paralel Fase 2 aset tetap). Muncul di tab **Data Accurate** yang sama lewat **pemilih tipe Aset Tetap / Perlengkapan** (tukar tabel+kolom) — **bukan** tab baru, **bukan** dicampur ke tabel aset tetap. Kerja baru: sync `integration` menangkap **kategori item** Accurate (`accurate_stocks` belum simpan; prefix `item_no` tak bersih) + store record opname. Keputusan: [[ADR - 0065 Opname Perlengkapan GA via Rekonsiliasi Accurate]].
