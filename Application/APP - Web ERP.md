...
**Marketing Insight** (`/marketing-insight`, feature `src/features/marketing-insight/`) — landing card + sidebar submenu; submodul: Customer Demography, GMV Max Monitoring, Product Performance, dan **Affiliate Performance** (`/marketing-insight/affiliate`): KPI cards (GMV, komisi dibayar/estimasi, order·creator), tabel Top Creator + Top Produk, tabel detail order dengan checkbox per-baris + select-all; date range picker tanpa default (data baru di-fetch setelah rentang dipilih, resolve semua mode Monthly/Weekly/Yearly/custom, max 3 bulan); kartu Validasi Komisi (cross-check finance statement: tervalidasi/selisih/belum-ada-bukti/menunggu) + badge & filter status validasi di tabel detail; fetch react-query → `/api/integration/affiliate/*` ([[API - Integration Service]])
