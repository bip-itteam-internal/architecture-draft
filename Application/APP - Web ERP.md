...
- **Gross Profit / Laba Sejati** (`/integration/gross-profit`, feature `src/features/integration/gross-profit/`) — ⚠️ *working branch `feat/gross-profit`, belum merge*: laba kontribusi per produk/SKU TikTok mengikuti blueprint dashboard marketing (`GMV − biaya MP − promo = net settlement − HPP − iklan = laba`; retur kolom informasi). 3 tab: **Profit** (kartu ringkasan alur GMV→Net→Laba; toggle Produk|Bundle; tabel FLAT breakdown grup kolom berwarna + kolom Aksi SCALE/JAGA/PERBAIKI/HENTIKAN + badge estimasi; klik baris → panel samping waterfall Rincian Biaya + tabel per-SKU qty/GMV/Net/Laba, porsi bundle expandable), **HPP** (upload xlsx finance + filter nama/periode + badge Aktif/riwayat; HPP bundle terhitung read-only), **Mapping** (generate 2 fase dari master+nama finance, tabel Single|Bundle grouped per SKU, edit nama inline, panel approve saran). Mutasi hanya role `supervisor|admin` modul `integration` ATAU `finance`. Backend: [[API - Integration Service]] rute `/profit/*`.
