...
- **Gross Profit** (`/integration/gross-profit`, feature `src/features/integration/gross-profit/`) — ⚠️ *working branch `feat/gross-profit`, belum merge*: profit per produk/SKU TikTok (`revenue − iklan − fee MP − HPP`). Tab **Profit**: tabel produk expandable → breakdown SKU (kolom revenue/iklan/fee/HPP/profit/margin, profit negatif merah), filter account (credential shop) + toko + date range WIB + search produk/SKU, banner transparansi (settlement pending, SKU belum ter-mapping, HPP kosong). Tab **HPP & Mapping**: upload xlsx COGS finance + CRUD mapping SKU↔item_id↔produk — mutasi hanya role `admin|finance` modul integration (cookie `system_roles`), lainnya view-only. Backend: [[API - Integration Service]] rute `/profit/*`.
