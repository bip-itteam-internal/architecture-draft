...
> **Status, diukur 2026-09-17 (`gh pr view`, `gh api compare`)**: my-bharata PR [#151](https://github.com/bip-itteam-internal/my-bharata/pull/151) (branch `feat/post-test-peserta`) **merged ke `dev` 2026-09-16** dengan head `c988d43e`, jadi Tahap 3b (pre-test) di bawah ikut masuk `dev`. **Belum ada di rilis GitHub**: rilis terakhir `v1.14.5+135` terbit 2026-08-03. ⚠️ **Backend pre-test-nya BELUM merge**: rute `POST /me/pre-test/:trainingId/start`, penanda `pre_test_tersedia`/`pre_test_selesai`, dan `pre_score` pembanding baru ada di branch bip-erp `feat/learning-post-test-batas` (HEAD `42e7760c`, 6 commit di depan `main`, tanpa PR). Server yang belum membawanya tak mengirim penanda pre, dan penanda yang absen dibaca `false` (`_bool`, `data/models/my_training_model.dart:57-58,64-68`), sehingga tombol dan peringatan pre-test tak muncul: gagal-tertutup, tanpa tombol yang pasti ditolak.
