...
- **Halaman pengisian `/survey/:id`** merender **9 tipe pertanyaan** (`short_text`, `long_text`, `number`, `date`, `time`, `dropdown`, `radio`, `checkbox`, `scale`; branch T6 menambah `boolean` dan `file`, lihat [[#Satgas 5R dan K3]]), memvalidasi cermin aturan backend sebelum kirim, lalu menyegarkan section supaya form yang baru diisi langsung lenyap.
...
> Status: 🔜 **selesai di branch** my-bharata `feat/satgas-inspeksi` 2026-09-12, belum dicoba di perangkat. T6 (`1.18.0+162`, PR draft [#144](https://github.com/bip-itteam-internal/my-bharata/pull/144)), lalu cek ulang lintas bulan T3 di branch yang sama (commit `0972270c` sampai `97032157`): belum merge, belum rilis, dan versinya tidak dinaikkan (`pubspec.yaml` tetap `1.18.0+162`). Kartu cek ulang butuh backend T3 bip-erp (branch `feat/satgas-cek-ulang`, belum PR); tanpa field barunya kartu itu tak muncul dan halaman isi jatuh ke `/me/forms`. Keputusan: [[ADR - 0090 Inspeksi Satgas 5R dan K3 di Form Builder dengan Nilai dari Cek Ulang Terakhir]] §3, §5, dan §7. Kontrak: [[API - Form Builder Service]].
...
- ⚠️ Versi lama menampilkan kedua tipe sebagai "belum didukung" dan melewatinya saat memeriksa isian, jadi larangan berkas wajib di editor web baru dicabut sesudah adopsi versi ini terukur.
