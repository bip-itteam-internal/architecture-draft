...
- **Status**: ✅ Implemented — aplikasi HRIS mobile produksi (Flutter, Android/iOS). **Rilis di HP pemakai: `1.14.5+135`** (`origin/main`, GitHub Release 2026-08-03; masih rilis GitHub terbaru per 2026-09-15); `origin/dev` sudah di **`1.18.3+165`** (Booking Ruang #149 merged 2026-09-15, diukur hari itu) dan belum ditarik ke `main`, jadi PR sesudah 135, termasuk **Pelatihan Saya** (#112) dan **Pengajuan Pelatihan** (#115), belum ada di rilis GitHub itu. Angka lama "rilis dev 1.10.2+104" dan "dev 1.14.9+143" di baris ini sudah tidak berlaku.
...
- ⚠️ **Koreksi 2026-09-17: versi yang beredar di HP pemakai TIDAK terbaca dari repo.** Pemilik proses menyatakan MyBharata **1.20.0** sudah dirilis ke toko, dibangun dari `dev` (`1.20.0+167`) tanpa tag git maupun GitHub Release; `origin/main` dan GitHub Release terbaru tetap `1.14.5+135`. Jadi klaim "belum rilis" di dok ini yang bersandar pada tag, GitHub Release, atau `main` perlu dicek ulang ke pemilik proses atau Play Console/App Store Connect sebelum dipakai. Fitur yang merged ke `dev` sampai `1.20.0+167` (antara lain Pelatihan Saya, pre/post-test, penilaian trainer) kemungkinan sudah beredar; yang merged sesudahnya (mis. pertanyaan kesesuaian materi, my-bharata [#156](https://github.com/bip-itteam-internal/my-bharata/pull/156)) menunggu rilis berikutnya.
...
- **Review Submission** (peninjau): inbox **lintas jenis** (Izin/Cuti/Sakit/Dinas/Koreksi/Tukar) dalam satu daftar via `/hr/requests` (`?as=reviewer` antrian / `reviewed` sudah); detail fetch-on-tap via `/hr/requests/detail`. "Aktivitas Saya" & kartu beranda "menunggu persetujuan" juga pakai endpoint terpadu (`/requests/mine`, `/hr/requests`). Kontrak: [[API - Attendance Service]]. Sejak irisan 2 Booking Ruang ([#149](https://github.com/bip-itteam-internal/my-bharata/pull/149) merged ke `dev` 2026-09-15, belum rilis store) daftar terpadu berhalaman 20 item (muat saat digulir, coba lagi di kaki daftar, angka kartu beranda dan badge antrean dari `total`) dan meminta `include=booking`; lihat [[#Booking Ruang]].
...
- **Lampiran di detail peninjau** (2026-09-01): kartu lampiran tampil untuk **Izin/Cuti/Sakit**, bukan cuma di detail pemohon. Sebelumnya `ReviewerSubmissionInfo` tak pernah merendernya sejak dibuat, padahal `/hr/requests/detail` mengirim `document.file_object` utuh ke peninjau dan `SubmissionHistoryModel` sudah mem-parse-nya — jadi yang hilang murni lapisan tampilan, dan gagalnya senyap. Peninjau **Dinas** sudah benar sejak awal; **Koreksi** dan **Tukar** memang tak punya field dokumen di modelnya, jadi tak ada yang bisa ditampilkan. Detail: [[HRIS - Leave Request]]
- **Koreksi Presensi** — jenis pengajuan tersendiri di layar yang sama (**Pengajuan → Human Resource (HR)** → Tipe Pengajuan "Koreksi Presensi"), dengan sub-pilihan **Clock-in** / **Clock-out**; `HrCorrectionFormSection` menerjemahkannya jadi `checkin`/`checkout` (tak ada `both` di jalur ini). Pemilih tanggal "7 hari terakhir" di-feed `GET /correction/candidates?type=`, jadi isinya **sudah difilter per tipe oleh server** dan aplikasi tidak menilai ulang kelayakannya. Mengganti tipe **mengosongkan** tanggal yang sudah dipilih, sebab entri yang sah untuk Clock-in belum tentu sah untuk Clock-out. ⛔ Sampai **2026-09-04** aplikasi memegang salinan aturan kelayakannya sendiri yang tak mengenal `status`, sehingga setiap entri **Terlambat** ditolak di layar dengan "Presensi pada tanggal ini tidak memenuhi syarat untuk dikoreksi" padahal server menerimanya — sengketa telat karena itu mustahil diajukan sejak fiturnya lahir. Perbaikannya menunggu rilis store. Aturan lengkap + riwayatnya: [[HRIS - Attendance Correction]]
- **Pengingat surat dokter** (2026-09-01, PR [#132](https://github.com/bip-itteam-internal/my-bharata/pull/132)): formulir pengajuan menampilkan `SickNoteReminderCard` khusus jenis **Sakit**, mengingatkan menyerahkan surat **asli** ke ruang HRD saat masuk kerja kembali. Bukan pengingat mengunggah — lampiran foto sudah wajib dan ditolak server bila kosong; yang tak punya jejak di sistem justru dokumen fisiknya. Kartunya sendiri yang menyaring jenisnya (meniru `SubmissionTermsCard`) **dan membawa jarak bawahnya**, sebab elemen kosong tetap memakan ruang dan spacer di halaman akan menyisakan celah menggantung untuk empat jenis lain. ⚠️ Kalimatnya **tidak ada** di ketentuan resmi `sakit_terms.md`, jadi kartu ini satu-satunya tempat aturan itu muncul.

### Booking Ruang

> Status: ⚠️ **merged ke `dev`** 2026-09-15 lewat [#149](https://github.com/bip-itteam-internal/my-bharata/pull/149) (versi `1.18.3+165`), belum rilis store dan belum dicoba di perangkat karena VM dev belum melayani. Butuh attendance-service ber-`include=booking` dan inventory-service riwayat terstruktur naik lebih dulu. Tanpa keduanya aplikasi tetap jalan: attendance lama mengabaikan `include` dan tak mengirim `degraded`, jadi booking tak muncul di daftar terpadu **tanpa keterangan apa pun**; inventory lama masih menulis kalimat server Bahasa Indonesia di `alasan`, dan kalimat itu yang tampil di riwayat. Keputusan: [[ADR - 0094 Booking Ruang lewat MyBharata, Penyetuju Ditunjuk HR, Satu Sumber Ruang Kantor]] dan [[ADR - 0095 Pengajuan dan Persetujuan Booking Ruang Juga Lewat Web]]. Kontrak: [[API - Inventory Service]] dan [[API - Attendance Service]]. Konsep: [[GA - Asset Loan & Room Booking]].

- **Pintu masuk**: menu Pengajuan mendarat di grid tipe pengajuan (`SubmissionTypeSelectionPage`, redesign `origin/dev` PR #147); kartu terakhir **Booking Ruang** membuka form booking. Kartu itu tetap tampil walau daftar tipe HR gagal dimuat, sebab datanya milik inventory. Mengajukan cukup identitas, tanpa izin modul.
...
- **Rapi-rapi form dan layar Jadwal Ruang** (2026-09-22, sudah ada di `origin/dev`; catatan "belum merged" di versi lama dok ini diukur ulang 2026-09-22 dan sudah basi — nol PR terbuka di my-bharata, ketiga berkasnya ada di `origin/dev`):
  ...
  - **Jadwal Ruang** (ikon kalender di kanan atas form): daftar ruang terpakai dari `GET /peminjaman/jadwal`, dikelompokkan per tanggal seperti notifikasi. Rentang yang ditampilkan dibaca dari respons server, bukan dihitung ulang di aplikasi. `SliverGroupedList` mendapat flag `menaik` karena daftar ini menatap ke depan; bawaannya tetap menurun sehingga lima pemakai lain tak berubah.
  - ⚠️ **Sejak [[ADR - 0121 Jadwal Ruang Publik Menyertakan Nama dan Divisi Pemohon]] (2026-09-22) kartunya menyebut nama dan divisi pemesan**, menggantikan "tanpa identitas pemesan" yang berlaku sebelumnya. Alasannya alur: tanpa itu orang yang melihat ruang terpakai tak punya cara menindaklanjuti di dalam aplikasi. Yang **tetap** tak ikut: keperluan, nomor WA, nomor booking, posisi, riwayat. Booking lama tanpa identitas tidak menggambar pemisah menggantung, karena backend menghilangkan keynya (`omitempty`), bukan mengirim string kosong.
  - **Rentang tanggal pindah ke bawah judul halaman** lewat `CustomAppBar.subtitle` yang memang sudah ada, bukan baris tersendiri di badan. Tahunnya dicetak **sekali di ujung** bila kedua tanggal setahun ("22 Sep sampai 5 Okt 2026"), dan **dua kali** bila berbeda tahun, karena "28 Des sampai 5 Jan 2027" akan terbaca seolah keduanya 2027.
...
- ⛔ **Sejak 2026-09-22 daftar klub HANYA di halaman Semua Event, tidak lagi di beranda** (permintaan pemilik produk: event sudah punya rumah di section "Event terkini", jadi daftar tersendiri di beranda cuma pintu kedua). Konsekuensinya halaman Semua Event menjadi **satu-satunya** jalan masuk ikut klub, karena klub yang belum dijadwalkan tak pernah muncul sebagai event di mana pun. Karena itu:
	- **Subtitle section "Event terkini" WAJIB menyebut klub dan SELALU tampil**, termasuk saat tak ada event (`homeEventSectionSubtitle`, menggantikan `homeCultureEventSubtitle` yang pemakainya jadi nol). Tombol "Lihat Semua" memang selalu terjangkau, tapi labelnya tak menjanjikan apa pun tentang klub; subtitle inilah satu-satunya yang menutup alurnya. Kewajiban itu dipatok test di kedua bahasa, bukan cuma ditulis di komentar.
	- **Jangan menghapus `ClubCommunitySection` dari `all_events_page.dart`** tanpa menyediakan jalan masuk lain lebih dulu. Dijaga gerbang susunan `test/features/home/event/susunan_event_section_test.dart`, yang dibuktikan tidak kosong lewat kontrol mutasi.
- **Kartu event jadi `EventCard`** di `widgets/event/event_card.dart` (dulu `CultureEventCard` di `widgets/home_page/`). Namanya netral supaya jenis event berikutnya — jadwal rutin klub, pelatihan, agenda kalender — masuk ke kartu yang SAMA alih-alih melahirkan tampilan kedua. ⚠️ Masukannya masih `CultureEvent` apa adanya: view-model netral ditunda sampai feed kedua benar-benar ada, karena bentuk yang ditebak dari satu pemakai akan dibongkar. `HomeCultureEventLive` ikut berganti nama jadi `HomeEventSection`.
...
- **Susunan beranda**: section Event terkini ada **di atas** `HomeCarousel` (yang memuat kartu promo "Punya Kendala dalam aplikasi?"). Sejak 2026-09-22 ia **tidak lagi** menampilkan daftar klub.
- ⛔ **Menu "Program Culture" DICABUT dari Akses Cepat** (2026-09-22, permintaan pemilik produk). Halaman `/program-culture` cuma punya dua pintu: menu itu dan deep link notifikasi, jadi mencabutnya membuat isinya nyaris tak terjangkau. Yang dikerjakan supaya tak ada yang hilang:
	- **Riwayat kehadiran Program Culture pindah ke tab Riwayat**, diangkat jadi `CultureHistorySection` (`program_culture/presentation/widgets/`) dan dipakai **dua** layar: tab Riwayat dan halaman `/program-culture` yang masih hidup. Satu widget, bukan salinan kedua. `HistoryNavigationPage` menyediakan `CultureHistoryBloc`-nya. Tanpa pemindahan ini riwayatnya tak terjangkau dari mana pun, dan hilangnya tak menghasilkan satu pun galat.
	- **Rute `/program-culture` sengaja TIDAK dihapus**: `notification_route_mapper.dart` masih memetakan notifikasi culture ke sana, dan menghapus rutenya membuat ketukan notifikasi mendarat di layar galat go_router. Dipatok test, sekelas gerbang `/pengajuan-pelatihan` yang sudah ada.
	- ⚠️ **Yang tersisa tak terjangkau dan belum dibereskan**: daftar program PIC (QR + absen manual) hanya terjangkau dari kartu event saat eventnya **sedang berlangsung**, dan `CultureScanPage` (`/program-culture/scan`) memang sudah yatim sebelum perubahan ini — nol pemanggil di `lib/`. Keduanya task tersendiri.
- **Utang yang diterima sadar**: `ClubEntity.clubColor` **tidak** jadi dihapus — `event_card.dart` masih memakainya untuk chip logo klub di kartu event. Itu masih palet di luar token tema, dan jadi task tersendiri.
...
### Payroll
> **Dikoreksi 2026-08-05 (grounded ke kode).** Catatan lama di sini menyebut "payslip bulanan PDF terenkripsi, dilindungi PIN, ditampilkan in-app" beserta riwayat tahunan. **Enkripsi PDF tetap tidak ada** dan tak pernah ada. Yang sudah berubah sejak koreksi itu: slip terbit **sudah** dipanggil aplikasi (merged ke `dev` 2026-08-2x, commit `3fbf44d4`), dan **gerbang PIN sudah ada** walau bentuknya bukan "PDF terenkripsi" melainkan verifikasi sebelum membuka layarnya.

- **Slip gaji terbit + unduh PDF** (✅ **merged ke `origin/dev`**, commit `3fbf44d4`): `GET /payroll-runs/my` menampilkan slip dari run **published** saja, dan `…/pdf` mengambil berkasnya (lihat [[Microservices - Payroll Service]]).
...
- ⛔ **Daftar berhenti menampilkan nominal, dan itu SYARAT gerbangnya, bukan pilihan gaya.** Kartu lama memasang "Gaji Bersih" tepat di daftar, jadi PIN yang diminta sesudahnya tak menjaga apa pun yang belum terbaca. Menambahkan nominal kembali ke daftar akan terasa seperti perbaikan UX dan diam-diam membatalkan seluruh fiturnya; yang menahannya cuma satu test.
- ⛔ **Gerbang PIN pindah dari `onTap` ubin menu ke `PayrollPages.getPage`.** Bentuk lama hanya menjaga SATU pintu: rute `/payslip` sendiri polos, sehingga menu developer dan rute `/payslip/detail` masuk tanpa PIN, dan deep link notifikasi akan menyusul begitu notifikasi slip dipasang. Semua rute payroll bermuara di `getPage`, jadi membungkusnya di sana menutup ketiganya sekaligus.
	- **Bukan** di `AppMiddleware.authGuard`: redirect GoRouter mengembalikan `String?` dan **tak bisa membawa `extra`**, padahal halaman PIN menerima judul dan pesannya lewat `extra`. Lagipula `authGuard` dipakai setiap rute di aplikasi ini, jadi ongkosnya jatuh ke semua sementara manfaatnya ke satu.
- **`PinSession`** (core, hanya di memori, tak pernah dipersist): verifikasi berlaku selama aplikasi ada di depan mata. Dibersihkan saat **`paused`/`detached`** dan saat **logout**. ⚠️ Sengaja **BUKAN** `inactive`: state itu muncul untuk gangguan sementara yang pemakainya tak pernah pergi (menarik pusat notifikasi iOS, banner telepon masuk), dan membersihkannya di sana membuat PIN diminta berulang tanpa sebab yang terlihat. **Tanpa batas waktu numerik**, karena angka timeout apa pun tak punya dasar selain selera.
	- ⛔ **Guard wajib meninjau ulang sesinya saat `resumed`, bukan cuma saat dipasang.** Iterasi pertama membaca `PinSession` sekali di `initState`, sehingga pembersihan-saat-latar tak berpengaruh pada halaman yang SUDAH terbuka: pemakai membuka slip, menekan home, menyerahkan ponselnya, dan yang berikutnya membuka aplikasi langsung melihat nominalnya. Ditemukan saat `/review`, bukan oleh suite awal, karena seluruh test menguji keadaan saat dipasang.

⚠️ **Bagian "Perkiraan" DIBUANG**, dan ini **membalik keputusan yang tertulis di dok ini sendiri** (2026-08-05: *"keduanya hidup berdampingan… menggantinya akan membuat halaman kosong sampai run pertama diterbitkan"*). Pemilik produk memutuskan sebaliknya **2026-09-01**: dua sumber berbeda berdampingan di satu layar membuat uang final dan tebakan dari presensi terbaca setara. **Konsekuensi kosongnya diterima sadar** dan kini dijelaskan lewat empty state, bukan layar hampa. Seluruh rantainya mati bersih (card, bloc, usecase, entity, model, method datasource/repository, registrasi DI, konstanta URL, 5 key l10n).

- ⛔ **Akibat lintas-repo: `GET /me/payroll-approx` di [[Microservices - Employee Service]] kini YATIM**, nol konsumen kode. [[APP - Web ERP]] tak pernah memakainya (diverifikasi grep seluruh workspace). Endpointnya sengaja **tidak** dihapus, itu keputusan repo lain. `/payroll-supplement` di [[Microservices - Attendance Service]] **tetap hidup** karena [[Microservices - Payroll Service]] memanggilnya langsung, bukan lewat `payroll-approx`.
...
- **Identitas pemohon di form Buat Tugas ✅** (2026-08-22, branch `feat/task-create-identitas-pemohon`; ⚠️ belum dirilis ke store) — form kini merender **Nama Pelapor** dan **No Telepon** sebagai kolom yang terlihat dan bisa dikoreksi, dengan validator klien yang mencerminkan aturan server (≥2 dan ≥10 karakter).
	- ⛔ **Sebelumnya kedua nilai itu dikirim tanpa punya kolom sama sekali**, dirakit diam-diam dari `UserProfileBloc` lewat `_prefill()` yang hanya jalan pada state `UserProfileLoaded`. Ketika profil belum termuat, keduanya kosong, server menolak `400`, dan **tak ada apa pun di layar yang bisa diperbaiki pemohon** — ia hanya bisa menekan Kirim berulang kali. Terekam di prod 2026-08-21: empat percobaan ditolak berturut-turut lalu berhasil delapan menit kemudian. Sisi server kini juga mengisi kekosongan itu sendiri ([[API - Task Management Service]]), yang menutup versi aplikasi lama yang sudah beredar.
	- ⚠️ **Prefill hanya mengisi kolom yang masih KOSONG, dan hanya sekali.** Profil sering tiba sesudah halaman terbuka; menimpanya akan menghapus nomor yang sudah diketik orangnya tanpa satu pun pesan. Kedatangan terlambat ditangkap `BlocListener`, bukan dengan memanggil prefill di `build`.
	- ⚠️ **Galat `400` dibaca di datasource, BUKAN di `_handleError`.** Seluruh method `DioApi` memakai `validateStatus: (status) => true`, sehingga status 4xx kembali sebagai respons biasa dan `DioException` tak pernah dilempar — perbaikan yang dipasang di `_handleError` akan hijau di test-nya sendiri dan tetap mati di jalur nyata. `res.message` juga hanya statusMessage HTTP, bukan pesan server. Kunci `errors` dipetakan ke label l10n yang **sama** dengan label kolom di layar (`task_error_fields.dart`).
...
Mengisi menu **Pelatihan** (key l10n `menuLearning`; "Training" di locale en) yang selama ini `ComingSoon`. Daftar pelatihan yang diikuti (berjalan / riwayat), **tandai hadir mandiri**, dan **penilaian trainer** empat aspek.
...
- ⚠️ **Post-test dan course belum ada di `origin/dev`**: nol pemanggil `/me/post-test` maupun `/courses` (diperiksa 2026-09-15), dan komentar `home_quick_access.dart:95` menyebut isinya belum LMS penuh, padahal backend post-test sudah live ([[Microservices - Learning Service]]). ✅ **Post-test sudah menyusul**: my-bharata PR [#151](https://github.com/bip-itteam-internal/my-bharata/pull/151) (branch `feat/post-test-peserta`) **merged ke `dev` 2026-09-16** (diukur 2026-09-17), menambahkan pemanggil `/me/post-test` dan `/me/pre-test` sekaligus menata ulang menu Pelatihan; `/courses` tetap tanpa pemanggil di aplikasi. Belum ada di rilis GitHub. Lihat [[#Post-test Peserta dan Penyatuan Menu Pelatihan]] di bawah.
- **Unduh sertifikat pelatihan** (my-bharata PR [#150](https://github.com/bip-itteam-internal/my-bharata/pull/150), **TERBUKA ke `dev`, belum merge, belum deploy** — bagian Tahap 2 [[HRIS - Training Program]], lihat juga [[APP - Web ERP]]). Kartu pelatihan di tab "Pelatihan Saya" menambah blok **Sertifikat**, tampil hanya bila `item.sertifikatTersedia` bernilai `true` (`_Kartu`, `presentation/pages/my_trainings_page.dart:308-335`) — field ini **dari server** (sertifikat AKTIF milik pemakai untuk pelatihan itu; syarat terbitnya tak dihitung ulang di Dart), dan server lama yang tak mengirim field ini dibaca "belum ada sertifikat", bukan tombol yang pasti gagal (`domain/entities/my_training.dart:40-45`).
	- Tombol Unduh memicu event `DownloadCertificate` (`_unduh`, `my_trainings_page.dart:35-43`) → `TrainingBloc._onDownloadCertificate` (`presentation/bloc/training_bloc.dart:103-116`) → usecase `DownloadTrainingCertificate` (`domain/usecases/training_usecases.dart:46-52`) → `TrainingRemoteDataSourceImpl.downloadCertificatePdf` (`data/datasources/training_remote_datasource.dart:100-116`) → `GET api/learning/me/trainings/:id/certificate` (`Api.myTrainingCertificate`, `core/api/url.dart:218-219`). **Id yang dikirim `trainingId`, bukan id sertifikat**: server tak pernah mengirim id sertifikat ke peserta, beda dari alur unduh HR di web yang memakai `sertifikatId` (`url.dart:216-217`; lihat [[APP - Web ERP]]).
	- **State "sedang mengunduh" (`TrainingCertificateDownloading`) dipancarkan lebih dulu** supaya unduhan kedua atas sertifikat yang sama tetap memancarkan `TrainingCertificateReady` yang dianggap BARU oleh Equatable — tanpa itu bloc menelan state yang "sama" dan lembar berbagi tak muncul lagi (`training_bloc.dart:100-116`; `presentation/bloc/training_state.dart:68-83`).
	- **Bytes-nya dipulangkan lewat state, bukan ditulis ke berkas di BLoC**: penyimpanan + lembar berbagi (`ShareUtil.shareBytes`) urusan halaman, pola yang sama dengan slip gaji (`my_trainings_page.dart:70-76`). Nama berkasnya dari nomor sertifikat (karakter non-alfanumerik jadi "-"), jatuh ke `trainingId` bila nomor kosong (`namaBerkasSertifikat`, `my_training.dart:70-80`).
	- ⚠️ **Sertifikat yang belum terbit maupun yang sudah DICABUT sama-sama dibalas 404**, tak dibedakan di sisi klien (komentar `training_remote_datasource.dart:103-106`). `ApiInterface.getBytes` mengurai badan galat JSON dari respons bytes untuk status non-200 (`core/api/api.dart:265-297`), tapi **khusus 404 pesan spesifik server DIBUANG**: dipetakan paksa ke kunci `errorDataNotFound` ("Data tidak ditemukan", `l10n/app_id.arb:4971`) tanpa memakai pesan server yang berhasil diurai — beda dari status lain yang meneruskan pesan server apa adanya (`_failureDariBytes`, `api.dart:300-314`).
	- **Unduhan gagal memicu muat ulang daftar** (`gagalUnduh` di listener, `my_trainings_page.dart:85-99`): kegagalan unduh hampir selalu berarti daftarnya basi (sertifikat dicabut setelah daftar dimuat), jadi memuat ulang membuat tombolnya ikut hilang.

## Pengajuan Pelatihan & Tugas Onboarding — ✅ PR [#115](https://github.com/bip-itteam-internal/my-bharata/pull/115) merged ke `dev` 2026-08-22

> ⚠️ **Merged ke `dev` 2026-08-22, belum ada di rilis GitHub terbaru.** Merge-nya bukan leluhur tag `v1.14.5+135` (diperiksa 2026-09-15). Per 2026-08-19 PR ini masih OPEN, enam hari sesudah menunya dicabut di web, dan selama itu **karyawan biasa tak punya satu pun jalan bermenu** untuk mengajukan pelatihan: web sudah mencabut menunya 2026-08-13, aplikasinya belum punya layarnya. Data produksi masih konsisten dengan alur yang belum sampai ke orang: `training_request` **0 dokumen** per 2026-09-15, walau endpointnya live sejak 2026-08-11. Ini bukan sekadar fitur tertunda melainkan **alur yang terputus di tengah**, dan yang menutupnya rilis build yang memuat PR ini. Lihat [[Microservices - Learning Service]] dan [[HRIS - Training Program]].
...
- **Sejak 2026-09-22 daftarnya satu baris per KARYAWAN, bukan per tugas** (permintaan pemilik produk). Sebelumnya buddy yang memegang empat tugas atas satu karyawan melihat orang itu empat kali, dan tak ada satu tempat pun yang menyebut progres keseluruhannya. Kini: baris menyebut nama, jabatan · departemen, dan "2 dari 5 selesai"; ketuk → halaman detail `/tugas-onboarding/:instanceId` berisi kartu-kartu tugasnya.
	- Dikelompokkan per tanggal memakai `SliverGroupedList` seperti notifikasi, tapi **menatap ke depan** (`menaik: true`), sama dengan layar Jadwal Ruang.
	- ⛔ Dikelompokkan **`instanceId`, BUKAN nama**: dua karyawan bisa bernama sama, dan menggabungkan keduanya membuat progres salah satunya hilang tanpa satu pun galat.
	- ⛔ Tanggal kelompoknya diambil dari **tenggat terdekat yang BELUM selesai**. Tugas yang sudah beres tak lagi menuntut apa pun, jadi membiarkannya menarik barisnya ke tanggal lampau membuat karyawan tampak terlambat padahal sisa tugasnya masih lama.
	- ⛔ **Karyawan tanpa satu pun tenggat belum-selesai dirender di section "Tanpa tenggat" TERSENDIRI.** `SliverGroupedList` menuntut `DateTime` per item, jadi menaruh mereka di tanggal karangan sama saja berbohong, dan membuangnya membuat tugas yang benar-benar ditugaskan lenyap dari layar tanpa satu pun galat. Seluruh aturan ini hidup di fungsi murni `kelompokOnboardingPerKaryawan` (`domain/logic/`), bukan di widget.
- ⛔ **Halaman detail memuat ulang `GET /onboarding-tasks/assigned` lalu menyaring `instanceId`; JANGAN menggantinya dengan `GET /onboarding-instances/:id`.** Rute yang terlihat lebih langsung itu digerbang `PermRecruitmentView` + `isHR` (`services/recruitment/routes.go`), jadi buddy yang bukan HR menerima **403** dan layarnya mati justru untuk pembaca utamanya — kelas yang sama dengan `GET /kpi/sumber-katalog`. Ongkosnya satu permintaan berulang, dan itu yang membuat halamannya tahan deep link serta tahan restart aplikasi.
...
## Post-test Peserta dan Penyatuan Menu Pelatihan
...
- **Kartu "Pelatihan" baru di grid menu Pengajuan** (`submission_type_selection_page.dart`), sejajar **Booking Ruang** di ekor grid — kedua kartu ini BUKAN dari daftar tipe HR sehingga tetap tampil walau daftar tipe gagal dimuat (termasuk saat daftarnya kosong). Menekannya membuka form pengajuan pelatihan langsung, tanpa lewat picker HR.
- **Post-test peserta** (`post_test_page.dart`, rute `/pelatihan-saya/post-test`, argumen `PostTestArgs{trainingId, judul, jenis}`; `jenis` ditambahkan di Tahap 3b): kartu **"Kerjakan Post-test"** di tab Pelatihan Saya (`my_trainings_page.dart`) muncul hanya bila `bolehMengerjakanPostTest` server bernilai `true`; kuota percobaan, jeda mengulang, batas waktu, dan skor sepenuhnya diputuskan server, tak dihitung ulang di Dart. **Banner pengantar di puncak tab** menghitung berapa post-test yang menunggu (lulus kelas ≠ lulus post-test), sebab tombolnya selalu mendarat di section Riwayat, "tempat yang dibaca orang sebagai arsip, bukan sebagai daftar pekerjaan" (`my_trainings_page.dart:184-195` di HEAD `c988d43e`).
	- **Id percobaan berjalan disimpan di `SharedPreferences` per pelatihan dan per jenis** (`post_test_simpanan.dart`, kunci `post_test_attempt_<trainingId>` untuk post, `pre_test_attempt_<trainingId>` untuk pre sejak Tahap 3b) supaya aplikasi yang tertutup di tengah ujian **melanjutkan**, bukan memulai baru yang memakan kuota, sebab `POST /me/{pre|post}-test/:id/start` selalu membuat percobaan BARU. Hanya id yang disimpan; jawabannya sengaja TIDAK, sebab jawaban lama akan bertengkar dengan snapshot soal server (soal diacak per percobaan) dan yang kalah peserta. Endpoint dan pola resume ini **sama persis dengan sisi web** (`POST /api/learning/me/post-test/:id/start`, `GET`/`POST .../attempt/:id`, lihat [[APP - Web ERP]]), termasuk kontrak "-1 = tanpa batas waktu".
	- **Tenggat hitung mundur ditambatkan ke jam perangkat SAAT percobaan diterima** (`sekarang + sisa_detik` server), bukan dihitung dari `started_at` terhadap jam ponsel — supaya perangkat yang jamnya bergeser tak menampilkan sisa waktu yang salah. Server lama yang belum mengirim `sisa_detik` jatuh ke hitungan dari `started_at` sebagai cadangan.
	- **Waktu habis tetap menyisakan satu aksi**: tombol Kirim berganti **"Mulai Percobaan Baru"** (membuang simpanan lama lalu memulai baru), bukan mati tanpa jalan keluar.
	- Hasil percobaan dipulangkan lewat nilai balik rute (`Navigator.pop(true)`, `PopScope` menyamakan tombol kembali bawaan dan tombol di layar hasil) supaya tab Pelatihan Saya memuat ulang — lulus mengubah penanda post-test dan bisa menerbitkan sertifikat.
- **Pre-test peserta** (Tahap 3b, commit `d1ac1e6b` dan `c988d43e`; status merge di kepala bagian ini). Kelulusan post-test kini berarti **naik dari pre-test**, jadi pre-test jadi langkah wajib sebelum post-test (`domain/entities/post_test.dart:80-90`; teks pengantar `app_id.arb:77`; aturannya di server `NaikDariPreTest`, `services/learning/skoring.go:60` branch bip-erp).
	- **Tombol "Kerjakan Pre-test" di kartu** tampil bila `bolehMengerjakanPreTest` (`preTestTersedia && !preTestSelesai`, keduanya dari server), berganti "Pre-test sudah dikerjakan" sesudahnya, dan diletakkan SEBELUM tombol post-test (`presentation/pages/my_trainings_page.dart:373-404`; `domain/entities/my_training.dart:61-69,102-105`; `l10n/app_id.arb:81-82`). Praktiknya tombol ini tinggal di kartu section **Berjalan**, sebab server menutup `pre_test_tersedia` begitu kelas `Completed` (`my_training.dart:63-67`); kartunya sendiri tak memeriksa section.
	- **Kartu kelas buntu berbunyi "Terkunci: pre-test terlewat, hubungi HR"** (ikon gembok, warna galat tema) di posisi tombol post-test, hanya bila server mengirim `pre_test_terlewat = true` (kelas `Completed`, materi berbank soal pre, pre-test tak pernah dikerjakan; lihat [[API - Learning Service]]). Sebelumnya kartu semacam itu cuma kehilangan tombol, sama persis dengan kelas yang tak berujian. Keputusannya milik server, **tidak dirakit dari status kelas**, karena aplikasi tak tahu kelas mana yang bermateri; absen dibaca `false` sehingga server lama tak menuduh siapa pun (`presentation/pages/my_trainings_page.dart:441-467`; `domain/entities/my_training.dart:71-77`; `data/models/my_training_model.dart:60-63`; `l10n/app_id.arb:83`). Ditambahkan 2026-09-17 di branch `feat/post-test-peserta` sesudah `f076c04e`, **belum di `dev`**. Dikunci test model (kunci JSON dan absen) dan widget test berpasangan di `my_trainings_pre_test_button_test.dart`; mengembalikan rakitan dari status kelas membuat kasus tanpa penanda merah.
	- **Peringatan berjumlah di puncak Pelatihan Saya** ("N pelatihan menunggu pre-test..."), dihitung dari section Berjalan saja dan tampil di atas peringatan post-test, sebab pre-test punya tenggat yang tak bisa dimundurkan (`my_trainings_page.dart:153-183`; `app_id.arb:83`). Dikunci widget test `test/features/training/my_trainings_pre_test_button_test.dart` (tombol tampil dan hilang, kelas di Riwayat tak menambah angka, server lama tanpa penanda tak menawarkan apa pun).
	- **Layar ujian yang sama melayani pre dan post.** `PostTestArgs.jenis` **wajib, tanpa nilai baku**: baku `post` membuat pemanggil yang lupa mengopernya (notifikasi, deep link) tetap terkompilasi dan peserta mengerjakan post-test dari tombol bertuliskan Pre-test (`presentation/pages/post_test_page.dart:16-37`). Jenis memilih rute mulai `api/learning/me/pre-test/:trainingId/start` atau `.../post-test/:trainingId/start`; melanjutkan dan mengirim tetap `.../post-test/attempt/:id` untuk kedua jenis, karena server membaca jenisnya dari percobaan (`core/api/url.dart:220-233`; `data/datasources/training_remote_datasource.dart:160-181`). Judul, pengantar, dan kalimat waktu habis dibedakan per jenis (`post_test_page.dart:347,475,501-504`).
	- **Simpanan percobaan berjalan berkunci per jenis**, dan **kunci lama `post_test_attempt_<trainingId>` sengaja dipertahankan untuk post**: memindahkannya membuat percobaan post yang sedang berjalan di ponsel terpasang tak terjangkau sesudah pembaruan, sementara tanpa jenis di kunci, pre-test yang ditinggal setengah jalan akan "dilanjutkan" di layar post-test (`presentation/utils/post_test_simpanan.dart:16-25`).
	- **Hasil pre-test tanpa lencana lulus/gagal**: judul "Hasil Pre-test", ikon netral, dan catatan bahwa ini pengukuran awal. Kelulusan hanya ditampilkan bila `kind` hasil adalah `post` DAN layar tidak dibuka sebagai pre, sebab `kind` yang absen jatuh ke `post` (`post_test_page.dart:533-605`; `domain/entities/post_test.dart:20-26`). **Hasil post-test menyebut persen pre pembanding** ("Pre-test Anda X%, post-test ini Y%.") bila `pre_max_score` terisi; `null` eksplisit dibedakan dari nol (`post_test_page.dart:582-592`; `data/models/post_test_model.dart:42-50`; `app_id.arb:68`).
	- **Penolakan saat MULAI menetap di pengantar, di atas tombol Mulai** (commit `c988d43e`). Sebelumnya penolakan mulai dan kegagalan melanjutkan berbagi satu field, sehingga penolakan mulai merender blok "Coba lagi" yang tak melakukan apa pun tanpa percobaan tersimpan, dan tombol Mulai tak terjangkau lagi. Kini keduanya dipisah (`_ditolakMulai` vs `_gagalLanjut`, dipilih lewat `_sedangMulai`); kegagalan MELANJUTKAN tetap di bloknya sendiri dengan "Coba lagi" dan simpanan yang tak dibuang (`post_test_page.dart:98-121,307-321,421-435,463-472`). Kasus yang paling terasa: post-test ditolak karena pre-test belum dikerjakan, yang pemulihannya mengerjakan ujian lain, bukan mencoba lagi.
	- **Soal ujian dirakit dari komponen FORM** (keputusan pemilik proses 2026-09-17; commit `f076c04e`, PR [#153](https://github.com/bip-itteam-internal/my-bharata/pull/153) ke `dev`, belum merge per 2026-09-17). Sebelumnya tiap soal kartu buatan sendiri (`_KartuSoal` di atas `CustomCard`), sehingga ujian terlihat berbeda dari setiap isian lain di aplikasi. Kini mengikuti pola kartu isi form (`features/form/presentation/widgets/survey_fill_view.dart`, `_kartuPertanyaan`): seluruh soal dalam **satu** `Card` dipisah `Divider`, tiap soal sebuah `CustomFormField` (label bernomor, poin sebagai `subtitle`) berisi `RadioGroup` + `RadioListTile`, pola yang sama dengan `SurveyChoiceInput`. Satu beda yang disengaja: nilainya **indeks** pilihan, bukan teks pilihan, karena server menilai `selected_index` dan dua pilihan bertuliskan sama dalam satu soal tak bisa dibedakan bila yang disimpan teksnya. Ujian tetap bisa dikerjakan di web ERP juga (keputusan yang sama).
	- ⚠️ **Gap: layar hasil ujian belum punya widget test.** Nol berkas di `test/` yang merender `PostTestPage` (diperiksa `git grep` 2026-09-17); yang ada test bloc (`post_test_bloc_test.dart`), model (`post_test_model_test.dart`), pesan server (`post_test_pesan_server_test.dart`), dan tombol kartu (`my_trainings_pre_test_button_test.dart`). Cabang tanpa-lencana untuk pre, kalimat pembanding, dan blok penolakan mulai hanya terjaga oleh pembacaan kode.
- ⛔ **Tiga bug ditemukan `/review` atas PR ini sendiri, diperbaiki SEBELUM kode ini dibaca untuk dok ini** (commit `a0f627c8`, 2026-09-16): `response.message` yang dipakai jalur galat lama adalah `statusMessage` HTTP ("Forbidden", "Conflict"), BUKAN kalimat server — padahal seluruh guna pesan penolakan post-test justru pada isinya ("kuota percobaan sudah habis (3 dari 3)", "baru bisa diulang setelah 09:30..."). Diperbaiki lewat `_pesanServer` (`training_remote_datasource.dart:137`) yang membaca `response.data['error']`, dipasang di keempat jalur tulis training (kehadiran, penilaian trainer, pengajuan pelatihan, post-test) — sekelas dengan gotcha `validateStatus: true` yang sudah dicatat di bagian Task Management di atas: status 4xx tak pernah melempar `DioException` di `Dio` yang dikonfigurasi begitu. Dua bug lain di commit yang sama: galat JARINGAN saat melanjutkan percobaan sempat dibaca sebagai percobaan kedaluwarsa (membuang simpanan yang masih hidup di server); dan tombol Kirim yang mati saat waktu habis tanpa jalan pulih.
- Backend: [[Microservices - Learning Service]] · kontrak: [[API - Learning Service]].
