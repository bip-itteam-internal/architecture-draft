...
- **`SatgasFillPage`**, bukan `EvaluationFillPage` (yang menutup diri begitu semua orang pernah dinilai): definisi pertanyaan diambil dari `/me/forms` daftar penuh, dan kiriman membawa `subject_employee_id`. Form yang sudah tak ada di daftar menerangkan dirinya, bukan halaman kosong. Sesudah tersimpan, halaman Satgas memuat ulang.
...
- ⚠️ Menu dan halaman hanya menampilkan periode berjalan, mengikuti `/me/satgas`. Keputusan "cek ulang lintas bulan dihitung ke bulan temuan" belum terwujud (T3).
