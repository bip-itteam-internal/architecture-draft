...
- **`/psikotes/[token]`: mengerjakan psikotes** (`components/psikotes/`: `mesin-tes.tsx`, `ledger-kolom.tsx`, `sapaan.tsx`, `tes-sudah-selesai.tsx`; `lib/psikotes/mesin.ts`). Dibuka kandidat **tanpa login**, dijaga token di URL. Rincian Kraepelin: **[[HRIS - Psikotes Kraepelin]]**.
	...
	- **Revisi layar kandidat (2026-09-11, career-bharata #11 merged `441cf713`)**: `sapaan.tsx` menampilkan lowongan, telepon, dan email yang **sudah disamarkan server**. Semua label tahap lewat `labelTahap`/`labelBagian` (`lib/psikotes/paket.ts`), tanpa nama tes. Soal contoh sebelum soal asli: `contoh-pilihan-ganda.tsx` (dari `POST .../subtes/:s/contoh`), `contoh-disc.tsx` dan `contoh-kraepelin.tsx` (bawaan portal); opsi dan grup kata dipakai bersama contoh dan soal asli (`grid-opsi.tsx`, `grid-disc.tsx`). `kerja-pilihan-ganda.tsx` punya nomor soal, Sebelumnya/Berikutnya, Ragu-ragu (localStorage per token), dan konfirmasi Selesaikan bagian; `kerja-disc.tsx` punya Sebelumnya; pintasan huruf dan Enter di `lib/psikotes/use-pintasan-opsi.ts`. `AntreanJawab` tetap mengirim berurutan, kini supaya jawaban **terakhir** yang tersimpan saat soal dijawab ulang, dan memberi tahu pemanggil saat kiriman menyerah (`onMenyerah`) supaya jawaban itu dikirim ulang. Kapan contoh dilewati (subtes atau tahap yang sudah berjalan) diputuskan fungsi murni `keContohSubtes`, `contohBawaan`, dan `contohSubtesTunggal` beserta test. Kolom angka tetap maju saja.
	- ⚠️ **Inilah halaman yang dibuka kandidat di prod.** Tautan psikotes di email dan di layar HR dirakit recruitment-service dari env. Sampai deploy prod 2026-09-14 caranya lewat `ERP_FRONTEND_URL` yang di prod berisi `https://career.bharatainternasional.com` (dibaca 2026-09-10 dan 2026-09-12), sehingga tautan karyawan yang membaca env yang sama (penilaian onboarding, feedback interview) ikut ke portal ini lalu 404. ✅ **Sejak bip-erp #1870 (merged dan live prod 2026-09-14)** tautan psikotes membaca `CAREER_PORTAL_URL` (prod `career.bharatainternasional.com`, env yang juga dipakai logo email), tanpa jatuh ke `ERP_FRONTEND_URL`; rinciannya di [[Microservices - Recruitment Service]] increment **Tautan Email per Penerima**. Implementasi kedua di `erp-frontend` (`src/app/psikotes/[token]` + `src/features/psikotes/`) masih ada dan kemungkinan besar tidak disentuh kandidat prod; keputusan resmi mana yang dipertahankan belum ada.
...
| `GET /public/recruitment/psikotes/:token` + `POST .../start`, `.../columns/:index`, `.../finish`, `.../abandon`; tes berpaket (2026-09-11): `POST .../bagian/:b/{mulai,kolom/:index,selesai}`, `POST .../bagian/:b/subtes/:s/{mulai,jawab,selesai}`, `GET .../gambar/:nama`; *(revisi, merged 2026-09-11)* `POST .../bagian/:b/subtes/:s/contoh` | `/psikotes/[token]` |
