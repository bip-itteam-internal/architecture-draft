...
#
# pakai: kantor-agent.ps1 [-Workspace WS] [-ProyekDir DIR] [-Sekali] [-Interval 2] [-SepiMenit 60] [-TanpaBuka] [-Berhenti] [-Python EXE]
param(
...
if ($hidup) {
  ...
  $argLoop = @($dasar + @('--loop', $Interval.ToString($inv), '--sepi-menit', $SepiMenit.ToString($inv), '--log', $log)) |
