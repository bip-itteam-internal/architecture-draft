...
- [ ] **T0a. Status massal, laporan, dan hasil tes psikotes keluar dari cache gateway.** *(tidak bergantung apa pun; lahir dari review T0 2026-09-10)*
	...
	🔄 **Keadaan 2026-09-11: merged (bip-erp #1832 `4ef28e21`) dan terverifikasi di dev hari itu.** Bentuk yang dipilih: entri ber-`:id` di `noCacheRoutes` yang sama, dicocokkan per segmen, sementara 13 entri prefix lama tak berubah arti (dikunci test regresi); path ketiga rute dijaga test di recruitment-service. Di dev: API-Gateway naik 38 detik sesudah merge (biner memuat `psikotes/report` dan `test-results`); lewat gateway ketiga rute tak pernah `HIT` sementara GET kontrol `HIT`, Redis 0 kunci psikotes; gerbang di bawah lolos versi dev (status `finished` + `selesai_karena: "ditinggalkan"` seketika sesudah `abandon`); sebagai HR di Chrome, badge berganti Terputus 12,6 detik sesudah kandidat keluar, tanpa reload. **Belum tercentang** karena gerbangnya di prod: deploy api-gateway (manusia; kini satu deploy membawa #1824 dan T0a), lalu cek baca-saja biner dan Redis prod.
