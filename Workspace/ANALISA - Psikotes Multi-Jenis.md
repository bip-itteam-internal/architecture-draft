...
- [ ] **T0a. Status massal, laporan, dan hasil tes psikotes keluar dari cache gateway.** *(tidak bergantung apa pun; lahir dari review T0 2026-09-10)*
	...
	🔄 **Keadaan 2026-09-11: kode selesai di bip-erp branch `fix/api-gateway-psikotes-tanpa-cache`, sudah lewat review, belum merged.** Bentuk yang dipilih: entri ber-`:id` di `noCacheRoutes` yang sama, dicocokkan per segmen, sementara 13 entri prefix lama tak berubah arti (dikunci test regresi); path ketiga rute dijaga test di recruitment-service. api-gateway prod per 2026-09-10 masih biner lama (#1824 belum naik), jadi deploy gateway prod sebaiknya menunggu T0a merged supaya satu deploy membawa keduanya.
