...
- [ ] **B1. HRD menjawab legalitas item CFIT dan DISC.** Keduanya instrumen berlisensi. **Bank soal tidak boleh diisi sebelum ini dijawab.** Ini bukan pekerjaan teknis dan tidak menghalangi T0 sampai T2, tetapi **memblokir T3 ke atas**. **(2026-09-11)** T3 sampai T6 tetap dibangun di atas asumsi tertulis yang dipilih user: tak ada butir soal berlisensi di-seed, isi bank soal tanggung jawab HRD. Keputusan HRD-nya tetap terbuka.
- [ ] **B2. Putuskan resmi mesin kandidat mana yang dipertahankan**, erp-frontend atau career-bharata. **De facto di prod sudah career portal**: `ERP_FRONTEND_URL` prod dulu berisi alamat career portal (dibaca 2026-09-10; bip-erp #1870, merged dan live prod 2026-09-14, memindahkan tautan psikotes ke `CAREER_PORTAL_URL` karena env itu juga dibaca tautan karyawan yang jadi 404 pada 2026-09-12), dan commit career-bharata `761b4ec` berjudul "pindahkan halaman tes kandidat dari ERP ke portal karir", jadi arah pemindahannya pernah dinyatakan di kode. Selama keduanya hidup, tiap bentuk jawaban berisiko dibangun dua kali; menunda keputusan ini melipatgandakan T3 dan T5. T0 sudah merasakannya: perbaikan layar 404 hanya dibuat di career portal, dan versi erp-frontend sengaja dibiarkan dengan bug 404/410 lamanya. **(2026-09-11)** Asumsi: mesin baru hanya di career portal; halaman erp-frontend menolak sesi paket dengan arahan ke Portal Karir.
- [ ] **B3. HRD memutuskan pendampingan psikotes.** Tes hari ini dikerjakan mandiri tanpa pendamping dan tanpa jejak tempat pengerjaan, menyimpang dari keputusan HRD yang tercatat ("dilaksanakan staf HR langsung"). **Memblokir T3 (CFIT)**: menentukan apakah soal CFIT boleh dibuka lewat tautan publik, dan bila boleh, timernya wajib ditegakkan server. **(2026-09-11)** Asumsi: boleh jarak jauh; batas waktu tiap subtes ditegakkan server (deadline plus toleransi 5 detik). HRD tetap perlu menerima risikonya tertulis. Rincian: [[ADR - 0087 Katalog Tipe Psikotes Jadi Master Data, Tiga Bentuk Jawaban Tetap Kode]] §Belum Diputuskan.
...
- [ ] **D1. Setel ulang config Kraepelin ke permintaan HRD** (45 kolom, 30 detik per kolom, sekitar 22 menit). Rentang yang didukung sudah mencakupnya, jadi ini **penyetelan data, bukan pengembangan**. Verifikasi: terbitkan satu sesi uji dan pastikan konfigurasinya terbawa. **(2026-09-11)** Dengan kode T1 sampai T6, paket bawaan "Kraepelin" (45 kolom × 30 detik) jadi pilihan bawaan Kirim Tes begitu naik prod, jadi D1 terpenuhi lewat seed; sesi lama ber-`{config}` tetap memakai angka yang dikirim.
...
**Keadaan T1 sampai T6 (2026-09-11):** dikodekan di empat branch (bip-erp `feat/recruitment-psikotes-multi-jenis` + `feat/api-gateway-psikotes-catch-all`, erp-frontend `feat/psikotes-multi-jenis`, career-bharata `feat/psikotes-multi-jenis`); review empat dimensi (keamanan, konkurensi, kontrak, frontend) menemukan 13 temuan dan semuanya sudah ditutup kecuali satu keputusan i18n keterangan impor. Terverifikasi di DEV lewat gateway dan sebagai HR serta kandidat di Chrome. **Merged 2026-09-11 13.42 WIB (bip-erp #1837 `40323aae` dan #1838 `b73862af`, erp-frontend #1527 `42eccaf8`, career-bharata #10 `15240242`); belum prod, jadi belum dicentang.** Gerbang per task yang sudah dilewati di DEV: T1 jenis jawaban asing ditolak 400; T2 paket tiga bagian dikerjakan sampai selesai dan muat ulang melanjutkan di soal yang benar; T3 jawaban lewat deadline ditolak dan DTO kandidat tanpa kunci; T4 berkas lebih dari 2 MB ditolak 413 dan PDF bernama png 415; T5 tally DISC tanpa benar/salah, grafik terbaca di kedua tema; T6 impor dengan baris rusak ditolak per baris.
...
- [x] **T7. Sinkronkan dokumentasi dan peta kepemilikan data.** ✅ 2026-09-11: `HRIS - Bank Soal dan Paket Psikotes` ditulis ulang ke keadaan kode, ADR 0087 dan dok Kraepelin, API Recruitment, gateway, Portal Karir, dan Microservices Recruitment diperbarui, dan baris katalog psikotes plus salinan snapshot sesi masuk `REF - Kepemilikan Data`. Status di dok-dok itu sudah diperbarui ke merged (2026-09-11) dan wajib diperbarui lagi saat deploy prod.
