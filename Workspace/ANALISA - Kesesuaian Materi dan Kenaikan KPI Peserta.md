...
- [ ] **T7. Pertanyaan kesesuaian di lembar penilaian trainer.** Pakai ulang `SurveyScaleInput` + `CustomFormField`; model mengirim field baru hanya bila dijawab; test `toJson` diperbarui. l10n dua bahasa. Rilis menaikkan version name DAN code. Dependensi: T1.
	- ✅ **Merged ke `dev` 2026-09-17** lewat my-bharata [#156](https://github.com/bip-itteam-internal/my-bharata/pull/156): skala 1-10 opsional di lembar penilaian, jawaban diteruskan sampai datasource, kunci dihilangkan bila tak dijawab, judul "Penilaian Pelatihan", 23 test dan 4 kontrol mutasi, analyze folder training bersih. Koreksi: catatan "ditunda karena rilis terakhir `v1.14.5+135`" keliru; MyBharata 1.20.0 dari `dev` sudah beredar menurut pemilik proses tanpa tag atau GitHub Release. **Belum**: rilis store berikutnya (manusia), sebaiknya sesudah learning-service PROD naik. Sementara itu satu-satunya layar yang menanyakannya adalah web `/hris/pelatihan-saya`, yang sejak 2026-08-13 tak punya menu sidebar (dijangkau lewat tautan, mis. `deep_link` feed kalender pelatihan), jadi jumlah responden kesesuaian bisa lama di bawah 3. Server tetap menerima kiriman tanpa jawaban kesesuaian.
