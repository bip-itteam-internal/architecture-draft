...
**Urutan wajib**: learning-service dan employee-service naik BERSAMA (kontrak `GET /kpi/pelatihan` bertambah), lalu erp-frontend, lalu rilis MyBharata. **Prod dijalankan MANUSIA.** Tanpa env baru, tanpa kategori inbox baru, tanpa migrasi.

## Fase 0 — Prasyarat

- [ ] **T0. Ukur ulang prod sebelum mulai (baca saja).** `trainer_evaluation` (0 per 2026-09-17: bila sudah terisi, kiriman lama tak punya jawaban kesesuaian dan tak bisa dilengkapi), cakupan `kpi_score` per bulan (107-165 karyawan per 2026-09-17), dan jumlah peserta kelas `Completed` yang punya skor di bulan sebelum mulai dan sesudah selesai. Angka nol yang mencurigakan diperlakukan sebagai pertanyaan.
