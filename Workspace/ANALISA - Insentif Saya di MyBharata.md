...
- **Keadaan 2026-09-08**: ✅ **SELESAI, live prod** — bip-erp PR [#1778](https://github.com/bip-itteam-internal/bip-erp/pull/1778) (`keanggotaan_saya.go` + 23 test, `levelSkema` satu tempat di `func.go`); rencana `.task-plans/2026-09-08-insentive-keanggotaan-saya.md`. Sumber pemetaan toko: integration `GET /icc/mappings/me` dengan identitas diteruskan (bukan ringkasan profit). Prod di-deploy manusia 06:49 WIB dan diverifikasi dari dalam container untuk 7 orang (ICC, leader, SPV marketing, Host Live, tiga SPV non-marketing, tanpa header), silang dengan `/profit-dashboard/saya`; verifikasi dev dilewati karena VM dev tak terjangkau dari luar kantor. Fixture kontrak `/icc/mappings/me` diganti rekaman prod (nama dan id disamarkan) lewat PR lanjutan kecil.
