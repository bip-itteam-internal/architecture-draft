...
- **Keadaan 2026-09-08**: **merged ke `main`** — bip-erp PR [#1778](https://github.com/bip-itteam-internal/bip-erp/pull/1778) (`keanggotaan_saya.go` + 23 test, `levelSkema` satu tempat di `func.go`); rencana `.task-plans/2026-09-08-insentive-keanggotaan-saya.md`. Deploy dev lewat pipeline (ukur binernya: string `saya/keanggotaan`), prod oleh manusia. Sumber pemetaan toko: integration `GET /icc/mappings/me` dengan identitas diteruskan (bukan ringkasan profit). ⚠️ Verifikasi dev **tertunda**: VM dev tak terjangkau dari luar kantor saat dikerjakan; fixture kontrak `/icc/mappings/me` di test masih diturunkan dari kode integration, wajib diganti rekaman sungguhan saat VPN tersedia.
