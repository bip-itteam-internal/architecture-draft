**Status**: ✅ Perbaikan CEPAT (Temuan 2) sudah dikerjakan & terverifikasi live prod (2026-09-02, `common.IsMarketingDepartmentStaff`, bip-erp branch `fix/shop-acl-marketing-only`). **Redesign PENUH (Rekomendasi §1) SUDAH MERGE + DEPLOY + DIVERIFIKASI LIVE** — bip-erp branch `feat/acl-shops-department-source`, PR #1727 merge ke `main` 2026-09-05, `DepartmentShopLister` dikonfirmasi ada di biner prod `Integration-Service` per pengecekan 2026-09-14 (9 hari observasi live, lewat beberapa redeploy container lain). Riset awal dilakukan 2026-09-02 sebagai kelanjutan investigasi Fase Contract [[ADR - 0045 Identitas Tim Tunggal dan Peta Kepemilikan Marketing]], setelah rencana "cabut menu Teams sepenuhnya" dibatalkan karena ditemukan fungsi ACL yang masih aktif. **Dengan ini, syarat "redesign di-deploy DAN diverifikasi live beberapa waktu" di §Di luar lingkup terpenuhi — Fase Contract (lihat dokumen itu) sekarang bisa mulai direncanakan.**
...
- **Urutan migrasi** — status per 2026-09-14: langkah **migrate** (alihkan `AllowedShops` sepenuhnya ke `department_shops`) DIKODEKAN, lolos unit test (`marketing_team_usecase_test.go`: staf marketing dapat toko departemennya, cross-departemen tak bocor, kanonikalisasi kapitalisasi/spasi, departemen tak dikenal fail-closed, galat lister diteruskan), **MERGE (PR #1727, 2026-09-05) DAN DEPLOY** — dikonfirmasi live di biner prod `Integration-Service` 2026-09-14 (9 hari observasi, survive beberapa redeploy container lain). **expand paralel** tetap dilewati sesuai rencana (Temuan 1 sudah cukup membuktikan kecocokan data). **contract** kini **BOLEH DIMULAI** — syarat "verifikasi live beberapa waktu" terpenuhi; lihat [[ADR - 0045 Identitas Tim Tunggal dan Peta Kepemilikan Marketing]] §Migrasi langkah 4 untuk daftar pencabutan yang tersisa.
...
- **Fase Contract**: mencabut `team_members`/`team_shops`/koleksi/menu `/integration/teams`/`ListTeamIDsByEmployee`/`ListShopsByTeamIDs` — ✅ syarat "redesign di-deploy DAN diverifikasi live beberapa waktu" **terpenuhi 2026-09-14** (9 hari observasi live, lihat §Status di atas). Bisa mulai direncanakan; belum dikerjakan. Lihat [[ADR - 0045 Identitas Tim Tunggal dan Peta Kepemilikan Marketing]] §Migrasi langkah 4.
