...
*Status 2026-09-12*: rencana `.task-plans/2026-09-12-dokumen-pkwt-transisi.md` disetujui (ikut rekomendasi: PDF `go-pdf/fpdf`, gaji dari payroll dengan izin HR pemanggil, tanpa tanda tangan dan stempel direktur, nomor sistem ditandai, tanggal perjanjian titik-titik, kop teks tanpa logo, hanya PKWT dan PKWT (Evaluasi), kiriman dicatat). Implementasi dan /review selesai di branch bip-erp `feat/employee-dokumen-pkwt` dan erp-frontend `feat/hris-dokumen-pkwt`; PR dibuka 2026-09-12: bip-erp #1864 dan erp-frontend #1552, belum merged. Review menambah: izin `payroll.view` dinilai di employee-service, PDF deterministik + kunci idempoten per kiriman terakhir, keterangan Pihak Pertama dan lokasi/jam kerja dari template, tautan langsung ke detail karyawan. Sisa: PR, merge, deploy DEV (BE sebelum FE, env `PAYROLL_MODULE_URL`), verifikasi DEV dengan karyawan dummy beremail tim. Kekurangan yang ditemukan jadi masukan T5, T6, T7.
