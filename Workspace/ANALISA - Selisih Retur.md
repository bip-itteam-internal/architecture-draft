...
Papan kerja hasil `/analisa-kebutuhan` 2026-09-17. Keputusan dan alasannya di [[ADR - 0103 Selisih Retur Dihitung Terjadwal per Periode dan Bisa Ditandai Beres]]; cara kerjanya di [[Microservices - Integration Service]]. Mockup yang disetujui user: tab kelima Auto Sync Retur dengan kartu per jenis, daftar, panel detail tiga sisi (order · scan gudang · Accurate), dan dialog Tandai beres beralasan.
...
- `/start-task Tambah izin finance.returselisih.view/.tandai beserta gerbang menu & tab Auto Sync Retur (ADR-0103 §6)`
...
- `/start-task Model & repository temuan Selisih Retur per periode dengan status dan jejak beres (ADR-0103 §2, §4)`
...
- `/start-task Detektor Selisih Retur kelompok 1 dari data integration (ADR-0103 §3)`
...
- `/start-task Endpoint internal manufacture untuk data scan retur per periode (ADR-0103 §3, Consequences)`
...
- `/start-task Detektor Selisih Retur kelompok 2 dari endpoint manufacture (ADR-0103 §3, §5)`
...
- `/start-task Job malam Selisih Retur beserta saklar dan catatan kelengkapan (ADR-0103 §2, §5)`
...
- `/start-task API Selisih Retur: ringkasan, daftar, detail, tandai beres, unduh (ADR-0103 §6, §7)`
...
- `/start-task Tab Selisih Retur di Auto Sync Retur sesuai mockup (ADR-0103 §1, §6)`
...
- **Repo**: bip-erp (integration)
- **Bergantung**: T6
- `/start-task Penyegaran salinan dokumen retur Accurate terjadwal per periode (ADR-0103 §3 Tahap 2)`
...
- **Bergantung**: T10, T8
- `/start-task Detektor Selisih Retur kelompok 3: catatan ERP vs isi Accurate (ADR-0103 §3, §4)`
