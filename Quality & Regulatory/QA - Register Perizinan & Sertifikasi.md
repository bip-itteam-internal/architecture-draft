...
- **Path di repo**:
  - Backend: `bip-erp/services/employee/legal_perizinan.go` (routes+handler CRUD) · model `LegalLicense` di `bip-erp/shared-library/models/employee/models.go` · collection `legal_license` (`Collections.LegalLicense`) · RBAC `RequireLegalStaff`/`RequireLegalSupervisor` di `bip-erp/shared-library/common/roles.go` · seed department `legal` di `bip-erp/shared-library/models/employee/master_data.go` (`DefaultDepartments`).
  - Frontend: `erp-frontend/src/app/(main)/legal/perizinan/page.tsx` · `erp-frontend/src/features/legal/perizinan/*` (types, hooks fetch/upsert/delete, form modal) · entri menu `legal` di `src/components/layout/sidebar.tsx` · gating rute di `src/proxy.ts`.
- **Status**: ⚠️ Implemented (ada catatan). CRUD Perizinan **live di kode** (compile + typecheck lolos), **belum diverifikasi runtime** di stack dev (butuh redeploy container + akun ber-role `legal`). Unggah PDF, register Kontrak & Dispute, dan pemisahan ke service `legal` tersendiri **belum**.
...
- Model `LegalLicense`: `name`, `license_type` (BPOM/Halal/Izin Usaha/Sertifikasi/Lainnya), `number`, `issuer`, `pic`, `status` (Aktif/Dalam Perpanjangan/Kedaluwarsa/Tidak Perlu), `issued_at` & `expires_at` (ISO `YYYY-MM-DD`, string agar bebas dari friksi parse `time.Time`), `notes`, `file_object` (dicadangkan untuk unggah PDF), `metadata`.
- **Alert H-90 dihitung di sisi UI** dari `expires_at`: badge `H-{n}` (≤90 hari) / `Kedaluwarsa` (lewat tenggat) di halaman `/legal/perizinan`.
- **Frontend**: halaman list + modal create/edit (react-hook-form + zod) + hapus (ActionDialog). Menu `Perizinan & Sertifikasi` muncul di sidebar untuk pemegang role `legal`; rute `/legal/*` digating di `proxy.ts`.
- **RBAC & seed**: role key `legal` ditambahkan pada tier `system_roles` (`legal:staff|supervisor`), department `legal` di-seed di `DefaultDepartments` sehingga peran bisa di-assign lewat Master Data. Lihat [[CORE - RBAC dan Permission Set]].

## Belum Diimplementasikan / Catatan

- **Unggah PDF sertifikat**: field `file_object` sudah ada di model, tetapi endpoint unggah/preview (pola MinIO) **belum** dibuat. Kandidat pola: `services/recruitment` (`uploadCV`/`previewCV`) atau presigned-url `inventory` (`/api/inventory/item/upload/presigned-url`).
- **Register Kontrak & SLA Legal** dan **Register Dispute & Advis Hukum** (P2 pada blueprint) **belum** — akan mengikuti pola CRUD yang sama.
...
- [[APP - Web ERP]] — modul frontend `legal` (sidebar, gating, halaman Perizinan).
