...
- **Booking Ruang: layar web untuk memutus menyusul di irisan 1b** ([[ADR - 0095 Pengajuan dan Persetujuan Booking Ruang Juga Lewat Web]]; erp-frontend branch `feat/ga-peminjaman-aksi-web`, belum merge per 2026-09-14, nomor baris di bawah merujuk branch itu). Tab Perlu Keputusan membaca `GET /peminjaman/perlu-aksi` hanya bila `GET /peminjaman/penyetuju/saya` menjawab `true` (`erp-frontend/src/features/ga/peminjaman/hooks/use-peminjaman.ts:102`, `lib/tampilan.ts:30`); tombol Setujui dan Tolak di detail booking mengikuti `bolehDiputusOleh` (`lib/status.ts:156`) dan dikirim lewat `useSetujuiPeminjaman` serta `useTolakPeminjaman` (`use-peminjaman.ts:177`, `:197`). Aturan tombol itu hanya pemilih tampilan, bukan gerbang: server tetap memutus lewat `bolehMemutus`. Dua akibat yang diterima sadar: booking menunggu yang jam selesainya lewat tak mendapat Tolak di web walau `TentukanTolak` tak memeriksa jam (konsisten dengan antrean yang menyaringnya), dan penolakan server tetap tampil walau hak memutus hilang sesudah data dimuat ulang, karena komponen aksi dipasang menurut identitas pembaca (`components/peminjaman-detail-sheet.tsx:144`, `components/aksi-penyetuju.tsx:80`). Sebelum irisan 1b ter-merge, antrean dan aksi setujui atau tolak hanya terjangkau lewat API; layar MyBharata tetap direncanakan irisan 2 ([[ADR - 0094 Booking Ruang lewat MyBharata, Penyetuju Ditunjuk HR, Satu Sumber Ruang Kantor]]).
...
- [[ADR - 0094 Booking Ruang lewat MyBharata, Penyetuju Ditunjuk HR, Satu Sumber Ruang Kantor]] · [[ADR - 0095 Pengajuan dan Persetujuan Booking Ruang Juga Lewat Web]] (Booking Ruang) · [[Microservices - Inventory Service]]
