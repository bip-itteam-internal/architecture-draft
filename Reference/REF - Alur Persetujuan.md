...
- **Booking Ruang: layar web untuk memutus sejak irisan 1b** ([[ADR - 0095 Pengajuan dan Persetujuan Booking Ruang Juga Lewat Web]]; erp-frontend #1576, merged dan di prod 2026-09-15, nomor baris di bawah diukur di branch PR itu). Tab Perlu Keputusan membaca `GET /peminjaman/perlu-aksi` hanya bila `GET /peminjaman/penyetuju/saya` menjawab `true` (`erp-frontend/src/features/ga/peminjaman/hooks/use-peminjaman.ts:102`, `lib/tampilan.ts:30`); tombol Setujui dan Tolak di detail booking mengikuti `bolehDiputusOleh` (`lib/status.ts:156`) dan dikirim lewat `useSetujuiPeminjaman` serta `useTolakPeminjaman` (`use-peminjaman.ts:177`, `:197`). Aturan tombol itu hanya pemilih tampilan, bukan gerbang: server tetap memutus lewat `bolehMemutus`. Dua akibat yang diterima sadar: booking menunggu yang jam selesainya lewat tak mendapat Tolak di web walau `TentukanTolak` tak memeriksa jam (konsisten dengan antrean yang menyaringnya), dan penolakan server tetap tampil walau hak memutus hilang sesudah data dimuat ulang, karena komponen aksi dipasang menurut identitas pembaca (`components/peminjaman-detail-sheet.tsx:144`, `components/aksi-penyetuju.tsx:80`). Sejak 2026-09-15 antrean dan aksi setujui atau tolak terjangkau lewat web; layar MyBharata ada di irisan 2 (my-bharata #149, merged ke `dev` 2026-09-15, belum rilis store) ([[ADR - 0094 Booking Ruang lewat MyBharata, Penyetuju Ditunjuk HR, Satu Sumber Ruang Kantor]], [[APP - MyBharata]]).
