...
- **Paraf by jabatan (bukan ketik nama)**: panel **"Tim Batch — Jabatan Paraf"** di form — pilih **jabatan** sekali per peran (picker dari Master Karyawan HRIS, `GET /api/employee/list?type=employee` → `position`; **difilter divisi manufacture & quality** via `department`; dossier baru **default sesuai contoh dokumen asli** — Operator/Leader/QC Production, di-seed `seedParafRosterDefault` + `sebarParafRoster` saat create) lalu **tersebar otomatis** ke semua kolom paraf 7 lembar: `kesiapan_olah/kemas` (pelaksana/pemeriksa), `prosedur_kemas` (pelaksana/pemeriksa), `olah/kemas_ka_bag_produksi/mutu`, `pelaksana_uji`. Kolom paraf jadi **read-only** (sumber = `ParafRoster`). **Yang Mengajukan** & **Penanggung Jawab Teknis** distempel **nama + jabatan** (`diajukan/disetujui_oleh_jabatan` dari `BIP-Position`) saat Ajukan/Approve — orangnya bisa berganti, jabatan yang dipatok. Alasan: paraf harus tanpa ketik manual & tak selalu orang yang sama.
- **Paraf digital berjenjang (Mode 2)**: sebelum approval final, tiap peran memberi **paraf dari akunnya sendiri** — dilakukan **DI DOKUMEN** (buka via tombol **"Paraf Dokumen"** / "Paraf" → `BatchRecordPrint` mode paraf, mirip alur Menyetujui), **bukan** di form edit. Tombol **Paraf** hijau muncul **inline tepat di lokasi tanda tangan** (`SlotStamp`/`TtdBlok` paraf-aware) — hanya menyala bagi pemegang **jabatan** yang cocok. **Kesiapan Ruang & Prosedur Pengemasan diparaf PER BARIS** (tiap sel Pelaksana/Pemeriksa sendiri-sendiri, endpoint `{area,row,kolom}`, field `pelaksana/pemeriksa_nama+at` per `KesiapanItem`/`ProsedurKemasItem`). **Hasil (√) Kesiapan Ruang = bagian paraf Pemeriksa** — otomatis √ saat Pemeriksa memparaf baris itu (bukan diisi QC); approve QC tak menyentuh kesiapan; Ka. Bag Produksi/Mutu & Pelaksana Pengujian = slot tunggal (`{slot}`). Approve terkunci sampai **semua sel + slot** terparaf (`BIP-Position` == jabatan slot; PPIC/SPV super boleh override) — klik menstempel **nama + employee_id + waktu** (`BatchRecord.Paraf` = `ParafSet`, 7 slot). `POST .../batch-record/:id/paraf` `{slot, clear?}` (gate posisi di handler, bukan role, agar operator/leader/QC ikut paraf; terkunci saat LULUS/DITOLAK). **Persetujuan PJ Teknis terkunci** sampai **semua** jabatan yang ditetapkan roster terisi — `ApproveBatchRecord` menolak **422** dgn daftar jabatan yang belum paraf; tombol "Periksa & Setujui" disable + hint. Paraf tampil sebagai **stempel hijau miring** (`ParafStempel`: nama + jabatan di bawahnya) di layar & cetak — Kesiapan Ruang, Ka. Bag, Pelaksana Pengujian, Mengajukan/PJ Teknis.
...
- **Collection**: `manufacture_batch_record` (struct `BatchRecord`). Field kunci: `nomor_dossier` (auto `BR/<tahun>/<urut>`), `status`, header produk, `dibuat_oleh_*`, `diajukan_oleh_*`, `disetujui_oleh_*`, sub-struct 7 lembar (`ceklis`, `kesiapan_olah/kemas`, `penimbangan` [`no_batch_bahan`, `teoritis`, `nyata`=ditimbang, `rekonsiliasi`; + `qty_mo`/`sisa`/`pemakaian` **dormant** utk fitur rekonsiliasi MO], `fase`, `rekon_*`, `komposisi_kemasan`, `prosedur_kemas`, `foto_kemasan` [MinIO key], `uji_produksi`, `keseragaman`, `checklist_qa`, `paraf_roster` [jabatan tiap peran paraf] + `paraf` [ParafSet: paraf digital tiap peran] + `diajukan/disetujui_oleh_jabatan`, `rekon_mo_*` [status/ceklis/catatan/diperiksa — fitur Rekonsiliasi MO], dll).
- **Endpoint** (via gateway → manufacture service, lihat [[API - Manufacture Service]]):
  ...
  - `POST .../:id/ajukan` · `POST .../:id/paraf` (`{slot, clear?}`, gate posisi) · `POST .../:id/approve` (body opsional `reviewed:true` + field isian QC; **422 bila paraf belum lengkap**) · `POST .../:id/tolak` (wajib `reason`)
