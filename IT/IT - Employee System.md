...
- **Aktifkan / nonaktifkan akun** karyawan (`is_active`) — menonaktifkan akun menghilangkan kemampuan karyawan login ke sistem perusahaan. (`/account/activate|deactivate|status|toggle`)
	...
	- 🔜 **Menonaktifkan akun kini menuntut ALASAN** (branch erp-frontend `feat/hris-backlog-catatan-keluar`, belum merge). Dialognya menyebut nama dan ID orangnya, alasannya wajib diisi, dan kalimatnya menunjuk jalur yang benar: karyawan yang **keluar** dicatat lewat HRIS → Resign supaya tanggal & kategorinya ikut tersimpan; jalur IT ini untuk akun yang bukan kepergian karyawan (akun ganda, akun titipan, salah buat, insiden keamanan). Alasannya tersimpan di `account_status_log` beserta pelaku dan waktunya — sebelum ini, "siapa mematikan akun ini" memang tak terjawab untuk jalur IT. Latar & angka ukurnya: catatan perluasan [[ADR - 0035 HR Menonaktifkan Akun lewat Catatan Resign]].
	- 🔜 **`PUT /update/:employee_id/system-auth` menolak `is_active`.** Endpoint itu meneruskan apa pun yang dikirim ke dokumen akun, jadi ia sempat jadi jalur keempat yang mengubah status tanpa jejak. Kini dibalas 400 yang menunjuk `PATCH /account/active-status`.
