...
- **Status**: ✅ Implemented — 19 job terjadwal + 1 dispatcher webhook (per audit kode). *2 job reminder attendance + auto-ignore koreksi + pre-alokasi sadar-swap dari **PR #165** (Tukar Jadwal Kerja & koreksi) — belum rilis ke prod.*
...
| attendance | tiap jam (`0 * * * *`) | auto-ignore **leave + koreksi presensi** basi (>24j) + FCM | — | `services/attendance/cron.go:38` |
