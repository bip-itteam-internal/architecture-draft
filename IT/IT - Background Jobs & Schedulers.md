...
| integration | harian 05:00 (`0 0 5 * * *`) ⚠️ belum deploy | Affiliate v2: ledger order-level (`get_conversion_report`) → `shopee_affiliate_conversion` + tagihan komisi bulanan (`get_validation_list`/`report`) → `shopee_affiliate_validation_bill` + stamp/rekonsiliasi; window di-gate ke `get_performance_data_update_time` (fallback trailing-3-hari); iris per-hari (plafon `page*size≤10000`); per-shop isolation, skip tanpa cred AMS | Redis `lock:sync-shopee-affiliate-conversion` (30m) | `.../worker/tasks/shopee_affiliate_conversion_sync_task.go:26` |
