...
9. **Antrean kerja berat lintas sesi** (kit **1.30.0**, 2026-09-25). Batas "dua eksekutor wajar" di `/kerjakan` tadinya prosa, dan terukur 2026-09-25 13.39-13.53 beberapa sesi menjalankan `tsc`, `next build`, dan `vitest` (sekitar 20 worker per suite) bersamaan: CPU 83-88%, RAM 26 dari 32 GB, disk 73%, dan satu gerbang `go test` berjalan 2 jam. Sekarang **run penuh** (suite test, `build`, `tsc`, `lint`, `go build/test ./...`, `flutter test/build`) dari sesi mana pun mengantre satu per satu di mesin itu; test **tertarget** (satu berkas atau paket) sengaja bebas supaya siklus TDD tak menunggu suite penuh.
   - **Satu tempat**: `hooks/antre-lib.ps1` memegang klasifikasi perintah (`Get-KelasPerintah`: `penuh`/`tertarget`/`bukan`; teks berkutip dibuang dulu sehingga `git commit -m "pnpm test"` bukan perintah), daftar pola `if` hook, dan kuncinya.
   - **Kunci** = named **Mutex** per slot (`Local\agentkit-antre-slot-<i>`, jumlah slot `AGENTKIT_ANTRE_SLOT`, bawaan 1), bukan Semaphore: hitungan Semaphore bocor saat pemegangnya mati, sedangkan Mutex milik proses yang di-kill menjadi *abandoned* dan langsung diambil penunggu berikutnya. Pemegang memasang `AGENTKIT_ANTRE_PEGANG`, sehingga proses anaknya tidak mengambil kunci lagi (tanpa itu satu slot = deadlock).
   - **Tiga titik pasang**: `gerbang-lib.ps1` mengantre **per langkah** gerbang (tsc, lint, build, test masing-masing), bukan sepanjang gerbang, dan waktu antre tidak masuk `durasi_detik` maupun batas waktu `dart analyze`; `pre-push` membungkus `pnpm tsc/lint/build` dan `go build`; hook `PreToolUse` baru `antre-gate.ps1` **menolak (exit 2)** run penuh yang diketik agent tanpa `.claude\hooks\antre.ps1 -- <perintah>`, dengan pesan yang menyuruh `run_in_background: true` karena menunggu giliran bisa melampaui batas 10 menit tool. Jalan keluar sadar yang tercatat di transkrip: `$env:AGENTKIT_ANTRE_LEWATI=1;`.
   - **Terlihat**: penunggu mencetak pemegang slot tiap 60 detik ke stderr, tanpa batas tunggu (batas akan menggagalkan gerbang hanya karena antrean); `/papan-sesi` punya bagian ANTREAN KERJA BERAT dari berkas `.task-plans/antre/<pid>.json`, yang milik proses mati dibuang.
   - **Mac/linux: no-op tercatat** (`antre.sh` langsung menjalankan, `init.sh` tidak memasang hook). Masalahnya diukur di mesin dev Windows; implementasi posix ditunda sampai ada pemakainya.
...
- **Waktu tunggu push bertambah.** `build` erp-frontend tidak murah, dan di bip-erp perubahan `shared-library` membuat `pre-push` mem-build **semua** service. Bila ternyata mengganggu, yang diturunkan adalah cakupannya, bukan sifat menolaknya. Sejak 1.30.0 push juga bisa **menunggu giliran** di belakang gerbang sesi lain (butir 9), jadi `git push` dari agent sebaiknya di background.
- **Antrean tidak mengurangi total kerja**, hanya menghentikan perebutan CPU: tiap gerbang selesai lebih cepat dan lebih stabil, tetapi gerbang yang datang belakangan menunggu. "Gerbang lambat" belum tentu macet; lihat bagian antrean `/papan-sesi` dulu. Bentuk pemanggilan yang tidak dikenali klasifikasinya lolos tanpa antre (kembali ke keadaan sebelum 1.30.0, bukan kerusakan); kasus baru ditambahkan ke `Get-KelasPerintah` sekaligus pola `if`-nya.
