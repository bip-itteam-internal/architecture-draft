...
- ⚠️ **Diukur ulang 2026-09-06 lewat `gh workflow list --all` dan `gh run list`**: `deploy.yml` bip-erp ("Deploy to Dev") berstatus **`disabled_manually`**, begitu juga `CI — Verify PR` dan `frontend bip` di erp-frontend; run terakhir erp-frontend (2026-08-13) bahkan tidak sempat jalan karena **kegagalan billing GitHub Actions** ("recent account payments have failed or your spending limit needs to be increased"). `pr-notification.yml` bip-erp tercatat `active` padahal seluruh isinya dikomentari, sehingga tiap push memicu run gagal 0 detik; **PR [#1739](https://github.com/bip-itteam-internal/bip-erp/pull/1739) menghapusnya** (setelah merge, bip-erp tinggal `deploy.yml`). `main` kedua repo `protected: false`, rulesets 403 (paket akun). Kesimpulan yang mengikat: **tidak ada satu pun gerbang otomatis yang berjalan tanpa manusia** di kedua repo; yang ada kini gerbang LOKAL agent-kit 1.15.0 (pre-commit menolak commit di branch utama, pre-push menjalankan tsc/lint/build atau go build), lihat [[IT - Gerbang Repo dan Papan Sesi Agent]] dan [[ADR - 0077 Otonomi Merge Agent Digerbang Mekanisme yang Bisa Menolak]].
