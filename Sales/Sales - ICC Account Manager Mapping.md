...
| **13** | Induk sidebar "ICC" diganti **"Kelola Tim"** (ICC Management, Team Performance, Analisis Account Specialist); Performa Saya pindah ke induk **"Pekerjaan Saya"**; izin posisi modul `akuntoko` membuka Performa Saya untuk Marketplace Advertiser dan Ulasan/Komplain ke Gudang untuk Account Specialist. Lihat [[ADR - 0107 Alat Kerja Pemegang Akun Toko lewat Izin Posisi akuntoko]] | ✅ Merged 2026-09-17: erp-frontend [#1640](https://github.com/bip-itteam-internal/erp-frontend/pull/1640), bip-erp [#1963](https://github.com/bip-itteam-internal/bip-erp/pull/1963) + [#1964](https://github.com/bip-itteam-internal/bip-erp/pull/1964); PROD 2026-09-21 |
...
- ~~**Belum diputuskan**: apakah `icc_leaders`/guard-nya akan dihapus menyusul, atau backend ditulis ulang membaca hierarki HRIS langsung.~~ **TERJAWAB 2026-08-26**: [[Microservices - Insentive Service]] kini menurunkan Leader & Supervisor dari `work_data.supervisor_id`, memakai aturan yang sama persis dengan FE. `icc_leaders` **tidak dihapus** — ia turun pangkat jadi *fallback* bila hierarki gagal ditarik. Guard-nya di Integration belum disentuh, jadi jebakan 400 yang dicatat di atas **masih berlaku untuk alur assign**.

### Fase 4 (Insentive Service) — ✅ selesai 2026-08-26

- ~~Insentive membaca leader dari Integration untuk rollup ICC → Leader; `IncentiveOrg` dashboard profit belum disentuh.~~ Sejak PR #1431/#1432, dashboard profit menurunkan **Leader dan Supervisor** dari hierarki HRIS. `incentive_org` jadi penambal, bukan sumber. Hasil prod: 3 Leader (termasuk Ade yang sebelumnya mustahil muncul karena unique index) dan 2 Supervisor yang sebelumnya **selalu kosong** karena `incentive_org` tak pernah terisi dan tak punya layar pengisi.
- **Aturan yang menentukan** (disalin persis dari `hierarki-leader.ts` supaya tak lahir perbedaan halus): Leader = punya bawahan langsung ber-`position_key: icc` **dan bukan Supervisor**; Supervisor = atasan langsung seorang Leader; anggota langsung SPV (Affiliate, Host Live, Buzzer, dan ICC tanpa perantara Leader) diserap ke baris Supervisor.
- ⚠️ **Batas divisi wajib** — tanpa syarat "supervisor harus sudah memimpin Leader ber-anggota ICC", seluruh supervisor perusahaan ikut tertarik. Terukur prod: Supervisor HRD muncul membawa 27 anggota berisi security, office boy, dan legal. Ketahuan hanya karena verifikasi dijalankan atas muatan `/list` produksi, bukan fixture.
- Rincian lengkap: §Hierarki di [[Microservices - Insentive Service]].
