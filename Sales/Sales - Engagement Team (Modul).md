...
- **Perubahan 2026-09-21** (branch `feat/engagement-realisasi-per-jenis`, **belum merge, belum deploy**, keputusan [[ADR - 0110 Realisasi Engagement Dilaporkan Pengerja per URL per Jenis Pekerjaan sampai Akhir Bulan KPI]]): jenis pekerjaan dan target pindah ke tingkat URL, dan pengerja melaporkan realisasi per URL per jenis. Sebelum ini `volume_realisasi` **tak punya penulis mana pun** dan metrik Quantity selalu 0. Bagian yang sudah diperbarui: **Siklus hidup**, **Konsumen Data**, dan butir 8 di **Cacat yang Diketahui**.
...
- **Keputusan**: [[ADR - 0058 Tiket Engagement Memakai Koleksi dan State Machine Sendiri]] · [[ADR - 0059 Penugasan Langsung Menggantikan Antrian Bersama]] · [[ADR - 0060 Cakupan Keterlihatan Tiket Engagement]] · [[ADR - 0083 Alokasi Otomatis Round-Robin Menggantikan Penunjukan Manual AS]] · [[ADR - 0110 Realisasi Engagement Dilaporkan Pengerja per URL per Jenis Pekerjaan sampai Akhir Bulan KPI]]
...
1. **Account Specialist membuat tiket**: produk, jenis konten, jenis komen, jenis campaign, platform, deadline, guideline, plus **satu sampai lima baris target URL**. **Tiap URL memilih jenis pekerjaannya sendiri** (komentar, like, share, posting, review, vote, rating, save, lainnya) **dan mengisi target per jenis**, sehingga URL 1 cukup like dan komentar sementara URL 2 like, share, save, dan komentar. `jenis_pekerjaan` dan `volume` tiket **diturunkan server** dari rincian itu, bukan diketik terpisah ([[ADR - 0110 Realisasi Engagement Dilaporkan Pengerja per URL per Jenis Pekerjaan sampai Akhir Bulan KPI]]); tiket lama tetap memakai satu angka per URL tanpa jenis. Plafon **5 request per Account Specialist per hari**: yang melebihinya diantre (`ANTRE`, belum punya pengerja) dan dipromosikan otomatis besok. Ia **TIDAK LAGI menunjuk pengerjanya** — sejak [[ADR - 0083 Alokasi Otomatis Round-Robin Menggantikan Penunjukan Manual AS]] (1 September 2026), server mengalokasikan otomatis lewat giliran round-robin (sedepartemen requester secara default). Form menampilkan pratinjau read-only siapa yang AKAN dapat giliran (`GET /engagement/giliran-berikutnya`, sejak 2 September) — **perkiraan**, bisa berbeda dari hasil sungguhan bila giliran bergeser sebelum submit.
2. Sistem menerbitkan **nomor tiket** `ENG/YYYYMM/NNNN` (bulan menurut WIB) dan mengirim notifikasi ke orang yang teralokasi.
3. **Pengerja** menandai *mulai* (opsional) lalu *sudah dikerjakan* — **bukti pengerjaan OPSIONAL** sejak keputusan SPV melonggarkan syaratnya (semula wajib dan membuat tiket mustahil ditandai selesai, lihat ## Cacat yang Diketahui). **Pengerja juga melaporkan realisasi**: angka tercapai per URL per jenis (`PUT .../items/:itemId/realisasi`), boleh kapan pun dan boleh diperbarui **sampai akhir bulan KPI tiket itu ditutup**, walau Account Specialist sudah menekan Tutup. Requester tidak boleh mengisinya (ia yang memverifikasi); admin/supervisor dalam cakupan boleh, termasuk sesudah jendelanya lewat. Angkanya klaim pengerja: sistem tak punya cara memverifikasinya (akun boosting personal tanpa API), hanya jejak log dari→ke.
...
5. **Menugaskan ulang tidak lagi ditawarkan di layar** (keputusan produk 2026-09-17: tim Engagement Kyura hanya satu orang). Endpoint `POST .../reassign` tetap ada untuk pembuat tiket atau admin/supervisor (wajib alasan) dan menolak tujuan di luar departemen requester. Pembatalan tetap milik keduanya, wajib alasan. Tiket yang **salah input** (typo, salah pilih, submit dobel) dan masih `OPEN` atau `ANTRE` dapat **dihapus permanen** oleh pembuatnya atau admin/supervisor; yang sudah berjalan wajib lewat Batalkan supaya jejaknya tetap.

Status: `OPEN` → `IN_PROGRESS` → `DONE_BY_TEAM` → `CLOSED` (plus `ANTRE` sebelum `OPEN`, dan buka-kembali `CLOSED` → `IN_PROGRESS` oleh admin/supervisor/leader), dengan jalur revisi `DONE_BY_TEAM` → `IN_PROGRESS`, jalur tugas-ulang `IN_PROGRESS` → `OPEN`, dan `CANCELLED` dari `OPEN`/`IN_PROGRESS`. Tabel transisi + peran yang berwenang ada di [[Microservices - Task Management Service]].
...
- [[Microservices - Employee Service]] — sumber KPI **`kinerja_engagement`** (`kpi_sumber_engagement.go`, grup `marketing`), menarik `GET /kpi/engagement` dari task-management. **Tiga** metrik dari satu agregat, dipilih lewat `KPIAutoConfig.Metrik` (metrik `quality` **dicabut** 2026-09 mengikuti revisi template KPI Marketing Beauty Hacks tab Engagement Team, yang tak lagi memuat baris kualitas terpisah):

| Metrik | Menjawab | Bentuk nilai | Formula | Penyebut |
|---|---|---|---|---|
| `speed` | *Early Engagement Speed* | selisih menit **`deadline` − `done_at`**, per tiket, **bertanda** (positif = selesai sebelum/pas deadline, negatif = telat, tak pernah dinolkan) | `rasio_ambang`, ambang **tertanam 0** (HR tak perlu mengisi Ambang; layar Atur Target memang tak punya kolomnya) | tiket `CLOSED` periode itu |
| `quantity` | *Engagement Quantity* | rasio `volume_realisasi/volume_target`, per tiket (Σ lintas URL dan jenis), boleh > 1 | `rata_rata`; **target di Atur Target = 1** (skala rasio, bukan 100) | tiket **terukur** (`volume_terukur`): ber-target volume **dan** realisasinya sudah dilaporkan lengkap |
| `reporting` | *Reporting & Account Readiness* | 1/0 per tiket, 1 = punya lampiran bukti | `rasio_ambang` | tiket `CLOSED` periode itu |
...
- **Penyebut dasarnya tiket `CLOSED` pada periode itu**, dan periodenya dari `closed_at` (bulan **UTC**), bukan `created_at`. `DONE_BY_TEAM` sengaja tak masuk — menghitungnya gagal berarti menghukum pengerja atas kelambatan pemohon. `CANCELLED` juga tidak: permintaan yang batal bukan pekerjaan yang gagal. `quantity` lebih sempit lagi: **realisasi yang belum dilaporkan bukan rasio 0**, ia keluar dari penyebut dan tampil sebagai cakupan (`Ditutup` > `volume_terukur`). Pengerja yang tak pernah mengisi karenanya tampil "tak terukur" di panel Otomasi KPI, bukan mendapat skor 0. Bulan yang sama menentukan **jendela pengisian realisasi** (lihat Siklus hidup).
...
- ⚠️ **Konsekuensi yang perlu diketahui HR** (tertulis di kode): (a) metrik `speed` mengukur *selesai relatif terhadap deadline yang diminta pemohon* (`deadline` − `done_at`), bukan *berapa cepat merespons* dan bukan durasi kerja; (b) `speed` versi lama (menit `assigned_at` → `done_at`, tak bertanda) **tak pernah bisa bermakna**: `rasio_ambang` menuntut Ambang, layar Atur Target tak punya kolomnya, sehingga skornya konstan apa pun kinerjanya. Perbaikannya menyalin pola `SelisihJam` milik `kinerja_tiket` (nilai bertanda relatif deadline, ambang 0); field JSON berganti nama `menit_ambil` → `selisih_menit`; (c) angka `quantity` adalah **klaim pengerja** dan tak terverifikasi ([[ADR - 0110 Realisasi Engagement Dilaporkan Pengerja per URL per Jenis Pekerjaan sampai Akhir Bulan KPI]]).
...
**Ditemukan 2026-09-21, diperbaiki di branch `feat/engagement-realisasi-per-jenis` (belum merge/deploy):**

8. ⛔→✅ **`EngagementItem.VolumeRealisasi` tak punya penulis, sehingga metrik Quantity selalu 0 untuk semua orang.** Diukur `git grep` atas `origin/main` di kedua repo: satu-satunya sentuhannya guard `> 0` di `itemBolehDiubah`, penjumlahan `muatVolumeTiket`, dan pembacaan tombol di layar. Tak ada galat, angkanya wajar, dan tak satu pun test menangkapnya karena test memberi fixture realisasi dengan tangan. ADR 0058 §1 sudah menjanjikan pengerja yang melapor, rutenya tak pernah dibangun; Fase A (2026-09-16) hanya menyembunyikan gejalanya (tampilan "x/y" dihapus). Perbaikannya di [[ADR - 0110 Realisasi Engagement Dilaporkan Pengerja per URL per Jenis Pekerjaan sampai Akhir Bulan KPI]]. Sisanya yang belum tertutup: **tiket yang ditutup sebelum rilis** memakai bentuk lama dan harus diisi pengerja dengan satu angka sebelum akhir bulan; angkanya tetap klaim tak terverifikasi.
...
- **Penyelarasan template KPI produksi** yang masih bernama `Buzzer` dengan sumber `kinerja_engagement`. Template baru "Engagement Team Beautyhack" (Speed 45%, Quantity 55%) tampil di layar Atur Target; **nilai yang dianjurkan saat mengisinya** (belum diverifikasi tersimpan di lingkungan mana pun): Speed bertarget 100 (persen tiket selesai tepat/lebih awal dari deadline), Quantity bertarget **1** (rasio). `SATUAN_PER_METRIK` frontend belum memuat `speed`/`quantity`, jadi Tipe isian dipilih manual (Speed "Persen", Quantity "Nilai").
- **Apakah menandai Selesai mensyaratkan realisasi sudah dilaporkan**, dan apakah atribusi periode KPI diselaraskan ke WIB: lihat TBD di [[ADR - 0110 Realisasi Engagement Dilaporkan Pengerja per URL per Jenis Pekerjaan sampai Akhir Bulan KPI]].
