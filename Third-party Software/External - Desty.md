...
- **Auto-ship / auto-approve sadar-holiday** — saat order baru: ship hanya pada **00:01–14:59 WIB**; ditunda (`PENDING`) bila **>15:00 WIB** atau **hari libur** (varian Shopee juga menunda bila **besok** libur). Eksekusi per platform: Shopee/TikTok via ship-order masing-masing, selain itu via Desty `POST /api/order/accept`.
  - **Pengganti in-house UTAMA (2026-07-23): menu WMS "Perlu Diproses"** — meniru tab *Siap Dikirim → Perlu diproses* Desty. Admin gudang melihat order belum-berresi, memilih batch, klik **"Proses Pengiriman"** → resi terbit (`POST /fulfillment/arrange` → `ship-batch`). Dipilih **manual, bukan otomatis**, karena praktik lapangan menunjukkan admin butuh mengatur *ritme* (proses saat gudang siap kemas) dan *menyaring* (menahan order bermasalah). Lihat [[Microservices - Warehouse Service]] & [[WH - Fulfillment Flow & WMS Tinggarjaya]].
    - ✅ **Blocker cutover simulasi pertama (2026-07-24) DITUTUP hari yang sama (PR #638 / commit `14b9795c`).** Gejala: order yang di-arrange lewat menu ini tidak sampai ke WMS (marketplace tak mengirim webhook untuk perubahan yang kita sendiri sebabkan; `ShipBatch` tak memanggil `NotifyWarehouse`; reconciler kebetulan juga macet). **Perbaikan:** `ShipBatch` kini mendorong order ke WMS lewat dua jalur lepas-webhook (dorong `NotifyWarehouse` + tarik reconciler); deadlock reconciler diperbaiki; `raw_status` Shopee disegarkan. Terverifikasi live: arrange TikTok → muncul di "Pesanan Baru" <60 detik. **Menu "Perlu Diproses" kini siap dipakai simulasi penuh.** Sisa: 198 order Shopee "hantu" pra-fix + 2 order TikTok nyangkut belum di-backfill (pemulihan data terpisah). Detail: [[Microservices - Integration Service]] & [[Microservices - Warehouse Service]].
