**Status**: ⚠️ Implemented (ada catatan) — **belum berfungsi sebagaimana dimaksud.** Kode selesai di branch `feat/oneForAll` (2026-08-06), seluruh uji hijau, **belum merge dan belum deploy**. Aman dipasang (tak ada yang kehilangan akses), tetapi **mengaktifkannya belum benar-benar membatasi**: celah "akun tanpa paket" yang semula diduga kosong kini **terbukti tidak kosong** (bukti 2026-08-06 di §Consequences), sehingga akun finance/IT tanpa permission-set tetap bisa membuka halaman walau restriksi menyala. Prasyarat agar berfungsi: perbaiki fallback task-management lebih dulu. Penegakan backend juga sengaja parsial.
...
**Prasyarat agar berfungsi** (belum dikerjakan): ubah `effectiveTicketPermissions` di task-management dari menilai "klaim ada" jadi "klaim memuat izin **ticket**", menyamakannya dengan payroll/monitoring/procurement, sambil membuat penutupan akun vendor **eksplisit** lewat `IsExternalAccountPermissions` alih-alih menumpang perilaku lama. Sesudah itu penjaga "hanya menumpang klaim tak kosong" di `gabungPenandaMenu` bisa dicabut sehingga penanda menempel ke semua orang, dan `TestKaryawanTanpaPaketTetapKosong` diganti uji yang menguji maksudnya (fallback tier tetap hidup) alih-alih mekanismenya. Perubahan ini menyentuh jalur otorisasi modul lain untuk seluruh pengguna, jadi menuntut uji ulang keempat service konsumen klaim. Sifat "hanya menumpang klaim tak kosong" saat ini dikunci uji, jadi mencabutnya akan membuat CI merah lebih dulu — disengaja.
