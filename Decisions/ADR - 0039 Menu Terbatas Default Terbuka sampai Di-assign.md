**Status**: ⚠️ Implemented (ada catatan) — **sudah merge, tetapi belum berfungsi sebagaimana dimaksud.** Kode selesai di branch `feat/oneForAll` (2026-08-06), seluruh uji hijau, dan sejak itu **sudah mendarat di `main` kedua repo** (terverifikasi 2026-08-10: `services/employee/menu_terbatas.go`, `shared-library/common/catalog_menu.go`, gerbang di `services/integration/main.go`; sisi FE `utils/menu-terbatas.ts` + `components/menu-terbatas-guard.tsx`). Merge menaikkan taruhannya, bukan menurunkannya: yang menahan fitur ini kini tinggal keputusan untuk tidak memasang paketnya ke siapa pun. Aman dipasang (tak ada yang kehilangan akses), tetapi **mengaktifkannya belum benar-benar membatasi**: celah "akun tanpa paket" yang semula diduga kosong kini **terbukti tidak kosong** (bukti 2026-08-06 di §Consequences), sehingga akun finance/IT tanpa permission-set tetap bisa membuka halaman walau restriksi menyala. Prasyarat agar berfungsi: perbaiki fallback task-management lebih dulu (**langkah pertama SUDAH selesai 2026-08-09; per 2026-08-26 penyisiran ulang menemukan tak ada lagi konsumen yang menilai dari "klaim ada", jadi langkah kedua kini tak punya penghalang selain keputusan mengerjakannya**). Penegakan backend juga sengaja parsial. **Kunci kedua `menu.finance.insentif` ditambahkan 2026-08-26** (Dashboard & Master Data Insentif, sekalian pindah ke Portal Saya) dan mewarisi peringatan yang sama: jangan dinyalakan sampai langkah kedua tuntas.
...
**Langkah kedua BELUM dikerjakan**: penjaga "hanya menumpang klaim tak kosong" di `gabungPenandaMenu` masih terpasang, dan `TestKaryawanTanpaPaketTetapKosong` masih mengunci sifat itu. Selama keduanya ada, celah akun-tanpa-paket tetap terbuka dan fitur ini tetap **jangan dinyalakan**. Mencabut penjaga itu pekerjaan fitur menu terbatas sendiri, menuntut uji ulang tersendiri, dan akan membuat CI merah lebih dulu — disengaja.

**Diperiksa ulang 2026-08-26: prasyarat langkah kedua kini BERSIH.** Penjaga itu dipasang demi satu konsumen (`effectiveTicketPermissions` di task-management) yang menilai fallback dari "klaim ADA". Penyisiran `KlaimMemuatIzinModul|len(list) > 0` atas seluruh `bip-erp` hari ini menemukan **tak satu pun** konsumen klaim yang masih menilai dari "klaim ada": task-management (`identity.go`), learning, recruitment, form-builder, monitoring, payroll, hris, legal, ga, secretary, dan kpi semuanya lewat `common.KlaimMemuatIzinModul`, yang mengecualikan penanda ber-namespace. Artinya alasan asli penjaga itu sudah hilang, dan yang menahan langkah kedua tinggal **keputusan untuk mengerjakannya**, bukan lagi dependensi ke modul lain.

Selama langkah kedua belum dikerjakan, peringatan di atas berlaku **untuk SETIAP kunci menu terbatas**, termasuk `menu.finance.insentif` yang ditambahkan 2026-08-26. Konsekuensi konkret untuk kunci kedua itu: fallback-nya memuat `atasanInsentif` (supervisor / adv_leader / adv_marketplace / adv_meta), dan justru populasi itulah yang paling mungkin memegang `system_roles` tanpa satu pun permission-set — sama persis dengan akun bukti 2026-08-06 yang ber-`finance: supervisor` + `it: supervisor` tetapi berklaim kosong. Memasang paketnya hari ini akan membuka Dashboard Insentif bagi yang ditunjuk **tanpa menutupnya** bagi atasan marketing yang berklaim kosong.
