...
⚠️ **Lokasi gerbang `menu.finance.laporan` pindah dari HALAMAN ke TAB, 2026-09-26** (erp-frontend #1750/#1751, [[ADR - 0130 Dashboard FAT Diringkas, Isi Posisi Pindah ke Modul Kerjanya]]). Halaman `/finance/accounting` (Laba Rugi, Neraca, Neraca Saldo) dan halaman `/finance/gl` (Jurnal) digabung jadi satu halaman **Pembukuan** (`/finance/accounting?tab=`), tapi gerbangnya TIDAK dilebur: tab Jurnal tetap `finance.accounting.view`, tab laporan tetap menu terbatas `menu.finance.laporan` — persis dua gerbang lama, kini dinilai **per tab** lewat fungsi tunggal `aksesPembukuan`/`bolehPembukuan` (`erp-frontend/src/features/finance/accounting/lib/tab-pembukuan.ts`). Tab yang tak diizinkan **tidak didaftar** (tidak muncul di `CustomTabs` sama sekali), bukan tampil terkunci — konsisten dengan §Decision "menu tak boleh mengajak klik yang pasti ditolak". Halaman boleh dibuka bila SALAH SATU tab boleh; bila hanya laporan yang boleh, `MenuTerbatasGuard` yang dipasang (bukan `FinanceModuleGuard`), dan sebaliknya. Menu sidebar **"Pembukuan"** (menggantikan "Accounting" + "Jurnal & Buku Besar") memakai penanda `$pembukuan` (`IZIN_MENU_PEMBUKUAN`) yang dikenali `bolehMenu` (`utils/menu-permission.ts`) dan dinilai lewat fungsi yang SAMA dengan gerbang halaman — bukan izin tunggal, karena menu ini tak bisa digerbang satu izin. Semantik ADR ini (default terbuka, mengunci begitu ada assignment, fallback = perilaku lama) **tidak berubah**; yang berubah cuma lokasi pemeriksaannya.
...
- [[ADR - 0130 Dashboard FAT Diringkas, Isi Posisi Pindah ke Modul Kerjanya]] (gerbang `menu.finance.laporan` pindah ke tingkat tab, 2026-09-26)
