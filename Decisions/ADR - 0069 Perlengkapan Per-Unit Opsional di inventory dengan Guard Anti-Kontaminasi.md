...
- ⚠️ **KPI Fase 2 WAJIB menangani ini**: karena perlengkapan **campur** di `/items` (yang ditarik KPI `akurasi_aset_ga`), dan ia tak punya `accurate_asset_no`, ia akan dinilai **100% (ERP-only)** → menggelembungkan skor aset tetap. Saat Fase 2, `akurasi_aset_ga` **wajib mengecualikan** item ber-`accurate_item_no` (perlengkapan dinilai lewat metrik opname sendiri). **Belum masalah** — KPI belum attach ke template mana pun. ✅ **FE 2026-09-03**: tab Cocokkan (`reconcile-tab`) sudah mengecualikan item ber-`accurate_item_no` dari daftar + skor **per-item** aset tetap (`fieldScore`/`accuracy`) yang DITAMPILKANNYA (erp-frontend `fix/cocokkan-perlengkapan-dan-filter`; diskriminator `itemPerlengkapan` di `utils/reconcile.ts`). Ini menutup kontaminasi skor per-item di layar; BE `akurasi_aset_ga` (yang menarik `/items`) **tetap Fase 2**. ⚠️ **Amandemen 2026-09-07**: keputusan GA membalik SEBAGIAN exclusion ini — kartu headline **Skor KPI** & **Cakupan** (bukan Akurasi, bukan daftar ceklis) kini **menggabungkan** aset tetap + perlengkapan **50:50** (`gabungSkorJenis`), karena kedua jenis dianggap "satu kesatuan" tugas GA. Perlengkapan menyumbang lewat metrik **BARU** (`cakupanKuantitasPerlengkapan` = Σ unit ERP terlacak ÷ Σ qty Accurate Gudang Utama) — **bukan** `fieldScore` (yang tetap tak berlaku untuknya, tak ada harga/tanggal untuk dibandingkan). Jadi exclusion dari daftar ceklis & dari `computeFieldScore`/`accuracy` **tetap berlaku**; yang berubah cuma dua angka ringkasan headline. Detail: [[GA - Inventory Management]].
