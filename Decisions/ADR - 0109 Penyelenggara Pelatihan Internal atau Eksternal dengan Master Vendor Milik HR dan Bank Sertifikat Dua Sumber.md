...
- ✅ Perjalanan layar utuh sebagai orang **sudah dijalani 2026-09-21** di DEV (Chrome headless + CDP, login sungguhan lewat form): kelas `Completed` bermateri menampilkan peringatan, centang konfirmasi, dan tombol Tugaskan mati sampai dicentang; dua orang ditugaskan lewat SATU `POST .../participants/batch` dan blok hasil berbunyi "2 peserta ditambahkan"; kuota diisi 3 lewat isian baru lalu dua kandidat berikutnya ditolak `kuota_penuh` **dengan nama masing-masing** sementara pesertanya tetap 3. ⛔ **Perjalanan itu menemukan satu cacat yang 29 test lewatkan**: laporan per orang hilang begitu dialog ditutup lalu dibuka lagi, karena `trainings-page` mengosongkan `participantsTarget` sehingga `trainingId` jadi `""` dan efek pembuangnya membacanya sebagai pergantian kelas. Diperbaiki di erp-frontend branch `fix/laporan-peserta-bertahan` (belum PR per 2026-09-21), beserta uji yang kini meniru pemanggil sungguhan dan kontrol negatif; perbaikannya dibuktikan lagi di layar atas build yang memuatnya.
- ⚠️ **Yang menentukan nilai seluruhnya**: data prod 2026-09-21 masih **4 baris peserta** untuk 13 kelas (11 kelas nol, 0 sertifikat, pendaftaran terbaru 2026-09-17): alat pengisiannya sudah live, tetapi belum dipakai. Selama itu belum terjadi, bank sertifikat dan riwayat per karyawan akan tetap tampak kosong, dan itu keadaan data, bukan kerusakan.
