...
- **Status**: 🟡 **Diusulkan** 2026-09-18, disetujui pemilik proses (Opsi B: catatan dan bukti dulu, berkas besar menyusul). **Keputusan 1 (data peserta lebih dulu) sudah dikodekan**, di branch dan belum di-push: bip-erp `feat/learning-peserta-massal`, erp-frontend `feat/peserta-massal-laporan`, rencana `.task-plans/2026-09-18-peserta-massal-pelatihan.md`. Keputusan 2 sampai 11 **belum ada kodenya sama sekali**.
...
**Status kode T1** (2026-09-18): rute massal, laporan per orang, peringatan kelas selesai, dan isian kuota **selesai di branch** `feat/learning-peserta-massal` (bip-erp) dan `feat/peserta-massal-laporan` (erp-frontend); **belum di-push, belum PR, belum merge, belum deploy**. Yang belum bisa dibuktikan tanpa deploy: perjalanan lewat gateway DEV.
