...
4. ✅ **Contract — DEPLOY + BACKFILL TERVERIFIKASI LIVE (2026-09-02).** Kedua pembaca (**saringan `/divisi`** dan **kolom penanggung jawab** `icc`/blok "Konteks per departemen") sudah di-deploy dan diverifikasi langsung di prod: `docker exec Marketing-Analytics-Service grep -c 'team_shops' /service` → **0** kemunculan (dead-code-eliminated dari biner yang berjalan), `grep -c 'department_shops' /service` → 7, `grep -c 'Departemen '` (label baru menggantikan "Tim ") → 1. Backfill `POST /jobs/sync-profit-attribution/trigger?hari=120` sudah dijalankan dan menutupi **747.786 dari 871.708 dokumen** `mart_profit_attribution` (85,8%) — dikonfirmasi lewat query langsung: `icc.penanggungjawab.nama` berpola `"Departemen <X>"` untuk 747.786 baris. **Sisa 84.836 baris (9,7%) masih membawa label lama** (`"Tim GB-KY"`, `teamid` berupa UUID `marketing_teams` lama) — dikonfirmasi ini PERSIS batas jendela 120 hari, bukan kegagalan backfill: sampel baris tak ter-backfill bertanggal `2026-05-01`, tepat di luar jendela hitung mundur dari tanggal backfill dijalankan. Baris ini **aman dibiarkan** — nama sudah "dibakar" sebagai teks statis di dalam dokumen (bukan live-lookup ke `marketing_teams`), jadi mencabut koleksi lama tidak merusak tampilannya; labelnya sekadar mencerminkan struktur organisasi historis saat order itu terjadi.
   ...
   **Source code MASIH menyimpan fungsi mati** (`bacaTokoTim`/`bacaNamaTim` di `services/marketing-analytics/integration_db.go`, query langsung ke `team_shops`/`marketing_teams`) — dikonfirmasi TIDAK dipanggil dari jalur manapun (itulah sebabnya string-nya lenyap dari biner), tapi belum dihapus fisik dari repo. Ini AMAN dihapus kapan saja (scope-nya murni marketing-analytics-service, tidak menyentuh apa pun di bawah).
   ...
   >
   > ✅ **SUSULAN (2026-09-05) — ACL "siapa boleh lihat toko apa" SEKARANG PUNYA pengganti berbasis `department_shops`, tapi BELUM MERGE/DEPLOY.** Dianalisis di [[ANALISA - Redesign ACL AllowedShops Berbasis Department Shops]] dan dikodekan di bip-erp branch `feat/acl-shops-department-source` (commit `f4d45777`): `AllowedShops` kini `AllowedShops(ctx, department)`, baca `department_shops` + `Sedepartemen`, bukan lagi `team_members`/`team_shops`. **Cakupan sengaja dibatasi** — gate rute `RequireIntegrationStaff` dan exemption `!IsIntegrationAdmin` (Maftuhissaiin/Kyura, Aris Romadhoni/Beauty Hacks tetap lihat semua toko) TIDAK diubah. **Menu `/integration/teams` MASIH HARUS ADA** — pengganti ACL-nya sudah ada di kode, tapi belum merge, belum deploy, belum diverifikasi live; koleksi `team_shops`/`team_members` juga belum dihapus (masih dipakai fitur Teams CRUD lain). Fase **contract** (baru boleh mulai setelah verifikasi live pasca-deploy) masih sepenuhnya di depan.
...
- ⛔ **DIBATALKAN (2026-09-02) — "Satu menu hilang (Teams)" TIDAK terjadi, MASIH berlaku per 2026-09-05.** Fungsi kepemilikan toko→divisi memang pindah ke kartu team ICC Management, tapi menu Teams TETAP HARUS ADA: ia juga melayani ACL "siapa boleh lihat toko apa" (`AllowedShops`, dipakai puluhan halaman Accurate/payouts/gross-profit). Pengganti berbasis `department_shops` untuk ACL ini **sudah dikodekan** 2026-09-05 tapi **belum merge/deploy** — menu masih wajib ada sampai itu selesai DAN fase contract dijalankan. Lihat koreksi lengkap + susulannya di §Migrasi langkah 4.
