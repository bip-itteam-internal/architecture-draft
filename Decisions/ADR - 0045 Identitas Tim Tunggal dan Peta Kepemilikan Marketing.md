**Status**: 🟡 Sebagian dieksekusi (2026-08-29). **Expand** dan **Migrate** live di produksi: penyemaian awal 16 pemetaan (2026-08-11) tumbuh lewat entri manual susulan jadi **58 dari 59 toko terpetakan** per 2026-08-27 (satu yatim tersisa: `Series Glowbooster`), drift ICC-vs-`department_shops` (`Menyimpang`) nol. **Contract SUDAH DUA DARI DUA PEMBACA DIKERJAKAN, BELUM DI-DEPLOY/BACKFILL**: saringan `/divisi` di [[Microservices - Marketing Analytics Service]] sudah pindah penuh ke `department_shops` (bip-erp PR [#1520](https://github.com/bip-itteam-internal/bip-erp/pull/1520) & [#1522](https://github.com/bip-itteam-internal/bip-erp/pull/1522), erp-frontend PR #1305 — merged 2026-08-29, **belum di-deploy/diverifikasi lewat gateway**); kolom penanggung jawab toko (`icc`/blok "Konteks per departemen" di `/beranda`) **kode-nya sudah dipindah** ke `department_shops` (`sumber_penanggung_jawab.go`, branch `feat/penanggung-jawab-department-shops`, belum merged) — TAPI atribusi ICC dibakar ke `mart_profit_attribution.icc` saat sync, jadi data historis butuh **backfill eksplisit** (`POST /jobs/sync-profit-attribution/trigger?hari=120`) pasca-deploy, bukan otomatis berubah begitu kode di-deploy. Menu `/integration/teams` dan koleksi `team_shops`/`marketing_teams` **masih belum dicabut** (dipertahankan sengaja sebagai dead code sampai kedua pembaca terverifikasi stabil di prod). Rincian status per-langkah: §Migrasi. Menyusul [[ADR - 0030 RBAC Tiga Sumbu dengan Hak Menempel di Posisi]] dan [[ADR - 0043 Peran Sistem Diturunkan dari Jabatan]]; **tidak** menggantikan keduanya.
...
4. 🟡 **Contract — KODE KEDUA PEMBACA SUDAH DIGARAP, BELUM DI-DEPLOY.** **Saringan `/divisi`** (`services/marketing-analytics/divisi.go`) sudah pindah sepenuhnya ke `department_shops` (bip-erp PR [#1520](https://github.com/bip-itteam-internal/bip-erp/pull/1520) & [#1522](https://github.com/bip-itteam-internal/bip-erp/pull/1522), erp-frontend PR #1305 — merged 2026-08-29, **belum di-deploy/diverifikasi lewat gateway**). **Kolom penanggung jawab toko** (`icc` / blok "Konteks per departemen" di `/beranda`, `sumber_penanggung_jawab.go`) menyusul 2026-08-29 (branch `feat/penanggung-jawab-department-shops`, **belum merged**) — dipicu bug link mati di prod, bukan direncanakan sejak awal. ⚠️ **Beda krusial dari saringan `/divisi`**: atribusi ICC dibakar ke `mart_profit_attribution.icc` saat job sync (bukan dibaca live), jadi deploy kode SAJA tidak mengubah data historis — wajib disusul backfill eksplisit (`POST /jobs/sync-profit-attribution/trigger?hari=120`, maksimum yang diizinkan; data mart lebih tua dari itu tetap membawa atribusi lama sampai ada backfill khusus). Karena KEDUA konsumen kini punya jalur department_shops (walau belum di-deploy+backfill), `marketing_teams`, `team_shops`, `team_members`, dan menu `/integration/teams` **masih belum bisa dicabut** — pencabutan menunggu verifikasi stabil beberapa hari pasca-deploy+backfill, bukan lagi menunggu migrasi kode.
