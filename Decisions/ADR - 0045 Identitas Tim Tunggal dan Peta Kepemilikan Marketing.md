**Status**: ✅ Expand/Migrate/Contract (kode+data) DAN pencabutan fisik SELESAI dan terverifikasi live di prod. Penyemaian awal 16 pemetaan (2026-08-11) tumbuh lewat entri manual susulan jadi **58 toko terpetakan**; laporan kesehatan `/department-shops/kesehatan` per 2026-09-02 menunjukkan **2 yatim** (`Series Glowbooster` — lama, menunggu keputusan; **`BEAUTYHACKS BHARATA`** — toko baru 2026-09-01, **belum di-assign meski tab assign-nya sudah live**, actionable), drift `Menyimpang` nol. **Contract (kode+data) SELESAI DAN TERVERIFIKASI**: kedua pembaca (`/divisi` dan kolom penanggung jawab ICC) dikonfirmasi lewat grep biner produksi — 0 referensi `team_shops`, dead-code-eliminated. Backfill `POST /jobs/sync-profit-attribution/trigger?hari=120` sudah dijalankan, menutupi 85,8% dokumen mart (batas 120 hari menjelaskan sisanya, bukan kegagalan — lihat §Migrasi langkah 4). **Pencabutan FISIK SELESAI (2026-09-15, PR bip-erp #1888 + erp-frontend #1579, merge ke `main`)**: `bacaTokoTim`/`bacaNamaTim`/`indeksDivisiDari` (dead code) dihapus dari `integration_db.go`/`divisi.go`; CRUD Teams (`marketing_team_handler.go`, `marketing_team_repo.go`, entity `MarketingTeam`/`TeamMember`/`TeamShop`) dihapus total dari `services/integration`; route `/marketing/teams/*` dan menu sidebar "Teams" (`/integration/teams`) dihapus; `MarketingTeamUseCase` disusutkan+di-rename jadi `ShopACLUseCase` (isinya cuma `AllowedShops`, satu-satunya method yang masih dipakai — baca `department_shops`). Koleksi Mongo `team_shops`/`marketing_teams`/`team_members` sengaja **TIDAK di-drop** — tak ada lagi kode yang membaca/menulisnya, jadi aman dibiarkan sebagai data mati; drop fisik (opsional) tetap keputusan manusia lewat `mongosh` langsung, bukan bagian deploy kode. UI assign toko yatim sudah dibangun (tab "Kepemilikan Toko" di ICC Management, erp-frontend, 2026-09-01/02) — gerbang tulisnya dibuka lewat `RequireMarketingLeader` dengan penyempitan per departemen di handler (leader/SPV hanya boleh assign ke departemennya sendiri, laporan yatim tetap lintas-divisi), bukan lagi murni `RequireIntegrationAdmin` seperti disebut §Aktor dan wewenang di bawah — bagian itu perlu dibaca sebagai historis, lihat [[API - Integration Service]] untuk kontrak terkini. Rincian status per-langkah: §Migrasi. Menyusul [[ADR - 0030 RBAC Tiga Sumbu dengan Hak Menempel di Posisi]] dan [[ADR - 0043 Peran Sistem Diturunkan dari Jabatan]]; **tidak** menggantikan keduanya.
...
3. **Mencabut Teams begitu saja merusak dua hal.** `team_shops` adalah **satu-satunya sumber Shopee** untuk kolom penanggung jawab toko (3 dari 5 barisnya Shopee), dan `marketing_teams` memasok **saringan divisi** yang juga dipakai halaman Affiliate. `icc_account_mappings` punya `shopee_shop_id`, tetapi **kosong di seluruh baris aktif**.
   > ✅ **Sudah tidak berlaku sejak 2026-09-02** (fakta ini dipertahankan apa adanya karena ia yang membentuk keputusan saat itu). Diukur ke produksi: `shopee_shop_id` terisi pada **8 baris aktif**, mencakup **8 dari 8** toko Shopee yang punya belanja iklan, dan `department_shops` memuat 8 baris channel SHOPEE. Jadi risiko "mencabut `team_shops` menghapus satu-satunya kepemilikan toko Shopee" sudah tertutup. Rinciannya di [[HRIS - Otomasi Skor KPI]] § Yang harus diketahui sebelum angkanya dipercaya.
...
4. ✅ **Contract — DEPLOY + BACKFILL TERVERIFIKASI LIVE (2026-09-02).** Kedua pembaca (**saringan `/divisi`** dan **kolom penanggung jawab** `icc`/blok "Konteks per departemen") sudah di-deploy dan diverifikasi langsung di prod: `docker exec Marketing-Analytics-Service grep -c 'team_shops' /service` → **0** kemunculan (dead-code-eliminated dari biner yang berjalan), `grep -c 'department_shops' /service` → 7, `grep -c 'Departemen '` (label baru menggantikan "Tim ") → 1. Backfill `POST /jobs/sync-profit-attribution/trigger?hari=120` sudah dijalankan dan menutupi **747.786 dari 871.708 dokumen** `mart_profit_attribution` (85,8%) — dikonfirmasi lewat query langsung: `icc.penanggungjawab.nama` berpola `"Departemen <X>"` untuk 747.786 baris. **Sisa 84.836 baris (9,7%) masih membawa label lama** (`"Tim GB-KY"`, `teamid` berupa UUID `marketing_teams` lama) — dikonfirmasi ini PERSIS batas jendela 120 hari, bukan kegagalan backfill: sampel baris tak ter-backfill bertanggal `2026-05-01`, tepat di luar jendela hitung mundur dari tanggal backfill dijalankan. Baris ini **aman dibiarkan** — nama sudah "dibakar" sebagai teks statis di dalam dokumen (bukan live-lookup ke `marketing_teams`), jadi mencabut koleksi lama tidak merusak tampilannya; labelnya sekadar mencerminkan struktur organisasi historis saat order itu terjadi.

   ✅ **Dead code DIHAPUS (2026-09-15)**: `bacaTokoTim`/`bacaNamaTim`/`indeksDivisiDari` (dulu di `services/marketing-analytics/integration_db.go`/`divisi.go`, query langsung ke `team_shops`/`marketing_teams`) sudah dicabut fisik dari repo — lihat §Migrasi langkah 4 susulan 2026-09-15 untuk cakupan lengkap pencabutannya.
   >
   > ⛔ **KOREKSI PENTING (2026-09-02) — `team_shops`/`marketing_teams`/`team_members` TIDAK BISA dicabut, dan "Satu menu hilang (Teams)" di §Consequences TIDAK akan terjadi seperti ditulis.** Diperiksa lebih dalam setelah draf awal bagian ini: ketiga koleksi itu MELAYANI DUA FUNGSI YANG BERBEDA, bukan satu. Kepemilikan toko→divisi (yang dibahas ADR ini) SUDAH sepenuhnya pindah ke `department_shops` — itu benar. Tapi ketiganya JUGA jadi sumber **ACL "siapa boleh lihat toko apa"** yang independen: `transaction_handler.go` (integration-service) memanggil `marketingTeamUseCase.AllowedShops(ctx, employeeID)` di endpoint `GET /transactions/master/shops` (path benar sesudah gateway membuang prefix `/api/integration` — dikoreksi 2026-09-02, bukan `/transactions/orders/master/shops` seperti sempat tertulis di draf awal bagian ini) untuk memfilter daftar toko non-admin (`!IsIntegrationAdmin && scope != "all"`) — `ListTeamIDsByEmployee` baca `team_members`, `ListShopsByTeamIDs` baca `team_shops`. Endpoint ini dikonsumsi lewat hook FE `useFetchShops()` (`features/integration/transactions/hooks/use-fetch-shops.ts`) yang dipakai **puluhan halaman aktif**: Accurate auto-sync + auto-sync-return + receipts + rekonsiliasi, payouts (Shopee/TikTok/Lazada), gross-profit, dashboard transaksi, piutang TikTok. **Belum ada pengganti berbasis `department_shops`/`work_data.department` untuk ACL ini** — itu pekerjaan terpisah yang belum direncanakan. Menghapus koleksi/CRUD Team akan mematikan filter akses di seluruh halaman itu untuk staff non-admin, bukan cuma menghilangkan satu menu yang sudah tak berguna. **Menu `/integration/teams` dan seluruh `marketing_team_repo`/`marketing_team_handler`/route `/marketing/teams/*` HARUS DIPERTAHANKAN** sampai ACL-nya di-redesain — satu-satunya UI yang ada untuk mengelola keanggotaan+toko tim yang dipakai ACL itu. Link "Petakan toko ke penanggung jawab" di `blok-penanggung-jawab.tsx` (Beranda Marketing Analytics) yang tadinya mengarah ke `/integration/teams` sudah diperbaiki ke `/icc/management` (2026-09-02) — itu SATU-SATUNYA bagian dari rencana pencabutan menu yang boleh & sudah dikerjakan.
   >
   > ✅ **SUSULAN (2026-09-15) — Fase Contract SELESAI, menu `/integration/teams` DIHAPUS.** Redesign ACL berbasis `department_shops` (dianalisis di [[ANALISA - Redesign ACL AllowedShops Berbasis Department Shops]], bip-erp PR #1727, merge 2026-09-05) sudah live 10 hari tanpa masalah — syarat "diverifikasi live beberapa waktu" terpenuhi 2026-09-14. Fase Contract-nya sendiri dikerjakan 2026-09-15 (bip-erp PR #1888, erp-frontend PR #1579): CRUD Teams (handler, repo, entity `MarketingTeam`/`TeamMember`/`TeamShop`) dihapus total dari `services/integration`; `cmd/departmentshopsseed` (CLI seed sekali-jalan, tugasnya sudah tuntas) ikut dihapus; `MarketingTeamUseCase` disusutkan+di-rename `ShopACLUseCase` (isinya cuma `AllowedShops`); route `/marketing/teams/*` dan menu sidebar "Teams" dihapus; dead code pembaca lama di marketing-analytics (`bacaTokoTim`/`bacaNamaTim`/`indeksDivisiDari`) dihapus. Koleksi Mongo `team_shops`/`marketing_teams`/`team_members` **dibiarkan** (data mati, aman — tak ada kode yang menyentuhnya lagi); drop fisik opsional, keputusan manusia terpisah.
...
- ✅ **"Satu menu hilang (Teams)" AKHIRNYA TERJADI (2026-09-15), setelah ditunda dua kali.** Fungsi kepemilikan toko→divisi pindah ke kartu team ICC Management sejak Fase Migrate; fungsi ACL "siapa boleh lihat toko apa" (`AllowedShops`) pindah ke `department_shops` sejak 2026-09-05 dan terverifikasi live 10 hari; begitu keduanya stabil, menu Teams (`/integration/teams`) dan seluruh CRUD-nya benar-benar dihapus — bukan cuma tak berfungsi lagi. Lihat koreksi lengkap + susulannya di §Migrasi langkah 4.
...
- ✅ ~~**Pengisian `shopee_shop_id` di `icc_account_mappings`**~~ — **selesai, diukur 2026-09-02**: 8 baris aktif terisi, 8 dari 8 toko Shopee beriklan terpetakan. Yang tersisa bukan lagi kolom kosong melainkan **karyawan tanpa baris mapping sama sekali** (`BIP-0235-03-26`, `Marketplace Advertiser`), dan itu tetap pekerjaan data tim marketing.
