...
>
> ⛔ **Layar web untuk sumbu ketiga sempat dibangun, lalu DICABUT 2026-09-23 atas keputusan pemilik produk** dengan alasan *salah tempat, tidak sesuai rencana*. Riwayatnya: erp-frontend [#1691](https://github.com/bip-itteam-internal/erp-frontend/pull/1691) (`/portal/riwayat-pengajuan`) merged dan tayang di DEV 2026-09-22, dicabut keesokan harinya. Dicatat di sini supaya sesi berikutnya tidak membangunnya lagi sebagai "lubang yang belum diisi": ketiadaannya **disengaja**, bukan terlewat. Bila kelak diputuskan ulang, endpoint `GET /requests/mine` tetap lengkap dan tetap hidup di prod, jadi yang dibutuhkan hanya layarnya. ⚠️ Dua perbaikan yang lahir dari pekerjaan itu sengaja **DIPERTAHANKAN** karena tak berkaitan dengan layarnya: kunci `SUBTYPE_KEY` (`clock_in`/`clock_out` → `checkin`/`checkout`, bug senyap yang membuat subtipe Koreksi tampil mentah di `/hris/requests` dan detail pengajuan) dan pemindahan `rentangTanggal` ke `features/hris/requests/lib.ts`. `git revert` mentah atas PR itu akan menghidupkan ulang keduanya.
