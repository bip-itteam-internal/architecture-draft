...
- **Status**: ✅ **Implemented** — T1 sampai T6 live prod per 2026-09-08 (T3/T5/T6 pukul 11:29 WIB employee-service dan 11:27 WIB frontend-hris, terverifikasi baca-saja dari dalam container), T7 sinkron dok selesai. Catatan tersisa: jalur positif SPV di layar Atur Target belum ditempuh sebagai orang; 8 template prod masih menyimpan target profit lama sampai disimpan ulang atau skrip pembersihan dijalankan; catatan review T1 (indeks unik `incentive_profit_targets`, `kanonLevel` tiga tempat) masih terbuka. Disetujui manajemen 2026-09-07 lewat wawancara `/analisa-kebutuhan`. **T1 (gerbang tulis per level dan divisi)** di bip-erp PR [#1767](https://github.com/bip-itteam-internal/bip-erp/pull/1767) (**merged 2026-09-08 06:59 WIB, live di prod 07:03 WIB**; terverifikasi prod baca-saja: SPV Beauty Hacks menulis dirinya → 403 "tidak boleh menulis target diri sendiri", menulis level supervisor → 403 "level supervisor hanya boleh ditulis Finance, IT, atau Direktur", target yang ada tak berubah; jalur positif SPV menulis icc divisinya belum dicoba di prod karena itu menulis data); **T4 (Master Target per orang, sekaligus Dashboard Insentif, struktur HRIS + i18n) sudah dikodekan** di erp-frontend PR [#1473](https://github.com/bip-itteam-internal/erp-frontend/pull/1473) (**merged dan live di prod 2026-09-07 23:16 WIB**; jendela 403 pensil Supervisor karena dimerge sebelum #1767 tertutup sejak T1 live prod 2026-09-08 07:03 WIB); **T2 (KPI membaca target dari sumber insentif) sudah dikodekan** di bip-erp PR [#1775](https://github.com/bip-itteam-internal/bip-erp/pull/1775) (**merged 2026-09-07 14:42 UTC** ke `main` ce30a716; **live di dev 21:44 WIB dan di prod 22:16 WIB, keduanya terverifikasi 2026-09-07**: di prod `auto_target` Aan Budiyanto Agustus 44 jt (insentif) bukan 88 jt (template), Silvia 165 jt bukan 116,89 jt, Aan September "belum dapat dihitung: target profit belum diisi di Master Target insentif", skor beku Annisa Agustus tak berubah; lihat catatan transisi di §3 dan §7); **T5 backend (PR A, bip-erp [#1783](https://github.com/bip-itteam-internal/bip-erp/pull/1783)) dan T5 frontend + T6 (PR B, erp-frontend [#1484](https://github.com/bip-itteam-internal/erp-frontend/pull/1484)) di kode, dan T3 + penolakan §7 (PR C, bip-erp [#1788](https://github.com/bip-itteam-internal/bip-erp/pull/1788)) **merged berurutan lalu live prod 2026-09-08** (lihat §3 dan §6 "Sebagaimana dikodekan"); karena A dan C dimerge sebelum deploy, keduanya naik dalam satu biner employee-service berdampingan dengan frontend-hris. Daftar task: `Workspace/ANALISA - Target Profit Satu Pintu.md` (papan kerja, bukan dok terbit).
...
**Sebagaimana dikodekan (T5 backend, PR A, bip-erp PR [#1783](https://github.com/bip-itteam-internal/bip-erp/pull/1783), merged bad06408, live prod 2026-09-08 11:29 WIB).** T5 dipecah tiga PR berurutan supaya tak ada jendela HR terjebak: **PR A** backend melonggarkan validasi (`ValidateKPIAutoConfigTargetDariSumber`: aturan umum sama, target terisi ditolak) dan pada masa transisi **membuang** target yang masih dikirim layar lama (`validasiAutoKonfig(..., bersihkan=true)` di `POST /kpi/templates` dan pratinjau), sehingga template profit tetap bisa disimpan dari layar lama; **PR B** frontend (Atur Target menampilkan `auto_target`, berhenti mengirim target, T6 ikut); **PR C** backend menghentikan pembersihan sehingga target profit yang masih dikirim ditolak **400** — itulah penolakan tegas §7, dipasang saat tak ada lagi layar yang mengirimnya. Pembersihan 8 template prod: skrip dry-run + cadangan untuk manusia, opsional (`.task-plans/2026-09-08-bersihkan-target-profit-template.ps1`).
...
**Sebagaimana dikodekan (T6 frontend, PR B erp-frontend [#1484](https://github.com/bip-itteam-internal/erp-frontend/pull/1484), merged 8e31c57b, live prod 2026-09-08 11:27 WIB).** Sisi frontend dicabut lebih dulu: dua halaman, dua entri sidebar, antrean `ANTREAN_KPI` di Ruang Direktur beserta panel dan hook-nya, locale-nya, plus rantai Isi massal yang sudah yatim (`atur-target-marketing-field` → `konfigurasi-otomatis-field` → `target-massal-modal` → `use-target-massal` → `target-massal-rules`) dan hook `use-pratinjau-otomatis` — 37 berkas, sekitar 3.800 baris. Diukur prod 2026-09-08: koleksi `kpi_target_marketing` tidak pernah ada (nol dokumen). Backend (T3) dicabut di **PR C** (bip-erp [#1788](https://github.com/bip-itteam-internal/bip-erp/pull/1788), merged e20ee150, live prod 2026-09-08 11:29 WIB; `GET /kpi/target-marketing` terverifikasi 404): enam berkas target-marketing, `Collections.KPITargetMarketing`, dan `POST /kpi/auto-values/pratinjau` yang kehilangan pemanggil; sekaligus masa transisi §3 ditutup (`bersihkanTargetTransisi = false`, dikunci test) sehingga target profit yang masih dikirim ke `POST /kpi/templates` ditolak 400. Ketiga PR dimerge berurutan pada hari yang sama **sebelum** deploy, jadi A dan C naik dalam satu biner employee-service: deploy backend dan frontend-hris harus berdampingan, dan jendela beberapa menit saat simpan template profit dibalas 400 (layar lama di atas backend baru, atau sebaliknya) diterima sadar.
