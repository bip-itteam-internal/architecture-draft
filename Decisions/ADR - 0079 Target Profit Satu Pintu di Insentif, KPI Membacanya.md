...
**Sebagaimana dikodekan (T6 frontend, PR B erp-frontend [#1484](https://github.com/bip-itteam-internal/erp-frontend/pull/1484), 2026-09-08, belum merge saat ditulis).** Sisi frontend dicabut lebih dulu: dua halaman, dua entri sidebar, antrean `ANTREAN_KPI` di Ruang Direktur beserta panel dan hook-nya, locale-nya, plus rantai Isi massal yang sudah yatim (`atur-target-marketing-field` → `konfigurasi-otomatis-field` → `target-massal-modal` → `use-target-massal` → `target-massal-rules`) dan hook `use-pratinjau-otomatis` — 37 berkas, sekitar 3.800 baris. Diukur prod 2026-09-08: koleksi `kpi_target_marketing` tidak pernah ada (nol dokumen). Backend (T3) dicabut di **PR C** (bip-erp [#1788](https://github.com/bip-itteam-internal/bip-erp/pull/1788), bertumpuk di atas PR A; draft, belum merge saat ditulis): enam berkas target-marketing, `Collections.KPITargetMarketing`, dan `POST /kpi/auto-values/pratinjau` yang kehilangan pemanggil; sekaligus masa transisi §3 ditutup (`bersihkanTargetTransisi = false`, dikunci test) sehingga target profit yang masih dikirim ke `POST /kpi/templates` ditolak 400. **Merge hanya sesudah PR A dan PR B live prod**, supaya tak ada halaman yang memanggil rute yang sudah tiada dan tak ada layar lama yang ditolak 400.
