...
3. **Register QC mendapat daftar kategori tertutup, dan isinya ditetapkan tim QC.** Usulan awal dari penggolongan ulasan: `dugaan_tidak_asli`, `segel_terbuka`, `isi_tidak_sesuai`, `kedaluwarsa`. **Tingkat keparahan ditentukan QC saat validasi**, bukan ditebak pengaju.
   - *Irisan kategori dikerjakan 2026-09-18 (branch bip-erp `feat/employee-komplain-qc-kategori` + erp-frontend `feat/quality-komplain-kategori`, belum push/PR/merge):* keempat kode di atas dipakai apa adanya, `GET /quality/complaints/kategori` digerbang identitas saja, dan kategori **wajib** di POST maupun PUT (PUT karena ia mengganti dokumen utuh lewat `ReplaceOne`). Rinciannya di [[QA - Quality Operasional (CAPA, Incoming, Batch Release)]] dan [[API - Employee Service]].
