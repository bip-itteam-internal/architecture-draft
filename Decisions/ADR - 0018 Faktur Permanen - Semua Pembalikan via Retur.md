✅ **Implemented** (faktur permanen: PR #511; cabut guard HIGH-C: 2026-07-17)
...
- **⚠️ Kelas yang BELUM tertangani**: **pembatalan pasca-kirim** (COD gagal kirim). Karena faktur kini permanen, penjualannya **berdiri terus** dan **tak ada** yang membalik — auto-sync belum membukukan retur untuknya sama sekali (**0 dari 4.753** order sejak 1 Jul). Sistem lama finance menanganinya sebagai *Transaction Type: Return*. Rencana disetujui: `synthesizeCancelReturn` (baris ← `order.Items`, `Solution=0`, `Partial=false`), tanggal ← `cancelled_at` (source `cancelled`), pemicu saat CANCELLED **dan** `shipped_at` ada, cutover `cancelled_at ≥ 20260701`. **Grouping nol perubahan** ([[ADR - 0016 Retur Grouped per Faktur + Tanggal Retur]]).
