✅ **Implemented** (faktur permanen: PR #511; cabut guard HIGH-C: 2026-07-17; cancel pasca-kirim → retur: 2026-07-18)
...
- **✅ Pembatalan pasca-kirim → Retur Penjualan (2026-07-18, go-forward deployed).** COD gagal / batal setelah barang dikirim = barang balik; faktur permanen → retur ini satu-satunya pembalik. `synthesizeCancelReturn` membentuk retur dari `order.Items` (seluruh qty balik, `Solution=0` restock, `Partial=false`, `ReturnSN=""`); `effectiveReturn` dipakai `SyncOrderReturn` + rebuild grup (member cancel dapat barisnya). Tanggal = `cancelled_at` (`resolveReturnTransDate` cabang `cancelled`, **diutamakan** atas TikTok/accepted). Pemicu: `OnOrderCancelledOrReturned` (dulu no-op) saat CANCELLED **dan** `shipped_at` ada; cutover `cancelled_at ≥ 20260701` (via `returnEraDateWIB`). **Grouping nol perubahan** — cancel melebur ke grup faktur+tanggal yang sama, termasuk grup retur biasa ([[ADR - 0016 Retur Grouped per Faktur + Tanggal Retur]]). Sistem lama finance BERHENTI bukukan RETUR COD sejak 1 Jul (konfirmasi user → 0 dobel).
  - **Backfill backlog** (`cmd/cancelreturnbackfill`, DRY-RUN default, rem+verify, `--shop` bertahap): ~4.756 order sejak 1 Jul (~Rp523 jt) → ~1.988 dokumen setelah grouping. **⚠️ Guardian belum**: order ke-flip CANCELLED via order-sync (bukan webhook, `cmd/ordersync` rtsUseCase=nil) tak memicu — perlu sweep by `order_update_date` (lookback pendek, deploy setelah backlog beres).
