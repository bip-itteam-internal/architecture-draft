...
- **Status**: 🟡 **Diusulkan**, disetujui pemilik produk 2026-09-25 lewat percakapan `/analisa-kebutuhan`. **T1 (snapshot + persetujuan Finance) merged** (bip-erp PR #2062, 2026-09-25), **live DEV**, prod belum; kontraknya di [[API - Insentive Service]]. **T3 (komponen Insentif) merged** (PR #2073), live DEV. **T4 (impor dicocokkan snapshot + klaim + tanda terbayar) di kode, belum merged** (branch `feat/insentif-payroll-snapshot`). T2, T5 sampai T8 belum. Daftar task: `Workspace/ANALISA - Insentif Dibayar lewat Slip Gaji.md` (papan kerja, bukan dok terbit).
...
- **Tanggal**: 2026-09-25
- **Diamandemen**: 2026-09-26 oleh [[ADR - 0129 Persetujuan Payroll Run Bertingkat dan Dibayar per Badan Usaha sebelum Terbit]] §5. Snapshot ditandai `terbayar` saat **badan usaha karyawan itu ditandai lunas**, bukan saat run diterbitkan, dan gerbang `tandai-terbayar` mengikuti izin `payroll.bayar`. Sampai ADR itu di kode, perilaku T4 (tanda saat publish) tetap yang berlaku.
...
> ⚠️ **Dirinci saat T4 (2026-09-25).** (a) Satu baris `Insentif` per karyawan per run = jumlah **seluruh** snapshot disetujui dan belum diklaim milik orang itu, persis; tanpa bayar sebagian. (b) **Penjaga bayar ganda ada di insentive**: impor mengklaim snapshot atas nama run draft (`diklaim_run_id`), snapshot yang diklaim tak bisa dibatalkan, dan tanda terbayar hanya diterima dari run pengklaimnya. Rancangan awal menaruh penjaga di payroll saja; `/review` T4 membuktikan itu membiarkan jalur batal → bekukan ulang → bayar dua kali, karena pembatalan terjadi di insentive yang tak tahu klaim. Indeks unik di `payroll_run_line` tetap ada sebagai lapis kedua. (c) Insentive mati: impor ditolak, publish tetap jalan dengan tanda tertunda yang bisa diulang. (d) Rute internal digerbang izin payroll pemanggil yang diteruskan. Kontrak: [[API - Insentive Service]], [[API - Payroll Service]].
