...
- **Status**: ⚠️ **Diterima, sebagian terimplementasi** (2026-08-30). Berdiri di atas pengukuran langsung database produksi pada 2026-08-30 (lihat Context) dan di atas [[Microservices - Marketing Analytics Service]] yang berstatus ⚠️ Implemented dengan catatan, bukan di atas dokumen konsep.
  ...
  - ✅ **§5** (klien menampilkan seluruh sesi) — **web tayang di prod** 2026-08-30 (bip-erp #1537 + erp-frontend #1321/#1325); **mobile selesai di branch** `feat/live-shift-sesi-jamak`, merged ke `dev` lewat my-bharata #128 (2026-08-31). **Pembaruan 2026-09-11**: bagian web dicabut, halaman Sesi Live Host web dihapus ([[ADR - 0091 Pencatatan Sesi Live Host Hanya di MyBharata, Halaman Web Dihapus]]); tampilan N sesi kini hanya di MyBharata.
  - ✅ **§6** (lingkup IT) — sudah berlaku, diterima sadar.
  - ✅ **§2** (jam dinding union) — **selesai di jalur KPI** 2026-09-09 (`JamDindingDetik`, `live_shift_entity.go`) dan **di panel performa host ICC sejak 2026-09-11** (pembaruan di ujung butir ini). Sebelum itu masih utang di panel performa host ICC (`erp-frontend .../icc/lib/ringkas-performa-host-live.ts`) yang tetap menjumlahkan `durasi_efektif_detik` per baris porsi host. Akibatnya kedua permukaan kini menampilkan angka BERBEDA untuk host multi-akun: diukur 2026-09-09, Vani 1,67 jam di KPI vs 3,32 jam di ICC. Diterima sadar sebagai utang berjangka, bukan terlewat. **Pembaruan 2026-09-11**: panel ICC pindah ke `GET /live-shifts/performa` (jam dinding lewat `JamDindingDetik`, rumus jalur KPI individu) lewat bip-erp #1841 dan erp-frontend #1530, di PROD sejak 2026-09-11 (backend 15:33, frontend 15:41 WIB). Sumber jamnya kini fungsi yang sama dengan KPI; bukti di layar (host multi-akun menunjukkan jam yang sama di KPI dan ICC) belum diamati.
...
- **Path di repo**: `bip-erp/services/marketing-analytics/live_shift_penjualan.go` · `bip-erp/services/marketing-analytics/live_shift_pengingat.go` · `erp-frontend/src/features/marketing-analytics/components/live-shift/halaman-live-shift.tsx` · `.../panel-sesi-berjalan.tsx` · `.../dialog-mulai.tsx` (ketiganya dihapus 2026-09-11, [[ADR - 0091 Pencatatan Sesi Live Host Hanya di MyBharata, Halaman Web Dihapus]]) · `erp-frontend/src/features/integration/icc/lib/ringkas-performa-host-live.ts` · `mybharata-app/lib/src/features/live_shift/presentation/bloc/live_shift_bloc.dart` · `.../bloc/live_shift_state.dart` · `.../widgets/kartu_sesi_berjalan.dart`. Tidak ada berkas baru, tidak ada service baru, tidak ada perubahan gateway.
- **Tanggal**: 2026-08-30
...
Artinya staf IT melihat sesi berjalan dan riwayat seluruh host live, termasuk nama dan `employee_id` tiap orang. **Itu diterima sadar** (2026-08-30), dengan tiga alasan: IT memang yang menyelidiki saat angkanya janggal, akses baca itu sudah mereka miliki sebelum keputusan ini lewat gerbang rutenya sendiri, dan kartu performa tim di halaman ICC memang digerbang "marketing leader **atau** anggota IT" sehingga mempersempitnya justru mematikan kartu itu bagi mereka. Hal yang sama berlaku untuk `GET /live-shifts/performa` (halaman Analisis Live dan kartu ICC, di PROD sejak 2026-09-11), yang digerbang `common.RequireMarketingLeader` dengan predikat yang sama; frontend menggerbang bloknya dengan `bolehTim` (leader marketing **atau** anggota IT). **Pembaruan 2026-09-11** ([[ADR - 0091 Pencatatan Sesi Live Host Hanya di MyBharata, Halaman Web Dihapus]]): layar web yang membuka sesi berjalan dan riwayat per sesi seluruh host bagi leader dan IT dihapus. Di layar, visibilitas ini kini tinggal ringkasan per host (Analisis Live, ICC); rute API dan predikatnya tidak berubah.

⚠️ Dicatat di sini karena berkas peran itu sendiri memperingatkan agar predikat ini **tidak** dipakai sebagai penanda SPV marketing, dan peringatan itu benar untuk keputusan **wewenang**. Yang dipakai di sini adalah keputusan **visibilitas baca**, dan hanya untuk itu. Wewenang menindak sesi tetap tidak mengikuti predikat ini (lihat §3): leader tak boleh menghentikan sesi orang lain, dan satu-satunya pengecualian ber-peran adalah **supervisor/admin IT** sebagai jalan darurat, bukan staf IT. **Direvisi sempit 2026-09-11** oleh [[ADR - 0088 Ambil Alih Sesi Live oleh Host Terjadwal dan Tutup Otomatis Akhir Shift]] (🟡 diusulkan): host yang jadwalnya sedang berjalan boleh mengakhiri sesi orang lain, hanya lewat **ambil alih** pada akun yang sama; tombol Jeda dan Akhiri tetap seperti di sini.
...
- [[APP - Web ERP]]
- [[APP - MyBharata]]
- [[ADR - 0091 Pencatatan Sesi Live Host Hanya di MyBharata, Halaman Web Dihapus]] (halaman web Sesi Live Host dihapus; §5 dan §6 bagian web dicabut)
