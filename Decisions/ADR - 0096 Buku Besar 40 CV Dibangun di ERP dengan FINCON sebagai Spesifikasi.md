...
- **Status**: 🟡 **Diusulkan**, arah disetujui lewat `/analisa-kebutuhan` 2026-09-15. Berlaku setelah disetujui SPV FAT dan IT, pemutus yang ditetapkan ADR 0068. ⚠️ **§2 dan §3 (T1) MERGE 2026-09-16** (bip-erp [#1905](https://github.com/bip-itteam-internal/bip-erp/pull/1905), erp-frontend [#1592](https://github.com/bip-itteam-internal/erp-frontend/pull/1592)), **belum di PROD**. **§6 Jalur A (T2) dikodekan** di branch `feat/procurement-pembayaran-cv` kedua repo 2026-09-16, belum merge. §1, §4, §5, §7, dan §8 belum ada di kode. §2 diamandemen 2026-09-15 (pencocokan nama sesudah normalisasi format), lihat catatan di bawahnya.
- **Path di repo**:
  ...
  - `bip-erp/services/procurement/pengajuan_barang*.go`, `pembayaran.go`, `bukti_transfer*.go` (CV tujuan, antrean transfer per pemegang CV). T2 di branch: `cv_penugasan_sumber.go`, `cv_cakupan_permintaan.go`, `pengajuan_barang_pilihan_cv.go`
  - `bip-erp/shared-library/common/cv_penugasan.go` aturan "ditugaskan DAN CV aktif" yang dipakai BERSAMA finance dan procurement (T2, branch); `bip-erp/services/finance/akuntansi_cv_internal.go` rute internal penugasan (T2, branch)
...
>
> **Penerapan T1 (merge 2026-09-16, belum di prod).** Pemindai drift berbentuk laporan kecocokan yang dihitung saat dibuka (`GET /api/finance/akuntansi-cv/kecocokan`), tanpa penjadwal dan tanpa koleksi hasil; sumber yang gagal dibaca tidak melahirkan temuan. Pemetaan toko marketplace **ditunda ke T8** (keputusan user 2026-09-15): master T1 belum punya field toko.
