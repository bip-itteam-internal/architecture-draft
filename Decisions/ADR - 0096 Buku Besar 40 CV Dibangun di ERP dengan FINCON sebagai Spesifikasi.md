...
- **Status**: 🟡 **Diusulkan**, arah disetujui lewat `/analisa-kebutuhan` 2026-09-15. Berlaku setelah disetujui SPV FAT dan IT, pemutus yang ditetapkan ADR 0068. ⚠️ **§2 dan §3 (T1) MERGE 2026-09-16** (bip-erp [#1905](https://github.com/bip-itteam-internal/bip-erp/pull/1905), erp-frontend [#1592](https://github.com/bip-itteam-internal/erp-frontend/pull/1592)), **§6 Jalur A (T2) MERGE 2026-09-16 dan 17** (bip-erp [#1926](https://github.com/bip-itteam-internal/bip-erp/pull/1926) dan perbaikan header gateway [#1929](https://github.com/bip-itteam-internal/bip-erp/pull/1929), erp-frontend [#1614](https://github.com/bip-itteam-internal/erp-frontend/pull/1614), menu dikelompokkan di [#1617](https://github.com/bip-itteam-internal/erp-frontend/pull/1617)). T1 dan T2 **di PROD sejak 2026-09-17** (diverifikasi baca-saja hari itu), tetapi belum dipakai siapa pun: paket "Budget: Transfer Kas CV" belum dipasang dan data penugasan CV masih nol per 2026-09-17; ukur ulang sebelum dipakai memutuskan. §1, §4, §5, §7, dan §8 belum ada di kode. §2 diamandemen 2026-09-15 (pencocokan nama sesudah normalisasi format), lihat catatan di bawahnya.
- **Path di repo**:
  - `bip-erp/services/finance/` modul buku besar CV: master entitas, COA, jurnal, laporan, konsolidasi (baru). T1: `akuntansi_cv_{entitas,penugasan,kecocokan,sumber,handler,jejak,bawaan,semai}.go`
  - `bip-erp/shared-library/common/` katalog izin buku besar CV (baru). T1: `catalog_akuntansi_cv.go`, `akun_cv_accurate.go` (aturan rekening CV 1299 pindah dari procurement)
  - `bip-erp/services/payroll/company_internal.go` rute internal badan usaha untuk master entitas CV (T1)
  - `bip-erp/services/procurement/pengajuan_barang*.go`, `pembayaran.go`, `bukti_transfer*.go` (CV tujuan, antrean transfer per pemegang CV). T2: `cv_penugasan_sumber.go`, `cv_cakupan_permintaan.go`, `pengajuan_barang_pilihan_cv.go`
  - `bip-erp/shared-library/common/cv_penugasan.go` aturan "ditugaskan DAN CV aktif" yang dipakai BERSAMA finance dan procurement (T2); `bip-erp/services/finance/akuntansi_cv_internal.go` rute internal penugasan (T2)
  ...
  - `erp-frontend/src/app/(main)/finance/buku-besar-cv/` (baru) · `erp-frontend/src/features/finance/buku-besar-cv/` (baru). T1: `src/app/(main)/finance/entitas-cv/` dan `src/features/finance/entitas-cv/`
- **Tanggal**: 2026-09-15
...
> **Penerapan T2 Jalur A (merge 2026-09-16 dan 17, di prod sejak 2026-09-17).** Tahap transfer tetap SATU (`pb_ap_transfer`), pelakunya yang bercabang menurut ada tidaknya kode CV pada dokumen; jenjangnya tidak ditambah, mengikuti [[ADR - 0057 Penyetuju Pengajuan Pembelian Ditetapkan per Tahap]] yang menolak mesin alur yang dapat dikonfigurasi. Izinnya **baru**, `budget.cv.transfer`, bukan memakai ulang `budget.ap.bayar`: penugasan hanya membatasi CV mana yang boleh disentuh dan tak dapat menutup jalur PT, jadi satu izin bersama akan membuat pemegang CV ikut boleh memindahkan uang PT. Kode CV ditetapkan SPV Finance pada `pb_finance_setujui_bayar`, satu tahap SEBELUM transfer, sebab nilai itulah yang menentukan siapa yang boleh menindak tahap berikutnya; menyerahkannya kepada pelaku transfer (seperti `sumber_dana` hari ini) berarti wewenang ditentukan orang yang sedang memakainya. Kode CV **boleh kosong** dan kosong berarti dibayar PT. Pembacaan penugasan dari finance **gagal-tertutup**, menyimpang sadar dari ADR 0057 §8 yang memilih gagal-terbuka, karena jalur ini memindahkan uang. Antrean pemegang CV tinggal di layar **CV Saya**, bukan di `/finance/ap` yang digerbang `finance.ap.view` dan memuat utang seluruh perusahaan. Rinciannya di [[Microservices - Procurement Service]].
