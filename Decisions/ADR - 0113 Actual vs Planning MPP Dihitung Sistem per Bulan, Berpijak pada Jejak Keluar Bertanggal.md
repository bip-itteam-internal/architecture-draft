...
- **Status**: 🟡 **Diusulkan**, disetujui user 2026-09-21. **T1 (jejak keluar bertanggal) MERGED 2026-09-21** — bip-erp PR [#1984](https://github.com/bip-itteam-internal/bip-erp/pull/1984) (merge `3b32b03f`) + erp-frontend PR [#1670](https://github.com/bip-itteam-internal/erp-frontend/pull/1670) (merge `4ba0b2c72`); **belum deploy dan belum diverifikasi lewat gateway** (enam gerbangnya di `.task-plans/2026-09-21-jejak-keluar-bertanggal.md`). Lihat keputusan 3 & 4 dan catatan perluasan [[ADR - 0035 HR Menonaktifkan Akun lewat Catatan Resign]]. **T2 (master Divisi) MERGED 2026-09-21** — bip-erp PR [#1989](https://github.com/bip-itteam-internal/bip-erp/pull/1989) (merge `3d3fd08b`) + erp-frontend PR [#1990](https://github.com/bip-itteam-internal/erp-frontend/pull/1990); belum deploy, dan **pemetaan departemen→divisi masih kosong** (pekerjaan HRD, lihat keputusan 6). **T3 (perhitungan Actual per periode) MERGED 2026-09-22 WIB** — bip-erp PR [#1993](https://github.com/bip-itteam-internal/bip-erp/pull/1993) (merge `a6e26941`); **belum deploy, dan endpoint-nya belum SEKALI PUN dipanggil lewat gateway maupun jaringan docker** (ketujuh gerbangnya di `.task-plans/2026-09-21-headcount-periode.md`). Rute `GET /kpi/headcount-periode`, gerbang bulan lampau lintas tenant, dan kelima cacahan diagnostiknya di [[Microservices - Employee Service]] dan [[API - Employee Service]]. ⚠️ Bulan lampau akan **menolak** sampai T1 tuntas di data, dan itu perilaku yang dirancang. **T4–T6 belum ada kodenya** — diperiksa ke `origin/main` 2026-09-21: `mpp_actual` dan `bulan_berlaku` nol berkas di `services/` maupun `shared-library/`. ⚠️ Status deploy bergerak; ukur ulang sebelum mengandalkannya.
...
6. **Divisi jadi master data tersendiri** (`master_divisi`), dengan departemen menunjuk divisinya. Bukan daftar tetap di dalam kode, bukan kolom teks bebas.
   ...
   **T2 — 🔜 berkode, belum merge & belum deploy** (bip-erp `feat/employee-master-divisi`, erp-frontend `feat/hris-master-divisi`, 2026-09-21). Koleksi `master_divisi` ter-scope **per perusahaan** (mengikuti `master_department`), CRUD `/master/divisi` bergerbang `RequireHRISOrITSupervisor` untuk tulis, dan dibaca modul lain lewat `GET /data-type/divisi` — jalur yang sudah dipakai `useDataTypes`, bukan endpoint baca baru. Departemen menunjuknya lewat `divisi_key`; layarnya di Pengaturan → Organisasi, tepat di atas tabel departemen.
   - **Empat divisi di-seed tanpa pemetaan**: memetakan departemen ke divisi pekerjaan HRD, bukan tebakan sistem (keputusan user 2026-09-21). Yang belum dipetakan tampil "Belum berdivisi" beserta hitungannya.
   - ⚠️ **Kata "divisi" sudah dipakai untuk arti lain di kode** — `common.ReachDivision` pada paket izin dan `space.division` di Task Management, keduanya berisi NAMA DEPARTEMEN. Nama `divisi` tetap dipilih karena itu istilah HRD; perbedaannya ditulis di kode dan di [[REF - Kepemilikan Data]]. Keduanya **tidak** disentuh.
   - ⚠️ **Divisi yang dihapus HRD tidak boleh hidup lagi.** Seed hanya mengisi perusahaan yang belum punya satu pun divisi; bentuk pertamanya (upsert per-key tiap boot) membatalkan sendiri tombol Hapus di modul yang sama, tanpa galat.
