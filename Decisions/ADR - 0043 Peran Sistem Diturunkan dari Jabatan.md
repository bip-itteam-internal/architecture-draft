...
**Ini jembatan, dan jembatan harus dibongkar.** Begitu `manufacture` dan `insentive` berkatalog sesuai [[ADR - 0030 RBAC Tiga Sumbu dengan Hak Menempel di Posisi]], hak keduanya pindah ke permission-set yang menempel di posisi, dan tabel ini dicabut.

> ⚠️ **Pembongkaran dimulai 2026-09-05, separuh jalan.** `manufacture` kini BERKATALOG (bip-erp PR [#1723](https://github.com/bip-itteam-internal/bip-erp/pull/1723) + erp-frontend PR [#1463](https://github.com/bip-itteam-internal/erp-frontend/pull/1463), keduanya belum merge). Baris `manufacture` di `peran_dari_jabatan.go` **sengaja masih hidup**: fase satu menggabungkan matriks dan paket, jadi mencabut penurunan peran sekarang akan mencabut akses orang yang paketnya belum dipasang. Pencabutannya satu paket dengan `MANUFACTURE_TIER_FALLBACK=off` dan penghapusan matriks di `erp-frontend/src/features/manufacture/akses.ts` — ketiganya wajib berbarengan. Rincian: [[ADR - 0078 Fase Satu WMS Menggabungkan Matriks dan Paket Hak, Bukan Menggantikannya]]. Baris `insentive` belum tersentuh.
>
> Keberatan asli ADR ini terhadap katalog WMS ("menyalin aturan yang sudah ada ke bentuk kedua… matriks WMS saja 400 baris") **dijawab, bukan diabaikan**: matriksnya PINDAH ke `shared-library/common/catalog_manufacture.go` dan katalog serta paket bawaannya diturunkan dari peta yang sama, sehingga sisi Go tetap satu sumber. Membiarkannya hidup berdampingan dengan katalog akan menghasilkan dua sumber hak yang bisa menyimpang — persis keadaan yang ADR 0030 hendak akhiri.
