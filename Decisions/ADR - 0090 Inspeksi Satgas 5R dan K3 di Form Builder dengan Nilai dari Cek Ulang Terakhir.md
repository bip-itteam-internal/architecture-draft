> **Status**: ⚠️ **Diterima, sebagian terimplementasi.** T1+T2 **merged** 2026-09-11 (bip-erp PR [#1849](https://github.com/bip-itteam-internal/bip-erp/pull/1849)), status deploy belum diukur dan belum diuji lewat gateway. T4 **terimplementasi, PR terbuka** (bip-erp [#1852](https://github.com/bip-itteam-internal/bip-erp/pull/1852) + erp-frontend [#1542](https://github.com/bip-itteam-internal/erp-frontend/pull/1542)), belum merge. T3, T5, T6, dan T7 belum dikerjakan. Rincian di `## Deskripsi`.
...
- **Status**: ⚠️ **Diterima, sebagian terimplementasi.** T1+T2 merged 2026-09-11 (PR [#1849](https://github.com/bip-itteam-internal/bip-erp/pull/1849)), deploy belum diukur. T4 PR bip-erp [#1852](https://github.com/bip-itteam-internal/bip-erp/pull/1852) + erp-frontend [#1542](https://github.com/bip-itteam-internal/erp-frontend/pull/1542), belum merge. Daftar task: `Workspace/ANALISA - Inspeksi Satgas 5R dan K3.md`
- **Path di repo**:
  ...
  - bip-erp, **T4 (PR [#1852](https://github.com/bip-itteam-internal/bip-erp/pull/1852), belum merge)**: `services/form-builder/analytics.go` (`file_fields`)
  - erp-frontend, **T4 (PR [#1542](https://github.com/bip-itteam-internal/erp-frontend/pull/1542), belum merge)**: `src/features/form-builder/types/form.ts` · `src/features/form-builder/lib/{schema,field-types,field-type-icons,tabs,pivot}.ts` · `src/features/form-builder/components/{satgas-settings-fields,question-row,form-editor}.tsx` · `src/features/form-builder/components/kaizen/queue-view.tsx` · `src/features/form-builder/hooks/use-analytics.ts` · `src/features/form-builder/components/analytics/{lampiran-jawaban,question-tab,individual-tab,analytics-view}.tsx` · `src/features/hris/master-data/lib/label-modul.ts`
...
`metric_key` kini ikut dikirim di `/me/forms`, sehingga beranda bisa menyaring form Satgas dengan pola `isKaizen`, sementara `pendingOf` sengaja tidak disaring agar tautan notifikasi tetap jalan. Halaman isinya memakai `EvaluationFillPage`, dengan satu penyesuaian di T6: halaman itu menutup diri saat semua orang sudah dinilai, padahal cek ulang menilai orang yang sudah pernah dinilai.
...
Editor web bisa membuat field `file`, dan analitik menampilkan fotonya lewat rute pratinjau pengelola yang sudah ada (T4, PR bip-erp #1852 + erp-frontend #1542, belum merge). Rute pratinjau saja ternyata **tidak cukup**: backend mengeluarkan field berkas dari `fields` analitik (`analyzableFields`), padahal tab Individu dan Pertanyaan mengulang daftar itu. Karena itu respons analitik kini membawa daftar terpisah **`file_fields`** `[{key,label}]` (absen pada form tanpa berkas), dan tombol "Lihat berkas" mengambil presigned URL saat diklik. MyBharata memotret lalu mengompres sebelum unggah: resolusi dibatasi, ukuran diperiksa di bawah 4 MB, dan tombol kirim terkunci selama unggahan berjalan. Batas 4 MB file-service tidak dinaikkan. Kompresi di web tidak dibangun, karena erp-frontend tak punya jalur pengisian form.
