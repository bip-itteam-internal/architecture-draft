> **Status**: ⚠️ **Diterima, sebagian terimplementasi.** T1+T2 **merged** 2026-09-11 (bip-erp PR [#1849](https://github.com/bip-itteam-internal/bip-erp/pull/1849)), status deploy belum diukur dan belum diuji lewat gateway. T4 **terimplementasi**: bip-erp [#1852](https://github.com/bip-itteam-internal/bip-erp/pull/1852) **merged** 2026-09-11 (deploy belum diukur), erp-frontend [#1542](https://github.com/bip-itteam-internal/erp-frontend/pull/1542) **belum merge**. T3, T5, T6, dan T7 belum dikerjakan. Rincian di `## Deskripsi`.
...
- **Status**: ⚠️ **Diterima, sebagian terimplementasi.** T1+T2 merged 2026-09-11 (PR [#1849](https://github.com/bip-itteam-internal/bip-erp/pull/1849)), deploy belum diukur. T4: bip-erp [#1852](https://github.com/bip-itteam-internal/bip-erp/pull/1852) merged 2026-09-11, erp-frontend [#1542](https://github.com/bip-itteam-internal/erp-frontend/pull/1542) belum merge. Daftar task: `Workspace/ANALISA - Inspeksi Satgas 5R dan K3.md`
- **Path di repo**:
...
Editor web bisa membuat field `file`, dan analitik menampilkan fotonya lewat rute pratinjau pengelola yang sudah ada (T4: bip-erp #1852 merged, erp-frontend #1542 belum merge). Rute pratinjau saja ternyata **tidak cukup**: backend mengeluarkan field berkas dari `fields` analitik (`analyzableFields`), padahal tab Individu dan Pertanyaan mengulang daftar itu. Karena itu respons analitik kini membawa daftar terpisah **`file_fields`** `[{key,label}]` (absen pada form tanpa berkas), dan tombol "Lihat berkas" mengambil presigned URL saat diklik. MyBharata memotret lalu mengompres sebelum unggah: resolusi dibatasi, ukuran diperiksa di bawah 4 MB, dan tombol kirim terkunci selama unggahan berjalan. Batas 4 MB file-service tidak dinaikkan. Kompresi di web tidak dibangun, karena erp-frontend tak punya jalur pengisian form.
