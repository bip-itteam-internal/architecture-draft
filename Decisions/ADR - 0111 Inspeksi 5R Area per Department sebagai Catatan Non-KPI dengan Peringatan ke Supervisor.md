> **Status**: ⚠️ **Implemented sebagian, di BRANCH — belum merged, belum deploy.** T1+T2 (tulis+foto) mendarat di `origin/main` 2026-09-21 (`services/employee/area_inspection.go`, commit `6d5f4634`). **T3+T4+T5 dibuat 2026-09-23 dan di-push ke branch, BELUM merged ke `main`, BELUM prod, mobile BELUM diuji perangkat:** T3 baca/rekap di bip-erp `feat/area-inspection-baca-rekap` (`services/employee/area_inspection_read.go`); T5 rekap web SPV di erp-frontend `feat/area-inspection-rekap-web` (tab "Inspeksi Area"); T4 mode isi petugas di my-bharata `feat/faiz` (`lib/src/features/area_inspection/*`). **Tambahan 2026-09-23 (di branch yang sama, dev sudah di-rebuild dari branch & diverifikasi lewat gateway):** (a) **roster ikut department terpilih di form Satgas** — endpoint internal baru form-builder `/internal/satgas/area-departments` + penyaringan roster di employee-service (refinasi §6, lihat §Diputuskan saat implementasi); (b) **input area lewat WEB** (toggle Rekap/Isi Inspeksi di tab Inspeksi Area untuk petugas), tak lagi hanya mobile; (c) **menu web dipisah** jadi "Satgas 5R & K3" tersendiri berisi tab Individu + Area. T6 (KPI & push SPV) tetap **TBD**. Menambah track KETIGA di payung Satgas/Industrial Relation; **tidak** mengubah track per-PIC (ADR 0090) maupun catatan per-orang (ADR 0085). Rincian di `## Deskripsi`.
...
Petugas melihat **daftar department** dan mengisi kapan saja dalam periode berjalan — **tidak** perlu membuat form per inspeksi (inilah yang menjawab friksi "buat form tiap inspeksi ribet"). Periode **bulanan** (sejajar Satgas per-PIC). **≤15 foto per department per periode**, disimpan di file-service (MinIO) seperti bukti `compliance_note`; batas 4 MB/file file-service tidak dinaikkan.

⚠️ **Roster = department yang DIPILIH di Sasaran Penilaian form Satgas, bukan seluruh master department.** Rumusan awal berbunyi "seluruh master department (dev 2026-09-21: 12 department)" — **direfinasi saat implementasi** (lihat §Diputuskan saat implementasi): departemen di sasaran form Satgas adalah objek RUANGAN track ini (lihat `subjekTanpaDepartemen` di form-builder), jadi pemilihannya harus menentukan ruangan mana yang diinspeksi. Membiarkan roster = semua master department membuat pilihan department di form tak berarti apa-apa, dan ditemukan sebagai bug dari layar ("di pengaturan form beberapa department dipilih, tapi di mobile tampil semua").
...
- **Roster ikut department TERPILIH di form Satgas (refinasi §6).** Rumusan awal "seluruh master department" ternyata membuat pilihan department di Sasaran Penilaian form tak berpengaruh — petugas melihat SEMUA ruangan di mobile & web walau form hanya memilih sebagian (bug dilaporkan dari layar). Pilihan itu dimiliki **form-builder** (`subject.departments` form Satgas), bukan employee-service (pemilik `area_inspection`), jadi perbaikannya **lintas-service**: form-builder mengekspos `GET /internal/satgas/area-departments?company_id=&key=` (`satgas_area_departments.go`, union `subject.departments` seluruh form Satgas TERBIT, digerbang kunci layanan yang sama dengan `/satgas/metrics`), dan `handleAreaInspectionRoster` di employee-service (`area_inspection_read.go`) memanggilnya lalu menyaring `allMasterDepartments` ke department terpilih (`departemenRosterTerpilih`, murni). **Reuse `FORM_BUILDER_MODULE_URL` + `FORM_BUILDER_SERVICE_KEY`** yang sudah ada (sumber KPI `nilai_inspeksi_satgas`) — tanpa env baru. Gagal menghubungi form-builder → **500**, bukan diam-diam menampilkan semua. Form tanpa department terpilih → roster kosong (memang maksudnya). Menyaring roster cukup: rekap hanya menampilkan department yang benar-benar diinspeksi. Diverifikasi dev 2026-09-23 lewat gateway (roster mengembalikan tepat 8 department terpilih, bukan 12). bip-erp `feat/area-inspection-baca-rekap`.
- **Input area juga lewat WEB, bukan hanya mobile (menyimpang dari catatan "mobile HANYA petugas" di bawah).** Tab Inspeksi Area di web kini punya toggle **Rekap / Isi Inspeksi** untuk petugas (`kepatuhan.satgas.input`), sejajar dengan input inspeksi individu; SPV tetap rekap-saja. Alasan: petugas yang sama membuka web, dan memaksa mereka pindah ke MyBharata hanya untuk mengisi area adalah alur yang terputus. Backend tak berubah (endpoint submit/foto T2 sudah dipakai mobile); yang baru komponen web `area-inspection-{tab,input,fill-sheet}.tsx` + hooks `useAreaRoster/useAreaSubmit/useAreaPhotoUpload`, pola submit-dulu-lalu-unggah + kompres foto. Notifikasi push ke SPV tetap ditunda (§5). erp-frontend `feat/area-inspection-rekap-web`.
- **Menu web dipisah jadi "Satgas 5R & K3" tersendiri.** "Satgas 5R & K3" jadi menu sidebar sendiri (segrup Industrial Relation) berisi tab **Inspeksi Individu** (per-PIC, ADR 0090) + **Inspeksi Area** (ADR ini); menu lama "Industrial Relation" jadi induk bersarang berisi **Catatan Kepatuhan** (`/hris/industrial-relation`, ADR 0085) + **Satgas 5R & K3** (`/hris/satgas`). Satu form tetap satu form; hanya penyajian menu yang dipisah. erp-frontend `feat/area-inspection-rekap-web`.
