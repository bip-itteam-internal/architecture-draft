> **Status**: ⚠️ **Diterima, merged, belum prod**: disetujui 2026-09-10; T1 sampai T6 dikodekan, terverifikasi di DEV, dan merged 2026-09-11; prod belum. Revisi layar kandidat (2026-09-11) **merged 2026-09-11**. DISC tanpa kunci skor (2026-09-15, penyimpangan 9) **merged 2026-09-15** dan terverifikasi di DEV; prod belum diukur. Persen hasil akhir pilihan ganda (2026-09-17, butir 11) **PR terbuka**; percobaan pertamanya merged lalu sedang dicabut (11b). Rincian dan addendumnya di `## Deskripsi`.

## Untuk Manajemen

HRD bisa mengelola sendiri **daftar tipe tes** (CFIT, DISC, Kraepelin, dan tipe lain di kemudian hari), **soal-soalnya**, dan **paket tes** per posisi, tanpa menunggu developer. Kandidat menerima satu tautan, mengerjakan seluruh paket sekali duduk secara berurutan, dan sistem yang menghitung skornya. Tujuannya menghapus koreksi manual yang selama ini dikerjakan HR dengan tangan di atas lembar kertas. **Pembaruan 2026-09-15:** untuk DISC tujuan itu tidak tercapai. Kunci skor DISC tidak dimuat ke sistem, jadi sistem menampilkan lembar jawaban kandidat dan HR tetap menilai DISC sendiri, kini dari layar, bukan dari kertas. CFIT tetap dinilai sistem.
...
**Revisi 2026-09-11 (disetujui dari prototipe, belum dirilis):** kandidat tidak lagi melihat nama tes, hanya Tahap 1, 2, 3 beserta penjelasan singkat yang ditulis HRD. Setiap bagian diawali soal contoh yang tidak dinilai. Pada soal pilihan ganda kandidat **boleh kembali** ke soal sebelumnya selama waktu bagian itu masih berjalan, yang **menyimpang sadar** dari permintaan awal "CFIT tidak bisa kembali". Layar pembuka juga menampilkan lowongan serta nomor telepon dan email kandidat yang sebagian disamarkan. Konsekuensinya bagi HRD: tiap bagian CFIT perlu minimal satu soal contoh sebelum paket berisi CFIT bisa dikirim.
...
- **Status**: ⚠️ **Diterima, merged, belum prod**. Rencana disetujui 2026-09-10. Katalog, bank soal, paket, mesin pilihan ganda dan DISC, soal bergambar, dan impor Excel (T1 sampai T6) dikodekan dan merged 2026-09-11 (bip-erp #1837 `40323aae` dan #1838 `b73862af`, erp-frontend #1527 `42eccaf8`, career-bharata #10 `15240242`), terverifikasi di DEV lewat gateway dan layar; prod belum. Cara kerjanya di [[HRIS - Bank Soal dan Paket Psikotes]]. **Tahap nol (Decision 8)** dikerjakan 2026-09-10 di PR bip-erp #1828 · erp-frontend #1522 · career-bharata #9 (merged 2026-09-10: bip-erp `6681c997`, erp-frontend `5fd30185`, career-bharata `77bfafc5`; naik di dev dan terverifikasi lewat gateway hari itu; naik di prod 2026-09-11 oleh manusia dan lolos gerbang, bersama perbaikan cache gateway T0a bip-erp #1832); bentuk yang dipilih: sesi tak-tuntas menulis Hasil Tes **tanpa skor**, laporan tampil di kartu Psikotest, dan Kirim Ulang tersedia dari layar yang sama. Rincian: [[HRIS - Psikotes Kraepelin]]. Artefak kerja: `Workspace/ANALISA - Psikotes Multi-Jenis.md`
- **Penyimpangan sadar saat dikodekan** (2026-09-11): (1) `pilihan_ganda` menerima **1 atau 2 kunci** per soal lewat `jumlah_jawaban` subtes, bukan "satu kunci benar", karena Klasifikasi CFIT asli memilih dua; (2) `most_least` bawaannya tanpa timer tetapi HR boleh memasangnya; (3) tak ada berkas dispatcher `psikotes_jenis_jawaban.go`: sesi berjenis `paket` bercabang per bagian menurut `jenis_jawaban`, endpoint lama `columns` dan `finish` menolak sesi paket, dan sesi Kraepelin lama tetap di jalurnya; (4) gateway memakai satu rute umum untuk kandidat, dikeluarkan dari limiter umum 60 per menit dan dibatasi per token.
- **Addendum revisi layar kandidat** (2026-09-11, disetujui user dari prototipe; branch bip-erp `feat/recruitment-psikotes-revisi-kandidat`, career-bharata `feat/psikotes-revisi-kandidat`, erp-frontend `feat/psikotes-soal-contoh`; **merged 2026-09-11** sebagai bip-erp #1850, erp-frontend #1541, career-bharata #11). Penyimpangan sadar tambahan:
	- (5) **Pilihan ganda boleh kembali** ke soal mana pun selama waktu subtesnya berjalan, menyimpang dari permintaan manajemen "CFIT tidak bisa kembali". Batasnya tetap deadline server; subtes yang sudah ditutup tak bisa dibuka lagi. DISC boleh ke grup sebelumnya; Kraepelin tetap maju saja.
	- (6) **Kandidat tak diberi tahu nama tes.** DTO kandidat tanpa nama tipe, nama subtes, deskripsi, dan nama paket; teks tipe yang sampai hanya field baru `penjelasan_kandidat`, sementara `deskripsi` menjadi catatan internal HR.
	- (7) **Soal contoh wajib** sebelum soal asli: item bank bertanda contoh, minimal satu per subtes pilihan ganda agar tipe siap (kode `contoh_kosong`). **Kunci dan penjelasan soal contoh sengaja dikirim ke peramban** lewat endpoint contoh tersendiri; soal asli tetap tanpa kunci, sehingga konsekuensi "bentuk respons pengelola dan kandidat terpisah dan diuji" di bawah tetap dipegang. Contoh DISC dan Kraepelin bawaan portal, bukan dari bank.
	- (8) Tanda ragu-ragu hanya di peramban, tak memengaruhi nilai. Telepon dan email kandidat di layar sapaan disamarkan server.
	- Cara kerjanya di [[HRIS - Bank Soal dan Paket Psikotes]].
- **Addendum DISC tanpa kunci skor** (2026-09-15, keputusan user; **merged 2026-09-15** sebagai bip-erp #1900 `1683de56` dan erp-frontend #1589 `bb3a6117`, terverifikasi di DEV lewat gateway dan layar hari itu). Penyimpangan sadar dari Decision 2 ("tally empat dimensi"), Decision 7 ("menggambar grafik"), dan Consequences ("CFIT dan DISC berhenti dikoreksi dengan tangan"):
	- (9) **DISC dinilai HR dari lembar jawaban bila bank tanpa kunci.** Huruf dimensi `most_least` boleh kosong untuk SELURUH grup (kosong bukan N; grup setengah berhuruf ditolak `400`). Laporan HR bagian DISC membawa `skor_otomatis` (semua huruf snapshot terisi) dan `lembar[]` (pernyataan + indeks pilihan P dan K, tanpa huruf). Layar menyembunyikan tally dan grafik saat `skor_otomatis` false dan menampilkan lembar jawaban meniru kertas. Tally tetap dikirim demi bentuk JSON lama.
	- (9a) Lembar terbaca pemegang izin lihat rekrutmen tanpa izin terpisah (keputusan user). Tanpa huruf bukan jaminan kerahasiaan: pada bank BERKUNCI, hitungan + lembar dari cukup banyak laporan bisa dipakai menghitung balik huruf kunci. Hari ini laten; bila kunci kelak dimuat dan kerahasiaannya penting, perlu keputusan baru.
	- (9b) Bank Soal memilih tanpa kunci lewat satu sakelar "Pakai kunci skor" per grup; impor menerima kolom huruf opsional sebagai satu kesatuan. Ditunda: penanda bank campuran di Bank Soal dan penanda berkas tanpa kolom huruf di pratinjau impor.
	- Cara kerjanya di [[HRIS - Bank Soal dan Paket Psikotes]].
- **Addendum bank CFIT diisi soal internal** (2026-09-16, keputusan user: "hrd gak ada, jadi saya ingin km yg membuat soalnya sekalian"). Menjawab risiko legalitas CFIT dengan cara tidak memakai bahan berlisensi sama sekali, bukan dengan menunggu izin:
	- (10) **Butir CFIT berlisensi tidak didigitalkan.** Bank tipe berkode `cfit` diisi 50 soal penalaran gambar **orisinal** berformat CFIT (seri 13, klasifikasi 14 dengan dua jawaban, matriks 13, kondisi 10) plus 2 soal contoh per subtes, seluruhnya dibangkitkan skrip berbenih tetap di `erp/.task-plans/bahan-psikotes/cfit/`. Kuncinya dibuktikan pemeriksa yang ditulis TERPISAH dari pembangkitnya, ditambah kontrol negatif yang menuntut soal cacat buatan ditolak.
	- (10a) **Nama tipe diganti** jadi "Penalaran Gambar (internal)" dengan deskripsi yang menyatakan bukan CFIT berlisensi, skor jumlah benar mentah, tanpa norma dan tanpa IQ, supaya angkanya tak terbaca sebagai hasil CFIT resmi. Kode tipe tetap `cfit` (dipakai seed dan paket bawaan). Sesi yang terbit sebelum penggantian tetap membawa nama lama lewat snapshot.
	- (10b) **Tanpa perubahan kode repo**: soal dimuat lewat API yang sudah ada (unggah gambar lalu `POST /psikotes/item` per soal, karena impor Excel teks saja). DEV dimuat agent 2026-09-16 dan alur kandidat diuji penuh lewat API publik. **PROD dimuat hari yang sama** (58 soal + 24 grup DISC; tipe dan paket Staff siap) — ⚠️ oleh **agent atas perintah eksplisit user**, menyimpang dari keputusan tim 2026-08-14 bahwa eksekusi prod dilakukan manusia. Penyimpangannya disebut lebih dulu, ditegaskan user, dan pratinjau tanpa-tulis dijalankan sebagai gerbang; tak ada sesi kandidat uji yang dibuat di prod.
	- (10c) **Legalitas DISC tetap terbuka**: lembar DISC berasal dari bahan HRD, jadi butir B1 hanya tertutup untuk CFIT.
	- Cara kerjanya di [[HRIS - Bank Soal dan Paket Psikotes]].
- **Addendum persen hasil akhir pilihan ganda** (2026-09-17, keputusan user; ⚠️ **PR terbuka, belum merged**: erp-frontend [#1618](https://github.com/bip-itteam-internal/erp-frontend/pull/1618), bip-erp [#1933](https://github.com/bip-itteam-internal/bip-erp/pull/1933)). **Mempertegas Decision 7, bukan menyimpanginya:**
	- (11) **Persen ditambahkan di baris total laporan yang sudah ada**, hanya untuk bagian **tuntas**: `38 benar dari 50 soal · 76%`. Tetap skor mentah, hanya dinyatakan juga sebagai persen. Tanpa kategori, tanpa norma, tanpa verdict; `result` tetap `Pending`. Persen **per bagian**, bukan per paket, jadi larangan satu angka tunggal untuk paket campuran tak dilanggar.
	- (11a) **Kategori (Tinggi/Sedang/Rendah) untuk Penalaran Gambar ditolak.** Ditawarkan saat `/start-task` dan tidak dipilih: selain melanggar Decision 2 dan 7, ambang apa pun tak punya dasar psikometrik karena soalnya dibangkitkan internal, belum pernah diujikan, tanpa sampel norma dan tanpa psikolog. Preseden empat kategori Kraepelin tak berlaku di sini: ambang Kraepelin mewarisi konvensi instrumennya dan kodenya sendiri melarang mengubahnya tanpa validasi psikolog.
	- (11b) **Percobaan pertama dicabut.** Field `skor_otomatis` di `GET /candidates/:id/test-results` + kotak "Skor sistem" di kartu Hasil Tes (bip-erp [#1931](https://github.com/bip-itteam-internal/bip-erp/pull/1931), erp-frontend [#1616](https://github.com/bip-itteam-internal/erp-frontend/pull/1616)) merged 2026-09-17 sebelum `/review` jalan, lalu dicabut. Review menemukan kotaknya menduplikasi total yang sudah tercetak di kartu yang sama, namanya bertabrakan dengan `disc.skor_otomatis` (bool, penyimpangan 9) di berkas Go yang sama, dan skornya dipasangkan lewat `round_id` yang sama untuk semua kandidat. Rinciannya di [[HRIS - Bank Soal dan Paket Psikotes]] §Penilaian dan Hasil Tes.
- **Path di repo**: `bip-erp/services/recruitment/models_psikotes_katalog.go` · `psikotes_katalog_validasi.go` · `psikotes_katalog_handlers.go` · `psikotes_impor.go` · `psikotes_paket_sesi.go` · `psikotes_paket_skor.go` · `psikotes_paket_selesai.go` · `psikotes_paket_public.go` · `psikotes_paket_terbit.go` · `psikotes_paket_laporan.go` · `seed_psikotes.go` · `psikotes_selesai.go` · `psikotes_public_handlers.go` · `psikotes_hr_handlers.go` · `bip-erp/api-gateway/psikotes_publik.go` · `erp-frontend/src/features/hris/recruitment/psikotes-katalog/*` · `erp-frontend/src/features/hris/recruitment/psikotes/*` · `erp-frontend/src/app/(main)/pengaturan/rekrutmen/page.tsx` · `career-bharata/src/components/psikotes/*` · `career-bharata/src/lib/psikotes/paket.ts` · `career-bharata/src/lib/recruitment-api.ts` (halaman kandidat yang dipakai prod) · `erp-frontend/src/features/psikotes/*` (versi kedua, kini menolak sesi paket; nasibnya menunggu butir 4 Belum Diputuskan) · *(revisi)* `bip-erp/services/recruitment/psikotes_identitas.go` · `psikotes_katalog_tulis.go` · `seed_psikotes_penjelasan.go` · `career-bharata/src/components/psikotes/contoh-pilihan-ganda.tsx` · `contoh-disc.tsx` · `contoh-kraepelin.tsx` · `grid-opsi.tsx` · `grid-disc.tsx` · `career-bharata/src/lib/psikotes/use-pintasan-opsi.ts` · *(tanpa kunci)* `bip-erp/services/recruitment/psikotes_paket_skor.go` (`skorDISC`, `discBerkunci`, `pilihanDISC`) · `psikotes_paket_laporan.go` (`lembarDISC`) · `psikotes_katalog_validasi.go` (`validasiItem`) · `erp-frontend/src/features/hris/recruitment/psikotes/components/lembar-jawaban-disc.tsx` · `erp-frontend/src/features/hris/recruitment/psikotes-katalog/lib/dimensi.ts`
- **Tanggal**: 2026-09-10
...
> ⚠️ **Penyimpangan (9), 2026-09-15:** untuk `most_least`, "tally empat dimensi" hanya bermakna bila seluruh grup snapshot berhuruf (`skor_otomatis`). Grup tanpa kunci disimpan tanpa huruf, dan HR menilai dari lembar jawaban. Rincian di addendum `## Deskripsi`.
...
**7. Hasil tetap berhenti di `Pending`.** Sistem menghitung skor mentah dan menggambar grafik; ia tidak pernah menetapkan lolos atau gugur, tidak mengonversi ke IQ, dan tidak menyimpulkan profil. Tidak ada tabel norma yang disimpan. ⚠️ *(Penyimpangan 9)* Grafik DISC bersyarat: hanya tampil bila `skor_otomatis`; tanpa kunci, laporan menampilkan lembar jawaban.
...
- CFIT dan DISC berhenti dikoreksi dengan tangan. Ini kebutuhan yang sebenarnya, dan tahap 1 sampai 2 sudah menjawabnya. ⚠️ **Untuk DISC tidak tercapai** (penyimpangan 9, 2026-09-15): tanpa kunci skor di sistem, DISC tetap dikoreksi HR, kini dari lembar jawaban di layar.
...
- Gateway meng-hardcode kelima path psikotes. Endpoint publik berbentuk baru menuntut menyunting dan menaikkan gateway juga. **Dijawab 2026-09-11 (bip-erp #1838)**: kelima rute diganti satu rute umum berpenyaring ([[CORE - API Master Gateway]]), jadi endpoint kandidat berikutnya tak menuntut menyunting gateway; gateway tetap wajib naik sekali untuk perubahan ini.
...
- Selama **dua implementasi mesin tes kandidat** masih hidup berdampingan (erp-frontend dan career-bharata), tiap bentuk jawaban baru berisiko dibangun dua kali. **Di prod pilihannya sudah terjadi de facto**: `ERP_FRONTEND_URL` di container recruitment prod berisi `https://career.bharatainternasional.com` (dibaca 2026-09-10), jadi kandidat prod mengerjakan di career portal. Harganya muncul 2026-09-12: env itu juga dibaca tautan karyawan, sehingga tautan penilaian onboarding dan feedback interview 404. Sejak bip-erp #1870 (merged dan live prod 2026-09-14) tautan psikotes membaca `CAREER_PORTAL_URL` dan `ERP_FRONTEND_URL` prod kembali berisi web ERP ([[Microservices - Recruitment Service]] increment **Tautan Email per Penerima**). Keputusan resminya belum ada, dan biayanya berlipat justru karena ADR ini.

**Risiko yang diterima sadar**

- **Legalitas item CFIT dan DISC belum dijawab.** Ini risiko hukum, bukan teknis, dan berada di luar kendali sistem. Bank soal tidak boleh diisi sebelum HRD menjawabnya. ⚠️ *(internal)* **Untuk CFIT risiko ini ditutup 2026-09-16 dengan cara lain**: banknya diisi soal penalaran gambar **orisinal** buatan internal, jadi tak ada butir berlisensi yang didigitalkan (addendum butir 10). Untuk **DISC tetap terbuka**, karena lembarnya berasal dari bahan HRD.
...
3. **Aturan "halaman tersembunyi = tes berakhir".** Versi career portal mengakhiri sesi **seketika** begitu halaman tersembunyi (pindah tab, peramban diminimalkan, layar ponsel terkunci, telepon masuk) atau ditutup. Disengaja, karena Kraepelin mengukur ketahanan di bawah tekanan waktu tak terputus. Harganya: satu dari dua sesi terputus di prod cocok dengan pola ini (berakhir di kolom 3, sekitar satu menit sesudah mulai), dan kandidat tidak diberi tahu aturan ini selain anjuran di email untuk memakai komputer. Perlu diputuskan apakah dipertahankan apa adanya, diberi jeda toleransi, atau diberitahukan terang di layar petunjuk. **Dikodekan 2026-09-11 (bip-erp #1837, career-bharata #10)**: di sesi berpaket aturan ini hanya berlaku di bagian Kraepelin dan ditegakkan server; CFIT dijaga deadline server, DISC dijaga sapuan 30 menit. Layar petunjuk bagian Kraepelin menyebut aturannya ("Tes akan berakhir bila halaman ditinggalkan").
4. **Mesin kandidat resmi.** Di prod pilihannya sudah terjadi de facto: `ERP_FRONTEND_URL` di container recruitment prod berisi `https://career.bharatainternasional.com`, jadi kandidat mengerjakan di career portal. Sejak bip-erp #1870 (merged dan live prod 2026-09-14) arah itu dibaca dari `CAREER_PORTAL_URL`, bukan lagi dari env yang juga dipakai tautan karyawan. Versi erp-frontend masih ada dan perilakunya berbeda (tanpa aturan butir 3). Keputusan tertulis mana yang dipertahankan belum ada, dan bentuk jawaban baru harus dibangun di tempat kandidat benar-benar berada. **Asumsi tertulis 2026-09-11**: mesin CFIT dan DISC dibangun hanya di career portal; halaman versi erp-frontend menolak sesi paket dengan arahan membuka tautan lewat Portal Karir.
