...
| Tidak ada; permintaan pemilik 2026-09-15 | `/kantor-agent` (kit 1.20.0): denah kantor isometrik sesi Claude Code yang **hidup**, robot per sesi dan per subagent berpindah ke area sesuai tool yang sedang dipakai, diperbarui tiap 2 detik. Penulis Python membaca ekor transkrip dan menulis `.task-plans/kantor-agent-data.js`; halaman `file://` memuatnya ulang, tanpa server | §5 menolak dashboard karena papan yang menuntut layanan ikut mati bersama layanannya. Di sini tak ada layanan: penulis yang mati tampil sebagai banner data basi, bukan robot beku, jadi §5 tetap dipatuhi. Sumbernya transkrip, bukan berkas sesi §5, karena status `aktif` terukur tak bisa dipercaya (72 dari 99 berkas aktif, 7 yang benar-benar hidup) |
| Tidak ada; permintaan pemilik 2026-09-21 | peran eksekutor per **lapisan** tim IT (`loop-fe`, `loop-be`, `loop-mobile`, `loop-devops`) dipilih dari domain kali repo, dan dua brief di repo berbeda boleh berjalan **paralel** bila keduanya bertanda `Paralel: aman` | peran kit sebelumnya hanya per domain, sementara [[SCRUM SPECS]] menyebut Development Team diisi BE/FE/DevOps; `loop-devops` dibuat **tanpa shell di `tools`** supaya aturan "deploy PROD dijalankan manusia" jadi gerbang sesuai §3, bukan kalimat. §1 tidak tersentuh: agent tetap berhenti di PR |
| §3 dan §4 sendiri | **dua lubang ditutup** (kit 1.25.0, 2026-09-21): nol gerbang tidak lagi dihitung lulus, dan test milik kit ikut digerbang lewat `pre-push` vault | §3 menuntut gerbang punya exit code, tetapi `gerbang.ps1` mengembalikan lolos untuk repo yang jenisnya tak dikenali karena daftar gerbang kosong memenuhi "tak ada yang gagal" — diukur menyentuh **empat** repo. §4 menuntut test kit digerbang, tetapi `init` sengaja melewati vault saat memasang `core.hooksPath` sehingga tak ada yang menjalankannya. Keduanya membuat §1 belum punya mekanisme yang bisa menolak, jadi wewenang merge tetap ditahan sampai gerbang ini punya jam terbang |
| Otonomi sampai deploy | berhenti di PR | §1 |

Rincian bentuk dan keputusan kecilnya ada di `architecture-draft/.agent-kit/docs/2026-09-06-ai-engineering-loop-design.md` (Kantor Agent: `2026-09-15-kantor-agent-design.md` di folder yang sama); cara kerjanya untuk pembaca umum di [[IT - Gerbang Repo dan Papan Sesi Agent]]. Bukti pertama loop bekerja utuh: bip-erp PR [#1739](https://github.com/bip-itteam-internal/bip-erp/pull/1739), dari brief sampai PR lewat eksekutor, gerbang, dan judge, tanpa satu pun kode ditulis manusia.
