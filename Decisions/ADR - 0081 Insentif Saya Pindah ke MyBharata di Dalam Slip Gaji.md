...
Kartu Insentif tampil hanya bagi **anggota skema insentif profit**. Keanggotaan dijawab satu endpoint baru di insentive-service, `GET /profit-dashboard/saya/keanggotaan` (nama final ditetapkan di `/plan`), yang membaca hierarki dan pemetaan toko yang sudah dipakai penyusun baris dashboard untuk menjawab: apakah pemanggil adalah entitas di level `icc`, `leader`, atau `supervisor`. **Tanpa angka**, tanpa menghitung dashboard. Aplikasi menampilkan kartu Insentif untuk **dua belas bulan terakhir** bila pemanggil anggota; bulan tanpa baris menampilkan kalimat "belum ada baris insentif atas nama Anda di periode ini" di rincian, persis seperti web.
