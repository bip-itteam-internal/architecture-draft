...
- **Status**: ✅ **LIVE dan teruji end-to-end di dev DAN prod** (prod deploy manual + uji 2026-08-09). Merged 2026-08-08 (BE [#1099](https://github.com/bip-itteam-internal/bip-erp/pull/1099), FE [erp-frontend#866](https://github.com/bip-itteam-internal/erp-frontend/pull/866)). Bukti verifikasinya di [[Microservices - Form Builder Service]]. Desain kerja: `erp/docs/superpowers/specs/2026-08-08-tipe-form-per-departemen-design.md`
...
- **Aturan ini ikut menentukan apa yang TERLIHAT, bukan cuma apa yang boleh dibuat** (sejak 2026-08-10). Tab tipe di daftar form disaring dari jawaban yang sama, atas permintaan pemilik produk. Karena mencabut tipe tidak menyentuh form yang sudah ada, form yang terlanjur bertipe terlarang kehilangan tabnya sendiri dan hanya terbaca lewat tab Semua — konsekuensi yang diterima sadar, dan alasan lengkapnya di [[APP - Web ERP]]. Siapa pun yang melarang sebuah tipe perlu tahu bahwa ia sekaligus menyembunyikan jalan pintas menuju form lama bertipe itu.
...
- **Kepemilikan bersama menuntut tipe yang boleh di SEMUA owner** (🔜 branch `feat/formbuilder-owner-jamak`, terverifikasi lokal, **belum merge/deploy**). Sejak form bisa dimiliki beberapa departemen supervisi sekaligus ([[Microservices - Form Builder Service]]), aturan tipe dievaluasi **per owner**: form HR+GA hanya sah bertipe yang boleh dibuat keduanya (irisan). Aturan-per-departemen di ADR ini tak berubah bentuknya, cuma dijalankan berulang atas tiap owner — `createForm` menolak di owner pertama yang melarang, dan FE menawarkan irisannya. Ini memperluas sumbu `owner_department` dari satu nilai jadi **daftar**, kejamakan yang belum ada saat ADR ini ditulis; konsekuensi privasinya (analisa form bersama terbaca staf tiap pemilik) dicatat di dok service.
