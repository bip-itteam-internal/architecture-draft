...
- **Riwayat lama tetap milik perusahaan lama.** Entri presensi distempel `company_id` saat dibuat, jadi laporan bulan-bulan sebelum kepindahan tidak ikut pindah. Benar secara pembukuan, tapi berarti laporan seorang karyawan terbelah di dua tenant dan tak ada satu layar pun yang menyatukannya.
	...
	> ⚠️ **DIKECUALIKAN untuk perpindahan ELT → BIP (keputusan 2026-08-19).** Ke-242 entri presensi CV Elit (30 Jul sampai 19 Agu 2026) **ikut dipindahkan** ke BIP, membalik aturan di atas. Alasannya justru kalimat terakhir aturan itu sendiri: tak ada satu layar pun yang menyatukan riwayat yang terbelah, dan CV Elit diserap seluruhnya sehingga tak tersisa siapa pun di sana yang membutuhkan pembukuan kehadirannya.
	>
	> **Ini pengecualian, bukan pembatalan.** Aturan bawaannya tetap "riwayat tinggal di perusahaan lama", dan tetap itu yang berlaku untuk perpindahan orang-per-orang antara dua perusahaan yang sama-sama masih beroperasi. Yang membedakan kasus ini: seluruh badan usaha asal berhenti dipakai. Perpindahan berikutnya wajib menjawab pertanyaan itu sendiri, bukan menyalin keputusan ini.
	>
	> Konsekuensi yang diterima sadar: pembukuan kehadiran ELT untuk Juli sampai Agustus 2026 menjadi **kosong**, dan itu tak bisa dipulihkan tanpa backup. Karena itu migrasinya wajib didahului `mongodump` (dijalankan otomatis oleh `.task-plans/jalankan-migrasi-elt.ps1`).
