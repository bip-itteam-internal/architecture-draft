...
- **Status**: ⚠️ **Sebagian terimplementasi** (2026-09-10). **§3 (prasyarat `live_shifts.channel`) SUDAH DIKODEKAN**, di branch dan belum merged: bip-erp `feat/live-shift-channel` (field `Channel`, `channelNormal` dua sisi, `channelPerToko`, channel masuk predikat `jodohkanSesiDenganPorsi`, derivasi dari `department_shops` dengan 400 toko tak terpetakan vs 503 master data tak terbaca, resep `dropIndex` di `index.go`; `go build` OK dan `go test ./services/marketing-analytics` exit 0 sama dengan baseline, kontrol negatif dijalankan) dan erp-frontend `fix/live-shift-channel-pesan` (dialog Mulai menampilkan pesan server untuk 400/503 lewat daftar-izin, key `sebagianGagalSebab` di dua locale; `tsc` exit 0, `dialog-mulai.test.tsx` 27/27). ⚠️ **Belum diverifikasi lewat gateway maupun di layar** — keduanya menunggu deploy dev, jadi "bisa dipakai" belum terbukti. **§1, §2, §4 sampai §11 masih 🟡 konsep** dan sebagian prasyaratnya di luar kendali kita (otorisasi streamer Shopee). Diukur di prod 2026-09-10: `mart_live_sessions` berisi **6.363 dokumen, seluruhnya `channel: "TIKTOK"`**, jadi belum ada satu pun sesi non-TikTok yang perlu digabungkan. Sementara itu `department_shops` **sudah** memuat 9 baris SHOPEE dan 3 baris LAZADA di samping 49 TIKTOK, jadi dimensi channel sudah nyata di master data dan hanya sesi live-nya yang belum ada. Prasyarat yang **bisa** dikerjakan sekarang tanpa menunggu Shopee: field `channel` pada `live_shifts`, yang per 2026-09-10 **tidak dimiliki satu pun dari 29 dokumen** shift September 2026. Keputusan ini lahir dari penelusuran metrik `add_to_cart_rate` Host Live 2026-09-10 (lihat §Context).
