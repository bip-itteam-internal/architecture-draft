...
- **Inspeksi Satgas 5R & K3** (`metric_key: inspeksi_satgas`, gerbang izin `kepatuhan.satgas.input` di jalur isi, `metric_key` di `/me/forms`, rute `/me/satgas`) ⚠️ **merged 2026-09-11** lewat PR [#1849](https://github.com/bip-itteam-internal/bip-erp/pull/1849), **deploy belum diukur, belum diuji lewat gateway**. `file_fields` pada respons analytics **merged** 2026-09-11 lewat bip-erp PR [#1852](https://github.com/bip-itteam-internal/bip-erp/pull/1852), deploy belum diukur. Keputusan: [[ADR - 0090 Inspeksi Satgas 5R dan K3 di Form Builder dengan Nilai dari Cek Ulang Terakhir]].
...
**Respons analytics**: `total_responses`, `unique_respondents`, `audience_size`, `sample_size`, `truncated`, `response_rate` (opsional), `daily[{date,count}]`, `fields[{key,label,type,answered,skipped,options[{option,count}],average,min,max,sample_text[]}]`. Saat `truncated=true`, `response_rate` sengaja tidak dikirim karena tak bisa dihitung jujur dari sebagian data. **`file_fields[{key,label}]`** (⚠️ merged 2026-09-11 lewat bip-erp [#1852](https://github.com/bip-itteam-internal/bip-erp/pull/1852), deploy belum diukur): pertanyaan bertipe `file`, urut sesuai form dan mengikuti `?period=` seperti `fields`; **absen** pada form tanpa berkas. Terpisah dari `fields` karena berkas tak diringkas (`fields` memang tak pernah memuatnya); klien memakainya untuk tahu jawaban mana yang dibuka lewat `GET /forms/:id/uploads/:uploadId/preview`.
