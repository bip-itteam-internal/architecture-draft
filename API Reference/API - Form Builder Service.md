...
- **Inspeksi Satgas 5R & K3** (`metric_key: inspeksi_satgas`, gerbang izin `kepatuhan.satgas.input` di jalur isi, `metric_key` di `/me/forms`, rute `/me/satgas`) ⚠️ **merged 2026-09-11** lewat PR [#1849](https://github.com/bip-itteam-internal/bip-erp/pull/1849); biner form-builder dev memuatnya (diukur 2026-09-11), dan **gerbang Satgas diuji lewat gateway dev 2026-09-14** (tanpa paket: kiriman `403` berpesan paket dan `/me/satgas` `allowed:false`; berpaket: `allowed:true`, unggahan dan kiriman `201`), prod belum. `file_fields` pada respons analytics merged 2026-09-11 lewat bip-erp PR [#1852](https://github.com/bip-itteam-internal/bip-erp/pull/1852) dan ✅ **terverifikasi lewat gateway dev** hari yang sama; prod belum. Keputusan: [[ADR - 0090 Inspeksi Satgas 5R dan K3 di Form Builder dengan Nilai dari Cek Ulang Terakhir]].
- **Nilai Satgas per orang dan cek ulang lintas bulan** (T3 ADR 0090: `GET /internal/satgas/metrics`, `period_key` opsional pada kirim jawaban dan unggah, kartu cek ulang di `/me/satgas`, `subject_rule` di analytics) ⚠️ **merged 2026-09-14** lewat bip-erp PR [#1866](https://github.com/bip-itteam-internal/bip-erp/pull/1866) (`feat/satgas-nilai-kpi`) dan [#1867](https://github.com/bip-itteam-internal/bip-erp/pull/1867) (`feat/satgas-cek-ulang`); dev memuatnya dan diuji end-to-end 2026-09-14, **prod belum deploy**. Butir bertanda *(T3)* di dok ini berlaku di `main` dan dev, belum di prod.
...
| POST | `/me/forms/:id/uploads` | Unggah satu lampiran (**multipart**, field **`file`**; `field_key` tak dibaca, dan berkas baru dicocokkan ke pertanyaannya saat jawaban dikirim; diperiksa ke `uploads.go` 2026-09-12. ⚠️ *(T3)* Field **`period_key`** opsional ikut dibaca dengan aturan yang sama dengan kirim jawaban, dan wajib sama dengan `period_key` jawabannya: lampiran periode lain ditolak `400` saat jawaban dikirim). `201` membalas `{"data": {upload_id, file_name, size}}`. `400` bila field `file` tak ada. Cap 4 MB milik file-service; `413 {"error": "Ukuran berkas melebihi batas 4 MB"}` bila lewat. `409` juga bila form tak `published`. **`409` bila putaran form berulang belum dibuka** (sejak T3: seluruh tolakan `period_key`, `400` maupun `409`), diperiksa SEBELUM berkasnya naik supaya tak meninggalkan objek yatim. ⚠️ *(Satgas, merged #1849)* `403` berpesan pada form Satgas bila tak memegang izin, juga diperiksa sebelum berkas naik |
...
> ⚠️ **Merged 2026-09-11** lewat bip-erp PR [#1849](https://github.com/bip-itteam-internal/bip-erp/pull/1849), **live dev dan diuji lewat gateway dev 2026-09-14** (tanpa paket kiriman `403` berpesan paket, berpaket `201`), prod belum. Konsumen MyBharata (menu Satgas, halaman isi, unggah foto) ada di my-bharata PR [#144](https://github.com/bip-itteam-internal/my-bharata/pull/144), merged ke `dev` 2026-09-14; uji perangkat belum. Keputusan: [[ADR - 0090 Inspeksi Satgas 5R dan K3 di Form Builder dengan Nilai dari Cek Ulang Terakhir]].
>
> ⚠️ **T3 (nilai per orang, `GET /internal/satgas/metrics`, cek ulang lintas bulan) merged 2026-09-14**: bip-erp PR [#1866](https://github.com/bip-itteam-internal/bip-erp/pull/1866) (`feat/satgas-nilai-kpi`) memuat aturan nilai, tab analitik, endpoint internal, dan sumber KPI `nilai_inspeksi_satgas` di employee-service; PR [#1867](https://github.com/bip-itteam-internal/bip-erp/pull/1867) (`feat/satgas-cek-ulang`) bertumpu padanya dan menambah cek ulang lintas bulan. Biner form-builder dan employee-service dev memuatnya dan diuji end-to-end di dev 2026-09-14; **prod belum deploy**, dan jendela 1-5 Oktober 2026 (kartu cek ulang lintas bulan, kiriman tercatat ke bulan temuan, `409` bagi yang sudah selesai atau tanpa temuan, dedup cek ulang lintas bulan) belum teruji. Butir bertanda *(T3)* di bawah berlaku di `main` dan dev, belum di prod; cara kerja, alasan, dan hasil uji di [[Microservices - Form Builder Service]].
...
- Hanya form Satgas **terbit** yang pemanggilnya masuk audience. Kartu putaran berjalan hanya dikirim bila putarannya **buka**; sebelum T3 form di luar jendela periode tidak dikirim sama sekali. ⚠️ *(T3)* Sampai batas cek ulang, form yang sama bisa membawa kartu kedua untuk **bulan lalu** (di bawah), diletakkan sebelum kartu putaran berjalannya dan tetap dikirim di rentang mati awal bulan.
- ⚠️ *(T3)* Kartu putaran berjalan ikut membawa `fields` (susunan pertanyaan periode itu) dan `owner_department`, supaya halaman isi tak bergantung pada `/me/forms` yang menyembunyikan form di rentang mati.
...
**Cek ulang lintas bulan** ⚠️ *(T3)*. `POST /me/forms/:id/responses` (field JSON `period_key`) dan `POST /me/forms/:id/uploads` (field multipart `period_key`) memakai satu keputusan, `periodeKiriman`. Batas cek ulang temuan bulan M = tanggal **5 bulan M+1 pukul 23:59:59** (zona waktu service; compose memasang `TZ: Asia/Jakarta`).
