...
| Method | Path | Fungsi |
|---|---|---|
| GET | `/me/capability` | `{can_manage, can_write, can_manage_type_rules, departments[], form_types_by_department{}}` — apa yang boleh dilakukan pemanggil di Form Builder. `can_write` menyusul bersama katalog izin (PR [#1138](https://github.com/bip-itteam-internal/bip-erp/pull/1138), **belum merged**): pemegang paket "Lihat" membuka layarnya (`can_manage`) tapi tak boleh membuat atau menyunting form (`can_write`). Selama fase satu nilainya selalu sama dengan `can_manage` |
