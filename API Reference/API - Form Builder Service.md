...
**Tipe form** (`form_type`): `survey` · `evaluation` · `checklist` · `kaizen`. Kiriman kosong jadi `survey`; nilai tak dikenal ditolak `400` (bukan diam-diam diubah). **Terikat dua arah dengan `subject`**: `evaluation` wajib punya sasaran, dan yang punya sasaran wajib `evaluation`. ⚠️ `request` ("Pengajuan") **dihapus** dan kini masuk kelompok "tak dikenal" — 🟡 masih di branch `feat/form-builder-hapus-tipe-request`, **belum merge**, jadi `main` sementara masih menerimanya. Alasan, backfill dokumen lama, dan urutan deploy: [[Microservices - Form Builder Service]].
