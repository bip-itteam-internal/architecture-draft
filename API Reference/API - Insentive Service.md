## Deskripsi

*Endpoint **insentive-service**. Sejak 2026-07-30 skemanya **profit-based untuk seluruh jabatan** (SK 010 & 011/DIR/SK6/VII/2026); rute `/profit*` di bawah adalah yang berlaku. Rute warisan skema KPI-multiplier masih terdaftar tetapi sebagian **menolak** — lihat catatannya. Gateway: `/api/insentive/*`. Semua butuh gateway key; `/health` bebas. Grounded ke `services/insentive/main.go` + `func.go`.*

- **Implementasi**: [[Microservices - Insentive Service]] · **Status**: ⚠️ Implemented (ada catatan)
- **Indeks**: [[API - Index]]

## Profit-based (skema berlaku)
| Method | Path | Fungsi |
|---|---|---|
| GET | `/profit-dashboard` | Dashboard insentif per periode & level (`periode=YYYY-MM`, `level=icc\|leader\|supervisor`, `refresh=1` tarik ulang beban non-gaji). Tiap baris membawa `toko_tanpa_penjualan` — berapa dari `shop_ids` yang terpetakan ke orang itu tetapi nol order di periode berjalan, dipakai layar untuk menulis "9 dari 15 toko" alih-alih menyembunyikan yang belum berjualan (2026-08-26, PR #1455), dan `retur_gagal_booking` — order retur yang belum/gagal masuk pembukuan sehingga tak ikut rasio (2026-08-27, PR #1462). Jawabannya juga membawa, sekali per respons, `tarif_tiers` (tangga tarif SK siap tampil), `batas_retur_persen` (7), dan `batas_pencapaian_bebas_retur` (100) — dikirim backend, TIDAK disalin ke layar, karena tabel tarif pernah salah satu poin persen di produksi (2026-08-27, PR #1463). `gugur` hanya bisa `true` bila `target` > 0 (bip-erp [#1620](https://github.com/bip-itteam-internal/bip-erp/pull/1620), merged 2026-09-01): baris bertarget kosong tak digugurkan retur. Parameter `mode=bergeser` mengaktifkan jendela KPI (lihat catatan di bawah) |
| GET | `/profit-dashboard/saya` | ✅ live (ditambahkan ke dok ini 2026-09-07; rutenya sudah ada lebih dulu). Baris insentif **milik pemanggil sendiri** untuk satu `periode=YYYY-MM`, dipilih server dari `BIP-Employee-ID` yang gateway isi ulang dari JWT; menelusuri ketiga level, jadi satu orang bisa punya dua baris (ICC dan Leader). **Tanpa gerbang role maupun menu terbatas**, sengaja: ICC, host live, affiliate, dan CRM harus bisa melihat angkanya sendiri ([[ADR - 0039 Menu Terbatas Default Terbuka sampai Di-assign]]); `mode` dan `refresh` tidak diterima. Balasan `data{periode, rows[], rumus, tarif_tiers, batas_retur_persen, batas_pencapaian_bebas_retur}`; tiap baris membawa `biaya_gaji` orang itu sendiri (beban perusahaan, bukan gaji bersih). Konsumen: web `/finance/incentive/my-incentive` (sejak erp-frontend [#1448](https://github.com/bip-itteam-internal/erp-frontend/pull/1448), merged 2026-09-04; sebelumnya layar itu menarik `/profit-dashboard` tiga kali lalu menyaring di browser, sehingga seluruh dashboard berikut `biaya_gaji` semua orang sampai ke browser tiap pemakai); MyBharata menyusul ([[ADR - 0081 Insentif Saya Pindah ke MyBharata di Dalam Slip Gaji]]). Cache gateway 3 menit berkunci per pemakai |
| GET | `/profit-dashboard/saya/keanggotaan` | ✅ **live prod** 2026-09-08 (PR [#1778](https://github.com/bip-itteam-internal/bip-erp/pull/1778), [[ADR - 0081 Insentif Saya Pindah ke MyBharata di Dalam Slip Gaji]] M1; `services/insentive/keanggotaan_saya.go`). Terverifikasi prod 2026-09-08 dari dalam container untuk 7 orang: Account Specialist → `["icc"]`, leader pemegang toko → `["icc","leader"]`, SPV Beauty Hacks → `["supervisor"]`, Host Live dan SPV Quality/Manufaktur/Finance → `[]`, tanpa header → 401; tiap jawaban sama dengan himpunan `level` baris `/profit-dashboard/saya?periode=2026-09` orang yang sama. Level tempat pemanggil (`BIP-Employee-ID`) menjadi entitas skema profit, **tanpa angka dan tanpa menghitung dashboard**: balasan `data{levels: ["icc","leader","supervisor"] ⊆}` dalam urutan tetap, `[]` (bukan `null`) bila bukan anggota. Aturannya **tidak disalin**: `LevelKeanggotaan` memanggil `SusunEntitas` yang sama dengan dashboard untuk tiga level. Yang berbeda hanya sumber pemetaan toko: integration `GET /icc/mappings/me` dengan meneruskan identitas pemanggil (tanpa gerbang role), bukan ringkasan profit; "punya toko" = mapping aktif ber-`tiktok_shop_id`/`shopee_shop_id`/`lazada_shop_id`, meniru `petakanPemilikToko` integration, jadi mapping hanya-advertiser bukan ICC di keduanya. **Tanpa gerbang role maupun menu terbatas** (alasan sama dengan `/saya`). Status: **401** tanpa identitas; **503** hierarki karyawan tak terbaca; **502** integration gagal atau bentuk `data` tak dikenal (`null`/array sah, objek bukan); **500** `incentive_org` tak terbaca. ⚠️ Konsumen wajib membedakan 503/502 dari `[]`: yang pertama "server tak bisa menjawab", yang kedua "bukan anggota" |
| GET/POST | `/profit/org` | Struktur tim (ICC ↔ Leader ↔ Supervisor) |
| PATCH | `/profit/org/:id/tutup` | Tutup masa berlaku satu baris struktur |
| GET/POST | `/profit/targets` | Target profit per entitas/periode (ubah saat berjalan wajib beralasan ≥10 karakter). ✅ T1 [[ADR - 0079 Target Profit Satu Pintu di Insentif, KPI Membacanya]] live prod 2026-09-08 07:03 WIB (PR [#1767](https://github.com/bip-itteam-internal/bip-erp/pull/1767); terverifikasi prod: SPV menulis dirinya → 403 `tidak boleh menulis target diri sendiri`, SPV menulis level supervisor → 403 `level supervisor hanya boleh ditulis Finance, IT, atau Direktur`, target yang ada tak berubah; jalur positif SPV menulis icc divisinya belum dicoba di prod karena itu menulis data): gerbang per level dan divisi (`gerbangTulisTarget`); **400** level tak dikenal (disimpan kanonik); **403** target diri sendiri, beda divisi, atau level supervisor oleh bukan finance/IT/Direktur; **503** hierarki karyawan tak terbaca. Ubah saat periode berjalan tanpa alasan ≥10 karakter, atau target yang hasilnya sudah disetujui → **409**. Penulisan sukses **membuang cache dashboard periode itu** (`hapusCacheDashboardPeriode`, bip-erp [#1748](https://github.com/bip-itteam-internal/bip-erp/pull/1748), live prod 2026-09-11) supaya dashboard langsung memuat angka baru. Unik per `{level, entity_id, periode}` (indeks `kunci_alami_unik`, bip-erp [#1822](https://github.com/bip-itteam-internal/bip-erp/pull/1822), live prod 2026-09-10) |
| GET/POST | `/profit/opex` | Biaya operasional manual — kini **cadangan** (gaji dari payroll, non-gaji dari Accurate). Penulisan membuang cache dashboard periode itu (#1748); unik per `{level, entity_id, periode}` (#1822) |
| POST | `/profit/opex/distribusi` | Bagi satu angka divisi ke tiap entitas (pro-rata, metode sisa-terbesar). Membuang cache dashboard periode itu (#1748) |

> Sumber angka: komponen profit & beban non-gaji dari [[API - Integration Service]] (`/profit/incentive/summary`, `/profit/incentive/opex`), beban karyawan dari payroll-service `GET /employer-cost`.

> ⚠️ **`mode=bergeser` — dua angka profit yang sah berbeda.** Bawaan (tanpa parameter) memakai aturan insentif: order yang uangnya cair setelah tanggal 25 bulan berikutnya HANGUS. `mode=bergeser` memasukkannya ke periode berikutnya, dan itulah yang diminta adaptor KPI `insentif_profit` — insentif membayar periode yang sudah tertutup, KPI menilai kerja yang hasilnya baru cair terlambat. Terukur prod 2026-08-27: selisihnya +0,151% (Juli) dan +0,093% (Agustus). **Mode ikut kunci cache**; dua mode berbagi kunci akan membuat dashboard insentif menampilkan angka bergeser. Pre-warm menghangatkan KEDUA mode — tanpa itu panggilan KPI pertama memicu komputasi dingin ~2 menit lalu habis waktu di gateway. PR #1503, merged 2026-08-28.

> 🗑️ **Rute `/profit/internal-affiliates` DIHAPUS** (2026-08-27, PR #1474). Daftar putih affiliate kosong di produksi sepanjang umurnya sehingga penyaringan tak pernah menyala; premisnya juga tak ada di SK, dan komisi affiliate sudah terpotong di uang cair. Lihat [[Microservices - Insentive Service]].

## Engine lama & Master KPI (warisan)
| Method | Path | Fungsi |
|---|---|---|
| POST | `/calculate` | ⚠️ **Menolak seluruh role** — skema KPI-multiplier & ICC per-video dicabut; balasannya menyebut SK pencabutnya |
| POST | `/calculate/auto` | ⚠️ Sama; cron harian sudah dihapus |
| GET/POST/PUT/DELETE | `/master-kpi` · `/master-kpi/:id` | CRUD master KPI (bobot total 100) |

## Mappings & Audit
| Method | Path | Fungsi |
|---|---|---|
| GET/POST/PATCH/DELETE | `/mappings` · `/mappings/:id` | Mapping performa employee (PATCH wajib reason) |
| GET | `/audit-logs` | Audit log (filter target_collection/target_id) |

## Results & Workflow
| Method | Path | Fungsi |
|---|---|---|
| GET | `/results` · `/results/summary` | List hasil insentif (paginated) |
| GET | `/results/export` | Export Excel (single/pivot/history) |
| GET | `/results/me` · `/results/team` · `/results/:id` | Hasil sendiri / tim / detail |
| POST | `/results/bulk-approve` · `/results/bulk-unapprove` | Approve/unapprove massal (DRAFT↔APPROVED) |
| POST | `/results/:id/approve` · `/results/:id/unapprove` | Approve/unapprove per-hasil |
| PATCH | `/results/:id/override` · `/results/:id/daily-override` | Override (wajib reason; trigger recalc) |
| DELETE | `/results/:id` | Hapus hasil (ditolak bila APPROVED) |

## Stats · Config · Proxy
| Method | Path | Fungsi |
|---|---|---|
| GET | `/stats` | Dashboard total + tren bulanan |
| GET/PUT | `/configs/ppn` | Persentase PPN (shopee/tiktok/meta) |
| GET | `/accurate/summary` · `/accurate/income` · `/accurate/invoices` | Proxy data Accurate |
| GET | `/integration/shopee/item-performance` | Ambil GMS Shopee dari integration |
| GET | `/health` | Health check |

> ⚠️ **Cron harian sudah DIHAPUS** bersama skema KPI-multiplier (`cron_worker.go`, 2026-07-30). Tidak ada lagi job terjadwal di service ini — lihat [[IT - Background Jobs & Schedulers]].

## Dokumen Terkait
- [[Microservices - Insentive Service]] · [[HRIS - Key Performance Index]] · [[Finance - Incentive]] · [[API - Index]]
- [[ADR - 0033 Beban Operasional Insentif dari Proyek Accurate]] · [[API - Integration Service]]
