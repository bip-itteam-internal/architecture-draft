...
| GET/POST | `/profit/targets` | Target profit per entitas/periode (ubah saat berjalan wajib beralasan ≥10 karakter). ⚠️ [[ADR - 0079 Target Profit Satu Pintu di Insentif, KPI Membacanya]] T1 di branch `feat/insentive-gerbang-target`, belum merge: gerbang per level dan divisi (`gerbangTulisTarget`); **400** level tak dikenal (disimpan kanonik); **403** target diri sendiri, beda divisi, atau level supervisor oleh bukan finance/IT/Direktur; **503** hierarki karyawan tak terbaca |
