...
- **`GET /peminjaman/jadwal` sengaja DI LUAR `ga.view`**, dan itu bukan kelalaian: yang dipancarkannya hanya ruang dan jam, fakta yang sudah terbuka untuk semua karyawan lewat `GET /peminjaman/slot`. Jadwal **beserta nama dan nomor WA pemohon** tetap milik `GET /peminjaman` yang digerbang `ga.view` ([[ADR - 0094 Booking Ruang lewat MyBharata, Penyetuju Ditunjuk HR, Satu Sumber Ruang Kantor]] §7). Menggerbangnya ke `ga.view` akan mematikan layar Jadwal Ruang di MyBharata bagi pembaca utamanya, dan gagalnya senyap karena blok itu sekadar tak muncul.
...
| GET | `/peminjaman/jadwal[?ruang_id=&dari=&sampai=]` | Jadwal ruang yang sudah TERPAKAI (`DISETUJUI` saja) di satu rentang, **tanpa identitas pemesan**, `mulai_at` **menaik** (kebalikan `GET /peminjaman`) | identitas, **sengaja tanpa `ga.view`** | `{data: [JadwalTerpakai], dari, sampai, batas: 200, terpotong: bool}` |
