...
Grounded ke `services/inventory/peminjaman_*.go`. Keputusan desain: [[ADR - 0094 Booking Ruang lewat MyBharata, Penyetuju Ditunjuk HR, Satu Sumber Ruang Kantor]]. Konsep domain: [[GA - Asset Loan & Room Booking]]. Path di bawah adalah path service; gateway membuang prefix `/api/inventory`. Konsumen: [[APP - Web ERP]], [[APP - MyBharata]] (irisan 2), dan [[API - Attendance Service]] yang membaca `/peminjaman/saya` dan `/peminjaman/perlu-aksi` untuk daftar pengajuan terpadu (irisan 2; keduanya merged 2026-09-15, belum di PROD). MyBharata juga membaca `/peminjaman/jadwal` untuk layar Jadwal Ruang yang dibuka dari ikon kalender di form booking (2026-09-22; belum di PROD maupun store).
...
- **`GET /peminjaman/jadwal` sengaja DI LUAR `ga.view`**, dan itu bukan kelalaian: yang dipancarkannya cuma ruang, jam, **nama pemohon, dan divisinya**. Nama dan divisi dibuka sadar lewat [[ADR - 0121 Jadwal Ruang Publik Menyertakan Nama dan Divisi Pemohon]] (2026-09-22, mengamandemen [[ADR - 0094 Booking Ruang lewat MyBharata, Penyetuju Ditunjuk HR, Satu Sumber Ruang Kantor]] §7), supaya orang yang melihat ruang terpakai tahu harus menghubungi siapa. **Keperluan, nomor WA, nomor booking, posisi, dan riwayat keputusan TETAP** milik `GET /peminjaman` yang digerbang `ga.view`. Menggerbang rute ini ke `ga.view` akan mematikan layar Jadwal Ruang di MyBharata bagi pembaca utamanya, dan gagalnya senyap karena blok itu sekadar tak muncul.
- ⚠️ **`GET /peminjaman/slot` TIDAK ikut digeser** dan tetap sepenuhnya anonim (`TestSlotRuangTanpaIdentitas`). Keduanya tak lagi sekelas sejak ADR 0121, jadi jangan menyimpulkan isi salah satu dari yang lain.
...
| GET | `/peminjaman/jadwal[?ruang_id=&dari=&sampai=]` | Jadwal ruang yang sudah TERPAKAI (`DISETUJUI` saja) di satu rentang, `mulai_at` **menaik** (kebalikan `GET /peminjaman`). `JadwalTerpakai` = `{sumber_id, sumber_nama, mulai_at, selesai_at, pemohon_nama?, divisi?}`, dipatok `TestJadwalTerpakaiHanyaNamaDanDivisi`. ⚠️ Dua field terakhir `omitempty`: booking lama yang tak punya identitas **menghilangkan key-nya**, bukan mengirim string kosong, supaya klien tak menggambar pemisah menggantung di baris "Nama · Divisi" | identitas, **sengaja tanpa `ga.view`** | `{data: [JadwalTerpakai], dari, sampai, batas: 200, terpotong: bool}` |
...
- **Riwayat terstruktur** (irisan 2; bip-erp [#1899](https://github.com/bip-itteam-internal/bip-erp/pull/1899), merged 2026-09-15, belum di PROD): `alasan` hanya berisi ketikan manusia (tolak wajib, batal opsional). `jadwal_sebelumnya{sumber_id, sumber_nama, mulai_at, selesai_at}` diisi pada `ubah` yang memindah ruang atau jam, berisi jadwal **sebelum** perubahan. `nomor_terkait` diisi pada `tolak_otomatis` (booking yang mendapat slotnya) dan `batal_setujui_bentrok` (booking disetujui yang bertumpuk); `batal_setujui_tak_terperiksa` tanpa keduanya. Kunci yang kosong tak dikirim. Klien merakit kalimatnya lewat i18n; riwayat lama tetap membawa kalimat Bahasa Indonesia di `alasan`.
- **`JadwalTerpakai`**: `sumber_id`, `sumber_nama`, `mulai_at`, `selesai_at`. **Itu saja.** ⛔ Kontrak privasi, sekelas `SlotRuang`: tak memuat pemohon, keperluan, nomor WA, maupun nomor bookingnya. Daftar fieldnya dipatok `TestJadwalTerpakaiTanpaIdentitas`, dan badan respons mentahnya ikut diperiksa supaya kebocoran jadi test merah, bukan senyap.
...
- ⚠️ **Zona waktu respons tidak seragam, dan itu perlu diketahui klien.** `mulai_at`/`selesai_at` milik `Peminjaman` dan `JadwalTerpakai` berasal dari Mongo, yang didekode driver sebagai **UTC**, jadi terbit berakhiran `Z`. Yang dirakit di Go (`SlotRuang`, serta `dari`/`sampai`) berzona **WIB**. Klien wajib mengurainya sebagai **instan** lalu mengonversi sendiri ke WIB; memotong string tanggalnya (`mulai_at.substring(0,10)`) mengelompokkan booking dini hari ke hari yang salah tanpa satu pun galat.
...
- **`GET /peminjaman/jadwal`**: rentangnya **selalu terisi dan selalu berbatas**, tidak seperti `GET /peminjaman` yang boleh tanpa saringan. Bawaan **hari ini sampai 14 hari ke depan** (WIB), maksimum **31 hari**; tanggal bukan `YYYY-MM-DD`, akhir mendahului awal, atau rentang melebihi 31 hari → **400**. Jendela yang benar-benar dipakai dikembalikan sebagai `dari`/`sampai` (`sampai` inklusif) supaya layar bisa menyebutkannya dan klien tak perlu menyalin aturan jendelanya.
  ⛔ Alasan pembatasan itu bukan kerapian: `ListPeminjaman` mengurutkan `mulai_at` **menurun** lalu memotong di 200, jadi permintaan tanpa saringan pada perusahaan yang ramai membuang justru jadwal **paling dekat**, dan layar berbunyi "besok kosong" untuk hari yang penuh. `terpotong` menutup sisanya.
...
- **ubah (PATCH)**: badan pointer `{ruang_id, mulai_at, selesai_at, keperluan, no_wa, keterangan}`, field yang tak disebut tak berubah. Bukan pemohon **403** · perusahaan booking beda dari perusahaan pemanggil sekarang **403** · `DISETUJUI` yang sudah mulai **409** · status final **409**.
  - Ruang atau jam berubah: hasil gabungan divalidasi utuh seperti pengajuan baru (400/404/422/409; booking itu sendiri tak dihitung bentrok). Booking `DISETUJUI` kembali `DIAJUKAN` (slot lama dilepas), riwayat `ubah` membawa `jadwal_sebelumnya` (sejak irisan 2; sebelumnya kalimat `Jadwal sebelumnya: ...` di `alasan`), dan penyetuju dikabari lagi.
