...
| GET | `/today` · `/schedule` · `/sync/company-work-schedules` · `/data-type/:dt` | Jadwal harian/bulanan/sync/enum. ⚠️ **`/today` tanpa `?view` mengembalikan catatan kehadiran TERBARU milik pemanggil** (`FindOne` urut `date` menurun, `services/attendance/main.go`), **bukan** catatan hari ini, dan 404 `No attendance record found` bila belum pernah ada satu pun; pemakai yang menjanjikan "hari ini" wajib membandingkan `date` sendiri (contoh: beranda portal, [[ADR - 0105 Beranda Portal Menumpuk Zona Personal di Atas Ruang Kerja Posisi]]). `?view=resolve` = jadwal kemarin, hari ini, besok; `?view=team` = status kehadiran hari ini untuk departemen pemanggil beserta grupnya (employee-service memekarkan lewat `include_group=true`). `/today` membalas 403 bila header employee atau department kosong | open |
...
| Method | Path | Fungsi | Auth |
|---|---|---|---|
| POST/GET/PATCH | `/schedule-exchange/create` · `/consent` · `/partners` · `/view` · `/review` · `/cancel` | **Tukar Jadwal Kerja** (collection `schedule_exchange_request`). **create**: hanya karyawan shift → non-shift **403**; `type:"shift"`+`partner_employee_id` = swap antar-rekan (simpan `exchange_work_time`+`partner_work_time`), `type:"day"` = geser hari. `exchange_date` **H+1 untuk shift, H+3 untuk day** (per tipe sejak [#2033](https://github.com/bip-itteam-internal/bip-erp/pull/2033), 2026-09-23; pesan galatnya menyebut angka harinya). **consent**: rekan setuju/tolak swap (langkah 1; tolak→Canceled). **partners** `?date=`: kandidat rekan swap (role sama, terjadwal, **jam berbeda**; tiap butir membawa `work_time` pada tanggal itu). **view** `?as=reviewer/reviewed/partner` (`partner` = inbox consent rekan, 2026-06-29), `?filter=ongoing/past`, `?id=`, `?search=`. **review**: atasan→HRD, diblokir sampai consent; **`Disetujui` ditolak 400 bila tanggalnya sudah lewat** (`Ditolak` tetap boleh). **cancel**: hanya saat Waiting | header |
...
Ringkasan/detail **lintas jenis** (Izin/Cuti/Sakit/Dinas/Koreksi/Tukar, plus **Booking** Ruang GA bila diminta) dari satu endpoint: FE reuse satu kartu + stepper. Grounded ke `hr_admin.go` (`handleHRRequestsList`/`handleHRRequestDetail`), `self_requests.go` (`handleMyRequests`), dan `pengajuan_booking.go` (booking).

| Method | Path | Fungsi | Auth |
|---|---|---|---|
| GET | `/requests/mine` | Ringkasan pengajuan **milik pemanggil** ("Aktivitas Saya", Riwayat); `?filter=ongoing/past`, `?type=`, `?page=`/`?limit=` (bawaan 20, maks 100), `?include=booking`. Bentuk `HRRequestSummary` (header + `steps` timeline review + tanggal per jenis) | header |
...
- **Kapan ikut**: `GET /requests/mine?include=booking` membaca `/peminjaman/saya` (booking milik pemanggil); `GET /hr/requests?as=reviewer&include=booking` membaca `/peminjaman/perlu-aksi` (antrean penyetuju). Keduanya hanya bila `type` kosong atau `Booking`. `as=reviewed`, mode admin HR, dan `/hr/requests/summary` **tidak pernah** memuat booking. `include` boleh dipisah koma.
- **Bentuk item**: `id` = nomor booking (`PJR-...`), `request_type` = `Booking`, `subtype` = nama ruang, `employee_id`/`full_name`/`position`/`department` dari pemohon, `from_date`/`to_date` = jam mulai dan selesai (berjam, bukan tengah malam), `steps` = satu langkah `key: "penyetuju"` (nama dan waktu dari keputusan terakhir yang sesuai status; tolak otomatis tanpa nama; catatan hanya alasan tolak manual). `status` dipetakan ke kosakata ulasan: `DIAJUKAN` → `Menunggu persetujuan`, `DISETUJUI` → `Disetujui`, `DITOLAK` → `Ditolak`, `DIBATALKAN` → `Dibatalkan`; status tak dikenal diteruskan apa adanya.
- **Saringan antrean** (`overlayDaftar`, arti yang sama dengan koleksi lokal): `company_id` = perusahaan efektif pemanggil, `status` (kosakata ulasan), `department` persis, `search` atas `employee_id`/`full_name` tanpa beda huruf (pola regex yang tak sah dicocokkan sebagai teks biasa), `from`/`to` atas `metadata.created_at`. Di `/requests/mine`, `filter=ongoing|past` untuk booking memakai `bookingBerjalan`: ditolak atau dibatalkan = selesai, lainnya berjalan sampai `selesai_at`.
- **Urutan dan halaman**: seluruh item (lokal dan booking) diurutkan `created_at` menurun dengan seri dipecah `id` menurun, lalu dipotong `page`/`limit`. `limit` dijepit 100 untuk semua klien; halaman di luar total dibalas `data: []`.
- **`degraded`**: kunci ini hanya ada bila booking memang diambil. `[]` = booking terbaca; `["booking"]` = booking tak terbaca (env `INVENTORY_MODULE_URL` kosong, gagal dihubungi atau lewat 5 detik, balasan non-200, badan lebih dari 4 MB, atau kunci `data` absen), sementara pengajuan lain tetap dikirim. **403 dari `/perlu-aksi`** berarti pemanggil bukan penyetuju: bukan kerusakan, tanpa booking dan tanpa `"booking"` di `degraded`. Klien tanpa `include` menerima bentuk lama persis (`data`, `total`, `page`, `limit`), dikunci `TestHandlerDaftarSayaTanpaIncludeIdentikSebelumIrisan2`.
- **Panggilan ke inventory**: header `BIP-Gateway-ID` (`INTERNAL_GATEWAY_KEY`) dan identitas pemanggil disalin (`strings.Clone`) sebelum goroutine, berjalan berbarengan dengan pembacaan Mongo.
- **Detail dan aksi** booking tetap `/api/inventory/peminjaman/:nomor*` ([[API - Inventory Service]]); `/hr/requests/detail` tak mengenal `Booking`.
- **Env**: `INVENTORY_MODULE_URL` di blok `attendance-service` kedua compose, **opsional** dan di luar map `InternalURL` (kosong = `degraded`, presensi tak padam). Env baru menuntut `--force-recreate attendance-service`.
...
> 🔜 **Perubahan blok `kehadiran` yang BELUM live** (bip-erp [#1614](https://github.com/bip-itteam-internal/bip-erp/pull/1614), masih OPEN):
> `terlambat` berubah arti dari cacah status `Terlambat` menjadi **kejadian unik di buku tamu security**, dan jadi **nullable** — `null` berarti agregasi buku tamu gagal, BUKAN nol keterlambatan. Cacah status pindah ke field baru `terlambat_sistem` dan tetap dikirim. `persen` ikut berganti rumus jadi `(hari_kerja − terlambat) / hari_kerja`. Alasan dan konsekuensinya di [[HRIS - Attendance System]] §Kartu Terlambat.
