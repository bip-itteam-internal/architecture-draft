...
- Method: GET
- Auth: shop
- Deskripsi: Retrieve product performance of the shop.
- Sumber: open.shopee.com/documents/v2/ams.get_product_performance?type=1 (backend doc/api) — 2026-07-31
...
| `channel` | string | ya | Channel. Applicable values: - AllChannel - SocialMedia - ShopeeVideo - LiveStreaming Contoh: `AllChannel` |
| `item_id` | int64 | tidak | Item ID for query. Contoh: `123` |
...
| `response.list[].item_id` | int64 | Item ID. |
| `response.list[].item_name` | string | Item Name. |
| `response.list[].sales` | string | Total value of the product sold through affiliate marketing. |
| `response.list[].items_sold` | int64 | Total number of the product sold through affiliate marketing. |
| `response.list[].orders` | int64 | Total number of orders including the product generated through affiliate marketing. |
| `response.list[].clicks` | int64 | Total number of clicks on your product links through affiliate marketing during the selected period. |
| `response.list[].est_commission` | string | Estimated payout of the product sold through affiliate marketing. |
| `response.list[].roi` | string | Return on Investment, equal to GMV divided by Estimated Commission. It can be used to evaluate the efficiency of your investment in affiliate marketing on the product.If it does not exist, the return value is --. |
| `response.list[].total_buyers` | int64 | Total number of buyers who have purchased the product through affiliate marketing. |
| `response.list[].new_buyers` | int64 | Total number of new buyers who have purchased the product through affiliate marketing. |
| `response.total_count` | int64 | Total number of items that match the condition. |
