...
| GET | `/accurate/daily-returns/kpi/penanganan` | ⚠️ **belum merge/deploy.** Kecepatan penanganan retur satu periode (`periode=YYYY-MM`, wajib), bahan metrik KPI `retur_lewat_14_persen` di [[Microservices - Employee Service]]. Balas `{periode, total, dinilai, lewat14, lewat14_persen, belum_bisa_dinilai, catatan}`. Jeda diukur `trans_date` → selesai (`SENT` pakai `last_sent_at`, selain itu `warehouse_confirmed_at` — status yang menentukan, bukan keberadaan stempel). Penyebut `lewat14_persen` = `dinilai`, BUKAN `total`: yang belum jatuh tempo menurunkan cakupan, tidak dihitung telat. **Bulan berjalan → 400** (posisi belum final). `SKIPPED` dikecualikan. Beda tegas dari `/stats` yang mengukur UMUR terhadap hari ini dan karenanya tak dapat menilai periode |
