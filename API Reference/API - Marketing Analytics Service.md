...
*Daftar endpoint **Marketing Analytics Service** — grounded ke kode (enumerasi seluruh pendaftaran rute di `services/marketing-analytics/*.go` non-test terhadap `origin/main`; audit 2026-08-26, **40 route** di berkas produksi termasuk `/health`). Arsitektur & semantik data: [[Microservices - Marketing Analytics Service]].*

- **Status**: ✅ Grounded ke kode (2026-08-26), **keempat-puluh route punya baris di dok ini**. Sebelumnya 12 route tak terdokumentasi sama sekali; daftarnya + cara audit ulangnya ada di bagian **Cara mengaudit kelengkapan daftar ini** di kaki dokumen.
