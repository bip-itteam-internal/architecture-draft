...
| Method | Path | Catatan |
|---|---|---|
| GET | `/videos` | `VideoRow` (tab VSA/GMV Max/organik); + `gross_profit` (nil = tak pernah dihitung), `product_title`/`product_image_url`/`product_item_group_id`, `ad_id`, `channel`, `synced_at`, `published_at`. **Menolak `granularitas` (400)**, dan sejak 2026-09-02 juga **`dari`/`sampai` (400)** dengan pesan yang menunjuk ke `/videos/periode`. Keduanya dulu diterima lalu diabaikan diam-diam |
| GET | `/videos/periode` | Performa video **dalam rentang tanggal**. `dari` & `sampai` **WAJIB** (400 bila salah satu absen). Menolak `granularitas`, `matang`, `bulan`. Query lain: `channel`, `shop_id`, `divisi`, `sumber`, `limit`, `sort_by` (`revenue` bawaan · `ads_cost` · `gross_profit` · `hpp` · `hari_berdata` · `views` · `gmv`), `sort_dir`, `terbit_dalam_rentang` (hanya `"true"`/`"false"`, selain itu **400** berikut `terbit_dalam_rentang_valid`). Baris: identitas video + `revenue`/`ads_cost`/`gross_profit`/`hpp` + `views`/`gmv`/`units_sold`/`sku_orders` (**nullable**, null = belum ada riwayat harian, bukan nol) + `trend` per kolom + `hari_berdata`/`hari_diminta`. ⚠️ **Kolom yang diurutkan menentukan koleksi mana yang memikul populasi & limit** (`views`/`gmv` → deret harian, sisanya → `mart_profit_attribution`); tanpa itu tab Organik kosong selamanya. TikTok saja; channel lain dinyatakan lewat `unavailable_channels`. `handler_video_periode.go`, `sumber_video_periode.go`, `sumber_video_harian.go` |
...
| PATCH | `/live-shifts/:id/jeda` | Buka/tutup jeda bergantian. Tak ditemukan → **404**; sesi sudah diakhiri → **400**. **PATCH benar-benar sebagian** (hanya field `jeda`). Update bersyarat: menutup jeda mensyaratkan jeda terakhir **masih terbuka di Mongo**, membuka jeda mensyaratkan **panjang `jeda` belum berubah** — yang kalah balapan dibalas **409**, bukan menimpa diam-diam. 🟡 Branch di atas menambah gerbang kepemilikan → **403** bila pemanggil bukan pengelola sesi (lihat di bawah tabel) |
| PATCH | `/live-shifts/:id/selesai` | Akhiri sesi; jeda yang menggantung ikut ditutup di waktu selesai supaya durasi efektifnya tidak menggantung. Filter `{selesai: null}` menutup balapan cek-lalu-tulis → yang kalah **409**, bukan sukses palsu. 🟡 Branch di atas menambah gerbang kepemilikan → **403** bila pemanggil bukan pengelola sesi (lihat di bawah tabel) |
| GET | `/live-shifts/berjalan` | Sesi yang belum diakhiri. 🟡 **Di branch `feat/marketing-analytics-sesi-berjalan-pemilik`, BELUM merged**: disaring per pemilik — host biasa menerima sesi MILIKNYA saja, `IsMarketingLeader` menerima seluruh tim — dan tiap baris menambah **`boleh_kelola`** (bool) yang menyatakan pemanggil boleh menjeda/mengakhiri sesi itu. Baris tetap **DATAR** (`LiveShift` ditanam, bukan dibungkus) karena klien mobile mendekode `rows` langsung sebagai `LiveShift`. Sebelum branch itu mendarat, rute ini mengembalikan SELURUH sesi lintas host tanpa penyaringan apa pun |
...
| GET | `/live-shifts/akun` | Isi pemilih akun untuk satu toko. `shop_id` **wajib**, kosong → **400**. Diagregasi dari `mart_live_sessions` per `username` (`live_shift_akun.go`), membalas `akun` · `sesi` · `gmv` · `porsi_gmv` (**persen**, jangan dikali 100 lagi) · `utama`. `utama` hanya disematkan bila porsi GMV akun teratas **≥ 60%** (`ambangAkunUtama`), karena satu toko produksi cuma 49,8% dari 290 akun dan menandai yang teratas di toko semacam itu mendorong host memilih akun yang belum tentu benar. **Tidak** menyaring `hanyaSesiHost`, sengaja beda dari jalur KPI: layar pemilih harus menampilkan seluruh akun termasuk afiliasi. Pemecah seri deterministik (nama akun menaik). ⚠️ Toko yang tak ada dibalas **200 + daftar kosong**, jadi konsumen wajib membedakan "kosong" dari "belum dimuat" dan "gagal dimuat". 🔴 **Tanpa jendela waktu dan tanpa status akun**: `$match` hanya atas `shop_id`, jadi akun yang sudah mati (diban/ditinggalkan) tetap terbalas selamanya dan, karena peringkatnya memakai GMV **kumulatif**, bisa duduk di puncak bahkan ikut distempel `utama`. Rinciannya di [[Microservices - Marketing Analytics Service]] |

Dua bentuk penyaringan host, **tidak** saling menggantikan: `host=` eksplisit dan hanya boleh dipakai pemanggil yang lolos `IsMarketingLeader` — host biasa yang menyebut orang lain **ditolak 403**, bukan diam-diam dipaksa ke dirinya sendiri, supaya UI yang salah kirim terlihat alih-alih tersamar jadi "kosong tapi sukses" (host biasa yang menyebut dirinya sendiri tetap lolos). `milik_saya=true` menyaring ke `employee_id` dari **header gateway**, bukan query string yang bisa ditulis apa pun oleh klien.

⛔ **Keduanya kosong: baris ini SUDAH TIDAK BENAR sejak branch `feat/marketing-analytics-sesi-berjalan-pemilik` (BELUM merged).** Dulu berarti "tak disaring per host sama sekali", dan itu lubang: **host biasa yang memanggil rute ini tanpa parameter membaca riwayat SELURUH tim** lewat API, lengkap dengan `employee_id`, nama, dan ringkasan penjualan tiap orang. Aplikasi web selalu mengirim `milik_saya=true` untuk host biasa, tapi API tak pernah mewajibkannya. Sekarang: **leader** melihat seluruh tim, **non-leader dipaksa ke dirinya sendiri**, dan non-leader **tanpa identitas ditolak 403** alih-alih jatuh ke "tak disaring".

⚠️ Konsumen yang paling mungkin patah oleh penyaringan itu adalah **kartu performa tim host live di halaman ICC**, yang sengaja memanggil rute ini TANPA parameter. Ia aman karena gerbang halamannya (`bolehTim` = marketing leader **atau** anggota IT) seluruhnya lolos `IsMarketingLeader` di backend — daftar peran backend memuat modul `it`, sementara padanan frontend tidak. Lihat [[ADR - 0063 Siaran Serentak Dicatat sebagai Sesi Terpisah per Akun]] §6; menyamakan kedua daftar itu tanpa memeriksa akan mematikan kartu tersebut.
...
⚠️ **Satu host bisa punya BEBERAPA sesi berjalan sekaligus, dan itu sah.** Index unik parsialnya hanya atas `akun_live` (tanpa `shop_id` maupun `host.employee_id`), jadi `POST /live-shifts` menolak sesi kedua **pada akun yang sama** tetapi menerima sesi kedua pada akun berbeda milik orang yang sama. `GET /live-shifts/berjalan` karena itu bisa mengembalikan **beberapa baris untuk satu orang**; konsumen yang mengambil elemen pertama saja akan menyembunyikan sesi yang tak bisa diakhiri, dan sesi itu lalu menggantung melewati ambang 12 jam yang **memaksa porsi host jadi nol**. Aturan turunannya (jam siaran = union jendela waktu, tumpang tindih lintas akun ditandai sah bukan dinolkan) diputuskan di [[ADR - 0063 Siaran Serentak Dicatat sebagai Sesi Terpisah per Akun]].

### Gerbang kepemilikan PATCH (🟡 branch `feat/marketing-analytics-sesi-berjalan-pemilik`, belum merged)

Sebelum branch ini, `PATCH /:id/jeda` dan `/:id/selesai` **tidak memeriksa pemanggil sama sekali**: siapa pun yang lolos `RequireLiveShiftUser` bisa mengakhiri sesi host lain. Sekarang keduanya membalas **403** kecuali pemanggil salah satu dari:

| Yang lolos | Alasan |
|---|---|
| **Host yang tercatat di `host[]`** sesi itu | Pemilik sesungguhnya. Dinilai atas SELURUH larik, jadi co-host ikut |
| **Pembuat sesi** (`dibuat_oleh`) | Membatalkan tindakan sendiri, bukan hak istimewa atasan: SPV memang bisa mencatatkan sesi atas nama host lain |
| **Supervisor/admin IT** (`IsITSupervisor`) | Jalan darurat, pola sama dengan `superAksesIT` di procurement. **Bukan** `IsITMember` — staf IT tak perlu bisa menghentikan siaran orang |

⛔ **Marketing leader SENGAJA TIDAK lolos.** Keputusan produk 2026-08-30: leader boleh MELIHAT sesi orang lain, tidak boleh menghentikannya. Sesi yang lupa ditutup tetap tertutup sendiri oleh autoclose begitu hostnya absen pulang.

⚠️ Jalan darurat IT ada karena gerbang ini menutup satu jalan keluar yang dulu terbuka bagi siapa saja: sesi yang tercatat dengan `employee_id` salah ketik tak punya host nyata, **autoclose menolak menutupnya** (orang itu tak punya entri absensi sama sekali), dan index unik memblokir sesi baru pada akun itu. Tanpa jalan darurat, akun itu mati permanen bila pembuatnya sudah resign.

Urutan pemeriksaan: **404 dulu** (pemiliknya baru diketahui setelah dokumennya terbaca), lalu **403**, baru pemeriksaan keadaan. Konsekuensi yang diterima sadar: pemanggil bisa membedakan "id tak ada" dari "id ada tapi bukan milikmu"; itu dibiarkan karena `_id` berupa ObjectID acak sehingga enumerasi tak realistis, sementara 404 yang jujur jauh lebih menolong saat orang salah menyalin id.
...
`/divisi` · `/toko` · `/pagu` (GET+POST) · `/kurva-alokasi` · `/profit/items` · `/profit/skus` · `/kpi/anggota-tim-icc` · `/kpi/kinerja-affiliate-tim` · `/live-shifts` (POST, GET, GET `/berjalan`, GET `/akun`, PATCH `/:id/jeda`, PATCH `/:id/selesai`)
