...
> ⚠️ **Cache gateway 3 menit.** `GET /api/recruitment/*` di-cache api-gateway (modul `recruitment` tidak ada di `noCacheModules`, dan per `origin/main` 2026-09-11 tak satu pun rutenya ada di `noCacheRoutes`), dan penutupan sesi oleh kandidat atau sweep tidak mengosongkannya. Status massal, laporan psikotes, dan `/candidates/:id/test-results` bisa basi sampai 3 menit; periksa header `X-Cache` saat verifikasi. 🔄 **(T0a)** Ketiganya dikeluarkan dari cache di bip-erp branch `fix/api-gateway-psikotes-tanpa-cache`, **belum merged per 2026-09-11**. Path ketiganya lalu tertulis juga di `api-gateway/redis.go` dan dijaga test `TestRuteYangDisalinKeNoCacheGatewayTetapTerdaftar`, jadi rename salah satunya wajib menyunting gateway juga.
