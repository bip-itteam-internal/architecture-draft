...
> ⚠️ **Cache gateway 3 menit.** `GET /api/recruitment/*` di-cache api-gateway (modul `recruitment` tidak ada di `noCacheModules`), dan penutupan sesi oleh kandidat atau sweep tidak mengosongkannya. ✅ **(T0a, bip-erp #1832, merged 2026-09-11 `4ef28e21`)** Status massal, laporan psikotes, dan `/candidates/:id/test-results` dikecualikan lewat `noCacheRoutes`; di dev terverifikasi hari itu (tak pernah `X-Cache: HIT`, 0 kunci Redis). **Sampai api-gateway prod di-deploy, ketiganya masih bisa basi sampai 3 menit di prod**; periksa header `X-Cache` saat verifikasi. Path ketiganya tertulis juga di `api-gateway/redis.go` dan dijaga test `TestRuteYangDisalinKeNoCacheGatewayTetapTerdaftar`, jadi rename salah satunya wajib menyunting gateway juga. Rute recruitment lain tetap di-cache.
