...
## Perusahaan (Rekrutmen Lintas Perusahaan)

> ✅ **Merged dan live prod 2026-09-12** (bip-erp [#1853](https://github.com/bip-itteam-internal/bip-erp/pull/1853)). Terverifikasi lewat gateway dev 2026-09-12 dengan akun yang bukan admin pusat: `GET /companies` 403 tanpa paket dan 200 dengan paket; `POST /requisitions` ber-`company_id=ELT` tersimpan ELT bagi pemegang paket dan BIP bagi SPV tanpa paket. Keputusan & rincian lengkap: [[ADR - 0092 Rekrutmen Lintas Perusahaan lewat Paket Izin]].

| Method | Path | Fungsi | Role |
|---|---|---|---|
| GET | `/companies` | Daftar perusahaan grup untuk pemilih & kolom Perusahaan di layar rekrutmen: `{data:[{key,name,active}]}` urut nama; perusahaan **nonaktif ikut dikirim** (bertanda). **403** untuk pemakai non-lintas. **503** bila master perusahaan (employee-service) tak terjangkau tanpa salinan cache (bukan daftar kosong) | lintas / admin pusat |

- **Cakupan baca**: pemakai non-lintas (mayoritas pengguna) selalu terkunci ke perusahaannya sendiri: `?company=` **diabaikan diam-diam**. Pemegang izin `recruitment.cross_company` atau admin pusat (`system_roles.group=admin`) melihat **semua** perusahaan, atau satu lewat `?company=<key>`. Berlaku di daftar requisitions, postings, candidates, interviews, offers, manpower-plans (+ coverage), onboarding-reviews, onboarding-instances, `/mpp/vacancies`, dan status psikotes massal (`/candidates/psikotes/status`).
- **Per-ID di luar cakupan → `404`** (bukan 403): keberadaan dokumen milik perusahaan lain tak boleh bocor. Pengecualian: pengaju requisition selalu boleh membuka pengajuannya sendiri walau tercatat di perusahaan lain (recruiter lintas yang mengajukan atas nama perusahaan B, lalu paketnya dicabut, tak kehilangan jejak pengajuannya).
- **Tulis** (body `company_id`, mis. `POST /requisitions`): pemakai non-lintas → selalu perusahaan sendiri, kiriman body **diabaikan** (bukan ditolak). Pemegang izin lintas boleh memilih perusahaan lain, divalidasi ke master: tak dikenal → `400 perusahaan tidak dikenal: <key>`; nonaktif → `400 perusahaan nonaktif: <key>`; master tak bisa dipastikan → `503` (**tidak** diturunkan jadi perusahaan sendiri: kegagalan diam-diam akan menyimpan data ke perusahaan yang salah dengan status 201).
- **`company_id`** ada di dokumen `job_requisition`, `job_posting`, `candidate`, `manpower_plan`, `onboarding_review`, `onboarding_instance` (diturunkan satu arah dari requisition, lihat [[Microservices - Recruitment Service]]). Dokumen anak kandidat (interview, offer, hasil tes, background check, psikotes) **tidak** menyimpan salinan; **`candidate_company_id`** (virtual, dihitung saat respons dari kandidatnya) muncul di `GET /interviews` dan `GET /offers` untuk kolom Perusahaan di layar. *(Menu Offers `/offers*` sudah ada di `routes.go`, mencakup approve/send/negotiate/accept/decline, tapi belum punya tabel endpoint sendiri di dok ini; gap pra-eksisting, tak disebabkan fitur ini.)*
- **`POST /requisitions`** digerbang **`isSupervisor` (atasan di modul mana pun) ATAU pemegang `recruitment.cross_company`/admin pusat** (`requireAtasanAtauLintasPerusahaan`), pelebaran dari gerbang lama yang hanya `isSupervisor`. Lihat baris requisition di bawah untuk interaksinya dengan body `company_id`.
- **`GET /requisitions?scope=department`**: atasan/HR melihat cakupan departemennya **di perusahaannya sendiri** + pengajuannya sendiri; recruiter lintas yang **bukan** atasan/HR hanya melihat pengajuannya sendiri (menu ini terbuka baginya karena ia boleh mengajukan, bukan karena punya wewenang atas suatu departemen); selain itu `403`.
- **Notifikasi requisition baru** ke SPV HRD kini menyebut nama perusahaan requisition-nya (mis. "... untuk departemen Produksi, CV Elit."), diambil dari cache di atas di dalam goroutine (pengaju tak menunggu itu).
...
> ⚠️ **Gerbang tulis di tabel ini merged 2026-09-17** (bip-erp [#1961](https://github.com/bip-itteam-internal/bip-erp/pull/1961), erp-frontend [#1637](https://github.com/bip-itteam-internal/erp-frontend/pull/1637)); deploy dan panggilan lewat gateway belum diverifikasi, dan paket "Rekrutmen: Penyusun MPP" belum dipasang ke posisi mana pun. Sebelum #1961 gerbangnya `recruitment.work` (fallback `isHRSupervisor`) untuk tulis MPP dan `recruitment.work` (fallback `isHR`) untuk keputusan posisi kosong, sehingga setiap pemegang `hris` lolos lewat fallback tier (diukur prod 2026-09-17: 13 akun); prod tetap berperilaku begitu sampai recruitment-service baru di-deploy. Keputusan user 2026-09-17: menulis MPP dan memutuskan posisi kosong **hanya SPV HRD dan staf rekrutmen**, lewat izin aditif `recruitment.manpower_plan_manage` (paket "Rekrutmen: Penyusun MPP"). Kolom Role = izin (fallback saat kill-switch `RECRUITMENT_PERMISSION_ENFORCEMENT=off`).
...
| POST · PUT · DELETE | `/manpower-plans` · `/manpower-plans/:id` | Tambah / ubah / hapus baris (`tahun`, `department`, `posisi`, `jumlah_rencana`, `is_kritikal`, `catatan`, `company_id` untuk lintas). DELETE permanen | `recruitment.manpower_plan_manage` (`isHRSupervisor`); sebelum #1961: `recruitment.work` |
...
| PUT/DELETE | `/stages/:kind/:id` | Ubah / hapus record tahap (`:kind` yang dikenal saat ini hanya `interviews`). ⚠️ **PUT memakai `ReplaceOne`**, jadi field yang dikontrol server dikembalikan lewat penjaga tersendiri: `id`, `candidate_id`, `created_at`, **dan sejak 2026-09-16 `status`/`status_reason`/`status_at`** — tanpa itu menyunting jadwal sesi yang sudah ditandai "Selesai" mengembalikannya jadi "Terjadwal" tanpa satu pun pesan. PUT juga **bersyarat** terhadap status yang baru dibaca: penandaan yang mendarat di sela-sela baca dan tulis membuat suntingan dibalas **409**, bukan menimpanya balik | HR |
...
| GET | `/candidates/:id/test-results` | Daftar hasil tes kandidat, diperkaya `round_name`. ⛔ **Jangan membangun di atas `skor_otomatis`** di respons ini: field objek itu merged 2026-09-17 (bip-erp [#1931](https://github.com/bip-itteam-internal/bip-erp/pull/1931)) dan **sedang dicabut** (bip-erp [#1933](https://github.com/bip-itteam-internal/bip-erp/pull/1933), PR terbuka). Namanya bertabrakan dengan `disc.skor_otomatis` (bool) di `/candidates/:id/psikotes/report`. Persen hasil akhir pilihan ganda diturunkan layar dari `total_benar`/`total_soal` laporan psikotes; alasannya di [[HRIS - Bank Soal dan Paket Psikotes]] §Penilaian dan Hasil Tes | HR |
...
| GET | `/candidates/:id/psikotes/report` | Laporan individual (metrik 4 kategori + skor keseluruhan + `selesai_karena`; **(T0, bip-erp #1828)** + `col_index` = jumlah kolom yang sempat dikirim). **(2026-09-11)** Sesi paket + `paket_nama` dan `bagian[]` (`nama`, `jenis_jawaban`, `status`, `selesai_karena`, waktu, dan salah satu `kraepelin`/`pilihan_ganda`/`disc`; hanya hitungan). **Tidak memuat soal/kunci jawaban**. *(DISC tanpa kunci, merged 2026-09-15 #1900)* `disc` + `skor_otomatis` (bool: semua huruf snapshot terisi) dan `lembar[]` {`nomor` = posisi di sesi, `pernyataan[]`, `pilih_p`/`pilih_k` = indeks atau `null`}; tally tetap dikirim. Pengecualian sadar: teks pernyataan DISC ikut untuk lembar jawaban, huruf kunci tetap tidak. Kandidat tanpa sesi → `404 {"error": ...}` | HR |
...
| GET | `/offers` | Menu Offers: offer (terbaru dulu) + `candidate_name`/`candidate_position` + `candidate_company_id` (virtual; offer tak menyimpan perusahaan), daftar disaring lewat kandidat dalam cakupan perusahaan pemanggil (rekrutmen lintas perusahaan, lihat bagian **Perusahaan**). 🟡 **(bip-erp #1854 `feat/recruitment-alur-kerja-hr`, belum merge)** + **`can_approve`** (bool, tanpa `omitempty`): pemanggil lolos gerbang `POST /offers/:id/approve`, sama untuk seluruh daftar | HR (`recruitment.view`) |
...
| Method | Path | Fungsi | Role |
|---|---|---|---|
| GET | `/psikotes/tipe` (`?active=true`) · `/psikotes/tipe/:id` | Daftar/detail tipe + `total_durasi_detik`, `jumlah_soal` per kode subtes, `kesiapan`, `subtes_kurang`, `dipakai_paket`. *(Revisi, merged 2026-09-11 #1850)* + `penjelasan_kandidat`, `jumlah_contoh` per kode subtes (soal contoh aktif, terpisah dari `jumlah_soal`); kode kesiapan baru `contoh_kosong` (subtes pilihan ganda tanpa soal contoh aktif), dan `subtes_kurang` ikut memuatnya | lihat |
...
| POST/PUT/DELETE | `/psikotes/item` · `/psikotes/item/:id` | Buat/ubah/hapus soal, divalidasi terhadap tipenya (opsi 2 sampai 6, kunci sebanyak `jumlah_jawaban`, DISC tepat 4 kata berdimensi sah, gambar berpola kunci; *(DISC tanpa kunci)* kedelapan huruf DISC boleh kosong = grup tanpa kunci, campuran → `400` "huruf kunci grup ini harus diisi semua atau dikosongkan semua"). *(Revisi)* + `contoh` (pilihan ganda saja; DISC bertanda contoh → `400`) dan `penjelasan` (maks 500, hanya soal contoh); body PUT tanpa key `contoh`/`penjelasan` mempertahankan nilai lama | tulis |
| PUT | `/psikotes/item-urutan` | `{tipe_id, subtes_kode, ids}`: urutan baru satu subtes (tombol naik/turun) | tulis |
| POST | `/psikotes/impor-item` | `{tipe_id, dry_run, expected_hash, baris[]}`, maks 500 baris, divalidasi per baris. `dry_run` membalas `{baris[], valid, ditolak, hash}`; simpan wajib membawa hash yang sama, beda → `409`. *(Revisi)* Baris membawa `contoh` (bool) dan `penjelasan`, keduanya ikut dalam hash; *(DISC tanpa kunci)* baris DISC boleh tanpa `most`/`least` (grup tanpa kunci), aturan campuran sama per baris | tulis |
...
| GET | `/interviews` | **Semua** sesi interview (terbaru dulu), diperkaya nama/posisi kandidat + status feedback `feedback_submitted`/`feedback_total` — sisi HR (menu **Interviews**) — PR #536. **(2026-09-11, live prod 2026-09-12)** + `candidate_company_id` (virtual; kolom Perusahaan). Daftar disaring lewat kandidat, lihat bagian **Perusahaan** di atas. **(2026-09-16)** tiap sesi membawa **`status`** (+ `status_reason`, `status_at` bila ada); sesi lama yang tersimpan tanpa field itu dikirim sebagai **`Scheduled`**, diisi saat baca di ketiga jalur (`/interviews`, `/interviews/assigned`, `/candidates/:id/stages`) | HR |
| GET | `/interviews/assigned` | Sesi interview yang menugaskan saya sebagai pewawancara (+ nama/posisi kandidat + jawaban saya). Dipakai halaman link `/interview-feedback/:id`; menu "Interview Saya" sendiri sudah dihapus dari navigasi (dormant). **(bip-erp [#1886](https://github.com/bip-itteam-internal/bip-erp/pull/1886), merged 2026-09-15 08:21 WIB; live prod 2026-09-15 09:23 WIB, terverifikasi; dev belum)** tiap sesi juga membawa **`round_name`** + **`form_type`** babaknya (virtual, `muatBabakSesi` di `interview_babak.go`: satu `Find` `$in`, best-effort, galatnya dicatat log). `form_type` kosong pada babak lama dikirim `"generic"`; babak tak ditemukan atau `round_id` rusak membuat kedua field absen. Alasannya: katalog `/masters/interview-rounds` digerbang `recruitment.view`, jadi pewawancara non-HR menerima 403 di sana dan dulu diam-diam mendapat form generic. Hanya endpoint ini yang diperkaya. ⚠️ Gateway meng-cache GET ini 3 menit per karyawan; `POST` lewat gateway meng-invalidate cache modul recruitment, deploy tidak | auth |
| PUT | `/interviews/:id/status` | **Tandai hasil sesi** (2026-09-16, bip-erp [#1924](https://github.com/bip-itteam-internal/bip-erp/pull/1924)). Body `{status, reason}`; `status` ∈ `Scheduled`·`Completed`·`Cancelled`·`NoShow`. **`Cancelled` dan `NoShow` wajib `reason`** (400 tanpanya). Transisi sah hanya **Terjadwal → hasil** dan **hasil → Terjadwal** (pembatalan penandaan); loncat antar-hasil dibalas **409**, supaya koreksi salah tandai terbaca sebagai koreksi di audit, bukan sesi yang diam-diam berubah. Update **bersyarat** (filter menyertakan status lama): dua HR yang menandai sesi yang sama tak saling menimpa, yang kalah balapan dapat **409**. Menulis audit `interview.status` | HR |
| POST | `/interviews/:id/feedback` | Kirim/**ubah** penilaian (babak generic: rating 1-5 + recommendation; babak `hrd_interview`/`user_interview`: `form_type` + `aspects[]` + catatan bertipe + `decision`, tanpa validasi silang terhadap `form_type` babak). **Upsert** per (interview, pewawancara) → tak dobel. Boleh **pewawancara sesi ATAU HR**. **`interviewer_id`** di body hanya dipakai bila pengirim **HR** (rekap atas nama pewawancara lain) — non-HR **selalu** JWT sendiri (PR #536). ⚠️ **Sejak 2026-09-16 membalas 409 untuk sesi ber-status `Cancelled`/`NoShow`**: penilaian ikut **menggerakkan status kandidat** (`wireDecisionToStatus`), jadi sesi yang tak pernah terjadi bisa meloloskan atau menggugurkan orang. Yang tetap perlu dinilai harus dibatalkan penandaannya dulu | auth |
...
|---|---|---|
| GET | `/public/recruitment/postings` | Daftar lowongan Open (featured dulu) — tiap item memuat **`slug`**. **(2026-09-11, live prod 2026-09-12)** + **`company_name`** (nama perusahaan perekrut; satu portal melayani seluruh perusahaan grup, lihat bagian **Perusahaan** di dok [[Microservices - Recruitment Service]]) |
| GET | `/public/recruitment/postings/:id` | Detail lowongan. **`:id` menerima `slug` ATAU ObjectID** (dicoba ObjectID dulu; gagal parse → lookup by `slug`). Respons + `slug` & **`job_type`** (nama, hasil resolve `job_type_id` → master `job_types`). **(2026-09-11, live prod 2026-09-12)** + **`company_name`** |
| POST | `/public/recruitment/apply` | Pelamar mendaftar sendiri (email + `posisi_dilamar` wajib; **`posting_id` kini juga WAJIB** sejak branch 2026-09-11 yang belum merge, lihat catatan kontrak di bawah). Respons **`201`** berisi **hanya** `{"message": "Lamaran terkirim. Konfirmasi telah dikirim ke email Anda."}` — **tanpa** `tracking_token`/`track_url` (fitur tracking dihapus, lihat di bawah). Kandidat lahir `progress: "CV Screening"`, `status: Pending`. **Dua bentuk body**: (a) JSON, atau (b) **`multipart/form-data`**: field `data` = JSON kandidat + file **`berkas`** = PDF **maks 10 MB** → MinIO `recruitment/cv/<candidate_id>/berkas.pdf` → set `cv_object` (HR buka via `GET /candidates/:id/cv/preview`) |

| Method | Path (gateway) | Fungsi |
|---|---|---|
| GET | `/public/recruitment/psikotes/:token` | Kandidat membuka sesi psikotes lewat magic link. **Tidak memuat digit soal**; sesi paket + `paket` (bagian, `bagian_index`, tanpa soal). *(Revisi layar kandidat, merged 2026-09-11 #1850)* + `lowongan` (judul posting, cadangan `posisi_dilamar`), `telepon_samar`, `email_samar` (disamarkan server; nilai lengkap tak dikirim); `paket` tanpa `nama`; bagian tanpa `nama`/`deskripsi`, dengan `penjelasan` dan key `konfig_kolom` (dulu `kraepelin`); subtes tanpa `nama`, dengan `jumlah_contoh` |
...
| POST | `.../:token/bagian/:b/subtes/:s/contoh` | *(Revisi, merged 2026-09-11 #1850.)* Soal contoh subtes yang gilirannya: `{jumlah_jawaban, contoh: [{pertanyaan, gambar, opsi, kunci, penjelasan}]}`. **Sengaja membawa kunci** (contoh tak dinilai). Tak menulis deadline maupun status, hanya `last_seen_at`. Bukan pilihan ganda → 400; bagian belum mulai → `409 belum_mulai`; bukan subtes yang gilirannya → `409 bukan_giliran` |
| POST | `.../:token/bagian/:b/subtes/:s/jawab` | `{nomor, pilihan}` (pilihan ganda) atau `{nomor, most, least}` (DISC, keduanya beda). Maju saja (*(revisi)* nomor mana pun dalam rentang boleh dijawab ulang selama subtes berjalan; di luar rentang 400); lewat deadline plus 5 detik → `409 waktu_habis` |
...
> **Psikotes publik dijaga token, bukan sesi login.** Token 32 byte `crypto/rand` base64url, unik di level index. Tidak ada endpoint publik yang mengembalikan kunci jawaban, dimensi DISC, atau skor; soal CFIT/DISC dikirim **tanpa kunci** saat subtesnya dimulai. *(Revisi, merged 2026-09-11 #1850)* Pengecualian sadar: `.../subtes/:s/contoh` membawa kunci dan penjelasan **soal contoh**; soal asli tetap tanpa kunci. DTO-nya eksplisit dan ada test allowlist kunci JSON yang menggigit bila field internal bocor. Rincian: [[HRIS - Psikotes Kraepelin]] dan [[HRIS - Bank Soal dan Paket Psikotes]].

> **Galat mesin tes berpaket** berbentuk `{"error": <kalimat>, "kode": <kode>}` dengan kode `bukan_giliran`, `belum_mulai`, `waktu_habis`, `soal_terlewati` (*(revisi)* dibuang), `subtes_selesai` (409); career portal membaca `kode`. `POST .../columns/:index` dan `.../finish` menolak sesi paket (400). Gateway meneruskan semua rute ini lewat **satu rute umum berpenyaring** dengan limiter per token ([[CORE - API Master Gateway]]).
...
> **Catatan kontrak `/apply`:** `posisi_dilamar` **wajib** dan **tidak** diisi server dari posting — divalidasi lebih dulu, jadi klien harus mengirimnya walau sudah kirim `posting_id`. ⚠️ **(live prod 2026-09-12, bip-erp #1853)** `posting_id` **kini WAJIB juga**, koreksi catatan lama di dok ini yang menyiratkannya opsional (kontrak lama memang begitu; diganti karena perusahaan kandidat kini HARUS diturunkan dari lowongannya). Kosong → `400 posting_id wajib diisi`; bukan ObjectID sah → `400 posting_id tidak valid`; lowongan tak ditemukan → `404 lowongan tidak ditemukan`; lowongan sudah `Closed` → `409 lowongan sudah ditutup`; lewat `application_deadline` → `409 lowongan sudah lewat batas lamaran`. Perusahaan kandidat (`company_id`) **selalu** diambil dari `company_id` lowongan, **tak pernah** dari header atau body: rute `/public/*` tak melewati validasi JWT, jadi header `BIP-*` di sana bisa dikarang siapa pun. `tanggal_lahir` = **RFC3339** (samakan dengan model employee agar mapping saat hire tidak perlu isi ulang — lihat [[HRIS - Recruitment]]). Upload berkas **backward-compatible**: body JSON tanpa file tetap diterima; berkas non-PDF / >10 MB → 400.
