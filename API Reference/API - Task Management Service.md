...
- ⚠️ **Tak ada penyaringan departemen/space di `queue`, `dashboard`, `tickets/:id`, maupun `logs`.** Setiap pemakai ERP dapat membaca seluruh tiket lintas departemen berikut `guideline`, `tone_of_voice`, `kata_terlarang`, dan `target_url`. 🟡 **Aturan penggantinya sudah diputuskan** ([[ADR - 0060 Cakupan Keterlihatan Tiket Engagement]]): penyaringan per `requester_department` + field baru + migrasi, **belum ada di kode**.
