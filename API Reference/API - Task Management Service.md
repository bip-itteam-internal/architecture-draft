...
| Method | Path | Fungsi |
|---|---|---|
| GET | `/kpi/ticket` ✅ | Agregat tiket **satu orang** satu periode untuk penilaian KPI. Query wajib `employee_id`·`periode=YYYY-MM`·`key`. Balasan `{ditugaskan, selesai, sla_terukur, selisih_jam[], csat_rating[]}`. Live di prod sejak 6 Agustus 2026 ([#1055](https://github.com/bip-itteam-internal/bip-erp/pull/1055)); penanda "belum merge" di versi lama dok ini sudah usang. 🟡 Field ke-6 **`csat_bukan_penangan[]`** di bip-erp PR [#1962](https://github.com/bip-itteam-internal/bip-erp/pull/1962) (merged 2026-09-17, live di DEV dan terverifikasi lewat gateway 2026-09-17, belum deploy PROD), lihat catatan di bawah tabel |
...
> 🟡 **`csat_bukan_penangan[]`** (bip-erp PR [#1962](https://github.com/bip-itteam-internal/bip-erp/pull/1962), merged 2026-09-17, live di DEV dan terverifikasi lewat gateway 2026-09-17, belum deploy PROD; `services/task-management/kpi_ticket.go` `ringkasTiket`). Nilai bintang pada tiket selesai (Done/Selesai/Archive) yang penilainya (`csat.rated_by`) **bukan** salah satu `assign_to` tiket itu; `rated_by` kosong tidak dihitung. Ikut terkirim di `/kpi/space-group` karena muatannya sama. Konsumennya metrik `selesai_dinilai` sumber `kinerja_tiket` di [[Microservices - Employee Service]].
>
> ⛔ **Himpunan bagian dari `csat_rating`, BUKAN kolom sejajar: jangan dijumlahkan dengannya.** Selisih keduanya = tiket yang dinilai penangannya sendiri. Penilai CSAT memang selalu pembuat tiket (`canSubmitCSAT`, `csat.go:22`), tetapi pembuat tiket boleh sekaligus penangannya; prod 2026-09-17 punya **8 dari 55** tiket ber-CSAT yang dinilai penangannya sendiri. `csat_rating` sengaja TIDAK disaring karena metrik `csat` Tech Development yang sedang dipakai bergantung padanya. Selalu terkirim `[]`, bukan `null`: pembacanya memakai pointer, dan field absen berarti task-management versi lama.
...
| Method | Path | Fungsi |
|---|---|---|
| POST | `/engagement/tickets` | Buat tiket. Wajib: `produk`, `jenis_konten` (`monolog`·`ugc`·`testing`), `jenis_komen` (`cowok`·`cewek`), `deadline` (RFC3339), dan `items` (1 sampai 5). **Tiap item memakai `rincian: [{jenis, volume_target}]`** (minimal satu jenis, `volume_target` > 0, tanpa duplikat; `jenis` ∈ `komentar`·`like`·`share`·`posting`·`review`·`vote`·`rating`·`save`·`lainnya`). Server **menurunkan** `jenis_pekerjaan` dan `volume` tiket dari rincian dan mengabaikan kiriman klien untuk keduanya; galat rincian menyebut nomor target (`target 2: ...`). **Bentuk lama** (item `{target_url, volume_target}` + `jenis_pekerjaan` + `volume` di tingkat tiket) masih diterima sementara, tetapi satu tiket tak boleh mencampur keduanya. `assigned_to` **diabaikan** (round-robin, [[ADR - 0083 Alokasi Otomatis Round-Robin Menggantikan Penunjukan Manual AS]]); `client` tak lagi wajib. Plafon 5 request per pemohon per hari: yang melebihinya menerima `201 {data, antre: true}` dengan status `ANTRE`. `prioritas` ∈ `low`·`normal`·`high`·`urgent` (kosong → `normal`; asing → `400`). Nomor tiket `ENG/YYYYMM/NNNN` diterbitkan server. Balasan `201 {data}` |
| GET | `/engagement/tickets/queue` | Tiket **aktif** tim (`OPEN`+`IN_PROGRESS`+`DONE_BY_TEAM`+`ANTRE`), urut prioritas lalu deadline, dibatasi cakupan departemen ([[ADR - 0060 Cakupan Keterlihatan Tiket Engagement]]). Paginasi `{data, pagination}` |
| GET | `/engagement/tickets/mine` | Tiket yang ditugaskan ke pemanggil: status aktif **ditambah tiket `CLOSED` yang dihitung KPI pada bulan berjalan** (bulan UTC dari `closed_at`, bulan yang sama dengan jendela pengisian realisasi). Tiap tiket berstatus `DONE_BY_TEAM`/`CLOSED` membawa `realisasi_belum_diisi` (jumlah baris ber-target yang realisasinya belum dilaporkan; hilang bila 0). Field itu **hanya ada di daftar ini** |
...
| GET | `/engagement/tickets/:id` | Detail + baris target: `{data, items, boleh_isi_realisasi}`. Tiap item: `{id, target_url, volume_target, volume_realisasi, rincian?: [{jenis, volume_target, volume_realisasi}], realisasi_diisi_at?, realisasi_diisi_oleh?}`; `volume_target`/`volume_realisasi` di item = **total baris** (Σ rincian bila ada). Item lama tanpa `rincian` memakai satu pasang angka. `boleh_isi_realisasi` dihitung server dengan putusan yang sama dengan `PUT .../realisasi`, jadi layar tak menghitung jendelanya sendiri |
...
| POST | `/engagement/tickets/:id/done` | Pengerja menandai selesai (`OPEN`/`IN_PROGRESS` → `DONE_BY_TEAM`). Body `{catatan}` opsional. Bukti pengerjaan **tidak diwajibkan** (keputusan SPV; syarat lama membuat tiket mustahil ditandai selesai karena tak ada rute pengisi `attachments`) |
...
| POST | `/engagement/tickets/:id/reopen` | Admin/supervisor/leader membuka kembali tiket `CLOSED` → `IN_PROGRESS` (pengerja sama; `closed_at`/`done_at` dihapus). Satu-satunya jalan keluar dari `CLOSED` |
| PATCH | `/engagement/tickets/:id/items/:itemId` | Pemohon/admin menyunting satu baris target. Body `{target_url, rincian: [{jenis, volume_target}]}` mengganti **seluruh** set jenis+target baris itu (bentuk lama `{target_url, jenis, volume_target}` hanya untuk baris lama; baris ber-rincian yang disunting lewat bentuk lama ditolak `400`). Hanya saat tiket `OPEN`/`IN_PROGRESS` **dan** baris belum punya realisasi tercatat (angka > 0 atau sudah ditandai diisi, walau 0), selain itu `409`. `jenis_pekerjaan` dan `volume` tiket diturunkan ulang |
| DELETE | `/engagement/tickets/:id/items/:itemId` | Pemohon/admin menghapus satu baris target, dengan syarat yang sama dengan PATCH; baris terakhir tak bisa dihapus |
| PUT | `/engagement/tickets/:id/items/:itemId/realisasi` | **Pengerja pemegang tiket, atau admin/supervisor yang tiketnya masuk cakupannya, melaporkan angka tercapai satu baris** ([[ADR - 0110 Realisasi Engagement Dilaporkan Pengerja per URL per Jenis Pekerjaan sampai Akhir Bulan KPI]]). Body baris ber-rincian: `{rincian: [{jenis, volume_realisasi}]}` yang **wajib memuat seluruh jenis baris itu tepat sekali**, angka ≥ 0 (nol sah); baris lama: `{volume_realisasi: n}`. `200 {data: item, message}`. `400` isian tak sah/tak lengkap, `403` bukan pengerja/admin (**requester tak boleh**) atau di luar cakupan, `404` baris bukan milik tiket, `409` status `ANTRE`/`CANCELLED`, jendela tutup (tiket `CLOSED` lewat akhir bulan KPI dari `closed_at`, admin/supervisor melewatinya), atau baris berubah di antara membaca dan menulis (muat ulang). Tiap tulis dicatat di log (aksi `realisasi`, dari→ke) |
| DELETE | `/engagement/tickets/:id` | Pemohon/admin menghapus **permanen** tiket yang masih `OPEN` atau `ANTRE` (kesalahan input); status lain `409`, wajib lewat `cancel` |
...
- ⚠️ **`?prioritas=` dan `?status=` kini dibaca seluruh endpoint daftar** lewat satu fungsi (`terapkanFilterQuery`); status dari query **mengiris** himpunan status layar itu, tidak menggantikannya (`queue` tak pernah menampilkan arsip, `mine` boleh `CLOSED` tetapi hanya bulan KPI berjalan). Catatan lama di sini (diabaikan tanpa galat) sudah tidak berlaku.
- ⚠️ **Keterlihatan tiket kini dibatasi cakupan departemen** (`keterlihatanFilter`/`cocokKeterlihatan`, `engagement_visibility.go`): pembuat, pengerja, pengerja dalam kolam departemen, supervisor tercakup, atau admin ([[ADR - 0060 Cakupan Keterlihatan Tiket Engagement]]). `tickets/:id` membalas `403 tiket ini bukan cakupan Anda` di luar itu, dan `PUT .../realisasi` memakai pemeriksaan yang sama untuk selain pengerja. Catatan lama di sini (tak ada penyaringan) sudah tidak berlaku.
...
| Method | Path | Fungsi |
|---|---|---|
| GET | `/kpi/engagement` | Agregat tiket engagement **satu orang** satu periode. Query wajib `employee_id`·`periode=YYYY-MM`·`key`. Balasan `{periode, ditutup, selisih_menit[], rasio_volume[], volume_terukur, berbukti}`. `selisih_menit` = `deadline` − `done_at` per tiket, **bertanda** (negatif = telat, tak dinolkan); `rasio_volume` hanya memuat tiket **terukur** (target > 0 dan realisasi seluruh barisnya sudah dilaporkan), sehingga `volume_terukur` ≤ `ditutup` dan selisihnya menjadi cakupan. Field lama `menit_ambil` dan `tanpa_revisi` sudah dibuang |
