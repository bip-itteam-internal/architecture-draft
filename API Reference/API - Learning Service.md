...
| POST | `/training/:id/participants/batch` | work | ✅ **Live** (bip-erp PR [#1969](https://github.com/bip-itteam-internal/bip-erp/pull/1969), terverifikasi lewat gateway DEV 2026-09-21). Penugasan MASSAL dalam SATU permintaan. Body `{employee_ids: []}`, maksimal **250** id. Balas **200** `{diterima, gagal[]}`, sukses sebagian. Lihat [[#Penugasan peserta massal]] |
...
✅ **Merged dan live.** bip-erp PR [#1969](https://github.com/bip-itteam-internal/bip-erp/pull/1969) **merged**; kodenya ada di `origin/main` (dibuktikan `git grep`). Gerbang biner `Learning-Service` yang berjalan, DEV maupun PROD 2026-09-21: `participants/batch` 1, `PilihYangBolehMasuk` 1, `CanEnroll` **0**, kontrol negatif 0 (DEV juga `kuota_penuh` 1 dan `alasan_kode` 2). Sumber: `services/learning/training.go`, `services/learning/models_participant.go`, uji `services/learning/participant_simpan_test.go`.

**Terverifikasi lewat gateway DEV 2026-09-21** dengan akun uji, kelas uji dibuat lalu dihapus sehingga DEV kembali ke 1 kelas:

- `POST /api/learning/training/:id/participants` membalas **201** `{"message":"participant enrolled"}`; permintaan duplikat membalas **409** ``{"error":"employee `UJI-A` already enrolled"}``, jadi bentuk balasan rute satu peserta memang tidak berubah.
- `POST /api/learning/training/:id/participants/batch` dengan 3 id yang salah satunya sudah terdaftar membalas **200** ``{"diterima":2,"gagal":[{"employee_id":"UJI-A","alasan_kode":"sudah_terdaftar","alasan":"employee `UJI-A` already enrolled"}]}``. Sukses sebagian terbukti di jalur hidup, bukan hanya di test.
- Kelas berkuota **1** yang sudah berisi 1 orang, dikirimi batch 2 orang, membalas **200** `{"diterima":0,...}` dengan **dua** entri `kuota_penuh` berbunyi `training is full (1/1 participants)`, dan `GET .../participants` sesudahnya tetap `count 1`. Yang dibuktikan bukan pesannya melainkan bahwa kuota benar-benar ditegakkan.
- `employee_ids` kosong membalas **400** `employee_ids is required`; 251 id membalas **400** `employee_ids melebihi batas 250 per permintaan`, jadi pesan batasnya memang menyebut angkanya.

⚠️ **PROD sudah naik 2026-09-21 tetapi perilakunya TIDAK diuji**: menulis ke prod dilarang, sehingga yang ada di sana hanya gerbang biner di atas dan pengukuran bundel frontend (lihat § Utang yang wajib diketahui). ⚠️ **Satu perjalanan layar utuh sebagai orang belum dikerjakan** (gerbang alur pengguna), jadi jangan mengutip dok ini sebagai bukti layarnya sudah dipakai HR.
...
- **Klien**: ✅ penggantinya di erp-frontend **merged lewat PR [#1647](https://github.com/bip-itteam-internal/erp-frontend/pull/1647) 2026-09-18 02:30 UTC** dan ada di `origin/main` (dibuktikan `git grep`): memanggil rute batch dalam satu permintaan (`src/features/hris/training/list/hooks/use-participants.ts:42-45`) dan menerjemahkan `alasan_kode` lewat `KODE_TOLAK_PESERTA` (`src/features/hris/training/list/types.ts:53-59`). Bundel `frontend-hris-dashboard` **DEV** 2026-09-21 memuat kunci-kunci baru itu (`capacityHint` 0); bundel **PROD** (container baru dibuat 11:11 WIB 2026-09-21): `participantsTitle` 4 berkas, `assignNoneAdded` 3, `completedConfirm` 3, `capacityHint` 0, kontrol negatif 0. Yang dibuktikan pengukuran itu hanyalah bundelnya sudah baru, **bukan** layarnya sudah dijalani seseorang. ⚠️ **Bentuk balasan rute satu peserta tetap wajib dipertahankan** walau bundel baru sudah mendarat: tab yang belum disegarkan masih menjalankan bundel lama, dan pola lamanya justru yang melahirkan bug kuota (`Promise.allSettled` atas N permintaan paralel, yang tak pernah melempar sehingga 10 dari 10 penolakan tetap tampil sebagai toast sukses berbunyi "0 peserta ditambahkan"). Rute 201/409-nya diukur ulang lewat gateway DEV 2026-09-21 dan bentuknya tidak berubah.
