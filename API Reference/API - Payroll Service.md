...
🟡 **Enam izin baru ada di katalog tetapi belum menggerbang rute apa pun** ([[ADR - 0129 Persetujuan Payroll Run Bertingkat dan Dibayar per Badan Usaha sebelum Terbit]] P1, PR [#2095](https://github.com/bip-itteam-internal/bip-erp/pull/2095), merged + live DEV 2026-09-26): `payroll.approve.cost_control` · `payroll.approve.hrd` · `payroll.approve.finance` · `payroll.approve.direksi` (empat tahap tanda tangan) · `payroll.bayar` (tandai lunas per badan usaha) · `payroll.publish` (terbitkan). Rute approve/publish baru pindah ke izin-izin ini di P2. Fallback tier: izin tahap dan `bayar` **tidak** punya fallback (hanya lewat paket); `publish` jatuh ke tier `hris` supervisor dan admin. Tier admin dan paket `payroll_admin` kini daftar eksplisit, bukan "seluruh katalog", supaya HR admin tidak diam-diam memegang keempat tahap. Paket baru: `payroll_ttd_cost_control`, `payroll_ttd_hrd`, `payroll_ttd_finance`, `payroll_ttd_direksi` (tahap + `payroll.view`), `payroll_pembayar_cv` (`payroll.bayar` saja, tanpa view), `payroll_penerbit` (publish + view). ⚠️ Paket bawaan yang **sudah tersimpan** ikut berubah saat employee-service start: `selaraskanIzinPaketDefault` (`services/employee/selaras_izin_paket.go`) menambahkan izin bawaan yang belum ada (`$addToSet`, tanpa jejak metadata), jadi `payroll_admin` mendapat `payroll.publish` (terbukti oplog DEV 2026-09-26). Karena itu isi paket bawaan tak boleh ditulis sebagai "seluruh katalog": penyelaras akan menyebarkan izin baru apa pun ke paket yang sudah terpasang di prod.
...
| POST | `/payroll-runs/:id/tandai-insentif` | **`approve`** / `isApprover` | ⚠️ Branch `feat/insentif-payroll-snapshot`, belum merged per 2026-09-25 ([[ADR - 0125 Insentif Profit Dibayar lewat Slip Gaji dari Snapshot yang Disetujui Finance]] T4). Ulangi penandaan terbayar snapshot insentif untuk run **published** yang bertanda `insentif_tanda_tertunda`; sukses membersihkan tandanya. Run tanpa baris Insentif atau belum published → 400 |

> ⚠️ **Baris `Insentif` di impor dan publish (T4 ADR 0125, belum merged per 2026-09-25).**
> - **Impor**: baris pendapatan `Insentif` diterima **hanya** bila nilainya sama persis (2 desimal) dengan **jumlah seluruh** snapshot insentif yang disetujui Finance dan belum diklaim milik karyawan itu, lintas level dan periode tertunda. Payroll membaca snapshotnya dari [[API - Insentive Service]] `layak-klaim`, lalu **mengklaimnya** atas nama run draft sebelum baris disisipkan. Tidak cocok → `gagal[]` menyebut selisih rupiah dan snapshot yang tersedia (diurut nomor baris sheet); diklaim impor lain bersamaan → 409; insentive tak terjangkau atau `INSENTIVE_MODULE_URL` kosong → ditolak (gagal-tertutup). Baris run menyimpan `insentif_snapshot_ids`, dijaga indeks unik parsial multikey sebagai lapis kedua.
> - **Hapus draft impor** melepas klaim di insentive **lebih dulu**; bila tak bisa, draft **tidak** dihapus (502), supaya tak ada snapshot terkunci tanpa pemilik.
> - **Publish** menandai snapshot terbayar. Run ber-Insentif diterbitkan dengan `insentif_tanda_tertunda: true` di update yang sama, dan tanda itu dibersihkan hanya bila penandaan berhasil; gagal → respons membawa `peringatan`, ulangi lewat `tandai-insentif`. Publish **tidak** ditahan oleh insentive yang mati.
...
| Method | Path | Gerbang | Catatan |
|---|---|---|---|
| GET | `/internal/badan-usaha?key=` | kunci layanan `PAYROLL_SERVICE_KEY` di query | ⚠️ **Merge 2026-09-16 (bip-erp #1905), di prod sejak 2026-09-17.** Id dan nama badan usaha (`payroll_company`), terurut nama: `{"data":[{"id","name"}]}`, **tanpa** NPWP dan rekening. Env kosong = **503** (rute tertutup sampai diisi); kunci salah atau absen = **401**, dibandingkan `subtle.ConstantTimeCompare`. Pemanggil: finance-service (master entitas CV dan laporan kecocokan, [[Finance - Buku Besar CV]]), tanpa identitas orang. Penjaganya di handler, bukan prefix `/internal/` ([[ADR - 0031 Prefix internal Bukan Batas Keamanan]]). `GET /companies` tidak dipakai karena menuntut identitas HR, sedangkan `InternalRequest` tak meneruskan `BIP-Permissions` |
